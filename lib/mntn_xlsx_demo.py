...
head = pd.DataFrame([{
    "Sales vertical": v, "MM IVR": mi, "3P IVR": ti,
    "IVR advantage": mi / ti, "MM CPV": mc, "3P CPV": tc,
    "MM ROAS": mr, "3P ROAS": tr, "IVR advantage.": None,
    "MM advertisers": ma, "3P advertisers": ta,
...
wb.table(
    ...
    formats={"MM IVR": FMT.PCT2, "3P IVR": FMT.PCT2, "IVR advantage": FMT.MULT,
             "MM CPV": FMT.USD, "3P CPV": FMT.USD, "MM ROAS": FMT.ROAS, "3P ROAS": FMT.ROAS,
             "MM advertisers": FMT.INT, "3P advertisers": FMT.INT},
    heat={"MM IVR": "high", "3P IVR": "high"},  # heat and rag never on the same column (they collide in Excel)
...
wb.table(
    ...
    formats={"Advertisers": FMT.INT, "Campaigns": FMT.INT, "Spend": FMT.USD0,
             "Impressions": FMT.INT, "IVR (median)": FMT.PCT2, "CPV (median)": FMT.USD,
             "ROAS (median)": FMT.ROAS},
    heat={"IVR (median)": "high", "CPV (median)": "low", "ROAS (median)": "high"},
    kind="data", widths={"Sales vertical": 22, "Group": 15, "Spend": 12, "Impressions": 13},
    freeze="C",
...
wb.table(
    ...
    formats={"Impressions": FMT.INT, "Visits": FMT.INT, "Spend": FMT.USD0,
             "IVR": FMT.PCT2, "CPV": FMT.USD, "ROAS": FMT.ROAS},
    band=False, kind="detail", widths={"Advertiser": 16},
...
wb.glossary(
    ...
    rows=[
        ("How to use this", "Start on Overview for the takeaways and a clickable index. Each data tab states its finding at the top, with the method just under it in grey. Everything is filterable; the SQL is on the Queries tab."),
        ("", ""),
        ...
        ("Colour", "Darker green = larger on that metric (per-column magnitude). On lift columns a semantic scheme applies: red = significant negative, amber = not significant, green deepens with lift. Navy headers, banded rows."),
        ("", ""),
...
wb.notes(
    ...
    blocks=[
        ("Advertiser-weighted medians", "Each advertiser counts once. This keeps a single large account from setting the headline. Columns labelled 'pooled' are impression- or spend-weighted instead."),
        ("ROAS is directional", "Prospecting, last-touch. Revenue mostly lands in retargeting (excluded here), and some pixels are unreliable. Visit rate and cost per visit are the solid metrics."),
        ("Minimum volume", "Advertisers under 20,000 impressions in a group are dropped from the median so a tiny campaign can't swing it."),
        ("This is a sample", "All figures are synthetic. The purpose is to demonstrate the shared format, not to report findings."),
    ],
)
...
# --- save + verify ----------------------------------------------------------
scratch = os.environ.get("CLAUDE_SCRATCH", "/private/tmp/claude-501/-Users-malachi-Developer-work-mntn-workspace/3b55570d-c509-4bdc-a8b6-68fa3f480871/scratchpad")
