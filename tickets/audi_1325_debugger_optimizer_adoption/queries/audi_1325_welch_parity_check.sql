WITH daily AS (
  SELECT * FROM UNNEST([
    STRUCT('synth' AS dag_id, 'spark' AS surface, DATE_ADD(DATE '2026-08-01', INTERVAL 0 DAY) AS d, 100.0 AS exec_h),
    STRUCT('synth' AS dag_id, 'spark' AS surface, DATE_ADD(DATE '2026-08-01', INTERVAL 1 DAY) AS d, 110.0 AS exec_h),
    STRUCT('synth' AS dag_id, 'spark' AS surface, DATE_ADD(DATE '2026-08-01', INTERVAL 2 DAY) AS d, 90.0 AS exec_h),
    STRUCT('synth' AS dag_id, 'spark' AS surface, DATE_ADD(DATE '2026-08-01', INTERVAL 3 DAY) AS d, 105.0 AS exec_h),
    STRUCT('synth', 'spark', DATE_ADD(DATE '2026-08-10', INTERVAL 0 DAY), 60.0),
    STRUCT('synth', 'spark', DATE_ADD(DATE '2026-08-10', INTERVAL 1 DAY), 55.0),
    STRUCT('synth', 'spark', DATE_ADD(DATE '2026-08-10', INTERVAL 2 DAY), 70.0)
  ])
), fix_rows AS (
  SELECT
    dag_id,
  ...
  SELECT 'synth' AS dag_id, 'spark' AS surface, DATE '2026-08-09' AS applied_date
), rates AS (
  SELECT
    j.dag_id,
    j.surface,
...
SELECT dag_id, before_days, after_days, ROUND(before_rate,4) before_rate, ROUND(after_rate,4) after_rate, ROUND(df,6) df, ROUND(se,6) se, ROUND(exec_h_saved,4) exec_h_saved, ROUND(half,4) half, exec_h_saved - half > 0 AS proven FROM scored
