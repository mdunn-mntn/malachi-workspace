"""
TI-813: Executive-quality visualization charts — 500 advertiser scale.
Generates charts from CSV data in outputs/.
...
def load_rank_buckets():
    rows = []
    with open(os.path.join(DATA_DIR, 'ti_813_rank_bucket_visit_rates.csv')) as f:
...
def load_per_advertiser():
    rows = []
    with open(os.path.join(DATA_DIR, 'ti_813_per_advertiser_rank_lift.csv')) as f:
        for row in csv.DictReader(f):
            rows.append(row)
    return sorted(rows, key=lambda r: float(r.get('median_lift', r.get('lift_top_vs_bottom', 0))), reverse=True)

def load_per_vertical():
    rows = []
    with open(os.path.join(DATA_DIR, 'ti_813_per_vertical_rank_lift.csv')) as f:
        for row in csv.DictReader(f):
            rows.append(row)
    return sorted(rows, key=lambda r: float(r.get('median_lift', r.get('lift_top_vs_bottom', 0))), reverse=True)
...
def chart_rank_buckets():
    ...
    # Title and subtitle — use actual lift from data
    top_lift = lifts[0]
    ax.text(0.0, 1.18, f'Top-Ranked Keywords Drive {top_lift:.0f}x More Visits',
            transform=ax.transAxes, fontsize=26, fontweight='bold', color='#111111')
    ax.text(0.0, 1.10, f'500 advertisers · 30.9B IPs scored · {top_lift:.0f}x lift for top-5 keywords vs rank 51+',
    ...
    fig.savefig(os.path.join(OUT_DIR, 'ti_813_chart_rank_bucket_visit_rates.png'),
...
def chart_per_advertiser():
    all_data = load_per_advertiser()
    all_lifts = [float(r['lift_top_vs_bottom']) for r in all_data]
    # Show top 15 + summary row
    data = all_data[:15]
    names = [r['advertiser_name'][:30] for r in data]
    ...
    # Title and subtitle
    n_above_10 = sum(1 for l in all_lifts if l > 10)
    ...
    fig.savefig(os.path.join(OUT_DIR, 'ti_813_chart_per_advertiser_lift.png'),
                dpi=200, facecolor=BG)
    plt.close(fig)
...
def chart_contrast():
    ...
    top_lift_val = lifts[0]
    fig.text(0.5, 0.91, f'Global keyword ranking captures 3x differentiation. Per-advertiser ranking captures {top_lift_val:.0f}x.',
    ...
    improvement = top_lift_val / 3.0
    fig.text(0.5, 0.05, f'Per-advertiser ranking captures {improvement:.0f}x more signal than global ranking',
    ...
    fig.savefig(os.path.join(OUT_DIR, 'ti_813_chart_global_vs_per_advertiser.png'),
                dpi=200, facecolor=BG)
    plt.close(fig)
...
def chart_per_vertical():
    all_data = load_per_vertical()
    # Show top 20 verticals
    data = all_data[:20]
    names = [r['vertical_name'][:35] for r in data]
    lifts = [float(r.get('median_lift', r.get('lift_top_vs_bottom', 0))) for r in data]
    median_lift = np.median(lifts)
    ...
    fig.savefig(os.path.join(OUT_DIR, 'ti_813_chart_per_vertical_lift.png'),
                dpi=200, facecolor=BG)
    plt.close(fig)
