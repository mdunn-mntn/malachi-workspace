...
### Vertical corrections DEPLOYED via the sanctioned override mechanism (2026-08-11 22:20 UTC)
Found the real mechanism instead of guessing: `gs://mntn-data-archive-prod/vertical_categorizations/vertical_manual_overrides/` — a parquet with schema `domain_name, current_vertical_name, new_vertical_name, new_vertical_id, new_bucket_id, is_manual_override, create_ts, last_modified_ts`, 412 existing rows from 2025-07-08. **Verified it actually propagates before writing to it:** all 412 override targets appear in the live wcv with `vertical_name == new_vertical_name` and `is_manual_override = 1` (100%).
- **26 of the 36 live wrong verticals deployed** as overrides (412 -> 438 rows). Backed up first to `backup_pre_audi431_2026-08-11/vertical_manual_overrides/`. Post-deploy verification re-read the live object: byte-identical to the validated local file, original 412 rows unchanged, dtypes preserved (including the `int32` on `is_manual_override` and `datetime64[ns]` timestamps — a naive rebuild produced `datetime64[s]` and was caught by the dtype assertion).
- Examples: `dailymail.com` Gifts & Specialty Stores -> Current Affairs; `bleacherreport.com` Self-Help -> Sports Teams & Leagues; `letterboxd.com` Apparel Premium -> Theatre, Dance, & Films; `audiogon.com` Apparel -> Consumer Electronics; `equibase.com` B2B Promotional Products -> Sports Betting.
- **Takes effect on the next wcv rebuild** (the override file is an input to it, not a live lookup). That rebuild is manual and infrequent, so the corrections are staged, not instantly live.
- **10 of the 36 have no honest vertical** (`facebook.com`, `bing.com`, `mail.com`, `viber.com`, `time.is`, `savvytime.com`, plus adtech `nextmillmedia.com`, `smilewanted.com`, `pubfinity.com`, `pixfuture.com`). The override schema has no "no vertical" option and forcing one would manufacture signal. **Recommendation: blocklist them** — same class as the yahoo.com/aol.com portals prod already blocks, and 4 are pure ad-serving domains that should not be in the visit stream at all. Not done unilaterally: it re-deploys the blocklist we just shipped.

### The 362 both-list domains (answered 2026-08-11)
Full list with each domain's current vertical: `outputs/audi_431_both_lists_detail.csv`. All 362 are in wcv and carry a vertical, which never applies because the blocklist wins first — e.g. `quora.com`, `aljazeera.com`, `neopets.com`, `perfumania.com`, `arkadium.com`, `reolink.com`, `betterment.com`, `timeout.com`. Their wcv verticals skew Current Affairs (56), Games & Comics (36), Live Music & Comedy (20). Left untouched: removing the whitelist rows is a no-op behaviourally (blocklist already wins) but is a 362-row deletion outside this ticket's adjudication.

### Round 2 deploy: the 10 not-verticalizable wcv domains blocklisted (2026-08-11 22:35 UTC)
Malachi: "are we not using them? if so, let's block as well." Confirmed — the 362 both-list domains are already inert (blocklist wins first), so nothing to do there. The 10 wcv domains with no honest vertical were blocklisted instead of being given a manufactured one.
- **Blocklist 4,385 -> 4,395** (+10): `facebook.com`, `bing.com`, `mail.com`, `viber.com`, `time.is`, `savvytime.com` and adtech `nextmillmedia.com`, `smilewanted.com`, `pubfinity.com`, `pixfuture.com`. 224M urls/7d. Whitelist unchanged this round.
- **These came from the corrections leg, not `missing_domains`, so they are NOT in the decision sheet.** Both the list builder and the gate now know that second provenance path, with evidence in `outputs/audi_431_extra_blocklist.csv` (domain + the wcv vertical removed + the reason). The gate was extended, not weakened: it still fails on any blocklist add lacking a traceable decision.
- **3 of the 10 (`smilewanted.com`, `pubfinity.com`, `mail.com`) are wrongly on the whitelist today.** Blocklisting is sufficient because the blocklist is checked first at every verified consumer, so it wins. **Deliberately did NOT delete the whitelist rows** — this deploy stays strictly additive and reversible. Cost: the known cross-list conflict set goes 362 -> 365. The gate distinguishes "reviewed" from "unreviewed" conflicts and only fails on the latter.
- Assertions that fired and were resolved rather than bypassed: BL-adds-in-existing-whitelist (3, reviewed), new-cross-list-conflict (same 3), provenance (second source taught), and the impact math KeyError (wcv extras have no `missing_domains` volume by construction — `reindex().fillna(0)`).
- Verified from the live object: SHA matches the validated bytes, original 1,464 rows preserved as an exact prefix, all 10 present.
