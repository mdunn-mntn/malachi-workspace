...
def model(dbt, session: SparkSession) -> DataFrame:  # type: ignore
    ...
    GCS_BASE_PATH = "gs://mntn-data-archive-prod/signals/site_visit_signal/"
    GCS_DOMAIN_VERTICAL_PATH = "gs://mntn-data-archive-prod/vertical_categorizations/website_crawl_verticals/"
    GCS_WHITELIST_PATH = (
    ...
    todays_folders = [
        f"{GCS_BASE_PATH}/dt={run_date_datetime.strftime('%Y-%m-%d')}/"
    ]

    signal_table_today_df = (
        session.read.option("basePath", f"{GCS_BASE_PATH}")
        .format("parquet")
        .load(todays_folders)
        .filter("ip NOT LIKE '%:%'")
        .filter("ip != '0.0.0.0'")
    ...
    yesterday_folders = [
        f"{GCS_BASE_PATH}/dt={(run_date_datetime - timedelta(1)).strftime('%Y-%m-%d')}/"
    ]
    ...
    whitelist_df = (
        session.read.csv(GCS_WHITELIST_PATH, header=False, schema=whitelist_schema)
        .distinct()
    )
    ...
    domain_to_vertical_df = (session.read.parquet(GCS_DOMAIN_VERTICAL_PATH)
                             .withColumnRenamed("domain_name", "domain"))

    final_df = (ddp_scored.join(domain_to_vertical_df.alias('v'), "domain", "left")
                .select("ip", "domain", "uid", "time", "vertical_id", "bucket_id", "vertical_name", "is_ecommerce",
