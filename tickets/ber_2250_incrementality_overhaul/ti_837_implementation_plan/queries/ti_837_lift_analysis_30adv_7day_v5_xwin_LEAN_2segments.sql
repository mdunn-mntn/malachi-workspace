-- TI-837 v5 CROSS-WINDOW VALIDATION — LEAN 2-segment variant (Phase 0c, 2026-04-30)
-- ----------------------------------------------------------------
-- Drops `all` and `stage1` segments to fit under BQ's 6-hour query timeout.
-- Two prior 4-segment attempts hit the 6.0 hr cap. Keeping `prosp` and `rtg`
-- — the two segments that drove v5's headline ("retargeting drives lift,
-- prospecting drives almost none"). Validates the deck's core claim on a
-- different window with halved query work.
--
-- Same query as v5 (queries/ti_837_lift_analysis_30adv_7day_v5_segments.sql),
-- shifted +2 days to the 2026-04-22 → 2026-04-28 window.
--
-- Purpose: validate that the segment ordering and approximate magnitudes
-- in v5 reproduce on a different week of data. Standard methodology
-- check before broader sharing.
--
-- Same: 30-advertiser cohort, hash, win_rate STRUCT (carried from v5
-- 04-20→04-26 — empirical drift across weeks expected to be small,
-- see caveat 1).
--
-- ADVERTISERS  = 30 advertisers (Phase 2 cohort, 2026-04-27)
-- ANALYSIS_WIN = 2026-04-22 00:00:00 UTC → 2026-04-29 00:00:00 UTC
-- VISIT_WIN    = 2026-04-22 00:00:00 UTC → 2026-04-30 00:00:00 UTC
--
-- Caveat: today is 2026-04-29 — visits for the latest impression days
-- are partial (04-28 impressions only get +1 day post-period vs +3 in
-- v5). Both arms equally affected → ATT is unbiased but lower power
-- on late days.
-- ----------------------------------------------------------------
...
augmentor_ips AS (
  ...
  WHERE DATE(time) >= DATE(TIMESTAMP('2026-04-22 00:00:00 UTC'))
    AND DATE(time) <  DATE(TIMESTAMP('2026-04-29 00:00:00 UTC'))
    AND ip IS NOT NULL AND ip != '0.0.0.0'
),
...
cost_imp_pairs AS (
  ...
  WHERE DATE(ci.time) >= DATE(TIMESTAMP('2026-04-22 00:00:00 UTC'))
    AND DATE(ci.time) <  DATE(TIMESTAMP('2026-04-29 00:00:00 UTC'))
...
-- Per-segment served_treatment (LEAN: prosp + rtg only)
...
cp_pairs AS (
  ...
  WHERE DATE(cp.time) >= DATE(TIMESTAMP('2026-04-22 00:00:00 UTC'))
    AND DATE(cp.time) <  DATE(TIMESTAMP('2026-05-01 00:00:00 UTC'))
...
-- Per-segment clickpass visits (LEAN: prosp + rtg only)
...
guid_visits AS (
  ...
  WHERE DATE(time) >= DATE(TIMESTAMP('2026-04-22 00:00:00 UTC'))
    AND DATE(time) <  DATE(TIMESTAMP('2026-05-01 00:00:00 UTC'))
    AND advertiser_id IN (30181, 30392, 30496, 31276, 31297, 31455, 31464, 32244, 32320, 32404, 32527, 32899, 33467, 33572, 33684, 34141, 34365, 34862, 35086, 35374, 35573, 37222, 37796, 38307, 38422, 42097, 43996, 46426, 50525, 56187)
    AND ip IS NOT NULL AND ip != '0.0.0.0'
),

-- Per-segment biddable holdouts (LEAN: prosp + rtg only)
bh_prosp AS (
  SELECT advertiser_id, ip, intent_tier
...
-- 2 union-combined "subjects" tables (LEAN: prosp + rtg only)
all_subjects AS (
...
-- Clickpass with segment label (LEAN: prosp + rtg only)
