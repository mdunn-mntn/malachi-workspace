/*
  TI-933 Phase 3: Pooled MNTN Select lift, 7-day window.
  Same methodology as v5 of TI-837 but Select-only (campaign_groups.product_id = 2),
  pooled across all 38 active Select advertisers, single "all Select" segment.

  Window: 2026-04-29 -> 2026-05-05 (7 days, ending yesterday, within augmentor TTL).
  Visit window extends to 2026-05-08 (+3 days for attribution lag).

  win_rates computed inline per advertiser. Outputs per-advertiser AND pooled rows.
*/
...
augmentor_ips AS (
  ...
  WHERE DATE(time) >= DATE '2026-04-29'
...
cost_imp_pairs AS (
  ...
  WHERE DATE(ci.time) >= DATE '2026-04-29'
...
cp_pairs AS (
  ...
  WHERE DATE(cp.time) >= DATE '2026-04-29'
