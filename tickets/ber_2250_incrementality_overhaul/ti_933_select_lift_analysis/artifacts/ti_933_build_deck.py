...
<section>
  ...
  <p style="margin-top: 0.4em; font-size: 0.8em;"><strong>How we count a "visit":</strong> an IP that landed on the advertiser&#39;s site between April 29 and May 8 &mdash; same window for treated and holdout. Detected via MNTN&#39;s identity graph (any traffic source, not just MNTN-attributed clicks).</p>
  <p style="margin-top: 0.3em; font-size: 0.75em; color: var(--text-light); font-style: italic;"><strong>Known limitation, fix next time:</strong> each impression gets 3-9 attribution days depending on when it served (later impressions get fewer). Affects both arms equally, so the lift comparison is fair &mdash; but the next iteration should fix every impression to a constant 3 or 7-day window.</p>
