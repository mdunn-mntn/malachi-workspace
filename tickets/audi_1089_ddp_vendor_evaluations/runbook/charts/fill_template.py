...
SCOPE = {
    ...
    99: "COMBINED pseudo-vendor: guid_log + augmentor treated as ONE source. Unique counts are the UNION (not the sum of the two columns — the logs overlap heavily). Percentage rows are ROW-WEIGHTED by volume: augmentor is 71.5% of combined rows, so e.g. % advertiser_id populated reads 28.5% (guid 100% x 28.5% share + aug 0% x 71.5% share) — the column describes the COMBINED feed, not its best half; use the per-log columns for per-log capabilities. Serving/performance cells are MEASURED union cohorts (q15); reach/stock/freshness cells are MEASURED union values (q15b). GRAIN CAVEAT: union reach is measured on URL-PARSEABLE rows only, so 'Unique IPs delivered' (186.9M) reads BELOW guid_log's own 195M - guid's ~20% URL-less rows carry IPs that q2's per-log raw reach counts but the union scan excludes.",
