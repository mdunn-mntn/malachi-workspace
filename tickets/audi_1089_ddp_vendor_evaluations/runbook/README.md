...
renewal status, ingestion + off-switch, blast radius, full verdicts, vendor asks, plus a
CONVENTIONS block). Regenerated by `charts/fill_template.py` (idempotent — rerun after any
scan refresh). Sources, in query order (row numbers below refer to the original question
template; the numbers sheet carries the same questions split one-per-row, same order):

| # | Query (queries/) | Output CSV (outputs/run_2026_07_10/) | Template rows it feeds |
|---|---|---|---|
| q0 | q0_roster_cost.sql | q0_roster_cost.csv | 3-8 contract/identity, 86 bills |
| q1 | q1_scale_by_day.sql | q1_scale_by_day.csv | 11-15 scale/liveness/IPv6, 25 path% |
| q1b | q1b_column_richness.sql | q1b_column_richness.csv | 25 field richness |
| q1c | q1c_content_quality.sql | q1c_content_quality.csv | 21-24 junk flags, 106 asks |
| q1d | q1d_billed_usage.sql | q1d_billed_usage.csv | 50 billed domains, 87 %billed, 92, 99 |
| q2 | q2_window_reach.sql | q2_window_reach.csv | 16-18 window reach |
| q2b | q2b_daily_drops.sql | q2b_daily_drops.csv | 29 drop decomposition |
| q2c | q2c_funnel.sql | q2c_funnel.csv | 28, 30-35 usable funnel |
| q3 | q3_usable_uniqueness.sql | q3_usable_uniqueness.csv + q3_pair_recency.csv | 38, 40-42 uniqueness/freshness |
| q3b | q3b_credit_reassignment.sql | q3b_credit_reassignment.csv | 43 marginal add-order, 99-101 LOO |
| q4 | q4_domain_value.sql | q4_domain_value.csv | 39 sole domains, 96 fee band |
| q5 | q5_score_tiers.sql | q5_score_tiers.csv | 46 touched/served, 54-57 score tiers |
| q6 | q6_value_tiers.sql | q6_value_tiers.csv | 47-49, 62-63, 67, 74-75, 79, 91-95 economics |
| q6b | q6b_sole_by_funnel.sql | q6b_sole_by_funnel.csv | 88 prospecting attribution |
| q7 | q7_sole_vr.sql | q7_sole_vr.csv | 76, 80 sole visits/IVR + Poisson CI |
| q7b | q7b_perf_by_cohort.sql | q7b_perf_by_cohort.csv | 58-59 avg HH score, 64, 68 visits/IVR touched |
| q7c | q7c_conversions.sql | q7c_conversions.csv | 65-66, 69-71, 77-78, 81-83 conversions/revenue/CVR/AOV/ROAS |
| q7d | q7d_platform_week.sql | q7d_platform_week.csv | 51 platform served-IP denominator |
| q8a | q8a_solo_stock.sql | q8a_solo_stock.csv | solo sheet: solo IPs/domains/classified + freshness-vs-free (pair + visit-day splits) |
| q8b | q8b_solo_perf.sql | q8b_solo_perf.csv | solo sheet: measured solo-cohort serving/performance (media, visits, conversions, HI/PP tier anchors) |
| — | (computed in fill_template.py) | — | 43 greedy add-order, 91-96 pricing lenses (margin ladder 15/20/30%), 104 curved score (q9b formula), 105-106 verdict/asks; solo sheet mask-exact rows (q3b/q3c/q3d holder masks), rebased shares, solo bill bounds |

Conventions: "% of column total" rows (33-35, 54-55 overall shares) sum >100% because
...
3. Run q0 -> q8b per the "Run" block in each header (bq_run.sh wrapper; svs = temp external
   table over GCS parquet). `q4_domain_value.sql` query A also emits `q3_pair_recency.csv`.
   Background class, ~1h each: q3, q3b, q3c, q3d, q5, q6, q7, q7b, q7c, q8a, q8b. The rest
   are minutes.
4. `python3 runbook/charts/fill_template.py run_<YYYY_MM_DD> <YYYY-MM>` — writes
   `outputs/audi_1089_quality_template_filled.xlsx`; the run must print `empty: none` for
   BOTH grids and `solo pending-scan cells: 0` (the script runs fine before q8a/q8b land —
   scan-fed solo cells print "pending scan (q8)" until then; rerun after they land).
   Solo anchors are asserted in the script: mask solo pairs == q3 net-new-vs-free (exact),
   q8a solo pairs == masks (<0.1% wcv/pc snapshot drift tolerated), q8b >= q6 sole
   (superset monotonicity), q8b HI/PP tier counts ~= q3d mask solo counts (<0.5%).
