...
(compound questions split so it's ONE question per row — 131 rows x 14 vendor columns).
Two sheets: **numbers** (real numeric values with Excel formats — counts, %, $, decimals —
sortable; "—" = not applicable) and **notes** (per-vendor text: scope, billing detail,
...
| — | (computed in fill_template.py) | — | 43 greedy add-order, 91-96 pricing lenses (margin ladder 15/20/30%), 104 curved score (q9b formula), 105-106 verdict/asks |
