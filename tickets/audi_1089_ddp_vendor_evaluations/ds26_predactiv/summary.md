...
- hashed_email_ds_26_signals is a DS26-exclusive severity-1 hourly DAG that explodes hem_sha256, applies privacy caps <=10 IPs/email and <=100 emails/IP, writing to hashed_email_signal/.../data_source_id=26/hash_type=sha256.
...
- DS26 registry SCD is broken: 4 valid_to-NULL physical rows; dedupe by max valid_from mntn_matched row (the orphan open row is stale type=crm). Fix in SteelHouse/sqlmesh:seeds/tpa__direct_data_partners.csv.

**Reuse when:** Renew/drop a DDP data vendor · Predactiv/DS26/ShareThis/DS17 questions · tracing hashed-email/HEM/CRM-identity resolution sources · valuing a vendor by classified-domain contribution vs sole-IP reach · AUDI-1089 DDP evaluation wave.
