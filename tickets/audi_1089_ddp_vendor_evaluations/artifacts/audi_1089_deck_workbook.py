...
notes = [
    ...
    "Universe = 13,286,674,041 distinct usable (ip x domain x date) triples across all 10 sources (measured deck_d1/d4, 2026-07-16 snapshots). 'Usable' = domain consumable by DS13 or DS19.",
    ...
    "CORROBORATION (2026-07-17, BAE billing table dw-main-gold.reporting.ddp_mm_winners_imp_202606): free-only-winner rows already bill $0 (tv_cpm=0 on 100%); rows where a free log co-wins with a paid vendor still bill $0.50 on 91.7% = 291.1M imps in June alone — the preemption gap is visible in the billing table itself (AUDI-1115 s4f).",
    "CREDIT-SEMANTICS NUANCE (2026-07-17): billing credit splits across matched DATA PATHS — 3P interest-segment paths (e.g. DS17 ShareThis @ $0.95 CPM) share the denominator with the MM path, so cross-path fractional credit is alive in June (the 'May+ integer single-credit' reading applies to the usage-report field, not the upstream allocation). No simple winners-table aggregation reproduces the usage meter exactly (+-7-42% by vendor) — exact BAE allocation rule = 2026-07-20 billing-sync question. This does not change any number in this workbook (bills come from the meter itself).",
    ...
    "'High-Graduated' (6666-9999 excl. 8000) is a score-value bucket, not a product tier: the v1 scoring component (DS13-based) pins HI/PP at exactly 10000/8000; Fangorn v2 (DS46) emits graduated scores filling those bands (8001-9999 = HI band, 6666-7999 = PP band). Both components are live today (rolling migration), so both value-shapes coexist in the logs. HI10000 + PP8000 + High-Graduated together ~= the HI/PP audiences across both. Mid (3333-6665) ~= MI; Max Reach = 1-3332.",
    ...
    "Rebuild: python3 artifacts/audi_1089_deck_workbook.py (all deck scans landed 2026-07-16; 0 pending cells expected).",
]
