...
### 4.12 Post-delivery audit — two more stale figures caught, the Queries tab found to be dropping content, and the HI tab rebuilt
...
**HI tab restructured to the ask, then de-jargoned.** Rows read "HI, audiences with no exclusions" / "HI, all MM audiences (incl. exclusions)" / "HI, only audiences with exclusions", echoing Paulo's wording. Row order mirrors the ask rather than ranking descending (a three-answer table keyed to a person's numbered questions is not a ranking; the detail tabs stay rank-desc).

**A `Stage` column with `Prospecting` / `Prospecting + MT-S2` rows was built and then REMOVED.** It was a robustness check leaking into the deliverable: six rows where three were asked for, and `MT-S2` / `Stage-1` are internal labels the workbook never defines, which is exactly the §9 "no internal vocabulary the artifact doesn't define" violation. The sensitivity now lives on Method & caveats as one plain sentence ("Including the next stage down the funnel adds 1,418 more and barely moves the answer: mean 4.77M to 4.76M, median 3.55M to 3.56M"), and the funnel-2 label inside the Queries tab literals reads "next stage down" rather than the internal name. Lesson: a robustness check belongs in the ticket record; only the answer belongs on the tab.

**Funnel 1 + 2 figures, kept here for the record rather than on the deliverable:**
...
**Audit result — all checks pass, reproducible from the committed CSVs by a recomputation that shares no code with the builder:** summary-table cells match recomputation · all 148 vertical rows exact · all 37 bucket rows exact · stale-figure AND internal-jargon sweep across every cell clean (`MT-S2`, `MT-S3`, `Stage-1`, and every pre-correction figure absent) · Queries tab carries both filenames, both full SQL bodies verbatim, the source GCS paths, the `funnel_level` filter, the `--location=us-central1` requirement and the cohort-size literals · derived prose claims re-derived (83.0x, 4.5M, 4.8M, 51.3M, 4.76M/3.56M/+1,418) · zero flat-10000 leakage in any reported cohort.
