...
**Design implication:** the ~$9.6k/mo prize is coupled to the HHST recommender, because an IP's score bucket requires scoring it — you cannot produce the recommender's per-bucket `ip_population` denominator over IPs you never score. Whether the prize survives is a design question for AUDI-1176 + Devon: does the recommender bucket on the **cheap household score** (`prospecting_high`, ~$100/day — keep full) or on the **expensive keyword scores** (`prospecting_keywords`, ~$396/day — the job we want to gate)? If the former, gate the keyword job and keep full household scoring → most of the prize survives. If the latter, the options narrow to (a) gate only the serving-store LOAD + IPDSC write (storage/downstream savings, NOT the compute prize), or (b) change the recommender's denominator to an addressable/sampled basis with Devon, then gate input fully. Also handle the recommender's 8-vs-30-day winnable mismatch and small-population guardrails. Clear AUD-5221 + LiftLab with their owners regardless.
