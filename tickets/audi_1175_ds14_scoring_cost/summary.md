...
**Authority — the live writer is PTV, not the Select-v4 system (correction, 2026-07-28).** Profiling `dso_household_score_thresholds` updates in the last day: 2,078 campaigns, ALL `product_id=1` (PTV) funnel-1, none template 55. But camperbid v4's scope is `product_id=2` (Select). **So the main production HHST writer is a different system than the auction-scoped v4 analyzed above.** What source then confirmed: the DDM recommender is scoped to `ddm.test_hhst_campaigns` (a test pilot — `inner join ddm.test_hhst_campaigns` in `hhst_generate_recommendation.sql`) and its generate step reads wins/spend (auction-scoped) → **DDM is NOT the PTV writer.** Every HHST implementation found (camperbid v4, legacy airflow `performance/intent_score_threshold`, DDM generate) builds population from AUCTION logs (bidder_bid_events / bid_price_log / wins); the only `prospecting_intent` read is in the confined DDM pilot's `hhst_bucket_collections`. **Gate-safety holds IF the PTV writer is auction-scoped like the others** — strongly indicated by the pattern + the 65%-Max-Reach distribution — but the exact PTV-writer job and its population source were NOT directly pinned. **This is the one residual to confirm before implementation.** Fastest close: Devon, or the existing "Threshold Investigation: BPL v. DSO" Mode notebook.
