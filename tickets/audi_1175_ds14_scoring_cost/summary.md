...
Caveat (monitoring only, no delivery impact): the one DDM routine `cache_hhst_population_filters.sql` DOES read the full scored `prospecting_intent`/`advertiser_intent` as a `pct_visible/pct_active` denominator — but it `INSERT`s to `ddm.hhst_population_filters` (an analytics cache), NOT `dso.household_score_thresholds`. A DS14 gate would shift that monitoring metric, not any threshold. Not-confirmable-from-code: v3's exact schedule (inferred from v4's `wait_for_v3` sensor) and the iDSO/BOS batch cadence.
