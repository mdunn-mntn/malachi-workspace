-- TI-650: v17 — CIDR-corrected v14 (campaign_group_id scoped resolution rates)
-- KEY FIX: event_log.ip has /32 or /128 CIDR suffix on ALL data before 2026-01-01.
-- This caused silent join failures in v14: s1_pool.match_ip (CIDR) != bid_ip (bare).
-- Fix: SPLIT(el.ip, '/')[OFFSET(0)] on all event_log.ip references.
-- CIL.ip (bid_ip) is always bare — no fix needed.
--
-- Changes from v14:
--   1. s1_pool: el.ip → SPLIT(el.ip, '/')[OFFSET(0)] AS match_ip (+ GROUP BY)
--   2. s2_chain_reachable: el.ip → SPLIT(el.ip, '/')[OFFSET(0)] AS vast_ip (+ GROUP BY)
--
-- Architecture: same v14 chain (S3→S2→S1 + direct fallback), pools keyed by campaign_group_id
-- Expected: resolution rates increase vs v14 (Nov-Dec 2025 vast IPs now matchable)
...
-- CIDR FIX: SPLIT(el.ip, '/')[OFFSET(0)] strips /32 suffix on pre-2026 data
-- ═══════════════════════════════════════════════════════════════
s1_pool AS (
    SELECT campaign_group_id, match_ip, MIN(impression_time) AS impression_time
    FROM (
        -- Vast IPs from event_log (vast_start + vast_impression) — CIDR-stripped
        SELECT c.campaign_group_id, SPLIT(el.ip, '/')[OFFSET(0)] AS match_ip, MIN(el.time) AS impression_time
        ...
        GROUP BY c.campaign_group_id, SPLIT(el.ip, '/')[OFFSET(0)]
        UNION ALL
        -- Bid IPs from CIL (covers display + failed vast events) — already bare
...
s2_chain_reachable AS (
    ...
    FROM (
        -- S2 vast events: (campaign_group_id, vast_ip, ad_served_id, earliest time) — CIDR-stripped
        SELECT c.campaign_group_id, SPLIT(el.ip, '/')[OFFSET(0)] AS vast_ip, el.ad_served_id, MIN(el.time) AS vast_time
        FROM `dw-main-silver.logdata.event_log` el
        JOIN campaigns c ON c.campaign_id = el.campaign_id AND c.funnel_level = 2
        WHERE el.event_type_raw IN ('vast_start', 'vast_impression')
          AND el.time >= TIMESTAMP('2025-11-06') AND el.time < TIMESTAMP('2026-02-11')
          AND el.ip IS NOT NULL
        GROUP BY c.campaign_group_id, SPLIT(el.ip, '/')[OFFSET(0)], el.ad_served_id
    ) s2v
    JOIN (
        -- S2 bid IPs: ad_served_id → bid_ip (earliest CIL record) — already bare
