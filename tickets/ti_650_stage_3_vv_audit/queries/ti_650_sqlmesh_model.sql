...
/* =============================================================================
   ...
   S1 RESOLUTION LOGIC (CASE in SELECT):
     vv_stage=1  -> current VV IS the S1 impression (use lt_ columns)
     pv_stage=1  -> prior VV IS the S1 impression (use pv_lt_ columns)
     pv_stage>1  -> second-level IP match: s1_pv join on prior VV's bid_ip with pv_stage=1

   JOINS (10 LEFT JOINs, 5 source tables):
     clickpass_log (anchor)
       -> el_all (event_log CTE, joined 3x: last-touch, prior VV, S1 chain — CTV)
       -> il_all (impression_log CTE, joined 3x: same slots — display fallback)
       -> ui_visits on ad_served_id (visit IP + impression IP)
       -> clickpass_log (self) x2: prior VV chain + S1 chain
       -> campaigns x1 (vv stage only; ft_stage removed — always 1, not needed)
...
, with_all_joins AS (
  ...
  /* S1 chain traversal — second-level match when pv_stage > 1.
     Finds Stage 1 VV where its redirect IP = prior VV's bid IP.
     Covers: S3/pv_stage=2 (most common gap), S3/pv_stage=3 when chain is same-IP. */
