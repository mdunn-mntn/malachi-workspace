...
--
-- S3 rows have full 5-source IPs with timestamps + impression type.
-- S2/S1 linked VV rows have clickpass details + channel (5-source unavailable
-- due to table TTLs and query cost).
--
...
-- ============================================================
-- 5-source IP extraction with timestamps (±30d around audit window)
-- ============================================================
ip_event_log AS (
    SELECT ad_served_id,
...
s3_all_ips AS (
    SELECT
        ...
        bid.bid_ip,       bid.bid_time,
        win.win_ip,        win.win_time,
...
-- ============================================================
-- Cross-stage VV matching (most recent before S3 VV time)
-- ============================================================
s3_s2_match AS (
    SELECT
...
-- ============================================================
-- Classify each S3 VV + generate deterministic UUID
-- (GENERATE_UUID is non-deterministic across CTE refs — use MD5)
-- ============================================================
s3_classified AS (
    SELECT
        FORMAT('%s-%s-%s-%s-%s',
        ...
        a.bid_ip,       a.bid_time,
        a.win_ip,        a.win_time,
        a.impression_ip, a.impression_log_time,
        a.viewability_ip, a.viewability_time,
        a.event_log_ip,   a.event_log_time,
        CASE
...
-- S3 VV rows (all — full 5-source IPs + timestamps)
SELECT
    c.trace_uuid,
    3 AS stage,
