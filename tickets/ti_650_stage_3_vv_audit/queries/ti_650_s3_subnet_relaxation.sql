-- TI-650: CGNAT /24 subnet relaxation for 752 unresolved S3 VVs
-- Tests approach #4: Relax exact IP match to /24 subnet match.
-- If unresolved S3 bid_ip shares /24 subnet with an S1 vast_start_ip, that's a link.
-- This is less precise but may be acceptable for CGNAT pools (172.5x.x.x, 40.138.x.x).
...
-- S1 /24 subnets (for relaxed matching)
s1_subnets AS (
    SELECT DISTINCT
        REGEXP_EXTRACT(vast_start_ip, r'^(\d+\.\d+\.\d+)\.\d+$') AS subnet_24,
        MIN(impression_time) AS earliest_s1_time
    FROM s1_by_vast_start
...
SELECT
    ...
    -- /24 subnet match
    COUNTIF(sub.subnet_24 IS NOT NULL AND sub.earliest_s1_time < u.vv_time) AS resolved_by_subnet_24,
    COUNTIF(sub.subnet_24 IS NOT NULL AND sub.earliest_s1_time < u.vv_time AND u.attribution_model_id IN (1,2,3)) AS primary_resolved_subnet,
    COUNTIF(sub.subnet_24 IS NOT NULL AND sub.earliest_s1_time < u.vv_time AND u.attribution_model_id IN (9,10,11)) AS competing_resolved_subnet,
    -- How many IPs are CGNAT (172.x.x.x)
    COUNTIF(STARTS_WITH(u.s3_bid_ip, '172.')) AS cgnat_172_count,
    COUNTIF(STARTS_WITH(u.s3_bid_ip, '172.') AND sub.subnet_24 IS NOT NULL AND sub.earliest_s1_time < u.vv_time) AS cgnat_172_resolved,
    -- Distinct IPs / subnets
    COUNT(DISTINCT u.s3_bid_ip) AS distinct_ips,
    COUNT(DISTINCT REGEXP_EXTRACT(u.s3_bid_ip, r'^(\d+\.\d+\.\d+)\.\d+$')) AS distinct_subnets,
    COUNT(DISTINCT CASE WHEN sub.subnet_24 IS NOT NULL AND sub.earliest_s1_time < u.vv_time
                        THEN u.s3_bid_ip END) AS distinct_ips_resolved
FROM unresolved u
LEFT JOIN s1_subnets sub
    ON REGEXP_EXTRACT(u.s3_bid_ip, r'^(\d+\.\d+\.\d+)\.\d+$') = sub.subnet_24;
