...
- **Lookback: 90 days** from VV start date (verified: 100% for S2→S1 with full pipeline; S3 patterns unknown, increase if resolution unexpectedly low)
...
- **90-day lookback** — from VV start date for S1/S2 pools (verified 100% for S2→S1 with full pipeline. S3 adds a VV bridge hop — patterns may differ. Use 90d initially, increase if resolution unexpectedly low)
...
For S2→S1, corrected analysis shows max 69 days (median 6d) using most recent match. Verified: 90d lookback = 100% for S2→S1 with full v8 pipeline (68,498/68,498). S3 adds another hop via the VV bridge, so S3 lookback requirements may differ. Measure the distribution (median, P95, P99, max) to determine the minimum lookback. **IMPORTANT:** Always use MAX(impression_time) WHERE time < vv_time, not MIN. The question is: "what's the MOST RECENT prior S1/S2 event that matches this S3 VV's bid_ip within the same campaign_group_id?"
