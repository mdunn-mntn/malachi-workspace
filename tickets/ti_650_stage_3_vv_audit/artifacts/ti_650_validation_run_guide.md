...
| **NO_BID_IP** | Breaks at step 2-3 | `bid_logs` record purged (TTL). We have `impression_log` but it only has internal NAT IP (10.105.x.x). Can't get the external bid_ip. | **No** — dead end, data is gone |
| **RESOLVED_EXTENDED** | Complete | Nothing — but the prior VV was found beyond our 365-day lookback. Our window was too short. | N/A — it resolved |
| **TRULY_UNRESOLVED** | Complete through step 3 | We have bid_ip, but step 4 finds no prior VV in clickpass_log matching that IP + campaign_group, even scanning all time | **Yes** — give to Zach |
