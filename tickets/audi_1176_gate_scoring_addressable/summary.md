...
- **Hard constraint (AUDI-1175 consumer audit):** the LIVE DDM/Redshift automated HHST threshold recommender (`ETL-DCO-Automated-Threshold-Adjustment.py` → `hhst_generate_recommendation`, owner **Devon Rogers**) reads the FULL scored `prospecting_intent`/`advertiser_intent` as its population denominator to SET the production HHST gate. Do NOT let a naive input-gate starve it. Because bucketing requires scoring, you can't produce its per-bucket `ip_population` denominator over IPs you don't score. Key design question: does it bucket on the cheap household score (`prospecting_high`, ~$100/day, keep full) or the expensive keyword scores (`prospecting_keywords`, ~$396/day, the gate target)? If cheap-score, gate the keyword job and keep full household scoring (most of the prize). If keyword-score, either gate only the serving-store load (storage savings, not compute) or change the recommender's denominator with Devon. Watch its `<100→6666` guardrail and 30-day-winnable vs 8-day-DS14 mismatch.
