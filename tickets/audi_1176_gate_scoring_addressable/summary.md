...
- **HHST constraint (AUDI-1175, updated 2026-07-28 — improved):** TWO HHST systems exist (both file-verified). The primary auction-scoped recommender (`airflow-camperbid intent_score_threshold_v4`) builds population from bid-side logs (`bid_price_log`/`bidder_bid_events`), which already exclude non-biddable IPs → **gate-SAFE, prize survives**. Only the DDM/Redshift DCO pilot (`ddm.hhst_bucket_collections`, `test_hhst_campaigns`, owner **Devon Rogers**) reads the full scored `prospecting_intent` — small-scope. Before rollout, run two shadow queries: (1) **starvation** — campaigns whose bid-log `population_rolling` at `prev_bucket_id` would fall below `next_population_required` under a DS14-only filter (+ how many already hit "HHST set to max reach"); (2) **Max-Reach inflation** — fraction of funnel-1 bid rows with score-failure reasons (`missingIntentScore`/`invalidHouseholdScore`) coalescing to `hh_score=0`. Confirm with Devon which system is authoritative + whether the pilot denominator can move to an addressable/auction basis.
