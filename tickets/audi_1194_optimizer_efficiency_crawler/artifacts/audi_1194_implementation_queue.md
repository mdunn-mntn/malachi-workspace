...
| 3 | `audience_intent / fangorn_score_monitor` | Stage 17 spills 1,138 GiB to disk, 954 exec-h worst run, 4 sweeps running | Raise `spark.sql.shuffle.partitions`; then executor memory if it persists | airflow-ti model config for `fangorn_score_monitor` | READY for a draft; verify partitions count from the log before choosing the number |
| 4 | `tpa_ipdsc_export / ipdsc_ds_35` | Stage 2 straggler 118.8x median on uniform data, 348 exec-h worst run | `spark.speculation=true`, `spark.speculation.quantile=0.9` | airflow-ti ipdsc batch config | READY; same speculation change validated on `intent_score_map` (Ryan, validation run 1) |
...
number; one branch per item, gauntlet once at the end; the fix lands with
