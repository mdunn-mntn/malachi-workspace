...
| 3 | `audience_intent / fangorn_score_monitor` | Stage 17 spills 1,138 GiB to disk, 954 exec-h worst run, 4 sweeps running | Raise `spark.sql.shuffle.partitions`; then executor memory if it persists | airflow-ti `models/monitoring/fangorn_score_monitor.py` | MERGED 2026-08-27 (PR #1231). PENDING: ledger `applied` write to the prod GCS ledger, blocked on gcloud reauth |
