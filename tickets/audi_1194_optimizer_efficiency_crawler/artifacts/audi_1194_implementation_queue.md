...
| 3 | `audience_intent / fangorn_score_monitor` | Stage 17 spills 1,138 GiB to disk, 954 exec-h worst run, 4 sweeps running | Raise `spark.sql.shuffle.partitions`; then executor memory if it persists | airflow-ti `models/monitoring/fangorn_score_monitor.py` | IN PR 1230: 256 -> 2048 in decorator and builder (verified: 1,267 GiB over 256 partitions) |
| 4 | `tpa_ipdsc_export / ipdsc_ds_35` | Stage 2 straggler 118.8x median on uniform data, 348 exec-h worst run | `spark.speculation=true`, `spark.speculation.quantile=0.9` | airflow-ti `models/ipdsc/ipdsc_ds_35.py` | IN PR 1230: speculation on, quantile 0.9 |
