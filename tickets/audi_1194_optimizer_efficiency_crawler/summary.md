...
**Mode savings $0 explained (2026-09-02 night): honest withdrawal, not a bug.** The $900+ figure rested on the blind window: fangorn_score_monitor ran 687.7 exec-h on 08-26, 954.3 on 08-27 (fix #1231 merged that day), then was invisible 08-28..09-01 while the downloader lost every batch. Today's first complete sweep restored visibility: 659.2 exec-h and the stage-17/19 disk-spill findings are CHRONIC again (fix_pr #1231 attached), so the measured-only rule zeroes the credit (after-rate 806.8 avg >= before-rate 687.7; GREATEST clamps to 0). The wide-shuffle components stayed resolved; ledger.py's per-finding view credits 28.5h. Verdict: #1231's spill half never held, its resolution was an artifact of blindness. Top hackathon re-fix candidate.
