...
- **Known metric sensitivity (TI-835):** total `guid_log` traffic barely moves with MNTN ads (~0% platform lift, `data_knowledge.md` L2589). This is WHY the visits test is **non-inferiority** (stable total visits ⇒ safe cap), not superiority; the attributed-VV diagnostic is kept for contrast. Expect a null / tight-CI safe-cap readout; power comes from large N (available on the eligible pool), not a large effect.
...
> **`PENDING-A` — total-visit source (RCT-grade join, to be BUILT):** primary = a **custom `dw-main-silver.logdata.guid_log` join** on **`(advertiser_id, ip)`** (ip CIDR-stripped), counting each household's total **visit-days** (`guid_log` is a PAGE-VIEW log → dedup page-views to distinct `(advertiser_id, ip, date)`; count = visit-days, not raw page-views; 366 TB physical → partition-prune + cohort-restrict, never full-scan; `first_impression` comes from the impression log, not `guid_log`) over a **post-anchored `[first_impression, +30-60d]` window that carries RCT arm membership** (arm from the §4 bucket hash). This is the ONLY source that yields the ≥30d, arm-A/B/C estimand the design requires. **`enriched.lift__ghost_bid_visits` is DEMOTED to a platform sanity/reference read only** — it is hard-coded to a **7-day** window and its arms are **ghost/submitted**, not the RCT's A/B/C, so it cannot deliver the ≥30d RCT-arm estimand (use it only to cross-check that the custom join reproduces the platform's 7d served/holdout rates). Attribution-independent: **yes** (site-pixel page views, fire with no impression; ghost/holdout reference empirically 0.886% 7d visit rate at 0.0% won-rate).
...
- [ ] Bidder feature (per-bucket cap in `do_fcap`) shipped + hash bit-match verified with `@SteelHouse/rtb` (16-hex TI-837 form, `CAST(advertiser_id AS STRING)` preimage).
- [ ] `PENDING-A` source BUILT: custom `logdata.guid_log` join on `(advertiser_id, ip)` over `[first_impression, +30-60d]` carrying RCT arm membership (the ≥30d RCT-arm estimand); `enriched.lift__ghost_bid_visits` used as platform 7d sanity/reference only. `PENDING-B/C` provisional (incidence-proxy ≈3% freq≥9, reference N/arm≈198K) — **confirm freq≥9 count MEAN + VARIANCE + N at Checkpoint β from Step 3's total-visit-by-frequency curve** (N off the critical path — does not gate lock).
- [ ] Eligible-stratum build validated: predicted ≥ cap+1 inflow measured (freq≥9 for arm B; ~1.62M/wk freq≥8 stock is an over-count proxy) + shared-IP purge applied + excluded volume quantified.
...
- [ ] Pre-registration `audi_1173_rct_prereg.md` FROZEN (this doc's §5/§6/§9 mirrored, count mean/variance anchor + N locked, δ = 5% relative recorded).

---

## 14. What would change the answer

- **Freq≥9 count mean `μ_C` (or its variance `σ_C`) materially different than assumed** → re-solve N via the §6 mean-difference formula at Checkpoint β (N off the critical path, so this does not move the calendar).
