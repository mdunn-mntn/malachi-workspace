...
- **Run the household-randomized cap RCT** (scope doc §6): cap arms {3,8} vs suppression holdout; randomize on `MOD(ABS(FARM_FINGERPRINT(CONCAT(CAST(advertiser_id AS STRING),':',ip))),1000)` (validated, 0-byte dry-run); primary metric = total visits/hh + cost/hh; power ~636K/arm; advertiser-clustered bootstrap; prospecting and retargeting as separate strata.
