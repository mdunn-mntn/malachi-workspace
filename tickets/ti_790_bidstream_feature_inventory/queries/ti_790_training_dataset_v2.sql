-- TI-790: Training dataset v2 — temporal leakage fix
-- Features: 2026-03-28 (full day)
-- Labels: 2026-03-29 clickpass_log (full day)
-- augmentor_log: 4-hour sample (12:00-16:00)
-- bidder_auction_events: full day
-- Scoped to (IP, advertiser) pairs

-- Base: IPs served impressions on 2026-03-28 (feature day)
WITH win_base AS (
  SELECT
    ip,
...
-- Labels: visits on 2026-03-29 (next day)
visits AS (
...
-- win_logs features (2026-03-28, full day)
wl AS (
  SELECT ip,
    COUNT(*) AS wl_n_wins,
  ...
  FROM `dw-main-silver.logdata.win_logs`
  WHERE DATE(time) = '2026-03-28' AND ip IS NOT NULL AND ip != ''
  GROUP BY ip
),

-- cost_impression_log features (2026-03-28, full day)
ci AS (
  SELECT ip,
    COUNT(*) AS ci_n_imp,
...
-- augmentor_log features (2026-03-28, 4-hour sample 12:00-16:00)
aug AS (
  SELECT ip,
  ...
  WHERE time >= '2026-03-28 12:00:00' AND time < '2026-03-28 16:00:00'
    AND ip IS NOT NULL AND ip != ''
  GROUP BY ip
),

-- bidder_auction_events features (2026-03-28, FULL DAY)
bae AS (
  ...
  WHERE _PARTITIONTIME >= '2026-03-28' AND _PARTITIONTIME < '2026-03-29'
