...
LAUNCHER = """
/* Mode renders the report body by assigning HTML, and scripts inserted that way never
   execute. So on a Refresh the markup is replaced by fresh placeholders and NOTHING runs
   to fill them: every earlier fix here was unreachable code. The first execution therefore
   installs a watchdog that outlives the markup and boots any unbooted copy it finds. */
(function () {
  function boot() {
    var roots = document.querySelectorAll('#mde');
    if (!roots.length) return;
    // Duplicate ids make getElementById resolve to the stale copy, so keep only the last.
  ...
  var tries = 0;
  (function wait() {
    if (window.datasets && window.datasets.length && window.Chart) return boot();
    if (++tries > 60) return boot();
    setTimeout(wait, 100);
  })();

  if (!window.__mdeWatchdog) {
    window.__mdeWatchdog = setInterval(boot, 400);
  }
})();"""
