...
<p class="note">The "Avon declined" premise (Paulo #1 / Mike Dolt) is the wrong window/lens: 2025→2026 Avon is healthy. The apparent decline lives in a 2024→2025 comparison (Avon scaled +71% then) and/or the client's industry_standard (last-touch + competing) lens.</p>
</section>
...
<p class="note">Avon <b>contracted</b> (spend −18%) — fewer users at higher frequency — but <b>converted better</b> and <b>ROAS rose</b>. Visit rate ~4.2% is 25–30× the cookware advertisers (HexClad/Caraway ~0.15%). The client's industry_standard view is also up (ROAS 9.4→10.4). Healthy on both lenses.</p>
</section>
...
<tr><td style="text-align:left"><b>Client UI / API (industry_standard = last-touch + competing)</b></td><td><b>692,888 / 598,436 EXACT</b></td><td><b>22.1 / 26.4 EXACT</b></td></tr>
</table>
<p class="note">The client UI/API = <b>CHAPI → ClickHouse</b>, not BigQuery. Three knobs: <b>(1)</b> attribution lens — <code>industry_standard</code> (last-touch <b>+ <code>competing_*</code></b>) vs a plain last-touch BQ pull → ~5× the visits, ~2–3× the ROAS; <b>(2)</b> Verified Visits = <code>clickpass</code> raw row count (~1.28×); <b>(3)</b> scope/aggregation. We reproduce the client's UI <b>to the dollar</b> in BQ by matching the lens. (Conversions are last-touch/last-tv-touch — Measurement; the delta is the <code>competing_*</code> credit, not a first-touch model.) <b>Watch the YoY trap:</b> comparing 2025 (last-touch) vs 2026 (industry_standard) manufactures ~52pp of an apparent crash — hold the lens constant.</p>
</section>
...
<ul class="tight">
<li><b>Avon did NOT decline</b> — spend −18%, ROAS +8%, conv-rate up, AOV flat, VR ~4.2% (25–30× the cookware advertisers). The premise is a wrong-window (2024→2025) / wrong-lens (industry_standard) artifact.</li>
...
<li><b class="navy">For any client-facing YoY, hold the attribution lens constant</b> (same reporting style both years) — the last-touch→industry_standard migration alone manufactures a large apparent decline.</li>
</ul></div>
