"""AUDI-1070 — Avon (31921) verified-case deck charts.
Tufte: no chartjunk, direct labels, color encodes meaning, NO embedded title
(the RevealJS slide H2 carries the finding). Reads outputs/avon_case_*.csv.
Outputs 6 PNGs (200 DPI, #FAFAFA). All numbers verified by workflow wf_733743cd-c9c."""
...
def read_csv(name):
    """Footer-tolerant read of a bq_run/plain CSV."""
...
# ============================================================ Chart 1 — HEADLINE
# Diverging YoY % deltas H1-2025 -> H1-2026: volume down, money up.
y = read_csv("avon_case_h1_yearly.csv").set_index("yr")
...
ax.text(-26, len(vals)-0.3, "VOLUME  v", fontsize=11, color="#5f6368", fontweight="bold")
ax.text(20, 0.0, "MONEY  ^", fontsize=11, color=GREEN, fontweight="bold", ha="center")
finish(ax); ax.spines["left"].set_visible(False); ax.tick_params(length=0)
fig.text(0.5, 0.015, "Avon H1-2025  to  H1-2026 (last-touch). Spend flat (−6%); fewer impressions/users, but more conversions, revenue, ROAS.",
         color="#666", fontsize=9.5, ha="center")
plt.tight_layout(rect=[0, 0.04, 1, 1])
plt.savefig(ART + "avon_case_1_headline.png", dpi=200, bbox_inches="tight"); plt.close()

# ============================================================ Chart 2 — TREND
# Monthly spend (bars) + ROAS (line). Performance holds across 30 months.
...
# yearly H1 avg ROAS callouts
for yr, x in [(2024,"2024-03-15"),(2025,"2025-03-15"),(2026,"2026-03-15")]:
    avg = read_csv("avon_case_h1_yearly.csv").set_index("yr").loc[yr,"roas"]
    ax2.text(pd.to_datetime(x), m["roas"].max()*1.07, f"H1 avg {avg:.1f}×",
...
fig.text(0.5, 0.015, "ROAS stays in a healthy band (10–38×) the entire period and its yearly H1 average rises 21 to 17 to 19×. Volume swings with budget; performance does not erode.",
         color="#666", fontsize=9.2, ha="center")
plt.tight_layout(rect=[0, 0.04, 1, 1])
plt.savefig(ART + "avon_case_2_trend.png", dpi=200, bbox_inches="tight"); plt.close()

# ============================================================ Chart 3 — INFLECTION
# Quarterly reach-per-$1k (bars, highlight Q2-2026) + frequency (line).
...
ax.annotate("Q2-2026: reach/$ falls ~30%\nfrequency jumps 2.3 to 3.1",
            xy=(9, q["reach_per_1k"].iloc[9]/1000), xytext=(5.4, 12),
            fontsize=10, color=RED, fontweight="bold",
            arrowprops=dict(arrowstyle="->", color=RED, lw=1.4))
for s in ["top"]: ax.spines[s].set_visible(False); ax2.spines[s].set_visible(False)
ax.tick_params(axis="x", labelsize=9.5)
fig.text(0.5, 0.015, "Through Q1-2026 each $1k reached ~38k users; in Q2-2026 the same dollars reach ~26k — extra budget now buys frequency, not new users. A reach ceiling, not a performance loss.",
         color="#666", fontsize=9.0, ha="center")
plt.tight_layout(rect=[0, 0.045, 1, 1])
plt.savefig(ART + "avon_case_3_inflection.png", dpi=200, bbox_inches="tight"); plt.close()

# ============================================================ Chart 4 — NO EXPANSION
# Panel A: flagship 259556 impr 25 vs 26 (+ROAS). Panel B: prospecting-impr change vs peers.
camp = read_csv("avon_case_campaigns_h1_yoy.csv")
row = camp[camp["campaign_id"] == 259556].iloc[0]
pc = read_csv("avon_case_prospecting_change.csv")
fig, (axA, axB) = plt.subplots(1, 2, figsize=(12, 5.2), gridspec_kw={"width_ratios":[1, 1.25]})
# A
axA.bar(["H1-2025","H1-2026"], [row["imps_25"]/1e6, row["imps_26"]/1e6],
        color=[LIGHT, NAVY], width=0.55)
axA.text(0, row["imps_25"]/1e6+0.06, f"{row['imps_25']/1e6:.2f}M\nROAS 8.65×", ha="center", fontsize=10, color="#5f6368")
axA.text(1, row["imps_26"]/1e6+0.06, f"{row['imps_26']/1e6:.2f}M\nROAS 9.21×", ha="center", fontsize=10, color=NAVY, fontweight="bold")
axA.set_ylim(0, row["imps_25"]/1e6*1.3); axA.set_ylabel("Prospecting impressions (M)", fontsize=10.5)
axA.set_title("Avon's only prospecting campaign\ncontracted −20%, ROAS rose", fontsize=11, color=NAVY, loc="center", pad=8)
...
for i, v in enumerate(order["prospecting_impr_chg_millions"]):
    axB.text(v + (0.8 if v >= 0 else -0.8), i, f"{v:+.1f}M", va="center",
             ha="left" if v >= 0 else "right", fontsize=11.5, fontweight="bold",
             color=GREEN if v < 1 else "#5f6368")
axB.axvline(0, color="#444", lw=1)
axB.set_xlim(-6, 44); axB.set_xticks([])
axB.set_title("Change in prospecting impressions, H1-25 to H1-26\nAvon did NOT expand; the others added 31–38M", fontsize=11, color=NAVY, loc="center", pad=8)
finish(axB); axB.spines["left"].set_visible(False); axB.tick_params(length=0)
fig.text(0.5, 0.015, "100% of Avon's H1-2026 impressions came from campaigns also active in H1-2025; zero from new campaigns. Caraway/HexClad shown for contrast.",
         color="#666", fontsize=9.2, ha="center")
plt.tight_layout(rect=[0, 0.04, 1, 1])
plt.savefig(ART + "avon_case_4_no_expansion.png", dpi=200, bbox_inches="tight"); plt.close()

# ============================================================ Chart 5 — AUDIENCE TIMELINE
...
ax.set_ylabel("YoY change H1-25  to  H1-26 (%)", fontsize=11)
ax.set_ylim(-17, 11); ax.legend(frameon=False, fontsize=10, loc="lower right")
finish(ax); ax.tick_params(length=0)
fig.text(0.5, 0.015, "Three independent tables, one signature: visits down double-digit, revenue & conversions up mid-single-digit. The case does not depend on one source.",
         color="#666", fontsize=9.3, ha="center")
plt.tight_layout(rect=[0, 0.04, 1, 1])
plt.savefig(ART + "avon_case_6_triangulation.png", dpi=200, bbox_inches="tight"); plt.close()

print("wrote 6 charts: avon_case_{1_headline,2_trend,3_inflection,4_no_expansion,5_audience_timeline,6_triangulation}.png")
