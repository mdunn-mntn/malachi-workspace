...
exactly two knobs: SCOPE (prospecting -> all stages) and ATTRIBUTION (add CTV / last-TV-touch).
2025 Jan-May. API=9.40 (prospecting, CHAPI) -> +stages -> BQ=17.33 (all-stages, no CTV)
-> +CTV -> UI=22.12 (all-stages, incl CTV). Every source is up YoY."""
import numpy as np
...
ax.set_title("API vs UI vs BigQuery: same advertiser, two knobs — scope + CTV attribution",
             fontsize=14, fontweight="bold", loc="left", y=1.06, color=NAVY)
ax.text(0, 1.015, "The numbers aren't in conflict. API/chart = prospecting only; add the mid-funnel stages to reach all-stages; "
        "add CTV (last-TV-touch) conversions to match the UI. Every source rises YoY.",
...
print(f"API {API} -> +{BQ-API:.1f} scope -> BQ {BQ} -> +{UI-BQ:.1f} CTV -> UI {UI}")
