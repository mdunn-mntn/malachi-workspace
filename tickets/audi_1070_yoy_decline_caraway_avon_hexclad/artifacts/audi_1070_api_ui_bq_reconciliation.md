...
`default` ≈ `last_touch` at the **account total** (23,962 ≈ 23,961). But `last_touch_*` and `last_tv_touch_*` are **two separate last-touch models** (display-last-touch vs TV-last-touch) that credit the *same* conversion to *different* campaigns — they coincide at the account total but **diverge per stage** (fl=1: default 11.0 / last_touch 4.99 / last_tv 7.47). So summing `last_touch + last_tv_touch` **double-counts** any conversion exposed to both display and TV → account 23.49, overshooting the UI's 22.12. **Clincher that default already includes CTV:** the 100%-CTV prospecting campaign ("Beeswax **Television**") shows its full conversions in the *default* columns (would be ~0 if default excluded CTV). **Net: don't sum the variants.** CHAPI dedups to the single true last touch, so the UI (22.12) sits *between* the last-touch rollup (17.33) and the naive sum (23.49); the 17.3→22.1 gap is the CHAPI engine (~1.276×), not a recoverable BQ column.
