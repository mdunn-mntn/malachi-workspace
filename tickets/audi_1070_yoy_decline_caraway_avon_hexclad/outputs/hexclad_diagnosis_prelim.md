...
## CARAWAY DMA cells (613551 High-DMA, 613591 Low-DMA) — DIFFERENT settings, bring non-HI in (2026-07-01)
The May-13-2026 DMA test cells do NOT share the flagship's settings. Per-campaign May13-Jun30 2026:
- **439156 flagship: 96.4% exact-10000, 0% Fangorn-continuous, 5.9% RTC** (bucketed HI-only, gate 10000).
- **613551 High-DMA: 10.4% exact-10000, 36.4% Fangorn-High (8001-9999), 16.3% Fangorn-Peak (6666-7999), 14.2% mid/low, 21.1% unscored, 26.8% RTC.**
- **613591 Low-DMA: 10.4% exact-10000, 45.4% Fangorn-High, 11.3% Fangorn-Peak, 14.0% mid/low, 17.1% unscored, 19.5% RTC.**
THREE setting differences, each admits non-HI: (1) **SCORING = Fangorn continuous** (DMA cells = Caraway's Fangorn cohort; flagship bucketed) — Fangorn grades pull in the Peak band; (2) **GATE = lower/continuous** — DMA HHST ramped DOWN to 7201 (admits Peak on the continuous scale) and hit 0 May 24 + Jun 30 (= short-flight HHST=0; DMA cells run 3-5d flights so Tofer's practice applied, unlike the flagship's long flights); (3) **RTC heavy** (20-27% vs 5.9%). EFFECT: DMA cells REPLACED the flagship May 13, so May-Jun Caraway prospecting = the DMA cells' mix (~46-56% High Fangorn-inclusive + ~15% Peak + ~14% Mid + ~20% unscored + ~20-27% RTC) = the entire May-Jun HI-share drop (65%->51%). It is a DELIBERATE new regime (geographic DMA A/B on Fangorn with a broader intent gate), NOT flagship degradation, and it's AFTER the Jan-May decline window (doesn't change the core over-scaling finding).

## CHAPI/API vs BQ — HexClad + Caraway reproduction (2026-07-01), extends the Avon exact-reproduction
Ran the proven CHAPI formula (silver.summarydata.all_facts, industry_standard = last-touch + competing_*; FT visits = clicks+views+competing_views) for HexClad(34611)+Caraway(40341), prospecting (obj 1,5,6), Jan-May 25 vs 26:
- **HexClad:** 2025 spend $642K, LT_VV 111,108 / **FT_VV 553,590**, LT_ROAS 3.14 / **FT_ROAS 8.78**. 2026 spend $968K, LT_VV 69,092 / **FT_VV 504,976**, LT_ROAS 1.04 / **FT_ROAS 3.87**.
- **Caraway:** 2025 spend $278K, LT_VV 61,559 / **FT_VV 277,304**, LT_ROAS 4.34 / **FT_ROAS 9.71**. 2026 spend $809K, LT_VV 52,904 / **FT_VV 262,255**, LT_ROAS 1.26 / **FT_ROAS 2.84**.
FT (industry_standard, +competing_*) = ~5x the visits, ~2-3x the ROAS of a naive last-touch pull. **RECONCILES Mike's HexClad numbers = the LAST-TOUCH view:** Mike ROAS 3.15->1.09, spend $600K->$900K, OV $2M->$950K == BQ LT prospecting 3.14->1.04, $642K->$968K, $2.02M->$1.01M (matches to ~1%). When we quoted 8.78->3.87 we were on first-touch. Same delivery data, different attribution credit. **Decline is REAL on BOTH lenses** (HexClad LT -67%/FT -56%; Caraway both -71%) — the lens changes the LEVEL, not the DIRECTION. The 3 knobs: (1) FT vs LT attribution [dominant], (2) VV=clickpass row count ~1.28x, (3) scope/aggregation. Which number a client sees = their reporting_style setting (oscillated LT<->FT historically) x scope/card. Query pattern: `queries/avon_chapi_exact_reproduction.sql` (swap advertiser_id).

## AVON MoM — CLEAN (verified 2026-07-01): gate + low-volume noise only, no tracking outages
Avon prospecting S1 monthly VR (outputs/diag_avon/monthly_vr_roas.csv) is volatile 4.5-9.9% but NO sustained collapse and NO craters. Daily VR min never near-zero (~2-4% floor) = no tracking outage (contrast HexClad Mar'26 VR->0.006%, Caraway Nov gap + Jan conv-pixel break). VR swings are LOW-VOLUME noise (200-800K imps/mo, daily VR 4%<->20% intra-week). ALL composition changes gate-explained (Nov52.9/Dec8.2% HI = gate removed Nov19; Feb-Mar 99.9% = re-gated Jan6; May-Jun = loosening+Fangorn). ROAS healthy throughout 5.5-18.4x. Avon's VR did NOT crater even when HI dropped (Dec 8%HI/7.5%VR) — beauty audience converts regardless (vs cookware where HI vs non-HI is 3x). DEFINITIVE: Avon's only drivers = HHST gate changes + noise; no 0-HHST-campaign issue, no unexplained drastic change, no outage. HexClad/Caraway DO have non-gate craters (data-quality). Change-audit tables now consolidated in knowledge/data_catalog.md "Config-change AUDIT tables".
