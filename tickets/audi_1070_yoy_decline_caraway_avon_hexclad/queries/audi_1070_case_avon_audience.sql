...
-- NOTE: ORDER BY create_time DESC, NOT version — the `version` field is NON-MONOTONIC (wraps);
--       ordering by version returns a stale snapshot. See data_knowledge.md + proof pack Q5.
WITH ac AS (
...
seg AS (
  SELECT s.campaign_id, s.expression, s.is_targeted, s.expression_type_id,
    ROW_NUMBER() OVER (PARTITION BY s.campaign_id ORDER BY s.create_time DESC) rn
