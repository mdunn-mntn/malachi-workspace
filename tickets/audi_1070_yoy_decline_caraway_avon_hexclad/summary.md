...
- **Q (Mike): first-touch conversion table?** There's no *separate* FT conversion table, but the UI **is** first-touch-inclusive via `competing_*` (industry_standard). **The FT lens DOES move AID-total revenue/ROAS** (22.1 industry_standard vs 17.3 last-touch) — correcting the earlier verbal note (Lilit's "last-touch + last-TV-touch" described the conversion→impression event layer, not the reporting column selection). ROAS still rises YoY on every lens (UI 22.12→26.36, +19%; last-touch 17.33→20.68, +19%; prospecting 9.40→10.37, +10%).

### FULL AVON PROSPECTING STORY — campaigns → audience → first-touch metrics (2026-06-30) — deck `artifacts/audi_1070_avon_full_deck.html`
