...
keywords: [audi-1070, hexclad, caraway, avon, mntn matched degradation, hhst gate thrash, within-hi vr, over-scaling saturation, industry_standard competing attribution, household_score binary logging 2025-06-01, reach frequency expansion, visit-tracking outage, short-flight 72h, cohort saturation law]
---

## TL;DR

**Q:** AUDI-1070: Diagnose the YoY visits/ROAS decline (relative to spend) for Caraway (40341), Avon (31921), and HexClad (34611) — is there a common cause, specifically "general degradation in MNTN Matched (MM) over time"?

**A:** "General MM degradation over time" is NOT supported. The dominant lever across the advertisers examined is the HHST intent gate (config / campaign-management), not the MM audience model. Two failure modes emerged: (1) gate-removal/thrash — the HHST gate is removed (holiday spend or short <72h flights) so delivery LEAVES high-intent while the served-HI pool stays healthy (within-HI VR holds/rises); (2) over-scaling — pushing 2-3x budget through a finite HI pool exhausts it (within-HI VR falls). Of 5 advertisers run through the diagnostic, 4 are gate (HexClad, Avon-recovered, The Bouqs, Kindred Bravely) and only Caraway is true over-scaling; "MM is degrading" is wrong on all 5. within-HI VR is the discriminator between the two modes.

Per-advertiser: HexClad's decline is definitively the HHST gate on flagship 93373/446801 being thrashed — removed ~Nov 11 2025 (holiday), never durably restored (66 changes on 446801), so delivery repeatedly leaves HI (97.8% HI pre-gate -> 31% HI / 34% unscored post-gate); root cause CONFIRMED by Tofer/PEX 2026-07-01 (short <72h burst flights manually set HHST=0, plus a manual Nov 0-change left forgotten). Caraway is over-scaling: spend tripled (+191%) into flagship 439156 that HELD ~82-99% HI-share yet VR fell -69% (0.465%->0.146%) because household_score is binary/pinned at 10000 and blind to marginal-vs-best HI; a ~Jan-2026 conversion-pixel break additionally inflates its ROAS -71% (clean audience signal is VR -69%). Avon is the clean control / no real decline: flat-to-down spend, ROAS ROSE (+8 to +19% by window/lens), VR flat; it CONTRACTED (fewer users, higher frequency, higher conv-rate). Paulo/Mike Dolt's Avon concern sources to (a) the wrong window — 2024->2025 (Avon scaled +71% and dipped = saturation) not 2025->2026, and (b) the client-UI lens: reporting_style="industry_standard" = last-touch + competing_* (a competitive-scenario credit orthogonal to touch-order, NOT first-touch as originally mislabeled), and for Avon/HexClad the effective lens flipped LT(2025)->industry_standard(2026), manufacturing ~52-54pp of apparent visit crash.

Ruled out: no MM-core scoring change (DS13 vertical + DS19 keyword substrate continuous 2024-2026); the June-1-2025 score-column NULL->populated flip is a CIL LOGGING change, not a scoring turn-on (HHST gates configured at scale since Jul-2024; scores already in the model_params blob back to 2025-05-06); LiveRamp DS11->DS35 cutover (May-Jun 2025) is a 3P-layer upgrade not MM-core; Fangorn/DS46 is post-window and intent-raising; BUK/DS38 never went live; RTC absent for the first 3 AIDs. Data-quality craters (HexClad visit-tracking gap Mar 3-17 2026; Caraway conversion-pixel break ~Jan 2026; Avon 2024 impressions gap) explain the steepest non-gradual drops and should be excluded from client YoY. A cohort-wide spend<->VR saturation law holds (n=294; flat-spend VR rose x1.26, spend-growers fell) but is not MM-specific. Fix = pace HI spend to the sustainable live-30d-pool rate, hold/restore the HHST gate (esp. through holiday scale-ups), educate clients to run flights >=72h, adopt continuous scoring (Fangorn), and hold attribution constant across both years in any client YoY.

Note: §5 "Solution/Verdict" (pure spend-driven saturation/expansion) and the front-matter result are the earlier framing; §9-§12 supersede them (HexClad = gate-thrash; only Caraway = true over-scaling). Status: in progress — Avon & HexClad answered, Caraway definitive; Avon mid-campaign check still pending.

**How:** Read the full summary.md (624 lines, 3 passes); skimmed outputs/ and queries/ filenames (q0-q5 steps, inv1-4, per-advertiser diag_avon/caraway/bouqs/kindred folders, reusable_diagnostic_pack). Analysis chain per summary: Step 0 lens-reconcile + YoY headline -> Step 1 log-decomposition (Oaxaca) + reach/frequency HLL -> Step 2 expression/data-source classification -> Step 3/Inv2 CIL served-score bands joined forward to clickpass visits -> Step 4 18-month timeline -> Step 5 cohort falsification (n=294) -> Inv1 supply-side audience-size -> sections 9-12 per-campaign HHST gate event-study confirmed by Tofer/PEX meeting. Grep-verified all candidate durable facts already exist in knowledge/*.md.

**Tables:** sum_by_advertiser_by_day · cost_impression_log · clickpass_log · conversion_log · archives_audience_segment_archives · archives_advertiser_setting_archives · r2_advertiser_settings · dso_household_score_thresholds · household_score_threshold_archives · flight_cid_day_audience_sizes · silver.core.flights · silver.summarydata.all_facts · campaigns · audience_data_sources

**Learned:**
- Verdict evolved: section 5 (spend-driven saturation/expansion) is superseded by sections 9-12 — dominant cause is the HHST intent gate (config/campaign-management), not MM degradation; within-HI VR is the discriminator between gate-removal/thrash and true over-scaling.
- Of 5 advertisers, 4 are gate-removal/thrash (HexClad, Avon-recovered, The Bouqs, Kindred Bravely) and only Caraway is genuine over-scaling; 'MM is degrading' fails on all 5.
- reporting_style='industry_standard' = last-touch + competing_* (competitive-scenario credit orthogonal to touch-order), NOT first-touch — earlier mislabel corrected; it moves ROAS via competing on conversions/order value.
- The June-1-2025 household_score/advertiser_household_score NULL->100% flip is a CIL logging/ETL change, not a scoring turn-on — scores were in the model_params blob back to 2025-05-06 and HHST gates existed at scale since Jul-2024.
- household_score is effectively binary (unscored=-1/NULL vs scored pinned ~10000); it cannot distinguish best-HI from marginal-HI, hiding Caraway's within-10000-band VR collapse from score dashboards.
- Steepest non-gradual drops are visit/conversion tracking outages (HexClad Mar 3-17 2026; Caraway ~Jan 2026; Avon 2024) — measurement discontinuities to exclude from client YoY.
- All durable facts from this summary were already committed to knowledge/*.md (per sections 7/11/12); grep confirmed present, so delta_facts is empty.

**Reuse when:** Diagnosing any advertiser YoY visits/ROAS decline vs spend (is MM degrading?) · Questions about HHST intent-gate removal/thrash vs over-scaling as decline mechanisms · Reconciling client UI/CHAPI numbers to BigQuery, or FT vs LT vs industry_standard attribution lens disputes · Interpreting household_score/advertiser_household_score bands, the 2025-06-01 logging cutover, or within-HI VR as saturation-vs-gate discriminator · Reusing the advertiser YoY diagnostic pack (documentation/docs/advertiser_yoy_diagnostic; queries/reusable_diagnostic_pack).
...
- **✅ VERIFIED EXACT REPRODUCTION (supersedes the "1.276× engine, can't rebuild" framing):** the UI runs CHAPI's **`industry_standard` (NEW) reporting style = last-touch + `competing_*` (FIRST-TOUCH) columns** — NOT `last_tv_touch`/CTV. Extracted from `SteelHouse/chapi` source (metric registry `r2-metadata.xml`; `Views = views + competing_views`, etc.) and reproduced against `silver.summarydata.all_facts` (time col `hour`) to the dollar/visit, BOTH years: **Verified Visits 692,888 / 598,436 EXACT**, ROAS 22.09 / **26.36**, CPA $2.39/$2.03 & CVR 4.41%/5.26% EXACT, Households ~1–2% under (HLL engine only). Last-touch-only gives 526,929 / 17.3× — does NOT match. So the "1.276×" gap = the `competing_*` first-touch credit, and it **IS recoverable in BQ**. Query: `queries/avon_chapi_exact_reproduction.sql`.
- **Q (Mike): first-touch conversion table?** No — and "first-touch" is a **misnomer** here (corrected w/ Lilit/Measurement + schema proof, 2026-07-01). The client UI headline = **`industry_standard` = last-touch + `competing_*`**. `competing_*` is a *competitive-scenario* credit **orthogonal to touch-order** (`competing_last_touch_*` and `competing_last_tv_touch_*` both exist in `all_facts`), **not** first-touch. Conversions/order-value have **no** first-touch column (only `last_touch_*`/`last_tv_touch_*`/`competing_*`/`probattr_*`) → conversions are matched last-touch/last-tv-touch. The only `first_touch_*` column is `first_touch_visits` (visits only). **`industry_standard` DOES move AID-total revenue/ROAS** (22.1 vs 17.3 last-touch) because `competing_*` adds `competing_view_conversions`+`competing_view_order_value`. ROAS still rises YoY on every lens (UI 22.12→26.36, +19%; last-touch 17.33→20.68, +19%; prospecting 9.40→10.37, +10%). See `knowledge/data_knowledge.md` §5h.

### FULL AVON PROSPECTING STORY — campaigns → audience → industry_standard (LT + competing) metrics (2026-06-30) — deck `artifacts/audi_1070_avon_full_deck.html`
**Scope split:** prospecting = `objective_id != 4`; retargeting = `objective_id = 4` (funnel_level is mixed across both — do NOT use it to separate). Join `campaign_id → bronze.integrationprod.campaigns` (PK `campaign_id`).
**Campaigns (2yr):** Stage-1 MM prospecting = **259556 "Beeswax Television Prospecting"** (obj=1, **CTV**, $250K); S2/S3 = Multi-Touch / Multi-Touch-Plus (obj 5/6, display, DS16 taxonomy — NOT MM); retargeting = 6× "TV Retargeting" (obj=4, $142K, excluded).
**Audience of 259556 (proper MM prospecting, no issues):** MNTN Matched **DS19** (~180 cats) OR Oracle 3P **DS1** → MM ∪ 3P; AND MNTN Global **DS14**; **exclude** CRM (DS4) / MNTN Pageview (DS34, 30d) / MNTN Conversion (DS21, 30d); geo **US only** (237); **10% holdout**; **RTC** scoring. Delivery 92%+ MM-scored.
...
## 11. Deliverables & final corrections (2026-07-01)

**Attribution label correction (Lilit/Measurement + schema proof).** Relabeled "first-touch (FT)" → **`industry_standard` (last-touch + `competing_*`)** across all three decks (HexClad, Avon; Caraway was clean), the full-story doc, `knowledge/data_knowledge.md` (new §5h authoritative; §5e/§5f/§5e-bis marked partially-superseded), and memory. `competing_*` is a competitive-scenario credit **orthogonal to touch-order** (proven: `competing_last_touch_*`/`competing_last_tv_touch_*` exist in `all_facts`); conversions have no first-touch column → last-touch/last-tv-touch only. `industry_standard` **does** move ROAS (Avon 17.3→22.1) via competing on conversions+OV. **Every decline conclusion unchanged — only the label was wrong.**

**Diagnostic decision-tree flowchart (new).** `artifacts/audi_1070_diagnostic_flowchart.png` (static) + `artifacts/diagnostic_flowchart.html` (interactive: zoomable SVG + per-node question/table reference) + `artifacts/diagnostic_tree.json` (machine-readable). Canonical home: `documentation/docs/advertiser_yoy_diagnostic/flowchart/`. Walk **Q0→Q5**, cheapest-artifact-first; the **Q2 gate fork** splits the two failure modes (gate-removal / HexClad vs over-scaling / Caraway). Built via a design-panel workflow (3 lenses → synthesize → adversarial-logic + completeness + Tufte critics → finalize); the critics added the tracking-outage node, the RTC no-op guard, the 2025-05-06 served-score floor, and the reach-not-frequency saturation signature.

**Query index (new).** `queries/QUERY_INDEX.md` — every query tiered by importance/reusability. The reusable engine (7 parameterized queries + `run_diagnostic.sh` + README) is copied into `queries/reusable_diagnostic_pack/` so the ticket folder is self-contained for Drive upload (canonical source stays in `documentation/docs/advertiser_yoy_diagnostic/`).

**Share links:** HexClad `gist.githack.com/mdunn-mntn/64c97a58e2b479ffeeae1d79e3bfd81a`; Avon `.../94922dd1622c28549a4df86aa5acab6c`; Flowchart `.../55bf6c94d2795ae85c261b6834c53ec9` (all `/raw/<file>.html`).

**Client-chart identification (spend-match) + MoM lens.** A client "Performance Report – MoM" chart handed over as "Avon" was actually **HexClad** — identified by matching monthly spend to the dollar (spend is lens-invariant; HexClad prospecting $139k–$903k/mo vs Avon $6k–$26k). Reproduced HexClad's every month to the **exact visit + ROAS** on `industry_standard` (obj 1,5,6). A second "Avon" chart WAS Avon — reproduced exactly on **last-touch**. **The MoM widget lens is per-advertiser: Avon=last-touch, HexClad=industry_standard.** Method + cases → `knowledge/data_knowledge.md` §5i/§5j; memory `reference_client_chart_spend_match_id`.

**Gate × spend × flights synced (Johnny/Tofer request, 2026-07-01).** Overlaid the flagship's daily HHST gate, daily spend, and flight boundaries (`silver.core.flights`, group 93373) on one time axis. Findings: 73 active flights over the year; **short flights (≤72h) run ungated 45% of their days vs 28% for long flights** (14/31 short flights fully ungated) — Tofer's manual short-flight-HHST=0 is a real tendency, not a rule. The big damage is the **Nov–Dec holiday mega-flights** ($112k → $165k → **$409k** → $180k budgets) running while the gate was off and daily spend hit ~$100k/day. Chart `artifacts/hexclad_gate_spend_flights.png`; data `outputs/hexclad_flights.csv` + `_446801_daily_spend.csv`. Fix: flights ≥72h + hold the gate through holiday scale-ups. Documented in `knowledge/data_knowledge.md` (Short-flights section).

**Stable HI-share ≠ stable ROAS.** Avon's MoM ROAS swings 4.3×–16.8× despite staying gated-HI. AOV is flat ($47–56 all 17 months) → the swing is conversions-per-dollar, driven by (1) a diminishing-returns spend envelope (ROAS moves inversely with spend: lowest-spend ~$7k months = 16×, highest-spend Nov $25.6k = 5.9× — same finite-HI-pool saturation as Caraway, Avon just runs at the efficient end) + (2) small-volume/seasonal/attribution-lag noise. Composition = WHO you reached; ROAS = efficiency-per-dollar. Aggregate Jan–May ROAS still +8% YoY (7.93→8.59) on −18% spend = healthy. Chart `artifacts/avon_spend_roas_envelope.png`; data `outputs/avon_mom_lt_decomposition.csv`; `knowledge/data_knowledge.md` §5k; memory `reference_stable_hi_not_stable_roas`.

## 12. Two more advertisers — The Bouqs + Kindred Bravely (Paulo request, 2026-07-02)

Ran the full diagnostic (reusable pack + gate×spend×flights sync + within-HI VR + lens reproduction + expression audit) on two more advertisers via a parallel, adversarially-verified workflow. **Both are gate-removal/thrash — NOT over-scaling.** Decks: `artifacts/audi_1070_{bouqs,kindred}_deck.html`; data `outputs/diag_{bouqs,kindred}/`; charts `{bouqs,kindred}_*.png`.

**The Bouqs — eCommerce Unit (32147) — REAL decline, gate-removal/thrash.** Prospecting visits −55%, ROAS −54% (2.77→1.27) on −20% spend; VR −34%. AID-wide only −14% (retargeting held). **Decisive: within-HI VR ROSE 0.30%→2.40% while HI-share collapsed 55%→4%** → delivery LEFT a healthy pool (opposite of Caraway). Gate thrashed (51 changes), removed Nov 11–24 (holiday), clean re-gate to 10000 in March (HI→77% within days). 2026 relaunch (flagship 595017, grp 119362, launched Apr 15) runs ~83% ungated. **Nuances (adversarial-verify caught):** (a) "overall VR tracks the gate" is FALSE at monthly grain (corr −0.45) — the gate controls COMPOSITION, not blended VR; decisive evidence is within-HI VR. (b) ~47% of the "71% unscored" is MT2/MT3 **unscored-by-design** (595018 = 5.86M imps 100% unscored, obj=5) — only 595017 (obj=1, 49% unscored) is true gate-removal. (c) March re-gate: HI-share recovers overnight but within-HI VR ramps ~4 weeks (TI-780). (d) RTC is **live** here (real filter, unlike the first 3). (e) Subscriptions unit 31906 dark in 2026 ($0); Dec-2025 full-account pause. **Power line:** *"The Bouqs didn't run out of high-intent — it stopped asking for it."* Fix: hold HHST≥8001 on the stage-1 v2 fleet, stop the walking-ramp thrash, consolidate short-flighted frequency variants, leave MT2/MT3 alone, allow ~4wk ramp.

**Kindred Bravely (35094) — REAL decline, gate-removal/thrash + mix-shift; over-scaling DISPROVEN.** Prospecting spend **+65%**, imps +43%, visits −47%, VR −63%, conv-rate −41%, CPM +15%, AOV flat, ROAS −81% (9.76→1.81 last-touch; 19.13→3.53 industry_standard — **lens-invariant**). **Decisive: despite +65% spend, within-HI VR HELD ~0.7–1.1% (Pearson r POSITIVE +0.37/+0.61)** → the HI pool is NOT exhausted; over-scaling disproven (the analyst's initial "over-scaling primary" was corrected by the verifier — the "collapse" rested on a single May-2025 outlier). Flagship 261318 (both years, **198 gate changes**) → 0/−1 on Nov 19 (holiday, HI→2.6% by Dec, ungated to Jan 6), Feb daily thrash (10000↔0↔3334 Mid-Intent), May loosening. 2026 expanded into new Pop-tier + Q1 variant prospecting groups. Mix-shift: 2026 overall VR (0.43%) is half the within-HI VR (0.88%). **Power line:** *"Kindred pulled the intent gate off for the holidays — ROAS fell 81%."* Fix: restore & HOLD HHST=10000 year-round, stop the Feb thrash / May loosening, pacing as complement; investigate the CPM+15%/conv-rate−41% efficiency loss separately.

**META-FINDING (the reusable insight): within-HI VR is the discriminator between the two failure modes.** Gate-removal/thrash → the served-HI pool stays healthy (within-HI VR holds/rises) because delivery just LEFT it. Over-scaling → the pool is exhausted (within-HI VR falls with spend). Of the 5 advertisers: **HexClad, Avon(recovered), The Bouqs, Kindred = gate; only Caraway = true over-scaling.** So "MM is degrading" is wrong on all 5 — the dominant lever is the HHST gate (config/campaign-management), not the audience model. Even a hard-scaling advertiser (Kindred +65%, Caraway-like setup) turned out to be gate, not saturation, once within-HI VR was checked. **Also new:** the PORTFOLIO/MIX trap — MT2/MT3 companion campaigns are unscored *by design* and inflate the "% unscored" headline; only the obj=1 stage-1 ungated share is a gate to fix. Captured in `knowledge/data_knowledge.md`.
