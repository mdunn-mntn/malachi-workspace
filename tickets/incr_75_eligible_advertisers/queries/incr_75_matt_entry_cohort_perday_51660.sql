...
SELECT
  ...
  ROUND(100*(SAFE_DIVIDE(v_t,n_t) - SAFE_DIVIDE(v_h,n_h)), 4) AS itt_pp,
  ROUND(100*SAFE_DIVIDE(SAFE_DIVIDE(v_t,n_t) - SAFE_DIVIDE(v_h,n_h), SAFE_DIVIDE(v_h,n_h)), 1) AS rel_pct,
  ROUND((SAFE_DIVIDE(v_t,n_t) - SAFE_DIVIDE(v_h,n_h)) /
        SQRT(SAFE_DIVIDE(v_t/n_t*(1-v_t/n_t),n_t) + SAFE_DIVIDE(v_h/n_h*(1-v_h/n_h),n_h)), 2) AS z,
  IF(entry_dt >= "2026-06-24", "TRUNC", "full") AS window_obs
