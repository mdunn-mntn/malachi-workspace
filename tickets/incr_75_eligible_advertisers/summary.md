...
**Longer window won't rescue it within the flight.** Table floor = Jun 22 (raw `bid_price_log` ~10-day TTL → Jun 8–21 launch period unrecoverable); forward accrual ~1 holdout visit/day, so running to the Aug 1 flight end takes 19 → ~29 holdout visits. Binding limit = holdout visit count (10% × 0.09% VR), not window length. To measure cleanly: bigger holdout (25–50%) or pool multiple HI-excluded prospecting campaigns. Matt asked for a longer pull; answered with this accrual math.
