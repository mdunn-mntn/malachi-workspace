...
- **Value score → lift-adjusted (2026-07-05).** `confirmed +` lift adds +30 base + up to +15 by relative magnitude (capped at rel=30%); flat/too-early/no-data add +0. New col `apriori_value_score` keeps the pre-lift number. (Interim: score-band tiers Top≥90/Mid60-90/Low<60 gave a strictly-monotonic 67/126/1077 — but that let UNPOWERED advertisers into Top, superseded below.)
- **FINAL tier = POWER × CONFIRMED-LIFT 2×2 (2026-07-05, user: "powered-for-5% must be a hard limit for Top").** Score-band Top had 44 of 67 with `Powered-for-5%=No` — a current ghost-lift got un-testable advertisers into Top. Fix: **Top = `can_hit_ivr_5pct_8w='Yes'` AND `confirmed +`** (runnable for a rigorous 5% test AND proven lift); **Mid = powered-5% OR confirmed** (one, not both); **Low = neither**. Result **Top 28 / Mid 152 / Low 1,090** (1,270 eligible; 17 F4-excluded). **Accepted trade-off:** power is a hard Top gate, so value_score is now a WITHIN-tier quality rank, NOT a global high→low sort — 97 non-Top (mostly unpowered high-lift like **Axos +55% at MDE 5.19%**) outscore the lowest Top. The fundamental constraint: you can have only 2 of {power-gated Top, tier-grouped, score-monotonic}; user chose power-gate + grouping. Gruns (flat) is Mid; Axos (confirmed but unpowered) is Mid.
