...
- **FINAL design — lift-adjusted score + score-band tiers (2026-07-05, supersedes the "demote-to-Mid" gate above).** User wanted the value_score to read strictly high→low with the tier. Solution: (1) **fold measured lift into value_score** — a `confirmed +` lift adds +30 base + up to +15 scaled by relative magnitude (capped at rel=30%); flat/too-early/no-data add +0. New col `apriori_value_score` keeps the pre-lift number. (2) **Tiers become SCORE BANDS** on the lift-adjusted score: **Top ≥90 / Mid 60–90 / Low <60**. The 90 cutoff is set just above the max any unconfirmed advertiser can score (89.7 = Gruns, no bonus) → **Top is guaranteed all-confirmed AND the sheet is strictly monotonic** (verified: Top 90.0–126.9, Mid 60.0–89.7, Low 3.7–59.7; 0 Mids above any Top). Final tiers: **Top 67 / Mid 126 / Low 1,077** (still 1,270 eligible; 17 F4-excluded). This replaced the two-gate tier (which couldn't be monotonic — any tier rule based on something other than the score breaks high→low ordering). Gruns (89.7, flat) is now the top of Mid; Axos (+59% confirmed) is now Top. Chosen over keeping strict Top=21 because the future-MDE gate is moot for advertisers whose lift is already measured.
