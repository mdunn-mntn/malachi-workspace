...
def write_pass30_geo_narrow_only_sheet(wb: Workbook) -> None:
    """Pass 30 — primary permutation view. GEO-BROAD collapsed into default (US-only).

    Per user direction (2026-06-01): "If it's geo-broad, meaning all of USA, don't
    even include it as separated. We always filter to US as we only target US right
    now. So it's not helpful." Only GEO-NARROW-incl (state/DMA/city/ZIP) and
    GEO-NARROW-excl (geo exclusion) appear as geo axes.
    """
    if not PASS30_CSV.exists():
        return
    ws = wb.create_sheet("Pass 30 — primary view")

    ws.cell(row=1, column=1, value="Pass 30 — permutation matrix (PRIMARY VIEW — GEO-BROAD collapsed into default)").font = TITLE_FONT
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=19)

    note = (
        "8 axes (US-only campaigns are the default — GEO-BROAD-incl is NOT a separate column):\n"
        ...
        "  · GEO-NARROW-excl = any geo exclusion clause (carving a region OUT of the include set)\n"
    ...
    with PASS30_CSV.open() as f:
        ...
        for r in reader:
            ...
            apply(10, "Y" if is_true(r["has_geo_narrow_excl"]) else "")
    ...
    legend_entries = [
        (fill_mm_only,         "MM-only buyer targeting (no 3P/CRM clauses; may have narrow geo)"),
        (fill_mm_and_incl,     "MM + AND-include (real narrowing — 3P or CRM intersected with MM)"),
        (fill_mm_or_theater,   "MM + OR-include only (theater — bidder-inert under HHST > 0)"),
        (fill_mm_mixed,        "MM + both AND- and OR-include (mixed semantics)"),
        (fill_no_mm_targeting, "No MM but has 3P/CRM clauses (3P-only or CRM-only campaign)"),
