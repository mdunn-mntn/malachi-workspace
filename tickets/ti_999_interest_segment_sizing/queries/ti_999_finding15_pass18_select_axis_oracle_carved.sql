-- TI-999 Finding 15 Pass 18 — re-bucket with MNTN Select as 4th audience axis
-- and Oracle (DS1) carved out of 3P per dead-weight delivery finding.
--
-- Changes from Pass 17:
--   1. MNTN Select group {DS9, DS42} added as a 4th axis alongside MM / 3P / Advertiser CRM
--   2. DS1 Oracle removed from 3P set — Sean Yang (TI team, 2026-05-29) confirmed
--      Oracle is no longer in IPDSC. The 553 prospecting campaigns referencing it
--      positively are paying for dead-weight clauses.
--
-- Audience axes (per locked taxonomy 2026-05-29):
--   MM   = {DS13, DS19, DS38, DS46}
--   MNTN Select = {DS9, DS42}    -- DS42 has 0 expression refs but kept for symmetry
--   3P   = {DS17, DS18, DS35}    -- DS1 Oracle REMOVED (was in Pass 17)
--   CRM  = {DS4, DS8, DS47}
--
-- "Any signal" = positive OR negative polarity.
...
flags AS (
  SELECT p.campaign_id,
    -- MM (DS19 keywords, DS13 vertical, DS38 BUK queued, DS46 Fangorn)
    LOGICAL_OR(c.data_source_id IN (13, 19, 38, 46)) AS has_mm,
    -- MNTN Select (DS9 households, DS42 deal registry)
    LOGICAL_OR(c.data_source_id IN (9, 42)) AS has_select,
    -- 3P bought interest (Oracle DS1 carved out per Sean Yang 2026-05-29)
...
bucketed AS (
  SELECT ap.*,
    CASE
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_select,FALSE) AND COALESCE(f.has_3p,FALSE) AND COALESCE(f.has_crm,FALSE) THEN '16_MM_Select_3P_CRM'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_select,FALSE) AND COALESCE(f.has_3p,FALSE) THEN '15_MM_Select_3P'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_select,FALSE) AND COALESCE(f.has_crm,FALSE) THEN '14_MM_Select_CRM'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_3p,FALSE) AND COALESCE(f.has_crm,FALSE) THEN '13_MM_3P_CRM'
      WHEN COALESCE(f.has_select,FALSE) AND COALESCE(f.has_3p,FALSE) AND COALESCE(f.has_crm,FALSE) THEN '12_Select_3P_CRM'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_select,FALSE) THEN '11_MM_Select'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_3p,FALSE) THEN '10_MM_3P'
      WHEN COALESCE(f.has_mm,FALSE) AND COALESCE(f.has_crm,FALSE) THEN '09_MM_CRM'
      ...
      WHEN COALESCE(f.has_mm,FALSE) THEN '05_MM_only'
