-- TI-999 prospecting-only: top advertisers by stale-3P spend exposure WITHIN prospecting only.
...
prospecting_stale AS (
  SELECT * FROM flagged
  WHERE NOT uses_list_retargeting AND uses_stale_3p
),

per_advertiser AS (
  SELECT advertiser_id,
         SUM(impressions_30d) AS stale_impressions,
         SUM(total_spend_30d) AS stale_spend
  FROM prospecting_stale
  GROUP BY advertiser_id
  HAVING SUM(total_spend_30d) > 0
)
