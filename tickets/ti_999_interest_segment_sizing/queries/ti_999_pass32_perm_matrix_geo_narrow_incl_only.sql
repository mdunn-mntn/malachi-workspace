-- TI-999 Pass 32 — Permutation matrix with BOTH GEO-BROAD-incl AND GEO-NARROW-excl
-- collapsed into the default (only GEO-NARROW-incl survives as a geo axis).
--
-- User direction (2026-06-01): "GEO-NARROW-excl is likely not an issue since we're
-- only excluding a small number of people." Same logic as GEO-BROAD-incl: these are
-- small-effect axes that fragment the matrix without changing what the bidder targets.
--
-- 8 axes (down from 9 in Pass 30):
--   MM
--   3P-AND-incl, 3P-AND-excl, 3P-OR-incl
--   CRM-AND-incl, CRM-AND-excl, CRM-OR-incl
--   GEO-NARROW-incl  = at least one positive geo clause references sub-country (state/DMA/city/ZIP)
--
-- GEO-NARROW-excl is now collapsed: MM + GEO-NARROW-excl → MM. MM + CRM-AND-excl
-- + GEO-NARROW-excl → MM + CRM-AND-excl. Same for every parent pattern.
...
classified AS (
  SELECT
    ...
    -- ONLY GEO-NARROW-incl survives. GEO-BROAD-incl and GEO-NARROW-excl are dropped.
...
SELECT
  COALESCE(NULLIF(ARRAY_TO_STRING(
    ...
    ' + '
  ), ''), '(no buyer targeting — US-only or narrow geo, no MM/3P/CRM)') AS pattern,
  has_mm,
