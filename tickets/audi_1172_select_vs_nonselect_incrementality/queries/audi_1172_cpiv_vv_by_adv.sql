-- AUDI-1172 CPIV/CPIA PER ADVERTISER x product (same method + cohort as the pooled version,
-- audi_1172_cpiv_vv_correct.sql, but grouped by advertiser_id too).
-- Verified Visit = clicks+views+competing_views (AUDI-1070). incremental = VV x lift/(1+lift).
WITH aids AS (
...
pipe AS (   -- volume-weighted (raw-count) rates -> rel_lift, per advertiser x product
  SELECT advertiser_id, product,
  ...
  FROM cg GROUP BY advertiser_id, product
),
af AS (     -- Reporting Verified Visits + Conversions + spend, per advertiser x product
  SELECT c.advertiser_id, c.product,
  ...
  GROUP BY c.advertiser_id, c.product
)
SELECT p.advertiser_id, adv.company_name, p.product,
       p.vis_t/p.n_t AS rate_treatment, SAFE_DIVIDE(p.vis_h, p.n_h) AS rate_holdout,
       SAFE_DIVIDE(p.vis_t/p.n_t, SAFE_DIVIDE(p.vis_h, p.n_h)) - 1 AS rel_lift_raw,
       SAFE_DIVIDE(p.conv_t/p.n_t, SAFE_DIVIDE(p.conv_h, p.n_h)) - 1 AS conv_rel_lift_raw,
       af.vv_reported, af.conv_reported, af.spend
FROM pipe p
JOIN af USING(advertiser_id, product)
