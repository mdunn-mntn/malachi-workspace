...
**Next (computable without Matt):** measure pipeline→VV scaling factor from all_facts 7d visits; apply to `incremental_visits`; recompute pooled CPIV/CPIA on the reconciled basis; send Matt "here's what I did" for his nod. Resolved by us earlier: spend = total (media+data+platform, Select bundles platform into a gross CPM so media-only flatters PTV); denominator = raw-count `incremental_visits` (Matt confirmed); CPIV pooled-only (per-advertiser unstable, Select cheaper in 15/28 = concentration-driven); CPIA pooled-only. Full question list: `experimentation.md` §Ghost-bid lift. Outputs: `outputs/audi_1172_cpiv_raw.json`, `audi_1172_cpiv_by_adv.csv`.
