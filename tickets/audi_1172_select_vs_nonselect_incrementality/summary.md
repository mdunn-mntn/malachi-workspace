...
**Exec methodology one-sheeter + slide (2026-08-07, Kirsa Slack ask for the Mark conversation).** High-level ghost-bid explainer for a C-level audience (Kirsa: "what this new incrementality methodology is and how it works, not too in the weeds"; Hannah: info graphics; Malachi steer mid-build: **explain how it works, NOT what they saw — no Select results on the page**). Deliverables: `artifacts/audi_1172_incrementality_one_sheeter.html` (+`.pdf`, letter print-ready) and `artifacts/audi_1172_incrementality_slide.html` (16:9 single slide), both self-contained; Drive copies `AUDI-1172 Ghost Bidding One-Sheeter.pdf` + `AUDI-1172 Ghost Bidding Slide.html`. Content: attribution-vs-causation framing (eBay 4,100%→−63% anchor, matches the BER-2250 primer; positioned as adding to attribution, not replacing it), 4-step ghost-bid diagram (random 90/10 household split → identical auction valuation → ghost bid logged, never sent → visit-rate compare), 3 trust bullets, a generic "how to read a lift number" infographic (illustrative +12%, 100-vs-112 stacked bars), KPI band 10% / 1,100+ / $0. Chart palette validated (dataviz six-checks: `#1AC9AA` + `#0F6FBF`). 3-lens adversarial review (facts vs sources, exec-comms standards, cold exec reader) applied; mechanism corrections from it: ghost = **bid it would have PLACED** (wins never known — not "bid that would have won"), compare arm labeled **ad group vs holdout** (bid-grain ITT, not served-vs-ghost), seven-day visit window named as the conservatism, "every prospecting campaign" softened to "across prospecting campaigns", footer carries the plain-words ITT line. Results intentionally live only in the workbook the footer points to.
