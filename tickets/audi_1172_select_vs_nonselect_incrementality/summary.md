...
**Exec methodology one-sheeter + slide (2026-08-07, Kirsa Slack ask for the Mark conversation).** High-level ghost-bid explainer for a C-level audience (Kirsa: "what this new incrementality methodology is and how it works, not too in the weeds"; Hannah: info graphics). Deliverables: `artifacts/audi_1172_incrementality_one_sheeter.html` (+`.pdf`, letter print-ready) and `artifacts/audi_1172_incrementality_slide.html` (16:9 single slide), both self-contained; Drive copies `AUDI-1172 Ghost Bidding One-Sheeter.pdf` + `AUDI-1172 Ghost Bidding Slide.html`. Content: attribution-vs-causation framing (eBay 4,100%→−63% anchor, matches the BER-2250 primer), 4-step ghost-bid diagram (random 90/10 household split → identical auction valuation → ghost bid logged, never served → visit-rate compare), 3 trust bullets, Select +22% vs non-Select +4% bar pair with a "per 100 visits" read line, KPI band 5x / 27 of 35 / 1,100+. Chart palette validated (dataviz six-checks: `#1AC9AA` + `#0F6FBF`). 3-lens adversarial review (facts vs this summary, exec-comms standards, cold exec reader) applied: attribution positioned as complemented not replaced, "per 100" anchor promoted to body, "both products add visits" line so non-Select doesn't read broken, stale "Beeswax-only" caveat and estimator-inconsistent VR pairing kept OUT. **Note: the +22%/+4% headline is frozen to the shipped workbook; the rollup accumulates (drifting toward +23%) — re-check against the live workbook before the Mark meeting.**
