/* AUDI-1083 — MNTN Matched campaign classifier: paste-and-run test.
   Returns 200 sample rows of the classifier. Swap the final SELECT tail for the
   variants noted at the bottom (distribution / single campaign / active-only). */
...
FROM classified
ORDER BY mm_class, campaign_id
LIMIT 200;

/* ── Tail variants (replace the final `SELECT classified.* ... LIMIT 200;` above) ──
   Distribution by class:
     SELECT mm_class, tiers_reachable, COUNT(*) AS campaigns
     FROM classified GROUP BY 1,2 ORDER BY campaigns DESC;
   One campaign:
     SELECT * FROM (SELECT classified.*,
       (is_unmodified_mm AND mm_class='mm_flagship_fangorn') AS is_flagship
