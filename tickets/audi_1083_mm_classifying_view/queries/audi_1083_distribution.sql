...
SELECT
  mm_class,
  COUNT(*) AS campaigns,
  COUNTIF(tiers_reachable LIKE 'HI%')          AS reaches_hi,
  COUNTIF(tiers_reachable = 'PP·MI (no HI)') AS pp_capped,
  MIN(DATE(campaign_created)) AS earliest_created,
  MAX(DATE(campaign_created)) AS latest_created
FROM v GROUP BY mm_class ORDER BY campaigns DESC
