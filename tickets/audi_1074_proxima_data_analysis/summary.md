...
### Vendor responses (2026-08-25, Proxima via email thread — answers to the 9 questions)
**1. Flag "monotonicity violations" RETRACTED as a data defect — it is a DICTIONARY defect.** Proxima: the windows are DISJOINT BANDS, not nested: `_6mo` = past 6 months, `_12mo` = past **6-12** months, `_36mo` = past **12-36** months. The delivered dictionary says "in the past 12 months"/"past 36 months" (cumulative) — vendor's own doc contradicts their computation. Under band semantics, 6mo=true & 12mo=false is legal, so the 5-10% "violations" are expected behavior. Evidence: dictionary xlsx (cumulative wording) vs vendor email 2026-08-25 (band wording). Settling check if it matters later: per-customer flags vs observed in-file transactions restricted to each band. Consequence: any consumer of these flags MUST read them as bands; a "bought in past 12mo" audience = `_6mo OR _12mo`.
**2. Q6 reinterpretation:** the 54.73% of beauty-36mo-flagged customers with zero in-file beauty transactions is now EXPLAINED, not anomalous — `_36mo` = purchases 12-36 months ago, which is entirely BEFORE the 1-year file window by definition.
**3. Cadence (supersedes AUDI-935 "weekly standard"):** production = monthly, sometimes weekly; sub-weekly = custom scoping with commercial implications; eval = one-time drop + refreshes as needed. Freshness re-verdict: the ~2-day pipeline lag holds, but refresh cadence dominates staleness — at monthly refresh, mean purchase age at actionability ≈ 17d (15d cadence midpoint + 2d lag).
**4. Refresh semantics:** orders are RESTATED every delivery (full refresh, not append-only) — refund/chargeback states update in place; ingestion must be full-replace or merge-on-order_id.
**5. Brand names: permanently unavailable** ("commitment to the brands we partner with") — advertiser-roster overlap is structurally impossible; integration-cost row, not a fixable gap.
**6. IP fill grain reconciliation:** vendor's 60-80% claim is per-ORDER browser_ip fill; our 45% is customers-with-any-ip_mapping-row. Different grains, both can hold; settles only when browser_ip is redelivered.
