...
Databricks (under Repos)."""))
    ...
    nb.cells.append(md("""### 1a. Install runtime dependencies (Databricks only)

Databricks ML runtimes don't ship `google-cloud-bigquery` or `db-dtypes` (needed for
`.to_dataframe()` on BQ results). The cell below installs them and restarts Python.
Skip this cell on a laptop where deps are already installed."""))
