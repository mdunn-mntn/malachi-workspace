...
- **Q:** For each of the 15 DAGs, is the spilling stage's spill shuffle-side?
  **A:** Landed 2026-09-03: two are shuffle-side (vertical_size_monitor stages 11/17, guid_log_advertiser_id_dsc_id stages 13/24); nine map-side (fangorn_prospecting_scoring, ipdsc_ds_17, ipdsc_46/14/49_monitor, ipdsc_ds_13, fangorn_predictions_vertical, fangorn_household_predictions_vertical, advertiser_join); one BigQuery-read-side (ipdsc_ds_47); three no-spill (ipdsc_ds_14, guid_log_pivot_household_id_vertical_id, aug_log_ip); all handed to their respective mechanisms (AUDI-1273, AUDI-1275, code ownership, no action).
- **Q:** What partition count puts about 256 MiB per task in memory?
  **A:** vertical_size_monitor 600 (136.0 GiB in-memory / 256 MiB = 544 → 600, 232 MiB per task, 44 MiB shuffle read, sticks under AQE floor 32 MiB rule); guid_log_advertiser_id_dsc_id stages 13/24 compute 4100 and 3700 by in-memory bytes (1007.9 GiB and 907.6 GiB), versus AUDI-1269's 3400 (303 MiB in-memory per task). Decision D1: 1269 lands first at 3400, 1270 raises to 4100 only if spill persists.
- **Q:** Does the Dataproc Serverless runtime default `spark.sql.shuffle.partitions` to 200?
  **A:** No, 1000: confirmed on all four models that set nothing (ipdsc_ds_13, ipdsc_ds_14, ipdsc_ds_17, guid_log_advertiser_id_dsc_id), nine runs, event logs verified 2026-09-02.
- **Q:** When does adaptive coalescing undo a raised count?
  **A:** Only while two neighbouring partitions fit in the 64 MiB advisory: 1000 → 334 at 16.7 MiB each (ipdsc_ds_17), unchanged at 43 MiB (ipdsc_ds_13) and 51 MiB (ipdsc_ds_47). A count sticks while shuffle bytes / count >= 32 MiB; the gate is `shuffle_read_bytes / target >= 32 MiB`, preventing AQE from coalescing back.

## 7. Data Documentation Updates
Facts routed via `/capture` to knowledge/ and memory/:
- **reference_dataproc_eventlog_profiling.md:** three new mechanics from 31 parsed event logs: (1) target formula `max(shuffle_read_bytes, mem_spill_bytes) / 256 MiB` rounded to next 100, because shuffle bytes alone misses 5.3x expansion on wide string rows; (2) AQE coalescing threshold 32 MiB per partition (`shuffle_read / count >= 32 MiB` to stick); (3) Iceberg write reducers ignore `spark.sql.shuffle.partitions` and use their own `files.maxPartitionBytes` advisory (~512 MiB); (4) BigQuery connector reads report zero input bytes (stage type is read of streamed data, not HDFS input); (5) `.gstmp` gsutil temp files from sliced downloads arrive complete and parseable (no truncation).
- **project_airflow_optimizer.md:** appended 2026-09-03 update documenting the two tickets' joint outcome, the verdict per DAG, cross-ticket dependency (AUDI-1269 owns guid_log_advertiser_id_dsc_id stages 5/16 at 3400, AUDI-1270 stages 13/24 sizes to 4100 but defers if AUDI-1269 resolves spill), and routing of 9 DAGs to AUDI-1273 (input-side maxPartitionBytes), 1 to code ownership (ipdsc_ds_47 BigQuery read parallelism), 3 to no-action (ipdsc_ds_14/guid_log_pivot/aug_log_ip stopped spilling).
