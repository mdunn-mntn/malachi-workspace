...
## 5. Solution
**PR:** https://github.com/SteelHouse/airflow-ti/pull/1270 (opened 2026-09-03 PT after a fast-tier gauntlet: 2 skeptic findings, both refuted, 0 confirmed). Branch audi-1274-aqe-advisory-pivot at 26ff44d.

- Code: one builder line per model, `.config("spark.sql.adaptive.advisoryPartitionSizeInBytes", "16m")`, in `models/feature_store/feature_group_3_pivoted/guid_log_pivot_ip_vertical_id.py` and `guid_conv_log_pivot_ip_vertical_id.py` (line 54 in each). Committed 2026-09-02 on branch `audi-1274-aqe-advisory-pivot` in the dispatcher's worktree (subject `AUDI-1274: cap AQE coalesce target at 16m on the 2 guid pivot models`). Decorators, `dags/`, schedules untouched; `dags/model_task_config.json` regenerated by the dry run with zero diff.
...
- Landed 2026-09-02: branch committed, `artifacts/audi_1274_result_comment.txt` posted to Jira, status In Progress. Pending, dispatcher: `/pr_gauntlet`, PR against `main`, merge (Ryan), ledger `applied` stamp for `disk_spill:33` / `disk_spill:34`, then the post-merge log check (§3 steps 8 to 10). Done-when lands when both ledger keys read `resolved` after three quiet sweeps.
...
- **Q:** Does the TI-956 "extra installs" gotcha for `model_upload.py --dryrun` still apply?
  **A:** No, for airflow-ti main `825b07e`. `uv venv --python 3.11` + `uv export --no-hashes --only-group models` + `uv pip install -r` + `MNTN_SDLC_ENV=dev .venv/bin/python model_upload.py --dryrun` completes with no `ModuleNotFoundError`; `pyproject.toml` pins `pretty_html_table`, `matplotlib`, `sendgrid` in the `models` group. Marked superseded in `documentation/docs/airflow_ti_workflow.md`.
- **Q:** Does CI lint `models/` with ruff, and are the two files clean on main?
  **A:** No and no. `pr_model.yaml` runs no ruff step (only `pr_airflow_debugger.yaml` and `pr_spark_optimizer.yaml` lint, each scoped to its own `include/` package); ruff 0.16.1 reports `I001` and `DTZ007` on both files at HEAD before any edit.
- **Q:** How was the value syntax checked with no Java on this Mac?
  **A:** From the pyspark 3.5.3 wheel in the dry-run venv: `JavaUtils.byteStringAs` (in `spark-common-utils_2.12-3.5.3.jar`, not `spark-network-common`) accepts `b k kb m mb g gb t tb p pb`; `SQLConf$.class` carries the key and its `64MB` default literal. macOS `strings` refuses `.class` files, so the constant pool was read with a Python regex over the bytes.

## 7. Data Documentation Updates
The execute agent wrote nothing to `knowledge/` (masters are off-limits in the execute wave) and handed the facts back in its structured return. Routed by the dispatcher's `/capture AUDI-1274` sweep, 2026-09-02:
- `knowledge/memory/reference_dataproc_eventlog_profiling.md`: new section "AQE's coalesce floor on the batch fleet is registered cores at plan time, NOT the 64 MiB advisory size" (target = max(1 MiB, min(advisory 64 MiB, total / registered cores)) with `parallelismFirst` default true and `spark.default.parallelism` unset; 728 vs 800 proof; `shuffle.partitions` no-op above the core count; `advisoryPartitionSizeInBytes` is the lever; co-partitioned inputs coalesce on their summed per-index size so bytes / target is only the floor of the greedy merge; the optimizer parser does not see AQE nodes), the `SparkListenerExecutorAdded`-at-`StageSubmitted` event-log trick with the probe path, and the Spark 3.5.3 byte-string one-liner (`JavaUtils.byteStringAs` in `spark-common-utils`, suffixes `b k kb m mb g gb t tb p pb`, macOS `strings` refuses `.class` files). Keywords + `last_verified` updated.
- `knowledge/memory/reference_airflow_ti.md`: § Spark Config Placement gained "a builder `.config()` line NEVER reaches `dags/model_task_config.json`" (`compute.py:99-107`, dry run diff-clean, no JSON commit for builder-only PRs, rides the live `.py` read); the "must be regenerated" bullet now says decorator changes only; new bullet with the exact no-extra-installs dry-run recipe (`uv venv --python 3.11` / `uv export --no-hashes --only-group models` / two-line exit-0 output / gitignored leftovers); the ruff-scope bullet now covers `models/` (CI lints only the two `include/` packages) and the pre-existing `I001` / `DTZ007` on both guid pivot files. Keywords + `last_verified` updated.
- `documentation/docs/airflow_ti_workflow.md` § Local dev environment gotcha: TI-956 extra-install list marked superseded 2026-09-02 in place (kept as history), with the current recipe.
- `knowledge/memory/project_airflow_optimizer.md`: new dated section "AUDI-1274 landed; the guid pivot spill mechanism was the core-count floor, not the 64 MiB advisory size" (branch state, done-when, contradiction record with both evidence classes and the settling check, lesson for the detector's fix text, post-merge log timing); description + keywords updated.
- `tickets/audi_1194_optimizer_efficiency_crawler/outputs/audi_1194_hackathon_optimizations_2026_08_27.md` Row 10 / Row 11 evidence: dated correction lines appended (core-count floor, remedy unchanged, 3,100 to 4,000 task band); the original lines stay.
- `knowledge/memory/reference_sprint_skill.md`: new section "Lessons from the first run (sprint 8649)": plan agents overran the runnable-not-answered line (this ticket's plan agent among them); the execute wave was cut off by a session limit and re-dispatched with RESUME notes pointing at partial worktree diffs and §4-§5; plans are never posted to Jira.
- `self_review/self_review_2.md`: entry "AUDI-1274 — one config line on two models, and the spec's spill mechanism corrected from the event logs (2026-09-02)".
- Not routed (no home fits): nothing for `data_catalog.md`, `data_knowledge.md`, `mntn_business.md`, `experimentation.md`, `glossary.md`, `knowledge/decisions/`, or `MEMORY.md`.

## 8. Open Items / Follow-ups
- Dispatcher (commit and the Jira comment done 2026-09-02): run `/pr_gauntlet`, open the PR with `artifacts/audi_1274_pr_body.md`, stamp the ledger after merge, then the post-merge log check (§3 steps 8 to 10; pass band 3,100 to 4,000 pivot tasks, disk spill 0.0 GiB, every coalesced shuffle at about 16 MiB per partition).
