-- Baseline: production flag-off branch of population_histogram.sql, one hour, sources pinned, checksum instead of CREATE TABLE.
...
, temp_beeswax_bid_events AS (
    SELECT campaign_id, ip, conquest_score, conquest_score_ttl, time, household_score, advertiser_household_score
...
, temp_stacked AS (
    SELECT campaign_id, ip, conquest_score, conquest_score_ttl, time, household_score, advertiser_household_score FROM temp_beeswax_bid_events
    UNION ALL
    SELECT campaign_id, ip, conquest_score, conquest_score_ttl, time, household_score, advertiser_household_score FROM temp_mntn_bid_events
)
, temp_scored AS (
    SELECT campaign_id, ip, COALESCE(CASE WHEN conquest_score > 1 AND TIMESTAMP_SECONDS(conquest_score_ttl) > time THEN GREATEST(conquest_score, 0) WHEN household_score > 1 THEN GREATEST(household_score, 0) ELSE GREATEST(advertiser_household_score, 0) END, 0) AS hh_score
    FROM temp_stacked
)
, temp_aggregate AS (
