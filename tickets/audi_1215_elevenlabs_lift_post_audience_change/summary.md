...
- **Confirmed by Mike Dolt's account-team Slack recap (posted 6/30, resurfaced 2026-08-21 2:32 PM):** (1) audience swapped to "MNTN-suggested precision audience changes"; (2) **conversion window extended to 43 days** (explains the 43-day lookback in the holdout lineage; also means ATTRIBUTED pre/post carries a window-length confound — post got a LONGER window and still fell ~60%, so the attributed decline is conservative; the 7-day ITT window is unaffected); (3) site-visitor + converter blocks 30d → 90d. Outstanding at the time: CRM list upload (landed 7/1 as DS47 suppression) and Bombora audiences (never observed in the expression history — apparently not added).
...
**Customer-facing deck (built 2026-08-21 PM, per the working session):** `artifacts/audi_1215_presentation_deck.html` (+standalone, shared via githack), source `artifacts/audi_1215_presentation.md`, charts `artifacts/audi_1215_chart_{daily_lift,prepost_lift,frequency_lift}.png` from `generate_charts.py`. Headlines = relative lift; critique pass applied (reconciliation slide reframed as agreement, resume-the-campaign as ask #1, close pre-empts the significance question, holdout-baseline proof on the 6x claim). Client one-pager `artifacts/audi_1215_ghost_bidding_one_pager_client.{html,pdf}` (internal ticket reference stripped). Workbook Did Lift Change sheet reformatted: uniform percent pre/post columns, one-clause reads, multiplier dropped (user feedback: multipliers confuse readers).

**Deliverables (updated):** branded workbook now includes a How Ghost Bidding Works sheet (mechanism-only, from the AUDI-1172 one-sheeter; PDF copied to the Drive folder as Ghost Bidding One-Pager.pdf). ElevenLabs' June review deck filed at artifacts/audi_1215_elevenlabs_ctv_review_deck.pptx. Meeting audio (2026-08-21 14:10, Malachi/Matt/Alex) transcribing to meetings/audi_1215_01. Branded workbook `My Drive/Tickets/AUDI-1215/AUDI-1215 ElevenLabs Lift Pre Post Audience Change.xlsx` (10 sheets: lift tables, delta tests, holdout check, frequency, timeline, attributed panel, glossary, queries, method); this summary; Jira completion comment; Slack draft for Mike's thread.
