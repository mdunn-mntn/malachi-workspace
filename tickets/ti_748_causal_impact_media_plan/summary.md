...
**Key observations:**
1. **Every plan created before Feb 2026 has 25-26 publishers** (except Lighting NY at 16 — confirmed by Chris Addy: no override, natural pruning from lower budget/vertical with fewer networks clearing $0.50/hr spend capacity + old `min_allocation=1%` floor. Old config was `max_networks=18` initially, bumped to 25.)
...
| 4 | Deliverability score / publisher capacity table? | ✅ Answered (Chris Addy) | **Scores NOT in BQ** — computed transiently in memory. Stored as JSON in GCS: `media-plan-artifacts` bucket, path `media-plan/{version}/{advertiser_id}/{plan_id}/response.json`. BQ `core.media_plan_publishers` has only final allocations (name + percentage). Would need BQ sink or GCS JSON parsing for score-level analysis. |
...
| 6 | Can concentration be tuned? | ✅ Answered (Chris Addy) | Yes — `alpha` (softmax temp, default 5.0) is the main lever. Higher = more concentrated. Also `max_allocation` (12%), `min_allocation` (0.5%). Config change, not code change. Per-advertiser override not natively supported but easy to add as request-level param. Chris wants to make MediaPlanConfig runtime-updateable — enables alpha A/B AND other features (e.g., using config in select). |
...
| 8 | Why did Lighting NY get 16 publishers under old config? | ✅ Answered (Chris Addy, 2026-03-31) | **No per-advertiser override.** Old config had `max_networks=18` initially (later bumped to 25), and old `min_allocation=1%` (vs current 0.5%). Natural pruning: budget/vertical had fewer networks clearing the $0.50/hr spend capacity threshold, so allocation produced fewer publishers even though cap allowed more. The higher 1% min_allocation floor dropped more borderline networks. **Check their budget relative to peers — smaller budget concentrates into fewer viable networks.** |
| 9 | Are per-publisher scores queryable? | ✅ Answered (Chris Addy, 2026-03-31) | **Not in BQ.** Full score breakdown (score_semantic, score_performance_advertiser, score_quality, score_spendability, score_cpm_efficiency, score_scale, score_combined) exists in GCS JSON artifacts only: `media-plan-artifacts` bucket → `media-plan/{version}/{advertiser_id}/{plan_id}/response.json`. To query at scale: either add BQ sink or parse GCS JSONs. |
| 10 | Can old-config plans be refreshed? | ✅ Answered (Chris Addy, 2026-03-31) | **Yes.** Regeneration would produce plans under current config: `max_networks=15`, `min_networks=10`, `alpha=5.0`, `max_allocation=12%`, spend capacity filtering. Chris can trigger this for Boll & Branch and Tempo. |
...
5. ✅ **Follow up with Chris Addy (2026-03-31)** — All answered: (a) Scores not in BQ, stored as GCS JSON artifacts (`media-plan-artifacts` bucket); (b) Lighting NY = natural pruning from lower budget + old `min_allocation=1%`, no override; (c) Can regenerate B&B/Tempo plans under current config; (d) Alpha tuning per-advertiser easy to add as request-level override; (e) Chris wants to make MediaPlanConfig runtime-updateable (enables A/B + select integration)
