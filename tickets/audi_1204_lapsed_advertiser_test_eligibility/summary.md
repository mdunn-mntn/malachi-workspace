...
- **Whole-lapsed-cohort version — now has a business case, 2026-08-12.** Al Beretta asked whether he could run the assessment himself, because "if this goes well, we might end up trying to use this tool to pull back customers who've revenue churned." The self-service surface already exists (TI-1019's hosted MDE calculator) but embeds only the **879 currently-delivering** advertisers, so churned ones are absent. Its 12 fields map 1:1 onto this ticket's metrics query, so the work is a cohort run windowed per-advertiser on last-active day + a `lastActive` label + array merge + gist redeploy. Flagged to Malachi as a **Task** (defined deliverable, needs points/PMO/quarterly label); not opened pending his yes. Two open considerations: the gist URL is secret-but-public, and a ranked standing list likely beats a lookup tool for CS.
