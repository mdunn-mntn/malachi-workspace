...
| Group | n adv | IVR (median) | CPV (median) | ROAS (median)* |
|---|---|---|---|---|
| MM (gated) | 1,262 | 0.46% | $9.13 | 0.92 |
| MM (no gate) | 144 | 0.13% | $21.84 | 0.56 |
| MM restricted | 1,393 | 0.18% | $19.02 | 0.94 |
| 3P | 438 | 0.07% | $37.18 | 0.40 |

- Gated MM beats 3P ~6.6x on visit rate (0.46% vs 0.07%) and ~4x on cost-per-visit ($9.13 vs $37.18).
- Both restrictions hurt: MM (no gate) 0.13% and MM restricted 0.18% sit well below MM (gated) 0.46%.
  Restricted MM still beats 3P, and the intent gate is the single biggest lever.
- MM (gated) leads IVR and CPV in all 8 real verticals. Largest IVR gaps: CPG (17x), Gaming (8.3x),
  Auto (8.1x). Smallest: Education (3.2x).
...
- outputs/audi_1141_mm_vs_3p_scorecard.xlsx: shareable workbook (upload to Google Sheets). Tabs: Read me,
  MM vs 3P by vertical, Full scorecard, Overall, Campaign detail (advertiser names, pivotable), Queries.
  Rebuild: artifacts/audi_1141_build_xlsx.py.
- outputs/audi_1141_campaign_grain.csv, audi_1141_scorecard_overall.csv, audi_1141_scorecard_by_vertical.csv,
  audi_1141_advertiser_names.csv
