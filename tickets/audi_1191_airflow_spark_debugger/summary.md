...
- **Next (needs a decision):** the plan/metrics acquisition layer — Databricks Spark REST `/api/v1/applications/<app>/sql/<id>` (richest, per-node time+spill, key-free via OAuth) vs plan-text-from-driver-log (cheapest, both engines, no timing). Then a succeeded-DAG crawl → cross-DAG optimization backlog (the "check every DAG" vision).
