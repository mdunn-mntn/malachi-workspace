...
- `airflow_debugger/signatures.py` — 21-signature deterministic taxonomy classifier; most-specific-first (e.g. `TABLE_OR_VIEW_ALREADY_EXISTS` beats generic `AnalysisException`). Carries a `programmatic_fix` flag ("yes/sometimes/no") that gates the deferred auto-PR. Offline unit tests pass: `airflow_debugger/tests/test_signatures.py` (14 cases + 4 ordering/edge guards).
