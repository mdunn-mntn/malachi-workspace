...
Generated 2026-08-26 from `on-call/airflow_logs/` — 25 days pulled, failures on 19 of them, 2026-08-02 to 2026-08-25.
...
|---|---:|---|
| `vertical_classification_api/response_tests` | 39 | The task never ran. The failure is `ddp_vertical_classification_api`; diagnose that. |
...
| `tpa_ipdsc_export/insert_file_audits` | 3 | The task never ran. The failure is `ipdsc_ds_67`, `tpa_export`; diagnose that. |
| `tpa_ipdsc_export/trigger_crm_match_rate` | 3 | The task never ran. The failure is `ipdsc_ds_67`, `tpa_export`; diagnose that. |
| `tpa_ipdsc_export/trigger_tpa_daily_metrics` | 3 | The task never ran. The failure is `ipdsc_ds_67`, `tpa_export`; diagnose that. |
...
| `augmentor_daily_gcs/merge_day_d0` | 1 | The task never ran. The failure is `augment_hour_d0`; diagnose that. |
...
The task never ran. The failure is `ddp_vertical_classification_api`; diagnose that.
```

Slack block:

```
*What*  *vertical_classification_api/response_tests* — no-cause-in-log
*Where*  `vertical_classification_api/response_tests`
*Why*  (no cause in this log) The task never ran. The failure is `ddp_vertical_classification_api`; diagnose that.
*How*  Diagnose the upstream task named above; this one never started.
```
...
```
*What*  *mntn_match_verticals_precache_v1_1/pre_cache_verticals* — no-cause-in-log
...
*How*  Check node capacity and image-pull time for that pod, not the task's code.
```
...
```
*What*  *tpa_ipdsc_export/tpa_export* — no-cause-in-log
...
Dataproc/GCE could not get machines in the zone (transient GCP stockout). Usually self-recovers in ~1-2h; autozone re-picks. Delete any lingering ERROR cluster (it self-blocks the retry on quota), then re-ru…
Not a code fix (compute/infra or upstream).
...
```
*What*  *fangorn_inference_pipeline_run/inference_pipeline* — infra/zonal-stockout
...
The task never ran. The failure is `ipdsc_ds_67`, `tpa_export`; diagnose that.
```

Slack block:

```
*What*  *tpa_ipdsc_export/insert_file_audits* — no-cause-in-log
*Where*  `tpa_ipdsc_export/insert_file_audits`
*Why*  (no cause in this log) The task never ran. The failure is `ipdsc_ds_67`, `tpa_export`; diagnose that.
*How*  Diagnose the upstream task named above; this one never started.
```
...
```
*What*  *tpa_ipdsc_export/trigger_crm_match_rate* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/trigger_tpa_daily_metrics* — no-cause-in-log
...
The database rejected the credential itself, which is not the same as a missing grant: the password is wrong, rotated, or the secret the job reads is stale. Check when the secret last changed against th…
Not a code fix (compute/infra or upstream).
...
```
*What*  *bottom_up_keywords_pipeline_run/training_pipeline* — auth/database-credential
...
*Why*  (matched signature) The database rejected the credential itself, which is not the same as a missing grant: the password is wrong, rotated, or the secret the job reads is stale. Check when the secret last changed against the last green run before touching the job.
*How*  Not a code fix (compute, infra or upstream).
```
...
```
*What*  *databricks_guid_geos/run_databricks_job* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/ipdsc_ds_67* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/ipdsc_geo* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/trigger_mntn_id_export* — no-cause-in-log
...
```
*What*  *audience_intent/intent_score_map* — no-cause-in-log
...
*How*  Check whether it already retried; the worker died before the task could raise.
```
...
```
*What*  *audience_intent/wait_for_ipdsc_geo* — no-cause-in-log
...
*How*  Check whether the awaited object landed, and for a control-plane event at the last timestamp.
```
...
The task never ran. The failure is `augment_hour_d0`; diagnose that.
```

Slack block:

```
*What*  *augmentor_daily_gcs/merge_day_d0* — no-cause-in-log
*Where*  `augmentor_daily_gcs/merge_day_d0`
*Why*  (no cause in this log) The task never ran. The failure is `augment_hour_d0`; diagnose that.
*How*  Diagnose the upstream task named above; this one never started.
```
...
```
*What*  *fangorn_inference_pipeline_run/wait_for_challenger_features* — no-cause-in-log
...
```
*What*  *hashed_email_ds_26_signals/populate_hem_data_ds_26* — no-cause-in-log
...
```
*What*  *hashed_email_guid_log_signals/populate_hem_data_ds_23* — no-cause-in-log
...
```
*What*  *site_network_hourly/site_network_hourly* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/ipdsc_ds_17* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/ipdsc_ds_35* — no-cause-in-log
...
```
*What*  *tpa_ipdsc_export/tpa_export_enrich* — no-cause-in-log
