...
result: "CLOSED Done 2026-08-25, no AUDI build: empties resolve via MemDB dedup work + MNTN-ID migration, score delivery via AP waterfall migration (bidder stops reading AUDI's advertiser/campaign scores). Meeting 04 cancelled, resolved in-thread same day"
...
- **Meeting 04 scheduled for 2026-08-26, 1 hour: Eric + Zach + Ryan Kleck + Wei + Malachi** — re-run the same questions with the actual owners. Transcribe as `ti_1016_04_*` when done. **CANCELLED 2026-08-25: everything resolved in the Slack thread the same afternoon (all five questions answered, ownership routed, resolution by in-flight work).**
- **Eric (Slack, 12:10) — read-before-write CLOSED, and the metric reframed:** "The goal is to do a net reduction in scylla REQUESTS, not writes" — any read-before-write scheme adds pressure (read + possible write vs just write) unless fronted by a Redis cache, which worked in the past but costs ~$20k/mo; they chose to scale Scylla up instead. This largely answers agenda Q4 (the crate's dedup helpers exist but wiring them is counterproductive by Eric's own metric without Redis — residual question is only why the helpers were built) and eliminates Ryan's read-first proposal (Slack 11:47). Consequence: **upstream suppression is the only lane that reduces requests.** The success metric for the spike is Scylla requests/sec, not writes/sec.
- **Zach Schoenberger (Slack, 13:31) — scope ruling + Q5 CLOSED:** "this has literally 0 to do with anything scores. there's no aspect of this that is relevant to Alyson/Matt/Ryan/Wei." Q5: "no, we should not be stopping that [RTC DS13 publish]. it has nothing to do with empties." Implication: the producer-side fix is MemDB/TMUL (Zach's), the consumer is Eric's, and AUDI's role narrows to coordination + sizing — the spike's "work lands on TI/AUDI" premise is now disputed and must be settled at meeting 04, along with Q1-Q3 (all Zach's).
- **Zach (Slack, 13:35) — Q2 answered NO, with visible fatigue:** "i am beyond tired of this question... No. you cannot just ignore one or the other." As asked (can the topic as-is carry everything) the answer is no — Kafka today is a subset and both paths are required, matching his June "subset of the full dump" line. UNRESOLVED NUANCE: Eric's doc proposes ADDING change-only emission so the topic becomes complete (additive, dump untouched) — Zach's "no" may be answering the as-is question, not that proposal. If raised at meeting 04, frame it as Eric's additive design, never as re-asking this question.
- **Zach (Slack, 13:42) — ownership settled narrower still:** after Malachi's clarity note (brought to AUDI as a scoring issue; it isn't; different "scores"), Zach: "it is a question between Eric and AP. mainly Daniel/Jaime/Mike Dolt/me there." So the owning conversation = Eric ↔ AP (Daniel, Jaime, Mike Dolt) + Zach; AUDI/ID and Ryan/Wei are out; Malachi + Matt context-only. AUDI-1016's "work lands on TI/AUDI" premise is dead — the spike's remaining value is handoff: the verified problem statement, Eric's doc, the measured numbers (92.9%, 402k/s, 57% BQ), and the repeat-vs-first-empty distinction, delivered to that group.
- **HANDOFF COMPLETE (Slack, 14:05-14:08):** Malachi routed the fix to Eric ↔ Mike Dolt (AP) + Zach; AUDI formally out ("we do scores, but not the scores that affect this"). Eric's position: cares but won't "fight with everyone about it"; boss on vacation; has "carte blanche to run this until it blows up in my face." NEW LEVERAGE FACT: **DPlat wants the bidder to stream INTENT scores; Eric refuses until segments are fixed** — segments fix is his precondition, which gives DPlat a reason to push Zach/AP. Malachi named Eric's stakeholders (final roster, Slack 14:10): **scotty, Dustin Niehoff (DPlat), Zach Schoenberger, Mike Dolt, Jaime Mutale, Daniel Hartnett** (resolves Zach's "Daniel/Jaime" = Hartnett/Mutale); Eric agreed to state the conflict in the group chat so everyone can sync at once, and Malachi advised explaining the full dependency loop (DPlat wants X → X needs Y → Y is owned by Z). AUDI-1016's remaining step: close the spike as verified-and-routed once that sync lands.
- **Eric opened the AP conversation (Slack to Mike Dolt, 14:17):** origin confirmed — DPlat approached Eric at mountain meet about streaming membership-consumer consumption; his reply was that intent-score dumps don't matter to him, fixing segment scoring does (92% empty writes every 4h forever); he attached his doc and framed the consolidation as segments-streaming-first, which then frees up intent-score streaming for DPlat. Handoff is now live in AP's court.
- **POST-CLOSE TLDR (Eric, Slack 2026-08-26 13:23-13:30, on WHY neither issue needs work):** (1) Empties: "most of the segment updates will be coming in via mntn-id... mntn-id will provide DELTA updates instead of full updates, so this should go away (particularly for empty items)." Open residual he dislikes: "some ip address based stuff will remain effectively forever." (2) Score delivery: "advertiser (I believe) scores will be moved to a kafka topic and that entire flow will go away" — attributed to **Zach** ("he and Ryan talked about it"), heavily hedged ("I mostly followed the TL;DR... not an area I fully follow"). CONTRADICTION APPENDED, not resolved: Zach 08-25 described the score migration as AP re-serving the waterfall in segment_scores; Eric 08-26 describes Zach moving advertiser scores to a Kafka topic. Could compose (AP source + Kafka transport) or conflict; evidence classes: Zach firsthand-authoritative vs Eric secondhand-hedged. Settles via Zach or the eventual design doc; matters mainly for the Alyson follow-up ([[project_ap_score_feed_migration]]).
- **RESOLUTION (Eric, Slack 15:53-56):** after direct clarity with Zach, "we actually don't need to do very much here at this point." Issue 2 (score delivery) "is going to go away" (the AP waterfall migration subsumes it); issue 1 (empties) "is going to be vastly reduced as we move to mntn-id." NEW STRUCTURAL FACT: the **MNTN-ID migration is the expected fix for the empty-write volume** (household keying moves off bare IP), on top of Zach's ongoing dedup work. Both tracks resolve via in-flight work; no new build needed from anyone. AUDI-1016 spike is closeable: verified, sized, routed, resolved-by-roadmap.
- **Dustin Niehoff (Slack, 14:44-45) — DPlat's actual ask lands partly ON AUDI:** "What I told him [Eric] was that the intent scores needed to be an INCREMENTAL drop instead of a full drop — which is why AUDI got linked into it." So AUDI is out of the segment-empties problem but IN for intent-score delivery: DPlat wants AUDI's daily full intent-score dump replaced with incremental/delta drops (the mirror of the segments fix, on the pipeline AUDI owns and that pages Malachi). Also: "If AP takes it up, they can certainly reduce the extra sends of the empty IPs." Malachi's routing read, which Dustin accepted: intent-score streaming is a TGT/AUDI request, not AT/AP's. **Likely future AUDI ticket: incremental intent-score drops.** Dustin follow-up (14:46): he doesn't care who does what; **Forrest is already looped in for RTC**. No formal owner assignment from DPlat — the incremental-drop ask stays informal until someone files it.
- **Zach (Slack, 14:26) — AUTHORITATIVE score-flow map, and a fact that IS AUDI-relevant:** the two workstreams are "100% independent unrelated items." Score plumbing today: bidder loads **advertiser + campaign scores from AUDI** (the intent-score GCS dumps) and **segment scores from AP** (mostly RTC, for over a year). In-flight plan: **AP now consumes AUDI's advertiser/campaign scores, will test sending down segment scores carrying the same waterfall score, and the bidder then STOPS reading the advertiser/campaign scores AUDI sends.** None of this touches empty segment updates. Terminology resolved: Scylla `advertiser_scores`/`campaign_scores` ← AUDI; `segment_scores` ← AP. **Flag for AUDI proper: the bidder dropping AUDI's score feed is a consumer-migration of OUR pipeline — confirm Alyson/team know and that the waterfall-through-AP path preserves the scores' semantics.**
- **Zach (Slack, 13:41) — Q1 and Q3 answered:** Q1: "there is already ongoing work that should help reduce duplicates being sent downstream" — WHAT work, whether it covers the 92% duplicate-empty class, and its timeline are unknown; this is now the central meeting-04 question. Q3: "literally everyone. an empty list is what defines an id as no longer being targeted... no different than A becoming A-1" — he is defending the FIRST empty (the removal signal), which nobody disputed; the actual question was repeat re-sends of the same empty every 4h. The repeat-vs-first distinction still needs a careful, non-confrontational clarification at the meeting.
- **Redis-cache post-mortem detail (Eric↔Zach Slack, 13:20-13:28):** the rejected ~$20k/mo cache was cpu + memory + throughput combined — many small boxes because cpu couldn't keep up; memory ended up above the ~100GB Zach expected; replicas would be wanted to avoid a death spiral on cache loss. Scaling Scylla was cheaper. Zach had never heard the outcome before this thread.
...
#### Self-serve code verification (2026-08-25, GitHub source — settles several open items)

Read directly from `SteelHouse` repos after Ryan's Slack pointer:

- **TTL conflict #1 RESOLVED — both numbers are real.** `rtb-rs/crates/rtb-scylla-models/src/household_profiles.rs`: `DEFAULT_TTL_SECS = 604800` (**7 days**, the per-write `USING TTL`) while the table DDL in the same file says `WITH default_time_to_live = 2592000` (**30 days**, the table default Eric remembered). Per-write TTL governs cells written with `USING TTL`; the table default applies only to writes that don't set one.
- **The shared crate ALREADY implements consumer-side dedup** — supersedes the doc's "BID-3440 read-before-write was reverted" narrative: `SELECT_SEGMENT_WRITE_STATE_STATEMENT` + `SegmentWriteStateRow::matches_segment_write` (read-before-write unchanged-detection; a test comment says null-compares-equal-to-empty "or every empty record would rewrite forever") + `UPDATE_SEGMENT_EPOCH_STATEMENT` (epoch-only bump when data is unchanged with ample TTL — no full rewrite). Module docs also describe segments arriving via **Kafka** and intent via GCS, and UPDATE-with-map-append ("No tombstones") in place of the doc's REPLACE-with-tombstones. The doc describes an older state of this code. **Unknown: whether this crate version is deployed in the prod consumer, and how much of the 400k it already absorbs — ask Eric.**
- **Sweep timestamp RE-VERIFIED in source (membership-db `etl/src/service/mod.rs`, `process_update_stream`):** `epoch = current_time_us()` is captured ONCE per refresh run/stream (also embedded in the output filename), then stamped into every record via `MembershipUpdateLog::from_update_response(resp, epoch_sec, epoch_sec)` with `us_as_sec` truncation. So every record in a sweep carries the run-start timestamp, matching the BQ copy's constant per-sweep stamp. Consumer half verified via the crate: `epoch_secs_to_cql_micros(record epoch)` becomes the CQL write timestamp. Still doc-sourced only: the Kafka activity path's last-activity/day-truncation behavior.
- **Q8 RTC RESOLVED mechanically.** `ip-vertical-classification/app/handlers/vertical_lookup_handler.py` `_create_membership_update_message`: on every vertical lookup RTC publishes an `UpdateRequest` to Kafka topic **`vertical-classifications`** (INTO MemDB, not to the bidder consumer) carrying DS13 (`dataSources[13]`) with `categoryId` = vertical id + epoch, and a `scores["rtc"]` entry with **score=10000 constant** (the RTC gate value in `model_params`), `delta=True`. So Ryan's removal offer would switch off the real-time conquest signal — a product decision for Ryan/Wei, not a fix for the empties. It does not emit empty records; at most it keeps IPs looking active in MemDB (fresh epochs → more sweep/Kafka emissions).
