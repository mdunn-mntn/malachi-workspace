...
--     A4: Production audit table (A4a CREATE, A4b INSERT, A4c preview, A4d validate)
...
SELECT
    ...
    SUM(CASE WHEN el_joined = 1 AND bid_ip = el_ip
        THEN 1 ELSE 0 END)                                         AS bid_eq_el,
    ROUND(100.0 * SUM(CASE WHEN el_joined = 1 AND bid_ip = el_ip
        THEN 1 ELSE 0 END)
        / NULLIF(SUM(el_joined), 0), 2)                             AS bid_eq_el_pct,
    ...
    ROUND(100.0 * SUM(CASE WHEN el_joined = 1 AND el_ip = cp_ip
        THEN 1 ELSE 0 END)
        / NULLIF(SUM(el_joined), 0), 2)                             AS el_eq_cp_pct,

    -- Mutation at redirect (bid=el but el!=cp)
...
SELECT
    ...
    (el.bid_ip = el.vast_playback_ip
        AND el.vast_playback_ip != cp.ip)       AS mutated_at_redirect,
    -- Classification
...
LEFT JOIN `dw-main-silver.summarydata.ui_visits` v
    ...
    AND DATE(v.time) BETWEEN '2026-01-28' AND '2026-02-17'  -- ±7 days from CP range
WHERE DATE(cp.time) BETWEEN '2026-02-04' AND '2026-02-10';


-- A4c: SELECT preview (run this first to validate before INSERT)
-- Same logic as A4b but returns results instead of inserting.
...
-- A4d: Validation summary (run after INSERT to verify counts match A1)
-- Should produce identical totals to A1 scale run results.

SELECT
    advertiser_id,
    COUNT(*)                                                        AS total_rows,
    SUM(CASE WHEN el_matched THEN 1 ELSE 0 END)                    AS el_matched,
    ROUND(100.0 * SUM(CASE WHEN el_matched THEN 1 ELSE 0 END)
        / COUNT(*), 2)                                              AS el_match_pct,
    SUM(CASE WHEN vv_matched THEN 1 ELSE 0 END)                    AS vv_matched,
    ROUND(100.0 * SUM(CASE WHEN vv_matched THEN 1 ELSE 0 END)
        / COUNT(*), 2)                                              AS vv_match_pct,
    SUM(CASE WHEN mutated_at_redirect THEN 1 ELSE 0 END)           AS mutated,
    ROUND(100.0 * SUM(CASE WHEN mutated_at_redirect THEN 1 ELSE 0 END)
        / NULLIF(SUM(CASE WHEN el_matched THEN 1 ELSE 0 END), 0), 2) AS mutation_pct,
    SUM(CASE WHEN cp_is_new THEN 1 ELSE 0 END)                     AS ntb,
    SUM(CASE WHEN NOT ntb_agree THEN 1 ELSE 0 END)                 AS ntb_disagree,
    SUM(CASE WHEN is_cross_device THEN 1 ELSE 0 END)               AS cross_device
...
SELECT
    advertiser_id,
    COUNT(*)                                                        AS ntb_vvs_checked,
    -- Truly NTB: no prior conversions on either IP
    SUM(CASE WHEN has_prior_cp_ip = 0 AND has_prior_bid_ip = 0
        THEN 1 ELSE 0 END)                                         AS truly_ntb,
    ROUND(100.0 * SUM(CASE WHEN has_prior_cp_ip = 0 AND has_prior_bid_ip = 0
        THEN 1 ELSE 0 END) / COUNT(*), 2)                          AS truly_ntb_pct,
    -- Mutation-caused misclass: bid_ip has prior but cp_ip doesn't
    SUM(CASE WHEN has_prior_cp_ip = 0 AND has_prior_bid_ip = 1
        THEN 1 ELSE 0 END)                                         AS mutation_misclass,
    ROUND(100.0 * SUM(CASE WHEN has_prior_cp_ip = 0 AND has_prior_bid_ip = 1
        THEN 1 ELSE 0 END) / COUNT(*), 2)                          AS mutation_misclass_pct,
    -- Both IPs have prior (was returning, flagged NTB by pixel anyway)
    SUM(CASE WHEN has_prior_cp_ip = 1 AND has_prior_bid_ip = 1
        THEN 1 ELSE 0 END)                                         AS both_prior,
    -- Only cp_ip has prior (returning by redirect IP but not bid IP)
    SUM(CASE WHEN has_prior_cp_ip = 1 AND has_prior_bid_ip = 0
        THEN 1 ELSE 0 END)                                         AS cp_only_prior
FROM ntb_check
GROUP BY advertiser_id
...
WITH stage3_cp AS (
    SELECT
        cp.ad_served_id,
        replace(cp.ip::text, '/32', '')     AS cp_ip,
...
vv_enrich AS (
    SELECT
        cp.*,
        replace(v.ip::text, '/32', '')      AS visit_ip,
        v.is_new                            AS vv_is_new,
...
win_match AS (
    SELECT
        cm.*,
        replace(w.ip::text, '/32', '')      AS win_ip,
...
el_match AS (
    SELECT
        wm.*,
        replace(el.ip::text, '/32', '')     AS el_ip,
