...
ROWS = [
    {"dag": "category_taxonomy", "task": "load", "jobs": "12", "slot_h": "109.4",
     "tib_billed": "1.2"},
    {"dag": "category_taxonomy", "task": "verify", "jobs": "12", "slot_h": "0.3",
     "tib_billed": "0.0"},
...
def _costs() -> list[bq_profile.TaskCost]:
    return [bq_profile.TaskCost(project="p", dag=r["dag"], task=r["task"],
                                jobs=int(r["jobs"]), slot_h=float(r["slot_h"]),
                                tib_billed=float(r["tib_billed"])) for r in ROWS]
...
def test_heavy_task_is_a_finding_and_unattributed_is_not() -> None:
    ...
    assert [f.key for f in by_name["category_taxonomy"].findings] == ["bq_heavy_task"]
...
def test_bq_sweep_never_resolves_spark_keys(tmp_path: Path) -> None:
    ...
    spark = JobReport(source="a.zstd", app_name="Populate fangorn.F",
                      findings=[OptFinding("shuffle_fetch_wait", "Stage 9 slow",
    ...
    states = {e["state"] for e in ledger.read(p)
...
def test_spark_sweep_never_resolves_bq_keys(tmp_path: Path) -> None:
    ...
    states = {e["state"] for e in ledger.read(p)
              if e.get("dag_id") == "category_taxonomy"}
    assert "resolved" not in states


def test_savings_keep_surfaces_in_separate_units(tmp_path: Path) -> None:
    """A resolved bq fix lands in by_surface['bq'], never in the spark exec-h headline."""
    p = str(tmp_path / "l.jsonl")
    ledger.record(bq_profile.reports(_costs()), "2026-08-20", path=p, surface="bq")
    ledger.mark_applied("category_taxonomy", "bq_heavy_task",
                        "https://github.com/x/pull/1", "2026-08-21", path=p)
    # The second dag keeps firing so the ledger keeps seeing sweep-dates advance.
    light = [bq_profile.TaskCost(project="p", dag="category_taxonomy", task="load",
                                 jobs=12, slot_h=9.0, tib_billed=0.1),
             bq_profile.TaskCost(project="p", dag="other", task="t",
                                 jobs=1, slot_h=200.0, tib_billed=2.0)]
