...
export function registerTaskTools(server: McpServer, api: TodoistApi) {
  server.tool(
    "todoist_list_tasks",
    "List tasks from Todoist. Can filter by project, section, label, or Todoist filter query (e.g. 'today', 'overdue', 'p1').",
    {
  ...
  server.tool(
    "todoist_create_task",
    "Create a new task in Todoist. Use due_string for natural language dates OR due_date for YYYY-MM-DD, not both.",
    {
    ...
    async (params) => {
      try {
        ...
        if (params.due_string) {
          ...
          (args as Record<string, unknown>).dueDate = params.due_date;
        }
  ...
  server.tool(
    "todoist_update_task",
    "Update an existing task. Use due_string for natural language dates OR due_date for YYYY-MM-DD, not both.",
    {
  ...
  server.tool(
    "todoist_move_task",
    "Move a task to a different project, section, or parent task. Provide exactly one destination.",
    {
    ...
    async (params) => {
      try {
        let moveArgs: Parameters<typeof api.moveTask>[1];
        if (params.project_id) {
          moveArgs = { projectId: params.project_id } as typeof moveArgs;
        } else if (params.section_id) {
          moveArgs = { sectionId: params.section_id } as typeof moveArgs;
        } else if (params.parent_id) {
          moveArgs = { parentId: params.parent_id } as typeof moveArgs;
        } else {
