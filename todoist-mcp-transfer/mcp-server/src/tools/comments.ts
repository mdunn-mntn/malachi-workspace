...
export function registerCommentTools(server: McpServer, api: TodoistApi) {
  server.tool(
    "todoist_list_comments",
    "List comments on a task or project. Provide either task_id or project_id (not both).",
    {
  ...
  server.tool(
    "todoist_add_comment",
    "Add a comment to a task or project. Provide either task_id or project_id (not both).",
    {
    ...
    async (params) => {
      try {
        ...
        let comment;
        if (params.task_id) {
          comment = await api.addComment({ content: params.content, taskId: params.task_id } as Parameters<typeof api.addComment>[0]);
        } else {
          comment = await api.addComment({ content: params.content, projectId: params.project_id! } as Parameters<typeof api.addComment>[0]);
        }
