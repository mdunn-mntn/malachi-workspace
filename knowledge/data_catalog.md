...
- **`uniques` is keyed on RAW `device_ip`, NOT the resolved `cost_impression_log.ip` (TI-1019, verified 2026-06-24).** `uniques`/`uniques_arr` is the HLL distinct of `device_ip` (raw IPv4 bidstream IP; IPv6 lives in `device_ipv6` and is dropped — PR #1033). This is the source of `graph.usersreached` ("Households Reached") and `graph.sitevisitors`. **Despite the "Households Reached" label it is NOT identity-graph / household-resolved** — it is the *rawest* IP count. For WGU (AID 31357) trailing-30d: `uniques`=32.1M, `site_visitors`=1.90M, IVR=5.92%. The matching `cost_impression_log` (billed/won log) count is only **15.7M distinct `ip`** → an IVR built as `site_visitors/uniques` (=5.92%) is ~½ the `cost_impression_log`-based per-served-IP IVR (=10.70%). **The gap is NOT a raw-vs-resolved IP cleaning step** (verified 2026-06-24): `ip` = `ip_raw` exactly in both `clickpass_log` (1.94M) and `impression_log` (21.2M), so nothing is being merged. It is a **cross-table difference** — three impression tables count three different universes on different IP keys and do not reconcile: `cost_impression_log` 356M rows → **15.7M** `ip` (= **won** bids / served); `impression_log` 935M rows → **21.2M** `ip` (`bid_ip` 21.6M) (= **all** bids, won or not); `impression_facts`/`graph.uniques` → **32.1M** `device_ip` (broader still — source TBD). Exact decomposition of the 32.1M needs the `impression_facts` SQLMesh model source (not in this workspace). Bottom line: `graph.uniques` (device_ip) is the loosest/largest of the three and over-counts the served universe — for a served-IP baseline use the **won** log (`cost_impression_log.ip`), since an IP we never served can't have driven a visit. Pick ONE IP field for any rate; do not assume `uniques` ≈ a distinct-served-IP count from CIL. **RESOLVED 2026-06-24 — the experiment unit is the resolved `cost_impression_log.ip`, NOT `device_ip`:** applying the production holdout hash (`MD5(advertiser_id:ip)` → bucket, 0–99 = holdout) to WGU's *served* IPs in `cost_impression_log`, **0 of 2,356,886 served IPs (one day) fell in holdout buckets** (would be ~10% if the holdout used a different field). Since `MD5(aid:resolved_ip)` is independent of `MD5(aid:device_ip)`, 0% proves the holdout/serving suppression is computed on the resolved `ip`. `clickpass_log` (VV attribution) also keys on the same resolved `ip` (+ carries `is_control_group`). **So for incrementality/IVR/MDE the correct denominator is the resolved served-`ip` count (CIL, 15.7M → 10.70% IVR); `graph.uniques` (device_ip, 32M → 5.92%) is the wrong grain.** The graph table can supply the correct numerator (`site_visitors` is resolved-IP, ~1.9M) but NOT the correct denominator — `uniques` is the only unique column and it's `device_ip`. **Zach Schoenberger (authority) confirmed:** holdout = targeting (on the targeting-system IP = served event-log `ip`); VV = attribution (no md5; matches event-log `ip` to guid-log `ip`). Both use the resolved event-log `ip`, never `device_ip`.
