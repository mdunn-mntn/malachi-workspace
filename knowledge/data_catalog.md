...
- **Per-day sizes across the sibling prefixes (measured 2026-09-04, AUDI-1321):** `openai_batch_input_formatted/dt=D/` **~1,014 files x ~40 MB = 40.3 GB**; `openai_batch_results/dt=D/` **~46 GB**; `product_categorization/dt=D/` **~4.0-4.3 GB across ~50 parquet files on a ~1,000-batch day** — that partition's size tracks BATCH VOLUME, not completeness (7.5 GiB for `dt=2026-09-03` at 1,004 batches, 12.47 GiB for `dt=2026-08-27` at 1,261 batches; measured 2026-09-05). Whole pipeline at a 48h retention window ≈ **100 GB** against OpenAI's **2.5 TB** file cap; our own footprint peaked near **200 GiB** while holding four days of inputs (2026-09-05).
- **The 2.5 TB cap is on a COMPANY-SHARED DEFAULT OpenAI project and our key lists only OUR uploads (confirmed by Malachi 2026-09-05).** This CORRECTS the 2026-09-04 entry that read the byte inventory as proof of ownership: with #308/#309 deployed the sweep enumerated everything visible to our key at **129 files / 4.2 GiB (0.2% of the cap)**, this pipeline 2.8 GiB and everything else 1.4 GiB — a bound on OUR footprint, not on the pool. Contradicting evidence: two consecutive sweeps read `Listed 6221 files holding 198.8 GiB, 7.8% of the 2.5TB project limit` and `6040 files holding 191.8 GiB` while `files.create` still returned a deterministic `400` exceeded-quota. `batch_submit` dies on the FIRST ~40 MB `files.create`, so it needs ~40 GB of headroom, not a clean account: **a green submit is NOT evidence the store is clean, and headroom depends on other teams' usage.** Memory `feedback_scoped_credential_cannot_prove_ownership`, `reference_mntn_matched_batch_pipeline`.
- **The receipt count is the ONLY completeness check for a `dt` (AUDI-1321, 2026-09-04).** `product_categorization__record_count` compares `product_categorization` against `openai_batch_results_joined` at the same `dt` and both are built from whatever was FETCHED, so a half-fetched day passes it: `dt=2026-09-02` passed at 46% fetched (468 of 1,014 receipts `was_downloaded`, `openai_batch_results` 20.3 GB vs 46.2 GB normal). Count `was_downloaded=True` receipts against the partition's receipt count, never against a sibling table.
- **The double-submission guard keys on `openai_batch_id.notna()`, not on `was_submitted` (read from source 2026-09-04).** `openai/openai_wrapper/batch_submitter.py::get_files_without_batch` builds `submitted_file_names` from `batch_submissions_df.query("openai_batch_id.notna()")`, so a receipt naming an EXPIRED or FAILED batch counts as submitted and its input file is never retried. Delete the receipts before re-running submit on a partially recovered day.
- **`was_submitted` is what gates `batch_fetch`, and it is NOT set by submitting (read from source 2026-09-05, AUDI-1321).** `fetch_results.py::get_batch_ids()` queries `was_downloaded == False & was_submitted == True`, and a fresh submit writes `was_submitted=False`, so a re-submitted day downloads NOTHING until `batch_transition` runs — `batch_transitioner.transition_to_in_progress()` selects `was_downloaded == False & was_submitted == False` and flips the flag for `PROGRESSED_STATUSES = {in_progress, finalizing, completed}`. After any re-submit clear `batch_transition` then `batch_fetch`, without downstream. Tell: a `cohort dt=...` line in a **`batch_fetch`** log means the receipts are still untransitioned (`assert_cohort_alive()` returns early once any row is flagged).
- **A quota-failed submit is monotonic (2026-09-05):** receipts written before the `400` survive and the double-submission guard skips them on retry, so a retry resumes rather than restarts (`dt=2026-08-28`: 866 of 1,241, then 1,241 of 1,241).
- **Task timings (2026-09-04):** `batch_submit` ~84 min for 1,261 input files, ~82 min for 1,004; `batch_post.product_categorization` ~31 min; `keyword_ddp_reporting.write_targeted_signal_ds_19` 1.7-1.8h.
...
- **Query discipline (MANDATORY):** 107 B rows / 366 TB → **always partition-prune on `time` AND cohort-restrict** (advertiser set + date bound), or read via **Databricks / GCS Spark** on the archive (`gs://mntn-data-archive-prod/guid_log/`). **Never full-scan** — a naive scan bills the whole 366 TB.
  - **Observed anti-pattern (AUDI-NTB, 2026-08-19, 620.7 GB billed):** a keying-disagreement diagnostic (`ip_vs_guid_keying_disagreement_30d`) declared a bound date (`DECLARE d DATE DEFAULT ...`) then filtered on `DATE(time) = d` (or a derived `dd` column) inside/after the CTE instead of filtering the raw `time` column directly (e.g. `WHERE time >= TIMESTAMP(d) AND time < TIMESTAMP_ADD(TIMESTAMP(d), INTERVAL 1 DAY)`). Wrapping or deriving from `time` before filtering defeats DAY-partition pruning on the underlying `guid_log_physical` table even when a date bound is declared. **Filter the raw `time` column with a `TIMESTAMP` range directly in the first `WHERE`** touching `guid_log`, not on a computed date column.
...
- **(16) `n_treatment` / `n_holdout` are DEDUPED IPs over the window, so `abs_itt` is ALREADY "incremental visits per household" (Matt Brorby, 2026-09-03).** The arm counts answer "did we bid on this IP at all across the window, yes or no" — one row per IP, not per bid event. So `abs_itt = rate_treatment - rate_holdout` is incremental visits per bid-on household and needs no recomputation to report it; a workbook showing only `rel_itt` is hiding a column it already has. **"Household" here means the deduped IP, NOT an identity-graph household** (Matt drew that distinction explicitly). This is consistent with "bid-grain ITT" in (4): bid-grain describes the *eligibility* population (everyone bid on, ~50% of whom win an impression per `ip_compliance`), not the row grain.
- **(17) Ryan Kleck maintains `_experiments` variants of the ghost-bid lift tables that bring in SPEND (Matt Brorby, 2026-09-03).** Same `lift__ghost_bid*` family, `_experiments` suffix, with extra join logic Ryan has been adding to attach spend to the lift rows. Matt's caveat: **"not 100% live and useful"** — treat the numbers as unvalidated, but **the join logic and calculations are the reference** for anyone solving the no-spend-below-campaign-grain problem in (12). Check these before hand-rolling another spend allocation.
- **(18) ⚠️ `dso.frequency_caps` IS CURRENT STATE — use `archives.frequency_cap_archives` for any historical window (TI-1313, verified 2026-09-03).** `dso.frequency_caps` holds one row per campaign with no history, and it churns: **12,197 of 125,672 rows carried an `update_time` inside a single day**, and 23,106 inside a 22 Jun to 31 Aug window. Joining it to a historical measurement window silently mislabels the campaign. On TI-1313's 433 campaign groups, **144 (33%) ran a different cap during the window than they carry today**, and the mislabelling flipped the finding: with current-state labels "No household cap" pooled highest and the attribute read p=0.0002; with in-window labels it is **second from bottom** (+7.84%) and p=0.0112. **`dw-main-silver.archives.frequency_cap_archives`** is the versioned twin (same columns plus `version`, `archive_create_time`, `datastream_metadata`) and **covered 433 of 433** at the window end. Pattern: `ROW_NUMBER() OVER (PARTITION BY campaign_id ORDER BY update_time DESC)` filtered to `update_time <= <window end>`, take `rn = 1`. Query: `tickets/ti_1313_incrementality_attributes/queries/ti_1313_fcap_in_window.sql`. **Assume every `dso.*` and `public.*` config dimension has this problem** — check for an `archives.*_archives` twin before attributing a config value to a past window. Note the archive is large: the naive scan is ~3.5 GB, ~6.6 GB when joined to a campaign list.
- **(14) ⚠️ `stratum_type='bid_count'` IS POST-TREATMENT — the strata are not comparable arms (TI-1313, verified 2026-09-03).** Bid count is measured AFTER the auction, so winning an impression (and the frequency cap that follows from it) changes which stratum a household lands in. The fingerprint is unmissable: on the 190-campaign TI-1313 population the **1-bid stratum pools to -5.52% [-8.16%, -2.80%], netting -22,529 incremental visits**, against 11+ at +14.21% / +184,558. **A ghost bid cannot suppress visits**, so a significantly negative bottom stratum is a readout of the selection rather than an effect. Corroborating: stratum ghost fractions run 0.0967 / 0.0894 / 0.0871 / 0.1061 against 0.0948 overall, holdout households are over-represented in the 11+ band (28.4% of holdout vs 22.3% of treatment), and 100 of 102 testable campaigns have different bid-count compositions across arms (p<0.05). **Never rank bid-count strata as if they were a frequency treatment**; the sheet also covers only 119 of 190 campaigns. Matt Brorby flagged this independently on sight, 2026-09-03.
- **(15) ⚠️ `stratum_type='score_band'` is mostly ONE BAND PER CAMPAIGN — it is a between-campaign cut, not a within-campaign one (TI-1313, verified 2026-09-03).** On the TI-1313 population, 215 band rows came from 176 campaigns and **152 of the 176 appeared in exactly one band**; only 24 contributed two or more. So pooling by band compares different campaign sets and inherits every campaign-level confound, which is what produced an apparent "all bands are equal" result. Restricting to the **37 pairs across 22 campaigns** where one campaign served both High and a lower band reverses it: -0.2169 pp absolute (campaign-clustered CI [-0.397, -0.082]) and -5.90% relative (CI [-12.2%, -1.1%]), with Mid 11/11, Unscored 7/7 and Max Reach 4/4 pairs negative. **Band is also not verifiably pre-treatment** despite the score being assigned before the bid: within-campaign holdout share is heterogeneous across bands in 21 of 24 testable campaigns, and treated households are over-represented among scored households (p=2.9e-70), i.e. exposure partly feeds the score. Roughly 13x milder than the `bid_count` imbalance in (14), but not clean. **Count the campaigns contributing 2+ strata before describing any of these as a within-campaign split.**
...
- **Scan envelope for the per-advertiser eligibility query shape: ~471.7 GB (AUDI-1213, dry-run 2026-09-03).** `tickets/incr_75_eligible_advertisers/queries/incr_75_advertiser_metrics.sql` = a CIL trailing-30d `(advertiser_id, ip)` rollup plus a 56-day `APPROX_COUNT_DISTINCT(ip)` pass, left-joined to `logdata.clickpass_log` and `summarydata.ui_conversions` (both 30d) and `summarydata.sum_by_advertiser_by_day` (12 months). CIL is the dominant scanner. It sits above the 5 GB `--dry_run` abort cap by design, so run it through `bq_run.sh` on the `us-central1` flat-rate reservation, never on-demand. The 2026-09-03 run returned **1,859 rows = advertisers that delivered in the trailing 30d ending 2026-09-03**, with no spend floor applied.

| Column | Type | Notes |
|--------|------|-------|
| advertiser_id | INTEGER | |
| campaign_id | INTEGER | |
| group_id | INTEGER | **CREATIVE group, NOT campaign_group_id** — joins `core_creative_groups.group_id` (200k/200k), never `campaign_groups.campaign_group_id` (0/200k). See join-key note above. |
| creative_id | INTEGER | Joins `bronze.integrationprod.creatives.creative_id` (true PK) → `length` = video seconds. Cannot fan out. |
...
| browser | STRING | |
| user_agent | STRING | |
| device_type | STRING | Beeswax device type: SET_TOP_BOX, CONNECTED_TV, MOBILE, PC, TABLET, GAMES_CONSOLE. **⚠️ `SET_TOP_BOX` DOES NOT MEAN A CABLE BOX (TI-1313, verified 2026-09-03).** MNTN buys no linear TV; the value is streaming players. On a partner-8 day slice (2026-08-25) SET_TOP_BOX is **83% Roku OS** (22.6M of 27.2M), then Android/Fire TV 3.7M, tvOS 0.1M; **Comcast X1 is 48,617 impressions, 0.18%**. `CONNECTED_TV` is the smart-TV built-in OSes: Tizen 6.0M, Vizio SmartCast 3.5M, Android 3.1M, webOS 1.9M, Vidaa. So the split is **streaming device against TV with the app built in**, not cable against streaming. In plain writing say "streaming devices like Roku and Fire TV" and "smart TVs" — the enum name misleads a reader into thinking linear cable is in the mix. |
| sh_device | STRING | MNTN device classification. **⚠️ UNUSABLE on the Beeswax leg (`partner_id=8`) — TI-1313, 2026-09-01:** NULL on ~72% of impressions, and where populated the domain is `COMPUTER`/`MOBILE`/`TABLET` — it NEVER takes the values `CTV`/`Display`/`Mobile`, so `COUNTIF(sh_device='CTV')` returns 0.0000 for every campaign. **For a per-campaign-group DEVICE MIX use `silver.summarydata.spend_facts.device_type` — it carries `campaign_group_id` natively and is the working source (TI-1313, 2026-09-02; supersedes the earlier note naming `sh_device`).** For the CTV-vs-display split use `bronze.integrationprod.campaigns.channel_id` (8=CTV, 1=Display) or `partner_ad_format`. **⚠️ The device dimension exists ONLY on `media_spend` (TI-1313, 2026-09-04):** `spend_facts` carries `media_spend`, `data_spend` and `platform_spend`, but a device split is meaningful only for media. On 433 groups over 22 Jun to 31 Aug 2026 that is $13,613,708 of the $34,596,466 total, so **any "% of spend to TV" from this table is a share of media spend, not of what the advertiser is billed** - label it as such. It reconciles to `summarydata.sum_by_campaign_by_day.media_spend` to the dollar at every `funnel_level`, so the table is complete, and unattributed device is negligible ($1,192 of $11.9M at stage 1). |
...
- **`uniques` (= `graph.usersreached`) is a channel-conditional IP-OR-GUID HLL over the SAME served `cost_impression_log` — NOT `device_ip`, NOT a broader universe (TI-1019, verified 2026-06-24 from the SQLMesh model + BQ reconstruction).** Full mechanism is in the corrected `impression_facts` entry above (search "channel-conditional IP-OR-GUID key") and in `data_knowledge.md` ("What the reporting graph table actually IS"). Key facts:
  ...
  - **The in-product MDE path no longer reads `usersreached` (moved 2026-08-25, confirmed from code 2026-09-03).** See the `Graph.IPUserSiteVisitorRate` / `Graph.IPUserConversionRate` entry under "graph.visits vs graph.sitevisitors (Reporting Metrics)" below. The mixed-key warning above is unchanged and still applies to anyone computing `sitevisitors / usersreached` by hand.
...
- **✅ STALENESS WARNING RESCINDED — this table is CURRENT (re-verified live 2026-09-02, TI-1313): `MAX(day)` = 2026-09-02, i.e. through today, zero lag.** *(Superseded the 2026-05-01 note that it lagged ~17 days at max=2026-04-14; that lag was transient and is gone. Any doc still carrying a "stops around 2026-05-01" warning for this table is WRONG.)* Still worth a one-line `MAX(day)` check before a recent-window read, but do NOT route around this table on staleness grounds. The downstream `silver.aggregates.agg__daily_sum_by_campaign` was empty since 2026-03-31 and has since been **deleted entirely** (2026-08-19); the hour-grain fact tables `silver.summarydata.{impression_facts, visit_facts, conversion_facts, spend_facts}` remain available but are no longer needed just for freshness.
- **Cost + why to prefer it over `all_facts` (TI-1313, 2026-09-02):** day grain and partition-pruned — a **71-day read across a ~900-campaign cohort billed 0.11 GB**, against multi-TB for the same question on `all_facts`, and it sidesteps the `all_facts` `hour`-is-DATETIME-not-TIMESTAMP gotcha entirely.
- **Metric composition, verified against this table (TI-1313, 2026-09-02):** Verified Visit = `clicks + views + competing_views` · Conversions = `click_conversions + view_conversions + competing_view_conversions` · Order value = `click_order_value + view_order_value + competing_view_order_value` · Spend = `media_spend + data_spend + platform_spend`. (Same industry_standard composition proved against `all_facts` in the CHAPI reproduction below.)
...
- **⚠️ Count DISTINCT `campaign_group_id`, never rows, and expect a tiny cohort (TI-1313, 2026-09-04).** The
  table holds one row per write, so a single group can carry 66 rows and both an active (`3`) and an
  inactive (`8`) status at once. Grouping by `media_plan_status_id` and `is_manual` made 3 groups look like
  5. Adoption is also small and young: of 899 ghost-bid campaign groups, 60 had any plan, and only **3**
  survived a 75% days-live filter over 22 Jun to 31 Aug 2026 - the plan campaigns are new, not badly
  measured (52 of the 60 sat inside the 7-11% holdout validity band). **Any media plan lift analysis needs a
  later window**, not a looser gate.
...
## bronze.integrationprod.core_advertisers_x_features + core_features — the display multi-touch toggle (TI-1313, verified live 2026-09-02)
- **Display multi-touch is an ADVERTISER-level feature flag. There is NO campaign-group display multi-touch toggle anywhere in BigQuery.** Read it as `core_advertisers_x_features` **`WHERE feature_id = 1 AND active`**; `feature_id 1` = **`MULTI_TOUCH_ENABLED`** per `core_features` (verified: 1=MULTI_TOUCH_ENABLED, 2=EGO_CAMPAIGNS_DISABLED, 3=PIXEL_UI_V2_DISABLED, 4=LEGACY_BUDGET_MINIMUMS). Exposed as `silver.core.advertisers_x_features` / `silver.core.features`.
- **A SECOND, independent setting exists:** `ui_advertiser_premier_attributes.multi_touch_disabled_by_default`. Check both before concluding an advertiser has multi-touch on.
- **Searched and empty (so stop looking):** `core_campaign_group_channel_margins`, `public.campaigns`, `public.campaign_groups`, `public.campaign_groups_raw`, every `ui_*` and `dso_*` config-override table, and every `integrationprod` column matching enabled/disabled/toggle/display/channel/stage — none carries a campaign-group display multi-touch flag.
...
| length | INTEGER | Video length in **SECONDS** (confirmed TI-1313, 2026-09-01) |
| caas_video_id | STRING | Creative-as-a-Service video ID |
| create_time / update_time | TIMESTAMP | |

- **Also exposed unprefixed as `dw-main-bronze.integrationprod.creatives` — `creative_id` is a TRUE primary key, so a CIL creative join CANNOT fan out (TI-1313, verified live 2026-09-02):** 5,768,212 rows / 5,768,212 distinct `creative_id` (5,768,167 on 2026-09-01 — it grows). **Creative-length composition per campaign:** join `cost_impression_log.creative_id → creatives.creative_id` and weight `length` by impressions. **⚠️ A clean 15-vs-30 binary is NOT available from delivery — 47% of campaigns run BOTH lengths.** Observed over 877 prospecting campaign groups (TI-1313, 2026-09-01): '30s only' 379 · 'Mixed, 15s-led' 297 · 'Mixed, 30s-led' 124 · '15s only' 77.
...
- **GEOGRAPHIC TARGETING CLASS is parseable straight out of `expression` — and this beats the pre-built classifier (TI-1313, 2026-09-01).** Filter `expression_type_id = 2 AND is_targeted = TRUE`, then read the `$.geos.where.value` blocks: **include blocks are those whose `$.op` != 'not'**. Resolve their `location_id`s against `dw-main-silver.geo.location_data.location_type_id` — **2=country, 3/4=DMA, 5=state, 6=city, 7=zip**; `location_id` **237 = US**. **⚠️ Radius targeting lives in a DIFFERENT node, `$.value.geo_radii`, and must be counted separately or every radius-only campaign is silently dropped.** Class precedence when a campaign uses several: radius > zip > city > state > DMA. Coverage 874 of 877 prospecting campaign groups. **Strictly better than `dw-main-silver.audience.mm_campaign_classifier.geo_narrowest_type`**, which covers only 732/877 (83.5%) and returns `'none'` for 124 of the 163 radius-targeted campaigns because it does not model `geo_radii` at all.

## dw-main-silver.audience.audiences (audience-level expression)
- **CRM EXCLUSION (customer-file suppression) is detectable from `expression` (TI-1313, 2026-09-01):** test for `"data_source_id":4` inside the `$.interest.exclude` block. **DS 4 = 'CRM'** per `dw-main-silver.audience.data_sources` (DS 63 = 'CRM Inclusions', unused). **276 of 877 campaign groups (31.5%) exclude their own customer file.**
- **⚠️ CAVEAT — `audiences.expression` is LIVE config, not point-in-time.** It carries no history, so it cannot tell you what the audience looked like during a past measurement window: 397 of those 877 (45%) had an `update_time` INSIDE the window. For as-of-date targeting use `silver.archives` (`audience_segment_archives`, `audiences_archives`, `audience_x_campaign_group_archives` — see §"Config-change AUDIT tables").
...
- **⚠️ MULTIPLE ROWS PER ADVERTISER — a naive join CARTESIAN-MULTIPLIES your output (TI-1313, 2026-09-01).** `SELECT DISTINCT advertiser_id, vertical_name` still returns both the parent and the sub row, so joining it to a campaign-group cohort doubles the rows: **2,224 campaign groups became 4,409**, with advertiser 31357 (WGU) appearing under both "Education" (parent) and "Colleges & Universities" (sub). **Fix: filter `type = 0` for the parent vertical (or `type = 1` for the sub), and belt-and-braces it with `QUALIFY ROW_NUMBER() OVER (PARTITION BY advertiser_id ORDER BY vertical_id) = 1`.** The type filter is sufficient today — verified 2026-09-02, **zero advertisers carry more than one `type = 0` row** — but the QUALIFY makes the join fan-out-proof if that ever changes.
...
- **`Graph.IPUserSiteVisitorRate` / `Graph.IPUserConversionRate`: the per-IP-user rate columns (column names read from `SteelHouse/gary-ql@cbae0e94` / `premier-ui@aaf65d59` source 2026-09-03; NOT yet confirmed against a ChAPI/R2 metric list or a query).** ChAPI/Graph exposes a visit rate and a conversion rate already expressed per IP user, so no hand-rolled `sitevisitors / usersreached` division is needed. These are the baseline the in-product incrementality Testing tab reads: `Graph.IPUserSiteVisitorRate` for a visits goal, `Graph.IPUserConversionRate` for a conversions goal, selected by the experiment's `goal_metric`. The product moved onto them on 2026-08-25, off `graph.usersreached` and the FPA `totalConversionRate` (see the `uniques` / `graph.usersreached` entries above). Window and denominator grain are inferred from the column names, not confirmed against a metric definition: `experimentation.md` records that the tab's rate is a different source over a different window and does not reconcile to our per-served-IP `cost_impression_log` rate to the digit, so do not treat these as a drop-in replacement for the CIL baseline.
