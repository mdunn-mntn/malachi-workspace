...
**⚠ CORRECTION — `bronze.external.household_scoring__prospecting_intent__v1` CANNOT SEE RECENT PARTITIONS; read the GCS day directly (AUDI-1208, verified 2026-08-18).** The registered table is defined `sourceUris = ['gs://household-scoring-prod/output/scoring/prospecting_intent/*.parquet']` with hive `mode: CUSTOM` and `sourceUriPrefix = 'gs://.../prospecting_intent/'` — **no `{key:TYPE}` schema in the prefix, which CUSTOM mode expects.** Symptoms measured the same day:
- `WHERE year='2026' AND month='08' AND day IN ('16','17')` → **0 rows** (and 46,537 slot-sec burned, no pruning).
- `SELECT year, month, day … LIMIT 5` → `2026 / 07 / 13`.
- `gcloud storage ls` → `year=2026/month=08/day=13..17/` each hold **20,000** `.snappy.parquet` files.
So the partition-value format was never the issue (it is zero-padded STRING: `month='08'`). **The file discovery is.** Any prior analysis that date-filtered this table for a recent window and got few/no rows was silently reading a stale file set — treat those results as suspect.
- **WORKAROUND (what to actually run):** an inline external table over one day's directory —
  `--external_table_definition="pi::PARQUET=gs://household-scoring-prod/output/scoring/prospecting_intent/year=2026/month=08/day=17/*.parquet"`. One day = **251,588,309,448 rows**, so aggregate with `APPROX_COUNT_DISTINCT` and never `SELECT *`.
- **MUST pass `--location=us-central1`** on any inline-external GCS query, or BigQuery defaults the job to the US multi-region and bills on-demand (the AUDI-1089 ~$875 footgun).
- **`gcloud storage ls` of a day prefix shows only the dir and `_SUCCESS`** — it looks empty. Count files with `ls '<prefix>/**' | grep '\.parquet$'`.
- Same GCS-vs-registered-table split applies to the sibling `prospecting_active_campaign_categories` under `…/output/data_aggregation/` (read it inline too).
...
- `bronze.external.household_scoring__prospecting_intent__v1` retains ~35 days active (10-day in BQ; deeper via raw GCS) — can't reconstruct >35d pool history. **CORRECTED 2026-08-18 (AUDI-1208): the BQ-visible window is not a rolling 10 days, it is STUCK.** On 2026-08-18 the registered table returned **0 rows for all of August 2026** while every GCS day partition held 20,000 full parquet files; a `LIMIT 5` came back with `2026/07/13`, i.e. ~5 weeks stale. See the CORRECTION note under "Production Fangorn score sources". `TI_835_prospecting_scores` GCS files were deleted; `ddp_vertical_classification_api` is API logs, not a membership table.
