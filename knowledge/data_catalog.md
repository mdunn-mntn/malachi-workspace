...
| first_day_visits through seventh_day_visits | INTEGER | Day-since-impression attributed (Verified) visit counts. SUM(first_day..seventh_day) = a **7-day-attribution VV count that matches the ghost-bid lift pipeline's 7-day-from-first-bid window** — use it to bridge Matt's pipeline `vis_treatment` to Reporting Verified Visits (the pipeline→VV scaling factor for CPIV; AUDI-1172, 2026-07-29). |
