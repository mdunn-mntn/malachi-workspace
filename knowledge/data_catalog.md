...
**You cannot rebuild the client UI/CHAPI number from these pre-summed columns.** CHAPI dedups each conversion to its single true last touch across display+CTV, so the UI (account **22.1×**) sits *between* the last-touch rollup (**17.3×**) and the naive lt+tv sum (**23.5×**). The 17.3→22.1 gap is the **CHAPI→ClickHouse engine** — a separate pipeline attributing ~**1.2–1.28× more** outcomes *uniformly* (broader multi-touch visit firehose + ~1.2× more orders), NOT a recoverable BQ column. To match CHAPI exactly, query CHAPI/ClickHouse (or do conversion-level dedup via `conversion_facts`), not the summary columns. **Spend & impressions DO match the UI to the dollar** (deterministic delivery); attributed visits/conv/rev only bracket it — but the **YoY direction is identical across every variant** (all UP).
