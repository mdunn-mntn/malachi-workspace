...
**Follow-up — AUDI-1072 recurrence (2026-07-29):** aud22 re-fired on the same class of defect (6 CGs). An IP resolves to a wrong DMA (e.g. 638) via `geo.network_locations` / `v_location_data_lat_long` hierarchy while the audience config and ad-buying-ui ipdata resolve the correct DMA (Columbus). AUDI-1072 was closed 2026-07-23 with a **ZIP-specific** fix (PR #1147 SteelHouse/sqlmesh addressed only ZIPs 43221, 45814) + Benny's aud22 Template-55 suppression + a "false positive" call — **not a source-data fix**. Verified at geo_version `1783900800`: those 2 ZIPs are now clean, but the `metro_id`-vs-`hierarchy` inconsistency still affects **5 other US ZIPs** (01223, 17371, 66214, 92545, 95245 — each has a `metro_id` whose metro `location_id` is absent from that ZIP's `hierarchy` array). The defect class was never fixed at source, so it resurfaces on each geo_version rebuild with different ZIPs. Durable fix is source-level (align `metro_id`, `hierarchy`, and `network_locations` for all ZIPs on every rebuild), not a per-CGID redeploy. Diagnostic: left-join `location_type_id=7` ZIPs to `location_type_id=3` metros on `metro_id`, flag where `metro_location_id NOT IN UNNEST(hierarchy)`.
