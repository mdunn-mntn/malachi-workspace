...
**To reconcile to the client UI / CHAPI (= what clients see), use `last_touch_* + last_tv_touch_*` — but know the naive SUM brackets-HIGH.** Avon Jan–May 2025: default revenue $1.27M → ROAS **17.3** (too low — excludes CTV); naive `last_touch + last_tv_touch` SUM $1.72M → ROAS **23.5** (too high — see below); **CHAPI/UI = 22.1 (the truth, in between).** Conversions: default 23,962 vs naive-sum lt+tv 32,772 vs **UI's implied ~30,600.** **The naive SUM double-counts** any conversion that has BOTH a display last-touch and a TV last-touch; **CHAPI dedups each conversion to its single true last touch** across display+CTV, so it lands between display-only and the naive sum. (Prospecting proof: display-only 5.0× < **CHAPI 9.4×** < naive-sum 12.5×.) To match CHAPI exactly you need conversion-level dedup (`conversion_facts`), not the pre-summed columns — the summary columns only **bracket** the UI. **If you report ROAS for any CTV advertiser off the default columns you understate it; off the naive lt+tv sum you overstate it.** Spend & impressions DO match the UI to the dollar (deterministic); attributed revenue/conv/visits only bracket it.
