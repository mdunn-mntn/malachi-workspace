...
**Follow-up — AUDI-1072 recurrence (2026-07-29):** aud22 re-triggered on the same class of defect (6 non-World-Cup CGs). An IP's impression geo diverges from the audience's targeted geo: the served location resolves to a wrong DMA (e.g. 638 Toledo) while audience config + ad-buying-ui ipdata resolve the correct DMA (Columbus, metro 535). Root cause (Sean Yang, AUDI-1072): the `dw-main-bronze.geo.location_data` model has rows where `metro_id` and the parent `hierarchy` chain disagree on the metro (e.g. ZIP 43221 loc 708867: metro_id=535/Columbus but hierarchy→298>638/Toledo). TPA export/MemDB/audience read the hierarchy; CIL geo for non-template-55 campaigns comes from `geo.network_locations` (lizz, CIL owner — template-55 uses win_logs). **Fix = PR #1147 (SteelHouse/sqlmesh, "keep location_data.metro_id and hierarchy in sync") — a GENERAL metro_id COALESCE for all type-6/7 locations, NOT ZIP-specific.** As of 2026-07-13 it is **still OPEN/DRAFT, never merged/deployed**: `sqlmesh plan` tried to backfill and would break downstream (needs forward-only), and Nivas hit permission errors running plan → DevOps ticket DEV-8264. AUDI-1072 was marked Done because DM (Data Monitoring) believed they'd fixed the root cause on the AUDIT-LOGIC side — Sean (2026-07-29): the flagged campaigns had an **unapproved audience leaking past the exclusion filter**, and DM **tightened validation** to stop it (a substantive fix, not mere suppression). So there are TWO candidate root causes for aud22 triggering: (a) that audit-logic audience leak (DM, believed fixed Jul-21) and (b) this location_data metro_id/hierarchy mismatch (PR #1147, undeployed). Which drove the 07-28 recurrence is UNRESOLVED, though Harry's 07-28 geo trace (a real 638-vs-Columbus divergence) points to (b) and would not be explained by (a). **Open caveat (Nivas):** PR #1147 does not touch `network_locations`, and CIL's non-template-55 geo comes from `network_locations` — so whether #1147 alone stops it is unconfirmed (Harry's 07-28 trace shows the wrong 638 living in `v_location_data_lat_long.hierarchy`, which IS #1147's target, so likely yes). Independent check at geo_version `1783900800`: the 2 original ZIPs (43221, 45814) are clean now, but 5 other US ZIPs still carry the `metro_id`-vs-`hierarchy` mismatch (01223, 17371, 66214, 92545, 95245) — the affected ZIPs rotate across geo_versions because the systematic fix was never deployed. Not a per-CGID redeploy; deploy PR #1147 forward-only once DEV-8264 unblocks. Diagnostic: left-join `location_type_id=7` ZIPs to `location_type_id=3` metros on `metro_id`, flag where metro `location_id` NOT IN `UNNEST(hierarchy)`.
