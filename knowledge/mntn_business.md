...
### Status / contacts
- **DS51** in MNTN's 3P registry (the Bombora registration) **began delivering IPDSC volume directly ~2026-07-06** — the "~July 2026" platform milestone above landing. Intermittent: the Bombora feed drops ~every other day (`ipdsc/dt=D/data_source_id=51/`, source `partners/bombora/segments/{D-1}`); a skip day → 0 DS51 volume for that dt, self-healing on the next delivery (on-call INC-001). Present days ≈ 108K-141K DS51 impressions in `analytics_curated.enriched_impressions`. (Superseded the prior "currently 0 IPDSC volume" note.) The LiveRamp custom-segment pipe (workstream 1) also remains live.
