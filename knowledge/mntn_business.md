...
**The H2 optimizer is an AUDIENCE OPTIMIZATION TOOL that recommends or auto-applies audience changes (Mike Dolt, Insight Spelunking session 2026-08-21).** The problem it solves: **over 80% of clients launch a campaign and never touch it again**, which drags performance across the board. The tool will surface a recommendation, or make the change itself, without the advertiser acting. The 8/21 fieldwork session existed to get the squad fluent in what a client can actually see before that gets automated, not to generate the recommendations by hand ("this is not a sustainable way to optimize things"). Transcript: `tickets/audi_1083_mm_classifying_view/meetings/audi_1083_01_insight_spelunking_2026_08_21.txt`.

**Turning Peak Performance OFF is a legitimate optimization, and often the right one (same session, confirmed live by Mike Dolt).** PP **broadens** the audience beyond high intent, and advertisers assume a bigger audience is better. For most advertisers a plain MNTN Matched / Fangorn campaign with no additions is the best setup; the exceptions are advertisers with enough spend to exhaust their high-intent pool and start re-hitting the same households. The decisive check is the **audience-segment report**: on one campaign in the session Peak Performance was the highest-spending segment and returned **zero conversions**, which Mike called "100% a legitimate recommendation" to turn off. Do not reflexively read "PP present" as "PP is helping".

**Not every underperformer is optimizable (same session).** Rule out: conversions that happen off-site (a creative whose call to action is a phone number — MNTN attributes site visits and conversions only), and industries that structurally never convert on CTV (Johnny's list: dentists, foundation repair). A badly configured pixel IS optimizable. Goal values are frequently unrealistic ($6 CPA on a foundation-repair advertiser); goal recommendations are in flight, and the idea raised was to show the advertiser their vertical's average as a reference point, not just guardrails.

**Reporting UI facts a client hits (same session, for the reporting team).** The **audience-segment and keyword reports are not default reports** — a client must build one (Create Report → blank template dropdown → audience segment, or Performance Metrics → "Pivot by"; no need to save). **Two reporting-page versions are live at once** and the newer one is less intuitive. Delivery can appear in a geo that is not in the campaign's current geo filter because the audience was edited mid-flight and the report spans both configurations; narrowing the date window resolves it. The campaign ID appears nowhere in the UI. **All new signups land on MNTN Express**, not PTV; the upgrade path to full PTV is undocumented even internally.

**Peak Performance is the H2 2026 optimizer focus** (Bryce Wagg, squad announcement 2026-08-19): the 8/21 "Insight Spelunking" fieldwork session directs everyone to claim a Peak Performance campaign first from the bad-CPA list, "only default to other underperformers if the PP ones are taken." Live PP footprint as of 2026-08-19 = **2,651 Stage-1 Live campaigns / 1,087 advertisers** (`audience.mm_campaign_classifier`, `has_ds13 OR has_ds46`). **The H2 cohort is overwhelmingly v2:** on the 251-row bad-CPA list, 161 groups are PP and 128 of those run DS46 (Fangorn) against 33 on DS13, so any PP conversation defaulting to v1 mechanics is describing a fifth of the population. Lauren's sheet already carries its own `peak perf` column and it holds up — it agreed with the classifier on 250 of 251 rows (AUDI-1083 §6f).
...
**DM = Data Monitoring** (Jira project key `DM`). Owns the "feature audits" (e.g. `aud22` / FA022 Geo Includes/Excludes) that fire against mission-control. **Benny Poon = Engineering Manager, Data Monitoring + Production Operations** (owns the audits, made the aud22 audit-logic validation fix); Harry Connelly investigates on the audit side; Kirsa was formerly DM's PM. **Benny Poon also fronts the MNTN Public MCP beta** (announced to eng 2026-08-19, feedback to him or **Daniella Kubiak**; **Rob McKee** demoed it at Eng All Hands with Evan Klemen and Maribel). He turns reported defects fast: two filed 2026-08-19 were answered inside four minutes, one "being fixed and deployed" and one closed as dead config. See `knowledge/memory/reference_mntn_public_mcp.md`.
...
### Status / contacts
- **DS51** in MNTN's 3P registry (the Bombora registration) **began delivering IPDSC volume directly ~2026-07-06** — the "~July 2026" platform milestone above landing. Intermittent: the Bombora feed drops ~every other day (`ipdsc/dt=D/data_source_id=51/`, source `partners/bombora/segments/{D-1}`); a skip day = no ipdsc partition that day, self-healing on the next delivery (on-call INC-001); membership persists (~90d TTL per Jordan) so a skip does NOT zero downstream serving. Present days ≈ 108K-141K DS51 impressions in `analytics_curated.enriched_impressions`, but **all from ONE internal test advertiser** (30506 "MNTN - No ENG Testing", CG 131563) — the pipe is live and validated end-to-end, but this is test traffic, NOT production client Bombora delivery yet. (Superseded the prior "currently 0 IPDSC volume" note.) The LiveRamp custom-segment pipe (workstream 1) also remains live.
...
- **⚠️ `summarydata.sum_by_*` HEADLINE COLUMNS ARE LAST-TOUCH-EQUIVALENT regardless of `reporting_style`** (AUDI-1070, verified): the unprefixed `views/clicks/*_order_value` ≈ the `last_touch_*` columns even for FT-configured advertisers (checked HexClad 2026). The table does NOT honor the client's reporting_style; the **UI/API applies FT separately**. So a source-table YoY measures the LT view, NOT what the client sees in their MNTN dashboard. For the client's FT view, compute from log level (`clickpass_log.first_touch_ad_served_id`, ~40% NULL) or the reporting layer. When a client reports a YoY decline, **confirm both years use the same attribution** — a LT→FT migration (e.g., the bulk `r2_advertiser_settings.update_time = 2025-12-10` seen on Caraway/Avon/HexClad) mid-window inflates the apparent drop.
