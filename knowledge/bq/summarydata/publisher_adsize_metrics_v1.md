...
- If this is a VIEW, resolve the physical `sqlmesh__summarydata.summarydata__publisher_adsize_metrics_v1__4286446057` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `summarydata.publisher_adsize_metrics_v1` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
