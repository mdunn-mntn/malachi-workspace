...
- If this is a VIEW, resolve the physical `sqlmesh__summarydata.summarydata__icloud_guids__1701851882` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `summarydata.icloud_guids` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
...
## View definition
```sql
SELECT * FROM `dw-main-silver`.`sqlmesh__summarydata`.`summarydata__icloud_guids__1701851882`
```
