...
- If this is a VIEW, resolve the physical `sqlmesh__summarydata.summarydata__v_campaign_group_segment_history__1098153208` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `summarydata.v_campaign_group_segment_history` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
