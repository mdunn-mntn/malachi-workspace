...
- If this is a VIEW, resolve the physical `sqlmesh__summarydata.summarydata__tvo_prospecting_campaign_groups__1901231145` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `summarydata.tvo_prospecting_campaign_groups` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
