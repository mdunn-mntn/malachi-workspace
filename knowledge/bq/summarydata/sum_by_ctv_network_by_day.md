...
- If this is a VIEW, resolve the physical `sqlmesh__summarydata.summarydata__sum_by_ctv_network_by_day__211569296` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `summarydata.sum_by_ctv_network_by_day` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
