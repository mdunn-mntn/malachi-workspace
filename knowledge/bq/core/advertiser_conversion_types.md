...
- If this is a VIEW, resolve the physical `sqlmesh__core.core__advertiser_conversion_types__1595573418` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `core.advertiser_conversion_types` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
