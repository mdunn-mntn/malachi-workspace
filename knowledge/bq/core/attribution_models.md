...
object_type: VIEW
physical_table: sqlmesh__core.core__attribution_models__3133187567
...
- If this is a VIEW, resolve the physical `sqlmesh__core.core__attribution_models__3133187567` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `core.attribution_models` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
