...
- Partition `none`, cluster [audience_segment_id]. Always filter the partition column; avoid SELECT *.
...
## Example queries
```sql
SELECT <cols> FROM `integrationprod.audience_audience_segments` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
