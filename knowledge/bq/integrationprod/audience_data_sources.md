...
- Partition `none`, cluster [data_source_id]. Always filter the partition column; avoid SELECT *.
...
## Example queries
```sql
SELECT <cols> FROM `integrationprod.audience_data_sources` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
