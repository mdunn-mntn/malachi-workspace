...
- Partition `none`, cluster [audience_id]. Always filter the partition column; avoid SELECT *.
...
## Example queries
```sql
SELECT <cols> FROM `integrationprod.audience_audiences` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
