...
- **Keys:** natural key is roughly `(upload_id, order_id)`, but it is NOT unique — see Gotchas.
  `order_id` = the partner's event id; 35,260 distinct across 51,128 rows (as of 2026-07-29; repeats across upload batches).
  `upload_id` = the ingest batch; 2,938 distinct. `advertiser_id` = 24 distinct (small offline-conversion cohort).
...
| hashed_values | ARRAY&lt;STRING&gt; | REPEATED |  |  |
| time | TIMESTAMP | YES | DAY |  |
| order_id | STRING | YES |  |  |
...
- **Physical:** SQLMesh table `sqlmesh__ber_stg.ber_stg__offline__conversion_signal_orders__2749449162`,
  DAY-partitioned on **`time`**, no clustering, no TTL (expiration null). 51,128 rows / 15,683,224 bytes (~15.0 MiB) backing storage (2026-07-29).
- **Always filter `time`** (the partition). Confirmed empirically (dry-run, `SELECT *`): full scan = 15,683,224 bytes;
  `WHERE time >= '2026-07-01'` = 4,805,769 bytes (~69% pruned). Partition filter is NOT required by the table, but apply it.
- Table is tiny (~15 MiB) so cost is negligible today, but partition-filter + explicit columns keep it that way as it grows.
...
- 2026-07-29: enriched→verified. Re-derived from live source. Physical hash `__2749449162` unchanged; still DAY-partitioned on `time`, no cluster/TTL; 14-column schema unchanged; `hashed_values` mode confirmed REPEATED (ARRAY<STRING>). Added the previously-missing AUTO:SCHEMA block. Partition re-confirmed empirically (full 15,683,224 B → 4,805,769 B with `time>='2026-07-01'`). Refreshed counts to 51,128 rows / 15,683,224 bytes and cohort (35,260 order_id / 2,938 uploads / 24 advertisers; still single conversion_source_id). No schema/partition drift.
<!-- CHANGELOG END -->
