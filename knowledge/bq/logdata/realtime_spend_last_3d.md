...
- If this is a VIEW, resolve the physical `sqlmesh__logdata.logdata__realtime_spend_last_3d__2265771416` table to recover the real partition/cluster/TTL (cataloger's job); `partition_by: unknown` is an actionable gap, not a claim of "no partition."

## Example queries
```sql
SELECT <cols> FROM `logdata.realtime_spend_last_3d` WHERE <partition_col> BETWEEN '<start>' AND '<end>'
```
