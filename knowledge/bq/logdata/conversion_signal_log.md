...
- 2026-07-29: enriched→verified. Re-introspected LIVE source. Schema identical (16 cols, types/order unchanged) — AUTO:SCHEMA untouched. Partition/cluster/TTL re-confirmed on physical (`time` DAY, requirePartitionFilter null→false, no clustering, no partition expiration). **Drift fixed:** table grew 2.6M→**76,313,558 rows** / 1.23GB→**~127 GB** (front-matter approx_rows/bytes + Purpose + Cost notes updated); driven by a 2026-07-27/28 Mobile backfill (~71.8M rows in 2 partitions). NEW `data_source_id=57` (page_view + empty type) → data_source_id is now **5 values** (was 4) and mntn_conversion_type **7 values** (was 5); per-source counts refreshed (Mobile now 74M/97%). Advertisers ~41→**47**. Old ~20 MB full-scan cost figure obsolete (single-column full scans now bill ~1–2 GB). schema_synced + last_verified = 2026-07-29.
<!-- CHANGELOG END -->
