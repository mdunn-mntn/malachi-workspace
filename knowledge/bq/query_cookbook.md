...
| `temp_campaigns AS (SELECT DISTINCT campaign_id FROM hhst_v4__campaign_bucket ...)` joined/filtered into `bid_price_log` + `bidder_bid_events` over a 1h window — **rewriting the CTE shape alone did NOT help**: AUDI-1277's "baseline" and "candidate" query variants billed the **identical** 2212.129 GB (perf log `ed1af9b9…` and `a475e8ae…`, both same 1h window) | 2212.1 GB billed / ~195-214s wall, 1h window, one run each | **the cost is the 1h scan of `raw.bid_price_log`/`raw.bidder_bid_events` itself (hour-partitioned, ~717B rows/~1.06 PB, no clustering that helps a campaign-id filter) — a CTE/join rewrite can't prune it.** Actual levers, in order to try first: (1) shrink the time window below 1h if the analysis tolerates it (cost scales ~linearly with hours, per the `bid_price_log` 135GB@1h/268GB@2h precedent above); (2) compute `temp_campaigns` first as a **separate cheap query** and materialize the id list, then pass it as a literal `IN (...)` / scalar subquery restricted by the same partition filter, so BQ doesn't have to re-plan the small-side join against a huge partition scan; (3) confirm whether `raw.bid_price_log`/`raw.bidder_bid_events` is really the intended source vs the already-aggregated `external.camperbid_prod__hhst_v3/v4__campaign_bucket_population` view (see `data_catalog.md` — hourly, pre-aggregated, avoids the raw event tables entirely) if the population count is all that's needed. | perf log 2026-09-02/03, AUDI-1277, `bq_perf_log.jsonl` sha `ed1af9b9…`/`a475e8ae…` |
