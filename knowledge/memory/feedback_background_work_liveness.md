...
doc_type: memory
keywords: [background work liveness, stall detector, Monitor poll, hung task no notification, TaskOutput no task found, Workflow fan-out, AUDI-1173, mtime stale, find newermt, stat epoch, bsd find, bfs find alias, invalid timestamp, false stall alarm, stat -f %m, timeout not installed macos, timeout 127 missing binary, git ls-remote probe, false VPN off, false unreachable, bare probe exit code, with_deadline preflight, gnu-only wrapper]
domain: [workflow]
lifecycle: active
...
- **Recurrence, and it cost the USER this time (2026-08-15, on-call INC-017).** I probed repo reachability with `timeout 25 git ls-remote <repo> >/dev/null 2>&1 || echo UNREACHABLE`. `timeout` is still not installed, so the shell exited 127 on the missing binary, the `||` branch fired, and I told Malachi his VPN was off and asked him to reconnect. The network was fine and he had done nothing wrong; a bare `git ls-remote` succeeded. **Rule, stated positively: run a reachability/liveness probe BARE and read its exit code — never wrap it in `timeout` (or any GNU-only binary) on this Mac.** If a real deadline is needed use `with_deadline()` from `.claude/scripts/preflight.sh` (perl `alarm`). This is the third distinct victim of the same family (`find -newermt`, `timeout` on gcloud auth, `timeout` on git), and the second time the false negative was reported to the user as fact — see [[feedback_validated_is_not_correct]]: a check that can only report one outcome has not been validated, it has only been run.

- **Recurrence #4, harmless only by luck (2026-09-02, AUDI-1217).** Ran
  `timeout 30 gcloud projects list ...` inside a compound probe. `zsh: command not found: timeout` printed
  where the project list should have been. This time the output was visibly an error rather than a
