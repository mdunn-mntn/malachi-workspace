...
doc_type: memory
keywords: [background work liveness, stall detector, Monitor poll, hung task no notification, TaskOutput no task found, Workflow fan-out, AUDI-1173, mtime stale, find newermt, stat epoch, bsd find]
domain: [workflow]
lifecycle: active
...
- **BSD/macOS `find` rejects GNU-style relative `-newermt` dates (hit 2026-08-06, PS-8572 workflows).** `find ... -newermt '-15 minutes'` errors on Darwin, and a stall monitor built on it FALSE-ALARMS (every poll reads as idle/failed). Use `stat -f %m` epoch arithmetic as in the recipe above — never GNU find date strings on macOS.
