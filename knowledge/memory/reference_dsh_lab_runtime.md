...
**Egress cage (INSTALLED 2026-08-21):** dedicated macOS user `dshagent` (uid 502) + pf anchor `dsh` (default-deny out, loopback only, scoped to the uid) + tinyproxy on 127.0.0.1:3128 with a hostname allowlist (`$(brew --prefix)/etc/tinyproxy/allowlist`: api.anthropic.com, *.googleapis.com). `/Users/malachi` tightened 750->700 (staff group could read it). Verify: `sudo -v && dsh-lab/scripts/egress_selftest.sh` (needs a cached sudo cred; pfctl prints `user = 502` not the name). Unattended dsh runs go as dshagent with `HTTPS_PROXY=http://127.0.0.1:3128 NODE_USE_ENV_PROXY=1`. Revert: `sudo pfctl -a dsh -F rules`, drop the 2 `/etc/pf.conf` anchor lines, `sudo sysadminctl -deleteUser dshagent`, `brew services stop tinyproxy`.

**Automation LIVE (2026-08-21):** the full engine loop runs unattended daily 08:30 via launchd `com.mntn.engine.loop` (`engine/scripts/loop.sh`): harvest + `run_engine.py --auto --max-llm 3` (spend-bounded hypothesize) + entropy + observe. **Only rung-0 auto-commits** (index rebuilds, byte-stable); higher rungs append `ENGINE-PROPOSE` rows to `improvements_backlog.md` for human approval — surfaced on the Claude Code SessionStart banner (`Engine : N pending proposal(s)`). Logs: `engine/loop.log`. Stop: `dsh-lab/scripts/killswitch.sh` or `launchctl unload ~/Library/LaunchAgents/com.mntn.engine.loop.plist`. (The earlier harvest-only launchd job was replaced by this loop job.) To raise auto-adopt above rung-0, bump `current_rung` in `engine/engine.config.yml` after the ladder criteria are met.
