...
**Audience Intelligence owns the airflow-ti deployment and EVERY DAG in it (2026-09-03 sprint 8649 clarification).** This includes the seven DAGs that alert to `#monitor-attribution` (a channel name is NOT evidence of another team's ownership). All feature-store DAGs, all audience-intent scoring, all derived-attribute pipelines, all export tasks — AUDI owns the repo and the running platform. A Slack route to another team's channel is a notification, not a ownership transfer. Ryan Kleck (pipeline owner) is the decider on deploy and merge policy; the data-platform team (SQLMesh owner) owns `knowledge/recipes/sqlmesh/` and the Databricks workspace.
