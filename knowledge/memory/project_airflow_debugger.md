...
- **`SLACK_ALERT_CHANNEL` on prod is now `C08CURMGNMQ` ONLY** (monitor-tpa removed;
  [[reference_slack_debugger_app]] corrected). *(Superseded 2026-09-02: the comma list
  `"C08CURMGNMQ,C067ZM2EC5S"` was re-applied so replies thread under monitor-tpa alerts — see
  the 2026-09-02 late-night section.)*
...
- **Review queue: #1259 + #1260** (both verified OPEN 2026-09-02). *(Overnight: #1260 grew to
  ...
  Jira AUDI-1194 comment 614410. Detail: [[project_airflow_optimizer]].)* *(Both merged
  2026-09-02 — next section.)*

## 2026-09-02 (afternoon/evening) — PR #1260 merged; canary awaits its first daily; gsutil generalized

- **PR #1260 MERGED** (with #1259, #1262, #1263, #1264 across the day). **The canary's first
  live run is the 2026-09-03 17:00 UTC daily** — today's 17:00 run predates the 17:43 deploy,
  and `rca_2026-09-02.json` not existing is EXPECTED: dailies publish the PRIOR data day. Rapid
  runs green every 15 min.
- **The 2026-08-28 marker failure (PR #1243) is now generalized: gsutil itself is broken on
  Astro task pods** — the optimizer's downloader hit the same pod-only class (~2 of ~194
  objects landed in every gsutil mode) and was rewritten on the gcloud-token + GCS JSON API
  path (PR #1264, live on `deploy-2026-09-02T19-27-09`). JSON API is the path for ALL pod GCS
  I/O. [[reference_gcloud_storage_over_gsutil]]

## 2026-09-02 (late night) — reply upgrades PR #1268: human-triggered runs diagnosed, wrapper replies follow the dive

- **Human-triggered runs are now diagnosed like any other run.** `pull._person_triggered`, its
  `triggered_by` lookup, and `_RUN_ORIGIN_CACHE` are REMOVED from both candidate paths. Trigger
  case: the `bottom_up_keywords_pipeline_run`/`training_pipeline` manual rerun
  (`triggered_by=ui`) sat unanswered on 2026-09-02; the open "keep the skip or widen?" question
  closed as widen. The [[reference_airflow_run_origin]] mechanics stay true — the debugger just
  no longer FILTERS on run origin.
- **Wrapper signatures now defer to the engine dive's deeper evidence.**
  `vertex_pipeline_task_failed`, `vertex_await_died_no_payload`,
  `dataproc_await_died_no_payload`, and `downstream_job_no_local_cause` gain a
  `followed-downstream` Why source in `slack_block` (sits between the resolver and the
  classifier); the Fix line takes the deeper signature's remedy, and the canned "go look at the
  job" pointer survives only when the dive found nothing. User feedback trigger: the reply told
  the reader to go find the error the dive had already fetched.
- **Shipped as ONE combined PR #1268** (per user preference; #1266/#1267 closed superseded).
  277 tests, gauntlets PASS.
- **`SLACK_ALERT_CHANNEL` re-applied to `"C08CURMGNMQ,C067ZM2EC5S"` 2026-09-02** — prod held
  only the single channel, so debugger replies could not thread under monitor-tpa alerts.
  [[reference_slack_debugger_app]], [[reference_airflow_log_puller]].

## 2026-09-03 — AUDI-1280: alerting-tag coverage audit; PR #1274 (`Targeting` on the watch list + CI test)

- **32 of 67 alerting DAGs were invisible to both sweeps at `origin/main` (`825b07e`):** TGT 22, ATTRIBUTION 7,
  TARGETING 3 (the debugger's own `airflow_debugger_daily` / `airflow_debugger_rapid` and `spark_optimizer_daily`).
  One cause: `PAGING_TAGS` (10 entries after #1248) held DAG-level tags only, and the team tags `Targeting` /
  `Attribution` that `JobConfig.make_tags()` prepends to every DAG were not on it. `pull._has_tag()` passes when ANY
  live tag is on the list, so the team tag is the durable invariant to watch; DAG-level tags vary per file
  ([[reference_airflow_ti]] § DAG tags, alert routes). Static resolver == live tags for all 75 registered DAGs.
- **PR [#1274](https://github.com/SteelHouse/airflow-ti/pull/1274) OPEN 2026-09-03** (reviewer Ryan Kleck;
  medium-tier gauntlet, 3 findings refuted, 0 confirmed): `"Targeting"` appended to `PAGING_TAGS` in
  `include/airflow_debugger/daily.py` (fixes 25; live sweep candidates go **35 → 60 of 75** DAGs, from live tag sets
  intersected with the old and new list; the "23" quoted in planning was a two-tag server-side `?tags=` filter, not
  the full list); `tests/dags/test_alerting_tag_coverage.py` (159 tests: parse, readability, per-file coverage x78,
  per-file static tags x78, config-in-use; red run 26 failures without the tag, green 152 passed + 7 skipped with it);
  `.github/workflows/pr_alerting_tag_coverage.yaml` (Python 3.12, the PEP 701 file). Nothing under `dags/` changed.
  The PR is the workflow's first run.
- **Attribution's 7 alerting DAGs stay OUT by decision**
  ([decision 0005](../decisions/0005_attribution_dags_excluded_from_debugger.md)): `EXCLUDED_CONFIGS =
  {"ATTRIBUTION": ...}` in the test skips them; opt-in is a two-line PR once the Attribution team says yes (the ask
  is the user's to send).
- **Reply location is set by the alert route, not the tag:** threaded under the alert for the 48 DAGs routed to
  `#monitor-tpa` (TGT 36) or `#alerts-tpa-pipeline` (TPA_EXPORT 12), because `SLACK_ALERT_CHANNEL` =
  `C08CURMGNMQ,C067ZM2EC5S` (re-read 2026-09-03 03:45 UTC); `#airflow-debugger` digest for the 19 on `#monitor-emr`
  (ML 9), `#monitor-targeting` (TARGETING 3), `#monitor-attribution` (ATTRIBUTION 7). Widening threading is a
  deployment-variable change ([[reference_slack_debugger_app]]), not code.
- **Post-merge check (open):** `current_tag` matches the merge, then the next failure of a newly watched DAG
...
## 2026-09-04 — PR #1285 is a no-op on its own acceptance case (AUDI-1327)

#1285 shipped 2026-09-03 to make a wrapper failure name the exception the downstream job actually
ended on. Run against the real 2026-09-03 prod failure it was written for,
`conversion_signal_backfill_workflow/submit_batch_dsid_21`, it produces **none** of the three
expected strings and emits the same pasted traceback region it was meant to remove. Verified by two
independent agents in fresh worktrees at 277e5fd against live Cloud Logging.

**Two independent defects, both about the SHAPE Cloud Logging returns, not the parse logic.**

1. `dataproc_rca` fetches the driver log with `--order desc`, so the log arrives **newest first**.
   `error_region`'s `rfind("Traceback...")` anchor then lands on the OUTERMOST frame and the
   2000-char window runs forward into post-failure startup noise (`google-hadoop-file-system metrics
   system started`, `SPARK_EXTRA_CLASSPATH=`, `JAVA_HOME=`), discarding the exception chain entirely.
   **This is the same class of bug as the Cloud Monitoring v3 `timeSeries.list` newest-first defect
   that broke the pod surface on 2026-09-01** (PR #1259). Assume every GCP list API returns newest
   first until proven otherwise.
2. Cloud Logging **collapses each Java stack onto one tab-joined line**, so `exception_chain`'s
   MULTILINE `^\s*Caused by:` anchor never matches. Correcting the ordering alone does not satisfy
   the acceptance case; feeding the same log reversed still yields a 300-char truncated
   `PSQLException` blob rather than the nested `SocketTimeoutException`.

`db_unreachable`, the signature #1285 added, returns `None` on the actual reachability failure, so
the reader gets the generic "pull the downstream job's own log" remedy.

**Why CI said it was fine: all 53 tests pass.** `_JDBC_DRIVER_LOG` in
`include/airflow_debugger/tests/test_parse.py` is hand-written, newline-separated, ascending-order,
with `Caused by:` starting its own line. Production emits none of those three properties. **The
fixture, not the parser, is the root cause** — which is why AUDI-1327 builds the real-log corpus
first and fixes the parser under it. Every reply defect to date was caught by a person seeing a bad
Slack post, never by a test.

Expected rendering once fixed: `java.net.SocketTimeoutException: Connect timed out`, raised through
`org.postgresql.util.PSQLException`, at `spark_read_host.py:27`.

## 2026-09-04 (later) — AUDI-1327 / PR #1287: the corpus, and a hard ceiling on it

**Cloud Logging keeps Dataproc driver logs 7 DAYS. `dataproc_rca.py:40` sets
`_LOG_FRESHNESS = "45d"`.** Verified: `gcloud logging buckets list --project=mntn-prj-prod-00` shows
`_Default` `retentionDays=7`; the 400-day bucket is `_Required`, admin audit only. Of 27 distinct
failed Dataproc batches probed, only 5 returned any text, oldest survivor 2026-08-29. The code's
"no driver log via Cloud Logging (check freshness window)" note therefore misdirects: the window it
names is 6.4x wider than the data that exists. **This is a permanent ceiling on any driver-log
corpus**, and it is why the whole Spark-side taxonomy (OOM, shuffle, executor loss, TTL, GCS
listing) has zero retrievable real examples.

**The taxonomy is 48 emitted keys, not 44.** `signatures.py` holds 44; `external_task_rca.py` emits
four more that are NOT in `SIGNATURES`: `external_task_target_failed` (6 hits in 13 days of
published prod RCA), `external_task_target_skipped`, `external_task_target_unfinished`,
`external_task_window_mismatch`. Any coverage count scoped to "the 44" silently omits four live
classes. **All 44 signatures carry `engine="any"`,** so `classify()`'s engine filter
(`signatures.py:651-652`) is dead code; a per-engine split must come from the log, not the signature.

**Corpus scope, from 3,627 real logs across 33 dates scanned with the package's own
`parse_log()` + `classify()`:** 3,068 success / 245 failed / 173 skipped / 137 upstream_failed. Of
the 245 failed, `classify()` matched 215; 30 unmatched, 22 of them carrying error text and therefore
genuine taxonomy gaps. 28 of 48 classes have a real example; 16 are waived on a CI-read list
(7 fire in prod with no artifact retained, 9 never observed). Engine split of the failures:
databricks 153, dataproc 31, other 28, vertex 26, unknown 7.

**The fix that mattered was not `rfind` -> `find`.** Cloud Logging had split the Python traceback
across ENTRIES, and `--order desc` reversed the entries, so frames precede their own header. The
reversal has to happen on the ENTRY LIST, below the `--order` flag
(`--order desc` is at `dataproc_rca.py:247` inside `logging_messages`, wrapped by
`_logging_messages` at `:259`). Putting the replay seam above that flag leaves a fetch-order change
invisible to CI: flipping `desc`->`asc` in the source moved three goldens with the seam below it and
nothing with the seam above. `exception_chain`'s "deepest = last" assumption also breaks after
reversal, because the last `Caused by:` in the window becomes the newest event rather than the
deepest cause.

**The corpus expires 2026-09-10 and nothing detects drift.** The acceptance capture's source falls
out of the 7-day window, and CI has no gcloud auth, so the vendored bytes can never be compared to
prod. If the log shape changes again, the corpus will keep certifying the old shape.

**Redaction is a commit gate, not a nicety.** The acceptance log carries
`CURRENT vault https://vault.prod.in.mountain.com` and internal `10.140.4.x` addresses. Vendored
captures scrub internal hostnames to `redacted-host.invalid` and private IPv4 to `203.0.113.1`
(RFC 5737 TEST-NET-3). CGNAT worker addresses (`100.64.0.0/10`, RFC 6598) are deliberately kept and
the decision is currently undocumented in the repo.

## 2026-09-04 (AUDI-1329) — coverage measured, and recall is not accuracy

**The debugger names a root cause for 88% of the failures Airflow reports as failed** (215 of 245
across 3,627 real logs; 90.5% on prod terminal failures, the two agreeing once split by outcome).
**Against ALL failure events including retry-recovered it is 46.1%** (59 of 128) — quote that one if
"our failures" means every failure event, not only the ones a retry did not paper over.

**Recall is not accuracy. 4.3% of GREEN runs also match a signature** (132 of 3,068 success logs).
`path_not_found_late_data` fires 72x on success against 4x on failure; `spot_preemption` 50x against
2x. At least 10 of the 215 matches rest on a key that does not discriminate — a lower bound, since
the other 205 were not hand-verified. A coverage number that counts "classify() returned a key" is
measuring recall, not correctness.

**20 of the 48 keys have never matched a real log**; only 27 ever fire.

**24 of the 30 unmatched failures are detectable from artifacts already on disk**, in 8 classes. In
three of them `parse.py` already extracts the discriminating evidence and no signature consumes it
(`pod_wait_seconds`/`pod_deleted`; `poke_target`/`poke_count`/`reschedule_count`; the missing-batch-id
note). Writing those detectors moves 215/245 to ~239/245 = **98%** with no new data source.

**Retention is NOT the ceiling on the tool's operational coverage.** It binds only the retrospective
Dataproc deep-dive, which is not how the tool runs in production. (It DOES cap a driver-log corpus —
see the AUDI-1327 section above — those are different claims about different things.) 57% of
failures have their real cause in engine-side text and 80% of those are already past the 7-day
window when looked at retrospectively; live, the debugger reads them within minutes.

**`_LOG_LIMIT` is 80 and reads newest-first.** 80 newest-first lines from a batch emitting 500+ ERROR
lines anchors on the outermost frame — the same failure mode AUDI-1327 fixed one level up. Read
`stateMessage` first: it survives well past 7 days and keyed 10 of 10 old batches.

**The canary has never worked.** Its threshold `max(2*norm, norm+0.25)` is unreachable above norm
0.5, and the norm sits at 0.62. Benign `upstream_failed` empty logs are what pushed it there; the
empty-log denominator needs to exclude them.

**`pull.py:91` is why the silent class is at zero.** `state in FAILURE_STATES or try_number > 1`
never sees a green run, and it also leaves 3,417 `skipped` instances unreachable.

**`PAGING_TAGS` matching is case-sensitive** at `pull.py:104`; making it insensitive recovers
`augmentor_daily_gcs` and both `tmobile_*` DAGs. The 8-of-75 exclusion currently drops
`feature_store_hourly` and `feature_store_setup_model`, which carry 15 of the 33 silent losses.

## The silent-failure class belongs to NEITHER tool (AUDI-1329)

The debugger fires only on FAILED; the optimizer sweeps only SUCCEEDED and looks for inefficiency,
not correctness. **33 output partitions were lost in 30 days behind runs that reported SUCCESS.**
Three models swallow the exception in a per-partition loop and print the loss to driver stdout, which
nothing reads: `site_network_hourly.py:246`, `aug_log_ip_hourly.py:202`,
`aug_log_ip_vertical_id_hourly.py:288`.

**An output-contract check is the only thing that catches it: 18 of 18 recall at 100% precision**,
the only one of five candidate sources that worked. List the `hh=`/`dt=` directory ~2h after the run
and assert it exists and holds `_SUCCESS`, gated on the source partition being non-empty. It can be
hosted in the optimizer's daily sweep for convenience; it is not the optimizer's rule.

**`_SUCCESS` is currently decorative.** Neither `utils_model/gcs_path_utils.py:filter_existing_gcs_paths`
nor `writer.py:_filter_relpath` gates on it — both test for at least one object — so the 2026-09-02
`hh=05` partition (29 files, 0.482 GB, no `_SUCCESS`) reads as good downstream.


## 2026-09-05 — live catch: the `openai_file_quota` signature fired unprompted and called it right (AUDI-1321)

**The debugger posted the `dt=2026-08-28` `batch_submit` failure to `#airflow-debugger` unprompted**, matched
the signature `vendor-quota/openai` and the known pattern `openai_file_quota`, and **correctly called the
`400` deterministic and unfixable by retry** rather than routing it as a transient. That is the tool doing
exactly what it exists for, on a failure a human was already working, and it is worth recording as a positive
in the track record next to the false-fire measurements above: the failure was one of the backfill's own
re-submits, so the signature also held up on a manually triggered run (the person-triggered filter was removed
in PR #1268).

Nuance worth keeping, not a defect: "unfixable by retry" is true of the individual `files.create` call and was
the right operator guidance. The day's own recovery still came from a retry, because a quota-failed
`batch_submit` is MONOTONIC — the receipts written before the `400` survive and the retry resumes from there
(866 of 1,241, then 1,241 of 1,241). See [[reference_mntn_matched_batch_pipeline]] § 2026-09-05.
