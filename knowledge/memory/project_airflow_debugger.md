...
**Phases 0-2 done (2026-08-03).** Phase 2 added `orchestrate.py` (deterministic-first entrypoint), `incident_match.py` (lightweight lexical matcher over `incident_log.jsonl` — chose it over `all-MiniLM-L6-v2`/torch for a 9-row corpus), and `synth.py` (LLM fallback for unknown signatures only). **Orchestration uses the Anthropic Messages API directly (`claude-opus-4-8`), NOT the full Claude Agent SDK** — the deterministic pre-processor does the extraction, so the LLM's job is one bounded synthesis call; the `claude` binary + agent SDK aren't installed and aren't needed. `ANTHROPIC_API_KEY` is the LLM-orchestration credential, separate from the key-free data layer; swappable behind `synth.synthesize` (e.g. ChatGPT later). **Remaining:** wire `airflow_pull.sh --watch` → `orchestrate` → drop the report artifact in `on-call/` for `/oncall` to triage (the 3-surface write-back stays `/oncall`'s job). Phase 3 deferred (in-DAG auto-fire, sanctioned Slack reply, propose-only PR + adversarial reviewer).
