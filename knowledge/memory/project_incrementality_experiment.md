...
- **AUDI-1213 CLOSED Done 2026-09-03**, retitled "One weekly-refreshed Mode MDE calculator for every advertiser". Two builds shipped the same day: the delivering-cohort refresh (1,859 advertisers, trailing 30d, no $1k floor, up from 879, advertiser-facing CPM basis, all three defects fixed, republished to the same gist under the original filename so shared links stay valid), then the full cohort of **4,387 (1,857 delivering + 2,530 lapsed)** on a 365-day lookback with each advertiser windowed on its own last 30 DELIVERING days. The calculator was also ported to a Mode report in the "Audience Intelligence" space ([[reference_mode_dashboard_porting]]). Nothing is left open on the ticket: the lapsed cohort, the Mode port and the VR_STACK re-measurement all landed. **The weekly refresh has to be set by hand in the Mode UI** — the schedules API rejects every cron payload on a member key ([[reference_mode_dashboard_porting]]) — so confirm it is set before quoting the report as auto-refreshing. **AUDI-1324, filed to split the Mode port out as its own deliverable, closed as a DUPLICATE hours later**. The Mode report IS the calculator. Numbers and method: [[reference_test_budget_from_rates]]; which surface to send someone to: [[reference_mde_surface_choice]].
- **VR_STACK 0.595 REFUTED 2026-09-03.** The stacked variance-reduction constant does not hold: **1.0030 median across 421 clean ghost-bid campaign groups, none at or below 0.595 or 0.794**. The toggle, the post-stack hero number, its chart series and the constant were REMOVED from both builds on the user's call, so every MDE either build now reports is raw-variance. **Restating anything published earlier: every post-stack MDE is understated 1.68x and every post-stack budget 2.82x.** Measurement detail: `knowledge/experimentation.md`.
