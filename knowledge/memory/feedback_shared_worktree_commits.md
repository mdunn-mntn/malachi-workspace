...
**Partial-file staging when ONE FILE carries both your hunk and a foreign one (2026-08-27).** Path-limited commits cannot split inside a file: `improvements_backlog.md` held the engine's uncommitted proposal lines plus a new IMP row from this session. `git add -p` is unavailable (non-interactive), so stage only your hunk with: `git diff <file> | awk '/^diff|^index|^---|^\+\+\+/{print;next} /^@@/{keep=/<your-hunk-anchor>/} keep' > my.patch && git apply --cached my.patch` — the anchor is the `@@ -N,M` line of your hunk. Verified on commit `e7973a9b`: the engine's lines stayed unstaged, only the IMP-093 row shipped.
