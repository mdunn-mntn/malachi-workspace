...
description: "Bidder serving-store architecture — household profile now ScyllaDB (key=IP, migrated from Aerospike), scores in GCS not MemDB, two segment delivery paths + empty-dupe problem, ScyllaDB raw-wins pipeline, id-service Bigtable ID→household resolution rule, ownership/contacts"
metadata: 
  node_type: memory
  type: reference
...
**Serving stores:**
- **Household profile cache = ScyllaDB (CORRECTED 2026-08-25; was Aerospike below, migration complete per Eric Salinger).** PRIMARY KEY = IP address. ~300M IP keys, 3–5 TB, single-digit-ms, hit DIRECTLY on every bid (no in-bidder in-memory tier). Only IPs actually in use are stored. ~~IP with empty segment list is deleted~~ **CORRECTED 2026-08-25: nothing is deleted — an empty write is a full REPLACE that keeps the row and resets its TTL (Eric: ~30d, his doc: 7d, unresolved), so empty IPs recirculate forever; ~92% of segment writes are duplicate empties riding the 4-hourly GCS dump (AUDI-1016, now a spike: Kafka streaming vs deltas). Two delivery paths: GCS full-snapshot dump every 4h + Kafka `segment-updates-burnin-proto`. Consumer = Rust `rtb-membership-consumer-service`.**
...
- **Migration:** Aerospike → ScyllaDB + Redis (Aerospike expensive / poor support). **2026-08-25: DONE for the household-profile path — treat Scylla = current there; `rtb` namespace/aql notes above are the historical Aerospike layout, spend/recency/price set status unverified since.**
