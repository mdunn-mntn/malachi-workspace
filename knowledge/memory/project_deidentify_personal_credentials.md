...
**Databricks, corrected 2026-08-21:** the service principal **already exists** — `spark_optimizer`, appId `07f36af7-614d-4d57-8143-2dbcd3cb58c2`, `CAN_USE` on warehouse `14b311ac86ee2ca2`. Mint its secret with `databricks service-principal-secrets-proxy create <id>`; the non-proxy `service-principal-secrets create` is account-level and errors. **`system.lakeflow` IS obtainable internally, corrected 2026-08-24** — the 2026-08-21 claim here (Databricks-side only, support ticket) misread the enable error. Lakeflow is enabled automatically; the blocker is a missing **metastore admin**, which is a console-only MNTN assignment. It gates `system.billing` (IMP-062) identically, so the two are one ask, not two. See [[reference_databricks]].
...
**Both of those closed 2026-08-21.** There is no Cloud Run manifest, because there is no Cloud Run job; the SA takes its own direct bindings in the copied Terragrunt unit. What is still open: a metastore admin assignment, which unlocks `system.billing` AND `system.lakeflow` together (corrected 2026-08-24 — neither is Databricks-side), a Jira bot identity (nothing started), and moving the AUDI-1191 debugger off its laptop cron into an `airflow-ti` DAG — the same move, needing no new identity beyond a copy of the `spark-optimizer` unit.
