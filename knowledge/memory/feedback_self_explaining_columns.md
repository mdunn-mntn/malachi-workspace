...
**How to apply:**
- **Name the column as the sentence the number answers.** `Reading` → **Tracking history** · `Tier` → **Test readiness** · `Score` → **Candidate score**.
- **Give the comparison, not the rank.** A percentile makes the reader do arithmetic, and a verb like "beat" implies we won a contest. Put the benchmark in its own column and let the row read straight across: `Share of site visits 0.02%` · `Typical for this size 0.61%` · `Compared to sites with: Over 1.4M visits`. Three plain numbers beat one clever one.
- **Check the distribution before shipping any rank.** In the largest site-size band the share ran p10 0.0036% · p25 0.0297% · p50 0.6075% — a 20x break, so "bottom quartile" reads as *slightly behind* when the truth is *different cluster*.
...
- **Five tests before shipping:** (1) read each header aloud with no context — if "what would this cell contain?" is a guess, rename it; (2) read one value aloud — if it prompts "…of what?", the unit is missing; (3) for a bucket label, can the reader tell *why this row landed there*? Ordinals always fail, ranges always pass; (4) any rank/percentile/index — can it be replaced by the raw number plus its benchmark? then do that; (5) read a full row aloud as a sentence — if you jump columns or do arithmetic to reach the point, the order is wrong or a column is doing too much.
