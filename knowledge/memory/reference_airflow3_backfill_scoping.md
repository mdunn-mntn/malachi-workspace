...
- **Airflow API is under a PATH PREFIX** = the SPA `<base href>` (e.g. `/dokgryiq/api/v2/...`), NOT `/api/v2/...`. A bare `/api/v2` fetch returns the index.html or 403. Read the prefix from `document.querySelector('base').href`. Auth = the UI's Bearer JWT (cookie alone → 403 on some builds; grab `authorization: Bearer eyJ…` from a Network request or reuse the session). Safari console needs top-level `await` wrapped in `(async()=>{…})()`.
