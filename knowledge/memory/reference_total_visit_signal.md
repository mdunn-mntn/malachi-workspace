...
**enriched.lift__ghost_bid_visits = the INCR total-visit lift table (Matt Brorby).** Binary `visited`/`converted` per arm×ip; `arm` ∈ {`ghost`=holdout, `submitted`=treatment}; guid_log-derived outcome. **Hard-coded 7-day-from-first-bid window with fixed arms** → a **platform 7d sanity/reference** only, NOT usable to size a ≥30-day or custom-arm (cap-8/cap-3) estimand. For a custom-arm/longer design, build the total-visit outcome directly from guid_log. See [[reference_ghost_bid_lift_register]].
