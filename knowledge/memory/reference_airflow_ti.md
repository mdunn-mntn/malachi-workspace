...
- **`spark/` scripts sync to `gs://mntn-data-archive-prod/ti_resources/spark/` after a merge to main, with VARIABLE lag** (observed 2026-08-06: INC-012 fix v1 merged ~21:55Z → deployed 00:09Z, ~2h15m; fix v2 merged ~02:1xZ → deployed 02:20Z, minutes). Verify the GCS object timestamp AND grep the content before declaring a fix live or re-running — that check is how the failed 00:45Z batch was proven to have run the v1 code.
...
## DataprocCreateBatchOperator re-run trap (batch-id attach)
- **On "Batch with given id already exists" the operator ATTACHES to the existing batch** ("Attaching to the job ... if it is still running") and inherits its terminal state. So clearing ONLY the operator task reuses the id-minting upstream task's old XCom (e.g. `create_batch_id`) and instantly re-fails with the OLD batch's error. Correct re-run: clear the id-minting task WITH downstream so a fresh id is minted. Same class as the `tpa_mntn_id_export` batch-id trap (runbook INC-001); confirmed on `materialize_mntn_select` 2026-08-06 (INC-012).
