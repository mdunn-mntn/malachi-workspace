...
- **The three `tpa_ipdsc_export` sites are deliberately NOT fixed yet.** That DAG's `already_exported` check tests for ANY blob rather than `_SUCCESS`, so making its retries real would let a retry skip on a PARTIAL write and report success (workspace `improvements_backlog.md` IMP-041 must ship before IMP-042). The materialize DAGs write `.mode("overwrite")` with no skip path, which is why they were safe to fix alone.
- **Status 2026-08-15: [airflow-ti#1196](https://github.com/SteelHouse/airflow-ti/pull/1196) is OPEN and deliberately HELD, not stalled.** It carries the `_SUCCESS` gate (IMP-041) plus the three remaining `tpa_ipdsc_export` batch ids (IMP-042) in one PR; Sean asked to deploy it Monday with the team watching, because it touches the critical `tpa_ipdsc_export`. Its DAG docstring was amended to document the `create_*_batch` clear requirement (next section). **Consequence while it sits: `tpa_ipdsc_export` still has dead retries on all three tasks** — any try-1 failure there is a page, and recovery is the clear-the-PRODUCER procedure below.
