...
doc_type: memory
keywords: [airflow-ti, feature store pipeline, dataproc serverless, ryan kleck, model_run.py, model_upload.py, backfill, feature_group, parquet schema gotchas, GCS feature store paths, persistent history server, PHS, spark event logging, spark-job-history, PR 1169, deploy to prod, spark eventLog, dataproc-debug pam, dataproc-temp bucket access, ExternalTaskSensor, skipped_states, failed_states, skip as failure, ExternalTaskFailedError, wait_fpa, globStatus, flat glob, fs.gs.glob.flat.enable, gcs list timeout, literal partition paths, augmentor_log glob, INC-012, PR 1176, basePath, getFileInfoInternal, root stat, directory marker, COLUMN_ALREADY_EXISTS, PR 1177, batch-id attach, create_batch_id, batch with given id already exists, ti_resources spark deploy lag, model_backfill.sh, backfill runbook, PR 1180, INC-013, PR 1179, dsid30 augmentor reader, silent try except degrade, mntn_global_data, feature_store_setup_model schedule, dt D+1 offset, catchup false latest interval only, bae_ip disabled, INC-015, retired model gcs dir, guid_log_derived_advertiser_id_dsc_id, conversion_log_derived_advertiser_id_dsc_id, guid_and_conv_log_derived_advertiser_id_dsc_id, false missing partition, dev not a mirror of prod, INC-016, INC-017, PR 1195, dag_vars.py, create_batch_id task decorator, batch id try_number, retry reattaches failed batch, materialize_mntn_first_party, materialize_mntn_select, tpa_ipdsc_export call sites, IMP-041, IMP-042, already_exported _SUCCESS, XComArg templated field, xcom_pull template render, dropped dependency edge, template_fields batch_id, logical_date nullable manual run, DagBag parse validation, AIRFLOW_VAR env var, SUPERVISOR_COMMS ImportError, no pytest in venv, parse against main control, mark task success PATCH new_state, clearTaskInstances include_downstream, run_id url encode %2B, astro jwt expiry refresh, gcloud storage rm bulk delete, gsutil -m rm hang libressl, PAGING_TAGS, PR 1248, vertical_classification_api execution_timeout, dags ruff not covered, create_batch cached spec, create_batch clear with downstream, batch spec xcom cache, config rerun trap, run with latest bundle version insufficient, false success cleared running batch, cancelled batch green tick, materialize success under 3 min, driver memory 9600m default, MapOutputTracker OOM, map-output-dispatcher, java heap space driver, constant death interval, resource ceiling vs data bug, shuffle partitions 5000, PR 1198, PR 1196, pam grants create, pam grants describe, gcloud storage cat driveroutput, gsutil wildcard 403, dataproc staging bucket, INC-018, ip_data_staging retention, sh-dw-external-tables-prod retention, 5002 objects, PR 1209, hh_id_data, household export, tpa_mntn_id_export, MemDB, ID-431, ip_data data_sources empty cats, operator timeout not wall clock, wait_for_batch no deadline, _cancel_previous_tries, load_graph_ids 1to1 ipv4, dag_run.conf param validation, ParamValidationError, spark.speculation false, ManifestCommitter race, model-unit-test broken, test_model_read_write_in_dev, model_config.json fixture rewrite, mergeStateStatus UNSTABLE vs BLOCKED, squash and merge, MNTN_SDLC_ENV dev dryrun, uv group models, model-upload-dryrun check, PR 1230, PR 1231, PR 1249, ruff pinned 0.16, per-file-ignores tests glob, lint like CI, ruff format not in CI, zsh no word-split, zsh arrays lint command, ruff point release flags main files, astro cli silent empty output, astro deployment list refresh, builder config not serialized, builder-only PR no json commit, compute.py runtime_properties serialization, uv export only-group models, dryrun no extra installs, dryrun two lines exit 0, models ruff not covered, DTZ007, AUDI-1274, advisoryPartitionSizeInBytes 16m, guid pivot models, AUDI-1273, maxPartitionBytes, ipdsc_ds_49, ipdsc_ds_67, conv_log_derived_ip, tests/models local recipe, JAVA_GATEWAY_EXITED, PYTHON_VERSION_MISMATCH, PYSPARK_PYTHON pinned, openjdk 17 homebrew not linked, model-unit-test passes locally, model-unit-test contradiction, git archive no baseline, context.py git at import, uv only-group test, ruff F841, show-settings no config, AUDI-1275, spark.speculation true, straggler gcs writers, gcloud dataproc batches project, dataproc.batches.list PERMISSION_DENIED, labels.airflow-dag-id, labels.job_type, batch list retention 7 days, FileOutputCommitter v2 injected, OutputCommitCoordinator, ManifestCommitter FileNotFoundException, manifest.committer.io.threads, commit 009e99a, commit 40ecc73, PR 1271, site_network_hourly canary, BLE001, swallowed hour write, AUDI-1280, PR 1274, JobTeamConfig, make_tags, team tag invariant, job_team_config.py, task_fail_slack, task_fail_email unwired, airflowignore, 78 dag files 75 live dags, PEP 701, category_taxonomy, python 3.12 dags, uv no-project, ast-only dag test, test_alerting_tag_coverage, pr_alerting_tag_coverage, airflow variables empty, JOB_CONFIG override, dag_decorator alias, AUDI-1278, PR 1278, bq_job_labels.py, airflow_job_labels, python client bq labels, get_df configuration labels, QueryJobConfig labels, url_pattern_pipeline labels, test_url_pattern_pipeline REPO_ROOT, parents 2 path bug, FileNotFoundError pre-existing failures, root tests not in CI, no ruff config no pre-commit, ruff 0.16.1 homebrew, google_cloud_default get_df billing, get_current_context RuntimeError, IMP-105, hackathon merge train, pr number ticket number offset, gh pr view headRefName, resolve worktree from branch not number, rebased wrong branch, merge order sprint 8649, deploy between merges, astro build 4 to 8 minutes, superseded build gap, PR 1277, test_newest_logs_takes_the_tail_and_drops_inprogress, gsutil text listing mock stale, PR 1264 json api, red ci on main, merge queue already red, ruff return annotations test helpers, spark-optimizer ci job, MntnKubePodOperator env_vars, kube_operators.py, astro deployment variable not in pod, Environment tab variable, env-secrets single key mount, OPENAI_API_KEY mounted key, env_from empty, pod_template_file null, rendered_fields ground truth, rendered-fields 422, map_index path segment, env_vars template_field, var.value.get default, airflow Variable knob no deploy, STORAGE_ALARM_MIN_FILES never passed, OPENAI_FILE_MAX_AGE_HOURS inert, shopper_graph#307, AUDI-1321, clearTaskInstances body fields, dry_run clear, only_failed default true, reset_dag_runs, include_future include_past, run id percent encode colon %3A, dag run state lies, read task states not run state]
domain: [repos, infra]
lifecycle: active
...
- **A builder `.config()` line NEVER reaches `dags/model_task_config.json` (verified 2026-09-02, AUDI-1274):** `compute.dataproc_batch` (`utils_model/base_model/compute.py:99-107`) serializes only the decorator `runtime_properties` into `DataprocBatchCfgSerializable`; `model_upload.py --dryrun` after a builder-only edit left both generated JSON files (`dags/model_task_config.json`, `dags/ipdsc_third_party_audience_builders.json`) diff-clean. So a builder-only PR needs no JSON commit, and (by the runtime read-split below) it rides the live `.py` read on the next run after merge rather than waiting for a bundle redeploy. First fleet use of `spark.sql.adaptive.advisoryPartitionSizeInBytes` = the two guid pivot models on branch `audi-1274-aqe-advisory-pivot` (mechanism in [[reference_dataproc_eventlog_profiling]]). Re-verified 2026-09-02 on AUDI-1273 (`ipdsc_ds_49` + `conv_log_derived_ip` `maxPartitionBytes` edits; Python 3.11 and 3.12 venvs, byte-identical JSON): `--dryrun` imports each model module and runs its decorators without instantiating the class, so builder code never executes at compile time.
- **`spark.speculation=false` is PINNED on every GCS-writing model (verified 2026-08-27):** `models/audience_intent/advertiser_join.py` comments "Disabled to prevent race conditions with ManifestCommitter"; `intent_score_map.py:54` also pins false. **Speculation is NOT a safe straggler fix for those writers** — the PR gauntlet reverted an evidence-backed speculation change to `models/ipdsc/ipdsc_ds_35.py` on this ground (2026-08-27); straggler recs on GCS writers go owner-gated. Mechanism + contradiction record: [[reference_dataproc_eventlog_profiling]]. *(Contradicted 2026-09-03 on AUDI-1275, both claims kept: the pin exists on 2 of the 13 straggler models only, `advertiser_join` and `prospecting_join` (manifest committer); `intent_score_map.py:54` is a third pin outside that set. The other 11 run Dataproc's injected FileOutputCommitter v2 or Iceberg, where Spark 3.5.3's `OutputCommitCoordinator` denies every attempt after the first per partition (`CommitDeniedException` + `abortTask` deletes the attempt dir) or Iceberg's task abort deletes the attempt's files, and the `audience_intent` scoring batches (`dags/audience_intent/audience_intent.py:109`) have run `spark.speculation=true` on v2 with `mode("overwrite").parquet` to GCS since 2025-08-15 (`40ecc73`): 366 and 88 speculative attempts in the two PHS logs read, all `TaskKilled`, 0 failures. Canary: `site_network_hourly` gets `"spark.speculation": "true"` in `runtime_properties` via [airflow-ti#1271](https://github.com/SteelHouse/airflow-ti/pull/1271) (open 2026-09-03, reviewer rkleck-mntn); the other 10 safe-by-source DAGs wait for three clean sweeps and Ryan's answer; the manifest pair stays pinned. Reconciling hypothesis and settling check: [[reference_dataproc_eventlog_profiling]]; decision `knowledge/decisions/0004_speculation_safe_where_writer_discards_loser.md`.)*
- **The Nov 2025 "ManifestCommitter race" behind the pin, from git (AUDI-1275, read 2026-09-02):** `64afca4` 2025-10-08 (Dustin) added the manifest committer to `include/spark/data_source/ipdsc_emr_cluster.py` with speculation still `true`; `009e99a` 2025-11-12 (Ryan) flipped speculation to `false`, comment "Disabled to prevent race conditions with ManifestCommitter", empty body; `5fbeb38` 2025-11-17 (Ryan, mntn_select) body "ISSUE 1: FileNotFoundException during ManifestCommitter parallel rename", fixed with `manifest.committer.io.threads=1` + `validate.output=false`; `a3352d9` 2025-11-20 the same two settings on `tpa_ipdsc_export`; `6afc07f` 2025-12-09 copied the manifest committer + pin to `advertiser_join` and `prospecting_join` and removed `io.threads=1` from ipdsc. So the FileNotFound persisted 5-8 days after speculation was off and was fixed in the job-commit rename phase (driver side, after every task committed, no duplicate attempt alive); the git record does not show speculation causing it, and does not fully explain the GCS rename anomaly either, which is why the manifest pair stays owner-gated (ask 1 to Ryan). The incident predates `on-call/oncall_runbook.md`; Confluence "TI On Call Playbook" not searched (no access from the agent).
...
**Lint like CI or get phantom results (AUDI-1191 round 2, 2026-08-31):** CI lints
`include/airflow_debugger` with **ruff pinned `>=0.16,<0.17`**, invoked with `--config` from the
repo root. The `per-file-ignores` `"tests/*.py"` glob only resolves when ruff runs from INSIDE the
package dir with a newer local ruff — so either replicate the CI invocation (repo root, pinned
version, `--config`) or `cd` into the package before linting; any other combination reports
test-file violations CI will not, or misses ones it will. **CI does NOT run `ruff format`** — a
formatting-only diff cannot fail CI. Shell gotcha that bit here: **zsh does not word-split an
unquoted `$VAR`** — a lint command held in a variable must be a zsh array
(`cmd=(ruff check --config ...)` then `"${cmd[@]}"`), not a string.

**The CI ruff pin FLOATS within `>=0.16,<0.17` (2026-08-31):** a fresh ruff point release can flag
`main`'s own untouched files. When that happens, annotate the violation and include the fix in
YOUR PR — do not wait on a separate cleanup PR.

- **model_task_config.json must be regenerated** after any model config change — including any change to a model decorator's `runtime_properties`. Exact invocation (verified 2026-08-27): `MNTN_SDLC_ENV=dev python model_upload.py --dryrun` under the uv dependency group `models`, then commit the regenerated `dags/model_task_config.json`, else the `model-upload-dryrun` check fails. Decorator changes only: a builder-only change leaves the JSON diff-clean (2026-09-02, previous section). Re-verified 2026-09-03 on AUDI-1275 (`MNTN_SDLC_ENV=dev uv run --group models python model_upload.py --dryrun`): deterministic, one `runtime_properties` key change = one JSON line under `<model>.batch.runtime_config.properties`, keys sorted.
- **Replicating the `model-upload-dryryn` CI job locally needs NO extra installs as of main `825b07e` (2026-09-02, AUDI-1274):** `uv venv --python 3.11 .venv` → `uv export --no-hashes --only-group models -o requirements-model-upload.txt` (137 lines) → `VIRTUAL_ENV=$PWD/.venv uv pip install -r requirements-model-upload.txt` → `MNTN_SDLC_ENV=dev .venv/bin/python model_upload.py --dryrun`. `pyproject.toml` now pins `pretty_html_table`, `matplotlib`, `sendgrid` in the `models` group, so the TI-956 `ModuleNotFoundError` list in `documentation/docs/airflow_ti_workflow.md` is superseded for the current lockfile. Exit 0 prints exactly two lines (`Compiling all models` / `Skipping all models upload to 'dev' env`) and leaves gitignored `dags/current_branch.json`, `utils_model/model_core/model_config.json`, `uv.lock` behind; delete the untracked `requirements-model-upload.txt` before handing back.
- **GitHub Actions runs model-upload-dryrun** — validates compilation and checks config freshness
- **CI job `model-unit-test` (`pr_model.yaml`) is broken REPO-WIDE since PR #1209 merged 2026-08-26.** `tests/models/test_model_read_write_in_dev.py` rewrites the git-ignored generated `utils_model/model_core/model_config.json` with only its five pytest fixture models during collection, so #1209's own `tests/models/test_tpa_mntn_id_export_model.py`, which reads the real config at import, can never pass in the same run. Verified by local repro (compile then pytest → 41 `ValueError` model_id not found). Owner rkleck-mntn; diagnosis posted as a comment on airflow-ti#1231.
- **`model-unit-test` is NOT a required check.** GitHub `mergeStateStatus` distinguishes them: **UNSTABLE = red check, merge still allowed** (this case) vs **BLOCKED = a required check failing**. Do not treat a red model-unit-test as a merge blocker while the #1209 break stands.
- **CONTRADICTION on the `model-unit-test` break, both claims kept (AUDI-1273, 2026-09-02):** on this Mac at main `825b07e`, `tests/models` passed **145/145 in 25 s** (re-run independently by the verifier: 145 in 25.11 s) with the dryrun-generated `utils_model/model_core/model_config.json` present and no `ValueError` at collection. Evidence for the old claim: the 2026-08-26 local repro (compile then pytest gave 41 `ValueError` model_id not found) and the comment on #1231. Evidence for the new: AUDI-1273 `summary.md` §4.4 runs 1-3. Reconciling hypothesis: the 08-26 red came from the local environment, not #1209. Without Java on PATH the suite returns 62 `JAVA_GATEWAY_EXITED` collection errors; with Java but the worker resolving `/usr/bin/python3` (3.9) against a 3.11 driver it returns 47 `PYTHON_VERSION_MISMATCH` failures in every test that starts a local SparkSession (`test_household_resolution` 17, `test_hh_prospecting` 12, `test_tpa_hh_export` 8, `test_tpa_metadata_info_map` 4, `test_tpa_mntn_id_export_model` 3, `test_export_tpa` 3), identical with and without the model edits. Settling check: read the CI log of the `model-unit-test` job on the next `models/**` PR (AUDI-1273 or AUDI-1274). Until then a red `model-unit-test` is unproven either way; it is still not a required check. AUDI-1275 (2026-09-03), same Mac without Java on PATH: `tests/models` gave 6 passed, 1 collection error (`test_export_tpa.py`, PySpark needs a JVM), consistent with the environment hypothesis.
- **Local `tests/models` recipe that matches CI (145/145 at main `825b07e`, 2026-09-02):** `uv sync --python 3.11 --only-group models --only-group test` (CI's exact groups; `uv.lock` and `.venv` are git-ignored), then `MNTN_SDLC_ENV=dev .venv/bin/python model_upload.py --dryrun` (writes the git-ignored `utils_model/model_core/model_config.json` the tests import at collection), then `JAVA_HOME=/opt/homebrew/opt/openjdk@17 PATH=$JAVA_HOME/bin:$PATH PYSPARK_PYTHON=$PWD/.venv/bin/python PYSPARK_DRIVER_PYTHON=$PWD/.venv/bin/python .venv/bin/python -m pytest tests/models -q`. Homebrew `openjdk@17` is installed but not linked on this Mac, hence the explicit `JAVA_HOME`. A `git archive HEAD` copy cannot serve as a clean baseline: collection fails there (`git branch --show-current` exit 128) because `utils_model/model_core/context.py` shells out to git at import; baseline with `git show HEAD:<path> > <path>` and re-apply the diff instead. Same recipe in `documentation/docs/airflow_ti_workflow.md` § Running `tests/models` locally.
- **airflow-ti merges are squash-and-merge** (verified on #1230/#1231, 2026-08-27).
...
- **`gcloud dataproc batches list` / `describe` need `--project mntn-prj-prod-00 --region us-central1` (AUDI-1275, 2026-09-02):** the default gcloud project `dw-main-silver` returns `PERMISSION_DENIED` on `dataproc.batches.list`. The list retains about 7 days of batches, so exposure counts older than that are not API-verifiable. Filter on labels: `labels.airflow-dag-id` (e.g. `audience-intent`), `labels.job_type` (e.g. `site_network_hourly`), `labels.airflow-task-id`. In `describe --format=json` the Spark property keys are prefixed `spark:` (`spark:spark.speculation`). Batch names carry the model and the logical hour (`sit-net-hou-y3a-20260901-195000-1`). Recipe + CSVs: AUDI-1275 `summary.md` §3.7.
...
## Airflow 3 REST recipes for state surgery (verified live 2026-08-29)
- **Mark a task success:** `PATCH .../taskInstances/<task>` body `{"new_state": "success"}`.
- **Clear tasks:** `POST /api/v2/dags/{dag_id}/clearTaskInstances`. Full body (verified against 3.1.5,
  2026-09-05): `{dry_run, task_ids: [...], dag_run_id, include_downstream, include_upstream, include_future,
  include_past, only_failed, reset_dag_runs}`.
  - **`only_failed` defaults to TRUE in some versions — pass it `false` explicitly** or a task that SUCCEEDED
    is silently not cleared and the call returns success having done nothing.
  - **`reset_dag_runs: true`** puts the dag run back to `running`; without it the run keeps its terminal state
    while the task re-executes.
  - **`dry_run: true` FIRST.** It returns the exact task instances that would be cleared, which is the cheap
    guard against a wrong `include_*` combination clearing a neighbouring day.
- **`run_id` in URL PATHS must be percent-encoded** — `+` -> `%2B` and `:` -> `%3A` — or the API 404s
  (manual run ids carry both, e.g. `manual__recover3_dt_2026-09-02T09%3A00%3A00%2B00%3A00`).
- **A DAG RUN's state can lie about its tasks.** A run whose recovery left one stale task failed reads
  `failed` while the task you care about succeeded (AUDI-1321: `batch_post.openai_batch_joined` stale from the
  outage, `batch_fetch` green). Read TASK states, never the run state, when reasoning about a half-recovered
  or backfilled run.
- **There is NO `/rendered-fields` sub-path on Airflow 3.1.5** (Astro deployment
  `cmd6bd10c0gl901rfuokgryiq`). `GET .../taskInstances/<task>/rendered-fields` **422s** — the router parses the
  next path segment as `map_index`. Read `rendered_fields` off the task-instance detail response itself:
  `GET /dags/<dag>/dagRuns/<run>/taskInstances/<task>`. Worth knowing because `rendered_fields` is the ground
  truth for what env a KubernetesPodOperator pod ACTUALLY received (see the KPO env section below).
- **The astro CLI JWT expires ~1h, and lapsed auth can be SILENT.** Deployment API tokens read
  from `~/.astro/config.yaml` expire ~1h, and astro CLI subcommands can print NOTHING (no error)
  once auth lapses. Run `astro deployment list` to force a refresh, then re-read the token from
  `~/.astro/config.yaml` (2026-08-31).

## Bulk GCS deletes: `gcloud storage`, not `gsutil` (2026-08-29)
`gsutil -m rm` on ~1100 objects HUNG silently on this Mac (LibreSSL); `gcloud storage rm -r`
completed in ~2 min. Prefer `gcloud storage` for bulk ops.

## Debugger paging-tag gap + vertical_classification_api timeout (2026-08-29, PR #1248)
- `PAGING_TAGS ["tpa","Machine Learning"]` missed `fetch_common_crawl` (tags
  `common_crawl_content`/`vertical_categorization`) and
  `audience_intent_scoring_household_14day_lookback` (tags include `ml`, not `Machine Learning`).
  Fix: [airflow-ti#1248](https://github.com/SteelHouse/airflow-ti/pull/1248), gauntlet PASS clean
  in one round.
- #1248 also raises `vertical_classification_api` `response_tests` `execution_timeout` 45m -> 68m:
  chronic timeout, that DAG failed 6x/day at the limit while successes run 39-42m.
- **`dags/` is NOT covered by airflow-ti CI ruff** (only `include/` packages) — pre-existing I001 in
  `ddp_vertical_classification_api.py` left untouched. **`models/` is outside it too** (verified from the
  workflow files 2026-09-02: `pr_model.yaml` runs no ruff step; only `pr_airflow_debugger.yaml` and
  `pr_spark_optimizer.yaml` lint, each scoped to its own `include/` package). ruff 0.16.1 flags both guid
  pivot model files on main (`I001` import order, `DTZ007` naive `strptime`) before any edit; a one-line
  config PR leaves them alone. Same on AUDI-1273 (2026-09-02): `ipdsc_ds_49.py` I001 and
  `conv_log_derived_ip.py` I001 + DTZ007 x2 + F841 all pre-exist on `git show HEAD:` copies, and
  `ruff check --show-settings` reports no settings path (no `ruff.toml`, no `[tool.ruff]`), so the rule set is
  the local ruff 0.16.1's resolution, not a repo choice. Diff against the HEAD copy before attributing any
  finding to a change. AUDI-1275 (2026-09-03): `models/bidstream_hourly/site_network_hourly.py` carries 3x BLE001, 2x DTZ007, 1x I001 on main; the same six sit one line lower after the one-line edit, none new.
- **2026-09-03 (AUDI-1280): the full-fleet audit found 32 of 67 alerting DAGs still unwatched after #1248** (the
  team tags `Targeting`/`Attribution` were never on the list). PR [airflow-ti#1274](https://github.com/SteelHouse/airflow-ti/pull/1274)
  appends `Targeting` and adds a CI test; next section.

## DAG tags, alert routes, and a static check over `dags/` (AUDI-1280, 2026-09-02/03, main `825b07e`)
- **Team tags are the durable debugger-visibility invariant.** `include/job_config/job_config.py` `JobConfig.make_tags()`
  returns `[team.value] + config.tags + dag_tags`, so every DAG built through a `JobTeamConfig` carries its team enum value
  (`Targeting`, `Attribution`, `Machine Learning`, `DataPlatform`, ...) whatever its file-level `tags=`. Verified: a stdlib
  `ast` resolver reproduced the live tag set exactly (P-tags included) for all 75 registered DAGs (`GET /api/v2/dags`,
  2026-09-03 03:22 UTC). DAG-level tags vary per file and are the wrong thing to key a watch list on.
- **Fleet at `825b07e`:** 92 `.py` under `dags/`, 14 in `dags/.airflowignore` (literal relative paths, one per line:
  `test_dags/*`, `exampledag.py`, the per-advertiser `tmobile_blocked_*_export_<id>.py`, the 4
  `gcp_pixel_page_view_signal_*_backfill`, `fpa/populate_site_visit_signal_bq.py`), 78 source files, 75 live DAGs
  (`dags/attribution/dlv_parse.py`, `url_pattern_discovery.py`, `url_pattern_pipeline.py` register no DAG).
  `job_team_config.py` has 22 `JobTeamConfig` members; 5 referenced from `dags/`: `TGT` 37 files (36 alerting +
  `models/feature_store_setup_model.py`, which binds `TEAM` and never calls `make_dag_args`), `TPA_EXPORT` 12, `ML` 9,
  `ATTRIBUTION` 7, `TARGETING` 3. 67 alerting files; 11 non-alerting (3 `send_email` to weiang@mountain.com:
  `augmentor_daily_gcs`, `tmobile_blocked_guids_workflow`, `tmobile_blocked_ip_workflow`; 8 with no failure callback).
  Per-file table: `tickets/audi_1290_pipeline_optimization_hackathon/audi_1280_debugger_tag_coverage_ci/outputs/audi_1280_tag_audit.md`.
- **Alert routes come from the team config, not the DAG file.** `make_default_args()` builds `on_failure_callback` as a
  `SlackNotifier` to `task_fail_slack` (prod: the `SlackChannel` enum value; non-prod: `#monitor-test`) and a PagerDuty
  callback only when `pagerduty_conn_id` is set AND `severity == 0` (8 P0 files today). `task_fail_email` is declared on
  `JobTeamConfig` but never wired into any callback (the `.email` property has no caller), so email is not an alert route
  through the team config. Routes: TGT `#monitor-tpa` (+`pagerduty_tgt_events`), TPA_EXPORT `#alerts-tpa-pipeline`, ML
  `#monitor-emr`, ATTRIBUTION `#monitor-attribution` (+`pagerduty_attr_events`), TARGETING `#monitor-targeting`.
- **Prod Airflow Variables are empty** (deployment `cmd6bd10c0gl901rfuokgryiq`, `GET /api/v2/variables?limit=200` →
  `total_entries 0`, 2026-09-03 03:22 UTC), so no `JOB_CONFIG_<dag_id>` runtime override (`job_env.get_job_config()`)
  exists and the static routes are the truth. Re-check before trusting a static read of routes.
- **Any static check over `dags/` needs Python >= 3.12.** `dags/tpa/category_taxonomy.py` L147
  (`values = f"({", ".join(['%s'] * len(COLUMNS))})"`) is a PEP 701 nested-quote f-string; 3.11 raises
  `f-string: expecting '}'`. Prod's dag-processor parses it, so prod runs >= 3.12. `dags/test_dags/test_ipdsc.py` L68 has
  the same construct but is in `.airflowignore`. `.github/workflows/pr_alerting_tag_coverage.yaml` pins `setup-python`
  3.12; `pr_airflow_debugger.yaml` stays on 3.11. L167 of the same file emits `SyntaxWarning: invalid escape sequence
  '\c'` on every parse (a `\copy` in a docstring), harmless.
- **Recipe, an ast-only test with no Airflow installed:**
  `uv run --python 3.12 --no-project --with pytest python -m pytest tests/dags/test_alerting_tag_coverage.py -q`.
  `--no-project` is required; without it `uv` resolves the repo `pyproject.toml` (Airflow, PySpark) first. Run only that
  file, never `tests/dags/` (its siblings import Airflow and PySpark). Same zsh trap as the ruff command above:
  `PY="uv run ..."` then `$PY ...` silently runs nothing (`command not found: uv run --python ...`), and a `$F` file list
  did the same to the dispatcher on 2026-09-03. Use a bash array or run the batch under `bash`.
- **CI at `825b07e` ran nothing under `tests/dags/`** (`pr_model.yaml` runs `tests/models` only; `pr_airflow_debugger.yaml`
  runs the debugger package only, path-filtered). PR [airflow-ti#1274](https://github.com/SteelHouse/airflow-ti/pull/1274)
  adds `tests/dags/test_alerting_tag_coverage.py` (stdlib + pytest, `resolve_dag_files(repo_root) -> list[DagFile]`, fails
  closed on a dynamic tag and on a `JobTeamConfig` reference with no module-level `TEAM = JobTeamConfig.X.value` binding;
  `EXCLUDED_CONFIGS` names the knowing exclusions with a reason) and the workflow that runs it on PRs touching `dags/**`,
  `include/job_config/**`, `include/airflow_debugger/daily.py`. DAGs are built three ways (`DAG(...)`, `@dag` from
  `airflow.decorators` (26 files) or `airflow.sdk` (12), a `dag_decorator` alias in 2 attribution files); the resolver keys
  on the `JobTeamConfig` binding and the `tags=` keyword, never on the constructor name.

## Python-client BQ job labels, root `tests/`, and the ruff / pre-commit facts (AUDI-1278, 2026-09-02/03, main `825b07e`)
- **`include/util/bq_job_labels.py::airflow_job_labels(context=None) -> dict[str, str]`** (airflow-ti [#1278](https://github.com/SteelHouse/airflow-ti/pull/1278),
  open 2026-09-03) is the helper for every python-client BigQuery submit: it mirrors `BigQueryInsertJobOperator._add_job_labels`
  (provider 22.4.0: `dag_id.lower()`, `task_id.lower().replace(".", "-")`, both must match `^[\w-]{0,63}$`, else `{}`), reads
  `airflow.sdk.get_current_context()` when no context is passed, accepts `ti` or `task_instance`, and returns `{}` outside a task
  (`get_current_context()` raises `RuntimeError("Current context was requested but no context was found! ...")` there), so DAG
  parse and local pytest never fail. Use `client.query(sql, job_config=bigquery.QueryJobConfig(labels=airflow_job_labels()))` or
  `hook.get_df(sql, configuration={"labels": airflow_job_labels()})`; `BigQueryHook.get_client()` returns a bare client with no
  default labels and `hook.labels` only applies on the cursor `run_query` / `insert_job` paths. `tests/test_bq_job_labels.py`
  imports the provider's `LABEL_REGEX`, so transform drift fails the test.
- **`tests/test_url_pattern_pipeline.py` has 2 pre-existing failures on main, a path bug:** `REPO_ROOT =
  Path(__file__).resolve().parents[2]` is one level above the repo, so `test_url_and_dlv_dag_calls_pass_shared_location[url|dlv]`
  die with `FileNotFoundError` before their AST assertions run (baseline 20 passed / 2 failed; with #1278 34 / 2). Fix is
  `parents[1]` plus whatever the assertions then show (IMP-105). Run:
  `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest tests/test_url_pattern_pipeline.py -q -p no:cacheprovider`.
- **Root `tests/` is not in CI** (`pr_model.yaml` runs `tests/models` only; the debugger / optimizer workflows run their own
  packages; `pr_alerting_tag_coverage.yaml` runs one file under `tests/dags/`), so the local pytest run plus the gauntlet is
  the whole gate for `dags/attribution/*` changes. **The repo root has no ruff config and no pre-commit** (`pyproject.toml` has
  no `[tool.ruff]`, no `ruff.toml`, no `.pre-commit-config.yaml`; only `include/airflow_debugger/` and `include/spark_optimizer/`
  ship their own); the venv has no ruff, so `/opt/homebrew/bin/ruff` 0.16.1 with its resolved defaults is the local linter. The
  six attribution DAG files touched by #1278 carry 43 pre-existing findings on main (unsorted import blocks, naive datetimes,
  root-logger calls); diff against `git show HEAD:` before attributing any finding to a change.
- **Three attribution DAGs do not bill in dw-main-bronze:** `set_gaclid_enabled_flag`, `blocked_ip_addresses_export`,
  `blocked_guids_export` read `dw-main-silver.summarydata.icloud_guids` / `icloud_ipv4` via
  `BigQueryHook(gcp_conn_id="google_cloud_default").get_df()` and produced zero jobs under `airflow-ti-prod@` in dw-main-bronze
  over 2026-08-26..09-01, so they bill to that connection's project and the optimizer's BQ surface never sees them. Which
  project: unresolved (check the connection's `project` extra on the prod deployment). The other python-client sites
  (`url_pattern_pipeline.py` via `BigQueryHook.get_client()`, both tmobile workflows) do bill there: 147 jobs/day,
  3.9 slot-h/day, all unlabeled before #1278. Detail: [[reference_bq_job_attribution]].

## PR 1209 review facts (household TPA export → MemDB, 2026-08-26)
[airflow-ti#1209](https://github.com/SteelHouse/airflow-ti/pull/1209) (Ryan, branch ID-431, OPEN): rewrites `tpa_mntn_id_export` to read prod `ip_data` JSONL, join the identity graph, keep ONE row per household (min-ip tie-break, no cross-IP merge), and write `hh_id_data/` for MemDB; deletes `intent_score_household_map`. Reviewed 2026-08-26, 9 inline comments posted ([review 5035551769](https://github.com/SteelHouse/airflow-ti/pull/1209#pullrequestreview-5035551769)); majors: empty-cats trim no-op, allowlist SQL narrower than the tested Python walker, household pick before DS trim, PACC bare-except, geo_version silent "0" default. Fix commit `10c9da65` verified 2026-08-26: all 5 majors fixed (trim+drop now BEFORE the household pick; PACC catch narrowed to missing-partition AnalysisException; geo_version raises). **Known residual limitation: `audience_clause_sql` still resolves only TWO nesting levels — a 3-deep or→and→any expression yields zero DS ids (now documented in its docstring, prod still SQL-only).** Minor residue: `_read_ip_enrich` wrapper dead, allowlist deduped 3x, normalize run twice. Malachi approved 2026-08-26; merge in Ryan's hands. Facts verified during the review:
...
## Local PySpark testing on macOS (AUDI-1276, 2026-09-03)
- **`/usr/libexec/java_home` returns nothing although Homebrew has `openjdk@17`** — the symlink is missing. Set `JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home` before `pyspark` or Spark 3.5.3 fails silently.
- **PySpark worker child processes inherit the driver's environment but execute `/usr/bin/python3` (3.9) by default, while the driver runs the venv (3.11+).** This causes `PYTHON_VERSION_MISMATCH` at every SparkSession creation. Fix: set `PYSPARK_PYTHON=$PWD/.venv/bin/python` and `PYSPARK_DRIVER_PYTHON=$PWD/.venv/bin/python` in the shell. `export` both; a bare `$VAR` reference inside a Python subprocess will inherit the parent's env, but setting in an arg list does not.
- **Stats-less JDBC relation emulation for local plan checks** (AUDI-1276): `spark.sql.autoBroadcastJoinThreshold=-1` (never broadcast at plan time) + `spark.sql.adaptive.autoBroadcastJoinThreshold=10MB` (broadcast at runtime only if the JDBC side shrinks below 10 MB). This mimics the prod Dataproc decision path where the planner has no stats and plans SortMergeJoin, then AQE rewrites it at runtime. Verified on Spark 3.5.3 with the airflow-ti `.venv` (group `models`).
- **Model-only changes without decorator edits leave `dags/model_task_config.json` byte-identical.** Evidence: `MNTN_SDLC_ENV=dev uv run --group models python model_upload.py --dryrun` after edits to SQL, logic, or read shapes that don't touch `@compute.dataproc_batch(runtime_properties=...)` decorators produces no diff (tested 2026-09-03 on AUDI-1276: four `.py` files edited with `F.broadcast()` hints, JSON unchanged). The model `.py` is read live from GCS on the next run after merge, so model-only changes take effect immediately without bundle redeploy.
- **Workspace `.gitignore` blocks `.csv` and `.zstd` (verified 2026-09-03, AUDI-1276).** Ticket output tables (e.g., `outputs/audi_1276_verdict_table.csv`, `outputs/app-*.zstd` event logs) are not tracked unless force-added with `git add -f`. A deliverable CSV that is the ticket's stated Objective needs explicit tracking or a `.md` companion to preserve it post-session.
- **zsh quoting: `echo =====` fails with "==== not found" because leading `=` triggers command-path expansion.** Use `echo "====="` or `echo $'====='` to echo literal equals.

## Merge order in sprint 8649: dags/model_task_config.json serializes config into git (2026-09-03)
- **Four airflow-ti PRs (#1273, #1275, #1281, #1271) all regenerate `dags/model_task_config.json` from the same base (825b07e, 2026-09-02) in sprint 8649.** Each decorator `runtime_properties` edit (e.g., `maxPartitionBytes`, `spark.speculation`) compiles into that one JSON file, so parallel PRs against different models create a file-level merge conflict on submit. The serialize happens during `model_upload.py --dryrun`, which is mandatory CI on all four. When they all merge in sequence: (1) first PR merges cleanly, (2) second PR must rebase on the merged JSON, re-run `model_upload.py --dryrun` to regenerate JSON from the new base, and re-push; (3) third and fourth likewise. The mechanism is Pydantic serialization, not git; the workaround is mechanical rebase+regenerate. **Actual sprint-8649 order, corrected 2026-09-03 from GitHub (the earlier line here wrote these four as "AUDI-1273 / AUDI-1275 / AUDI-1281 / AUDI-1271", which are PR numbers wearing an AUDI prefix — see the offset section below):** PR **#1273** (AUDI-1269, shuffle.partitions) 19:56 UTC, then **#1275** (AUDI-1270, shuffle.partitions) 20:04, then **#1281** (AUDI-1272, initialExecutors) 20:12, then **#1271** (AUDI-1275, speculation canary) 20:20. Documented for future sprint planning; conflict-free PRs (e.g., camperbid hand-off, shopper_graph labeling, Mode dashboards) do not block.

## The 2026-09-03 hackathon merge train: numbering is offset, and deploys serialize the merges

**PR numbers, AUDI ticket numbers and worktree/branch names are three DIFFERENT numbering spaces and they are
OFFSET in this batch.** PR **#1273 is AUDI-1269** on branch `audi-1269-shuffle-partitions-preverified`; **#1271 is
AUDI-1275**; **#1279 is AUDI-1281**. The tickets were filed before the PRs were opened, and the PRs did not open in
ticket order.

**Always resolve the worktree from `gh pr view <N> --json headRefName`, never from the PR number.** The first rebase
attempt of this train merged main into the WRONG branch — one that was already merged — because the number looked
like a match. Verified mapping (read from GitHub 2026-09-03): #1270→AUDI-1274, #1271→AUDI-1275, #1272→AUDI-1273,
#1273→AUDI-1269, #1274→AUDI-1280, #1275→AUDI-1270, #1276→AUDI-1276, #1277→AUDI-1277, #1278→AUDI-1278,
#1279→AUDI-1281, #1281→AUDI-1272.

**All 11 merged 2026-09-03, each individually deployed before the next merge**, in this order with the squash commit:
#1277 `b836214` 19:10 · #1278 `fc51c0c` 19:18 · #1274 `4091d33` 19:29 · #1279 `090a58f` 19:37 · #1270 `ca3b9e4` 19:44 ·
#1272 `370f2bd` 19:47 · #1276 `fac8e94` 19:50 · #1273 `96b020e` 19:56 · #1275 `f58f756` 20:04 · #1281 `cd353d7` 20:12 ·
#1271 `b9428f4` 20:20.

**Wait for a DEPLOYED status between merges** — merging the next PR before the previous one's Astro build finishes
leaves the superseded-build gap where a merged change never reaches prod. **Each Astro build took roughly 4-8 minutes
end to end**, which is what paced the ~70-minute train. Budget that when planning a multi-PR landing.

**A merged PR can leave main's CI RED if the merge queue was already red.** The `spark-optimizer` CI job on airflow-ti
main was failing before this train: `test_newest_logs_takes_the_tail_and_drops_inprogress` still faked **gsutil's text
listing** and had been broken since **PR #1264** moved the downloader onto the **GCS JSON API**; five test helpers also
lacked the return annotations the pinned ruff requires. Both fixed in **PR #1277**. Check main's CI state before
assuming your branch introduced a failure — and after merging a PR that changes an I/O layer, check whether any test
still mocks the OLD layer.

## An Astro deployment Environment-tab variable does NOT reach a KubernetesPodOperator pod (AUDI-1321, 2026-09-04)

**Highest-value infra footgun on this repo, and it applies to EVERY KPO task in `airflow-ti`, not just the OpenAI
pipeline.** Setting a variable on the Astro deployment's Environment tab configures the Airflow **component** pods
(scheduler, workers, triggerer) and, for secret ones, the `env-secrets` k8s secret. **Neither path reaches a pod
that `MntnKubePodOperator` launches.** The operator (`include/dbx/kube_operators.py`) builds a fresh pod from the
task's `env_vars` dict plus exactly **ONE mounted key** out of `env-secrets` (`OPENAI_API_KEY`) — which is itself
the proof: that key had to be mapped explicitly even though it is already a deployment variable.

**Proven from the prod rendered spec** of `batch_cleanup_1` on `mntn_match_incrementals_submit`, run
`scheduled__2026-09-03T09:00`:
...
`env_from` empty and `pod_template_*` null means nothing else was injected: three template-rendered variables and
one secret key, full stop.

- **To pass a new variable to a KPO task you must add it to the DAG's `env_vars` dict.** There is no deployment-side
  shortcut, so a shipped image reading a new env var is INERT until an `airflow-ti` PR lands.
- **`env_vars` is a `template_field`**, so `"{{ var.value.get('some_name', 'default') }}"` makes the value tunable
  through an Airflow **Variable** afterwards with no further deploy. Set it that way once and the knob is live.
- **Corollary that already burned us:** `STORAGE_ALARM_MIN_FILES` is read by the shipped `openai_batch_runner`
  image but is passed nowhere, so it has ALWAYS fallen back to its default (10,000). Same for
  `OPENAI_FILE_MAX_AGE_HOURS` (`shopper_graph#307`, deployed and inert). **A default that has never been
  overridden is not a configured value — grep the DAG's `env_vars` before believing any env knob is set.**
- **How to check, not guess:** read `rendered_fields` off the task-instance detail endpoint (REST recipes above)
  for the run that actually executed. Reading the DAG source tells you what main says; `rendered_fields` tells you
  what the pod got. See [[reference_mntn_matched_batch_pipeline]], [[reference_astro_deploy_mechanics]].
