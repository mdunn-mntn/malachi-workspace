...
- **model_task_config.json must be regenerated** after any model config change — including any change to a model decorator's `runtime_properties`. Exact invocation (verified 2026-08-27): `MNTN_SDLC_ENV=dev python model_upload.py --dryrun` under the uv dependency group `models`, then commit the regenerated `dags/model_task_config.json`, else the `model-upload-dryrun` check fails.
