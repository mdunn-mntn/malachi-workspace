...
- **Robin Fox stated the approval path himself, 2026-08-25**, which confirms the narrowing above from the person who owns the decision: create the app at `https://api.slack.com/apps/`, generate the bot token, then **request workspace install from inside app management** — that fires a notification to Robin, who reviews the requested scopes and approves. His one condition: "make sure the bot token is handled and stored correctly/safely", i.e. Vault/ESO (`teams/team-engineering-targeting/...`), never a laptop or the Pi. So a Slack bot is NOT banned outright; an unreviewed local one is. Ask for the narrowest scopes the job needs (`chat:write` plus `chat:write.public` if it posts to a channel it is not invited to) — scope review is the gate, so a wide ask is what gets it rejected.
