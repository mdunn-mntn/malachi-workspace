...
**`SLACK_ALERT_CHANNEL` takes a comma-separated list** (code shipped in the #1244-era `notify.py`); on Astro prod it was `"C08CURMGNMQ,C067ZM2EC5S"` 2026-08-28..31; **as of 2026-08-31 it is `"C08CURMGNMQ"` only** (monitor-tpa removed). `#monitor-tpa` carries forwarded emails, not Airflow alerts; the real `*FAILURE*` posts are in `#alerts-tpa-pipeline`, and `vertical_classification_api` does not alert there at all. **CORRECTION 2026-09-02 (appended, not overwritten): the comma list `"C08CURMGNMQ,C067ZM2EC5S"` is RE-APPLIED on prod** — the single-channel var left a debugger diagnosis unable to thread under a monitor-tpa alert (audience_intent_conversions_scoring_14day_lookback, 01:11Z), verified in the var list after the change. Verify the live var, not this memory, before trusting channel routing.

**CONTRADICTION 2026-09-03 (appended, not overwritten; AUDI-1280): `#monitor-tpa` (`C067ZM2EC5S`) IS an Airflow alert channel.** The line above says it carries forwarded emails, not Airflow alerts. Source says otherwise: `include/job_config/job_team_config.py` `JobTeamConfig.TGT` sets `task_fail_slack` to `#monitor-tpa`, and `JobConfig.make_default_args()` wires that into `on_failure_callback` through `SlackNotifier`, so the 36 alerting DAG files bound to `TGT` (`hhdsc_build`, `crm_match_rate`, every `hashed_email_*_signals`, ...) post task failures there; the two 2026-08-29 missed alerts were seen there. Evidence for the older line: the 2026-08-31 entry itself, no source cited. Reconciling hypothesis: the channel carries both, and forwarded emails dominated the window that was read. Settling check: the next `TGT` DAG failure shows up in `#monitor-tpa` as a `SlackNotifier` post from the Airflow app, not a mail forward. Live `SLACK_ALERT_CHANNEL` re-read 2026-09-03 03:45 UTC via `astro deployment variable list` = `C08CURMGNMQ,C067ZM2EC5S` (both channels thread); `SLACK_FALLBACK_CHANNEL=C0BT9TKRMKM`; `OPTIMIZER_SLACK_CHANNEL=C0BSTH6E84T` (`tickets/audi_1290_pipeline_optimization_hackathon/audi_1280_debugger_tag_coverage_ci/outputs/audi_1280_live_slack_channel_vars.txt`). The AUDI-1280 plan's "single channel since 08-31" fact was stale by execution time; read the var, never the memory. **Debugger reply location by alert route:** threaded for the 48 alerting DAGs on `#monitor-tpa` (TGT 36) or `#alerts-tpa-pipeline` (TPA_EXPORT 12); `#airflow-debugger` digest for the 19 on `#monitor-emr` (ML 9), `#monitor-targeting` (TARGETING 3), `#monitor-attribution` (ATTRIBUTION 7). Moving a digest route to threaded is a `SLACK_ALERT_CHANNEL` deployment-variable change, not code. [[project_airflow_debugger]]
