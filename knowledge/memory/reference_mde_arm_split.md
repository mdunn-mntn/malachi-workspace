...
metadata:
  ...
  type: reference
doc_type: memory
keywords: [MDE arm split, nTreated nControl, ghost bid holdout unserved, 1/sqrt(1-h), 1.0541x, 11.8% at h=0.20, spend_required, spendRequired 1.1111x, ti_884_mde_calculator, computeMde, impressionsPerIp, totalIps, AUDI-1323, AUDI-1213, round-trip check]
...
**It has now bitten three implementations:** the standalone gist calculator (`ti_xxx_mde_calculator_prefill.html`, fixed 2026-09-03), its `spendRequired` mirror (fixed 2026-09-03), and the in-product `computeMde.ts` shared by `gary-ql src/gql/types/IncrementalityExperiment/resolvers.ts` and `premier-ui src/app/scenes/Testing/ExperimentBuilder/useMdeForecast.ts` (open, filed as **AUDI-1323**, writeup `tickets/audi_1213_mde_calculator_refresh/artifacts/audi_1213_mde_arm_split_writeup.md`).
