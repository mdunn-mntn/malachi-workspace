...
- **`system.lakeflow` cannot be enabled by ANY customer-side admin — only Databricks can turn it on.** Enabling it via the API returns `lakeflow system schema can only be enabled by Databricks.` (2026-08-21, confirmed by Brian McAdams in the same thread). This **supersedes** the earlier reading that a Unity Catalog *account* admin could grant it: the route is a **Databricks support ticket**, not an internal escalation, so there is nobody at MNTN to ask. Malachi filed one 2026-08-21. Do not re-litigate this with an account admin; the metastore-level toggle is Databricks-side. `system.billing` is a separate schema and its enablement path is NOT known to have the same restriction (IMP-062) — verify before assuming the same answer.
