...
- AUDI-1016 → Eric Salinger: RESOLVED 2026-08-25, ticket VALIDATED not closed. Already on ScyllaDB; Eric sizes ~92% of segment writes as empty, conditional write ~400k→40k tps (10x). Posted to the ticket. Direction agreed 08-25: NO Scylla-side conditional write (needs ~$20k/mo cache to be performant, rejected); fix is upstream, stop supplying duplicate empty-segment records. Work lands on TI/AUDI: find which feed emits the empties, add the filter. Jira description + title rewritten 08-25 to the upstream-filter scope (options: filter at source, or diff-based Kafka delivery per Eric). Ticket is Malachi's to build, sprint-ready.
