...
**HI-tier segmented analysis (follow-up):** To rule out audience composition as the cause of the IVR gap, segmented by intent tier using `advertiser_household_score` (HHST >= 6666 = HI tier). Old prospecting campaigns were 89-99% HI tier traffic. Even within HI tier only, experiment campaigns ran at 0.08-0.53x of historical HI-tier IVR (historical: 0.025-0.117, experiment: 0.008-0.034). This confirms the gap is driven by `is_test = true` campaign delivery behavior, not audience composition differences.

- **Check `is_test` flag** — test campaigns deliver structurally different IVR than production campaigns at MNTN. Cannot use production campaign baselines for test campaign CausalImpact.
...
| TI-504 | Fangorn AIS experiment (RCT) | Direct RCT comparison (t-test, bootstrap CI) + CausalImpact validation + HI-tier segmentation | Edward Martin +41% IVR (p=0.015), Collector Store +40% (p=0.027) — significant. G-Shock/Reedsy/Zumba no effect. CausalImpact with proper covariates showed -37% to -79% (invalid — population discontinuity). HI-tier analysis confirmed gap is structural (is_test flag), not audience composition — even within HI tier, test campaigns run at 8-53% of normal IVR. | **Primary lesson:** Synthetic control fails when experiment creates new campaigns with split audiences. Parent campaign IVR (full audience) ≠ experiment campaign IVR (IP bucket subset). Direct RCT comparison is the correct method when real control/treatment groups exist. Also: `is_test = true` campaigns deliver structurally lower IVR than production campaigns — cannot use production baselines for test campaign CausalImpact. Self-referencing covariates (control_ivr = y in pre-period) produce artificially perfect fits — always verify covariates are external. Test campaigns excluded from all summary tables — must query log tables directly. |
