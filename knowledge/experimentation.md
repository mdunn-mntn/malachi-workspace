...
- **CPIV/CPIA — the "Select is 5x cheaper per incremental visit" headline is a PIPELINE-MEASUREMENT ARTIFACT; on the client basis it's ~1.6x (AUDI-1172, self-verified 2026-07-29; `queries/audi_1172_cpiv_vv_correct.sql` + `artifacts/audi_1172_cpiv_vv_compute.py`).** Two bases:
  ...
  The gap collapses because **Matt's ghost-bid pipeline massively UNDERCOUNTS Reporting visits for display** — non-Select: 223K pipeline incremental vs **6.5M** Reporting VV; Select (CTV) roughly agrees. So the pipeline basis overstates non-Select cost ~3-4x. **The client basis is the one to ship.**
  ...
  - **Residual (small, for Matt's nod not a blocker):** borrowing the 7-day pipeline rel_lift onto a full-window Reporting VV volume assumes the treated/holdout ratio is basis-invariant (unavoidable — holdout has no Reporting VV since it's suppressed).
