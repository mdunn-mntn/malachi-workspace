...
`ti_884_mde_calculator.py:104-109` is correct: `n_treated = n_total * (1 - holdout_frac)`, because only treated IPs receive impressions. The shipped HTML prefill calculator instead split a **spend-derived** IP pool 90/10, which charges the budget for impressions the holdout never gets. **Fixed in the standalone 2026-09-03, still live in the in-product Testing tab** (see the 2026-09-03 status block below). Consequences at the then-current June defaults (p=2.15%, 5% target, $24.84 CPM, 3.5 imps/IP, 10% holdout; the three cohort inputs (p, CPM, imps/IP) superseded 2026-09-03, new medians in that same block; the 5% target and 10% holdout are unchanged):
...
**Direction (settled 2026-09-03): this error is PESSIMISTIC, not optimistic.** Both an MDE quoted too high and a budget quoted too high make a test look HARDER to power and more expensive than it is; correcting the split makes tests look easier and cheaper. Do not describe the 1.0541x / 1.1111x arm-split error as over-optimistic or as overstating power. That is the opposite sign. (The over-optimistic power error in this file is the separate `graph.visits` baseline-inflation trap, TI-1019.)

**Fix `computeMDE` and `spendRequired` in the same pass.** Correcting one alone breaks the budget-to-MDE inverse: a corrected budget then renders 5.270% against a 5% target. The round trip `spendRequired(computeMDE(budget)) == budget` is the check that catches fixing only one side of the pair (held to <1e-9 on the 2026-09-03 standalone fix).
...
**Premier-UI status update (2026-09-03) — the in-product Testing tab and the standalone calculator are now SAME FORMULA, DIFFERENT DATA, and the tab carries the arm-split defect.** Verified against `SteelHouse/gary-ql@cbae0e94 src/utils/mde/computeMde.ts` and `SteelHouse/premier-ui@aaf65d59` (both live default branches) plus the RX-7420 PR series merged 2026-08-25 (gary-ql #4664 `27ffe77c`, #4662, #4665, #4666). **Two live call sites, each importing its OWN copy of `computeMde`:** `gary-ql src/gql/types/IncrementalityExperiment/resolvers.ts` → `forecasted_mde_percent` (the saved-experiment forecast) and `premier-ui src/app/scenes/Testing/ExperimentBuilder/useMdeForecast.ts` → ForecastSidebar + HoldoutSection (the live wizard). A formula fix has to land in both repos.
...
- **The standalone's IVR/CVR toggle never re-pulled the loaded advertiser's baseline (found and fixed 2026-09-03).** In `ti_xxx_mde_calculator_prefill.html`, `setOutcome(o)` used a hardcoded `{ ivr: 2.15, cvr: 0.054 }` and never read `S.advertiser`, so toggling IVR→CVR with an advertiser loaded silently dropped that advertiser's own rate and substituted the cohort default. It also never assigned `S.currentOutcome`, so `clearAdvertiser()`'s `setOutcome(S.currentOutcome)` always restored IVR. **This contradicts the TI-1019 summary claim that the toggle re-pulls the advertiser's actual baseline when one is loaded. That was never true in the shipped file.** Any CVR figure quoted off the standalone before 2026-09-03 is the cohort default, not the loaded advertiser's rate.
- **Which surface answers which question (the rule).** The in-product Testing tab makes you select an already-live campaign group and pins the budget to that selection, so it **cannot** do what-if budget exploration; it only answers "what MDE does this live campaign group's budget buy?". The standalone is the only surface that answers **"what budget would this test need?"**, which is the question customers are actually asking (Edgar von Trotha is fielding a rising number of lift-test budget requests). This breaks the 2026-08-25 premise recorded in AUDI-1213 and memory `project_backlog_gate_pings` that the UI covers delivering advertisers and the standalone only needs the lapsed cohort: the standalone is load-bearing for delivering advertisers too.
- **No ghost-bid *measurement* enters the forecast.** The MDE path and the ghost-bid lift path share no table. The tab's completed-report surface reads the ghost-bid rollup separately; a powered forecast is not a promise of a readout (cf. AUDI-1223 coverage gap).

**Standalone cohort defaults refreshed; the arm split, its `spendRequired` mirror, and the `setOutcome` toggle all fixed in the standalone, shipped 2026-09-03 (AUDI-1213). The tab-side arm split and the ForecastSidebar label are still live.** Re-ran `tickets/incr_75_eligible_advertisers/queries/incr_75_advertiser_metrics.sql` (dry-run estimate 471.7 GB, us-central1 reservation) over **1,859 delivering advertisers**, trailing 30d ending 2026-09-03, no >$1k spend floor. CPM is now the **advertiser-facing** basis (media + data + platform), not `media_cost`. WGU's CPM moving 4.607 → 9.222 is that basis change, not a rate move. The medians below are the CLEAR-button defaults in `ti_xxx_mde_calculator_prefill.html`:
