...
- Sizing the choice for the user: STRICT (require confirmed+ to be eligible) → 112 eligible; STAGED (exclude 17 neg + gate Top) → 1,270 eligible, Top 56→21 (33 demoted). Only 17 of the ~1,175 non-confirmed were truly negative; the rest were short-window artifacts. **Verdict buckets:** `confirmed +` (≥20 holdout visits, p<.05, positive) / `flat so far` (≥100 holdout visits, not sig — enough data, ~0) / `too early` (<100 holdout visits — thin) / `no data yet`.
