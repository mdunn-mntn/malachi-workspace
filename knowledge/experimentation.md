...
- **Gold layer** (`dw-main-gold.reporting.lift__ghost_bid_results` = per campaign×stratum with ghost_frac/compliance flags; `lift__ghost_bid_rollup` = advertiser & campaign_group weighted rollup w/ abs_itt, SE, CI, z, MH stratified estimator, conversions). **DO NOT use the rollup yet:** it is an ALL-TIME accumulation with no date dimension, so it cannot drop 06-22 → it reads spuriously **negative** (pooled −1.8%, ghost_frac 0.104). Verified: same advertisers flip positive (+5%, gf 0.100) once 06-22 is excluded at the silver grain. Matt is adding time-boxing to the gold rollup; until then use the silver visits table with the entry-cohort/exclude-06-22 method. Canonical: `artifacts/incr_75_fold_final.py`, `artifacts/incr_75_final_current_lift.py`; query `queries/incr_75_entry_cohort_excl_leftedge.sql`.
...
- **Score folded into value_score (2026-07-05):** a `confirmed +` lift adds +30 + up to +15 by relative magnitude (capped rel=30%); else +0; keep `apriori_value_score`. Interim score-band tiers (Top≥90/Mid60-90/Low<60) were strictly monotonic (67/126/1077) but let UNPOWERED advertisers into Top — superseded.
- **FINAL tier = POWER × CONFIRMED-LIFT 2×2 (2026-07-05).** Practical reframe: for a "which advertisers to run an incrementality test on" list, the binding constraint for TOP is **power** (can we run a rigorous 5% test?), not the current ghost-measured lift. **Top = `can_hit_ivr_5pct_8w='Yes'` AND `confirmed +`; Mid = powered-5% OR confirmed (one, not both); Low = neither.** → Top 28 / Mid 152 / Low 1,090. **Two durable lessons: (1) you can satisfy at most 2 of {power-gated Top, tier-grouped rows, score reads high→low} — a hard gate on anything other than the score breaks monotonicity, so when a gate is required, treat value_score as a WITHIN-tier rank, not the global sort. (2) A confirmed *current* ghost-lift does NOT make an advertiser testable — if it can't power a clean 5% 8-wk study it belongs in Mid, not Top (e.g. Axos +55% confirmed but MDE 5.19%).** The future-MDE 'power' gate is NOT obsolete after all — it's the practical Top gate. Only 17 of the ~1,175 non-confirmed were truly negative; the rest were short-window artifacts. **Verdict buckets:** `confirmed +` (≥20 holdout visits, p<.05, positive) / `flat so far` (≥100 holdout visits, not sig — enough data, ~0) / `too early` (<100 holdout visits — thin) / `no data yet`.
