...
- **CPIV/CPIA denominator = `incremental_visits` raw count (Matt Brorby confirmed, 2026-07-29).** `lift__ghost_bid_rollup/_results.incremental_visits` (= `abs_itt * n_treatment`) is the correct, sanctioned count to divide spend by for cost-per-incremental-visit; safe as-is (holdout 8-11% is his good-results band; ours ~10%). **Raw-count pooling vs IVW answer DIFFERENT questions:** SUM of `incremental_visits` = volume-weighted total (emphasizes high-spend campaigns) = the right denominator for a cost-per metric; IVW-pooling `abs_itt` = "average CAMPAIGN-level lift". Same cohort: raw implies ~+100% (treated 2.3% vs holdout 1.15%), IVW ~+22%, platform-clean ~+5% - not 3 estimates of one thing, but volume-weighted vs campaign-average vs platform-wide-clean. So a Headline that shows raw treated/holdout RATES next to an IVW abs-lift is internally inconsistent (they answer different questions) - pick one basis per table. **Leg alignment RESOLVED (Matt call, 2026-07-29): no spend restriction, no within-CG confound — every campaign_group sits entirely on ONE bidder (Select→Mountain/Rust p79, PTV→Beeswax p8, never split), so all_facts spend maps cleanly to a CG's single leg; the leg IS the product split.** **SPEND BASIS (Matt call, 2026-07-29) — the blocker is characterized and now bridgeable, not unknown.** Matt tracks NO spend (does not use CIL); his pipeline "visit" = pixel/page-view fire within 7d of an IP's earliest bid, NOT a Reporting Verified/attributed Visit. So dumping all_facts spend over his `incremental_visits` OVER-counts (his visit basis is a narrower subset than Reporting → CPIV biased high). Two fixes, both computable from data we have: (a) SPEND side — households reached ≈ `ip_compliance × n_treatment` (win rate ~50%, `ip_compliance` lives in `_results`) gives a cohort-matched delivery to cost, instead of full-campaign spend; (b) VISIT side (makes CPIV reconcile to the UI) — `all_facts` carries `first_day_visits…seventh_day_visits` (day-since attribution), which is EXACTLY Matt's 7-day window, so we can measure his pipeline→Verified-Visit scaling factor ourselves (compare `_results.vis_treatment` to all_facts 7d attributed visits on the same CGs) — the mapping Matt said he lacks. First-pass (unscaled, total spend media+data+platform, obj=1, 2026-06-22..07-27): pooled Select CPIV ~$6 vs non-Select ~$30 - directional only, do not publish until the pipeline→VV factor is applied. Transcript: `tickets/audi_1172_.../meetings/audi_1172_01_matt_brorby_spend_scaling_2026_07_29.txt`.
- **CPIV RECONCILED — the Select edge is basis-dependent (AUDI-1172, 2026-07-29; `queries/audi_1172_cpiv_reconcile.sql`).** Pooled, obj=1-matched metered `all_facts` spend: **Select CPIV $6.01 vs non-Select $30.47 on the pipeline-visit basis (5.1x cheaper); $21.21 vs $36.53 on the Reporting Verified-Visit basis (1.7x cheaper).** The gap compresses because the **pipeline→VV factor k differs wildly by product**: k = vv_7d / vis_treatment = **0.28 for Select (CTV)** vs **0.83 for non-Select (display)** — Reporting attributes CTV site-visits far more sparsely than Matt's pixel-fire-within-7d count. So "Select is Nx cheaper per incremental visit" MUST state the basis; on the basis advertisers actually see (Verified Visits) the multiple is ~1.7x, not 5x. **SPEND METHOD (the fix for Matt's blocker): don't reconstruct spend from compliance — join actual metered spend from `all_facts` on `campaign_group_id` scoped to `objective_id=1` (matches the lift cohort).** Matt's compliance path (households_reached = ip_compliance×n_treatment, then ×cost/household) algebraically expands to households×freq×CPM = impressions×CPM = the metered spend, so it's the same quantity minus the approximation. Validated 3 ways: cost/household ≈ $0.11 for BOTH products; implied ip_compliance 0.59 Select / 0.44 non-Select (in Matt's ~50% band); eCPM $34 Select / $24 non-Select. **CRITICAL scope gotcha: filter `all_facts` to `objective_id=1`** — non-Select CGs also carry obj 5/6 (MT-S2/S3) spend (~$0.6M) the obj=1 lift never measured; summing all objectives inflated non-Select CPIV $30→$33. Select is 100% obj=1. **Residual uncertainty = the k factor, not spend:** the day-bucket `first_day..seventh_day_visits` as "Verified Visit" and the k=0.83 display value (win rate is only ~44%, yet Reporting attributes 83% of pipeline visits — a bid-vs-impression anchor difference) both need Matt/Reporting confirmation before the VV-basis CPIV publishes.
...
Refreshed the persuadables gradient on the now-longer gold window (`dw-main-gold.reporting.lift__ghost_bid_results`, `stratum_type='score_band'`, clean-gated: `has_valid_holdout AND meets_min_n AND meets_min_compliance AND NOT ghost_frac_inflated AND NOT arm_imbalance_suspect`). Three durable lessons:
