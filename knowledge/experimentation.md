...
**Premier-UI status update (2026-09-03) — the in-product Testing tab and the standalone calculator are now SAME FORMULA, DIFFERENT DATA, and the tab carries the arm-split defect.** Verified against `SteelHouse/gary-ql@cbae0e94 src/utils/mde/computeMde.ts` (live default branch) plus the RX-7420 PR series merged 2026-08-25 (gary-ql #4664 `27ffe77c`, #4662, #4665, #4666).
...
- **Defaults deleted.** `DEFAULT_CPM = 24.84` and `DEFAULT_IMPRESSIONS_PER_IP = 1.5` are gone; CPM and imps/IP come live per advertiser from ChAPI and the tab refuses to forecast when either is missing or non-positive. `DEFAULT_COHORT_IVR = 0.0215` survives as dead code. The standalone gist still defaults to the June cohort ($24.84 CPM, 3.5 imps/IP).
