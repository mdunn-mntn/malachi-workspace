...
### Ghost-bid lift — bias register, the null result, and the persuadables gradient (INCR-66/69, Matt Brorby)

Source: `SteelHouse/databricks_targeting` branch `INCR`, `exploration/INCR-first-ascent/ghost_bid_lift_bias_register.md` (read via GitHub MCP). Matt's living bias register + run log for the production ghost-bid lift measurement. The durable conclusions:

**Headline causal result (2026-06-05, reconciled): internal ghost-bid lift ≈ 0 on BOTH bidders — null and underpowered, not negative.** The raw negatives were bias artifacts (A1/A2/A8 + rare-outcome relative blow-up); the interim positives (+7.2%) were selection (winner-selection A4) + clean-subset choice. After correction the effect is indistinguishable from zero. Binding limitation is **power + visit-window truncation, not residual bias.** This is the rigorous confirmation of the strategic-north-star "guid ≈ 0% lift" observation. Conversions also ≈ 0 (pooled +0.06%, tight null).

**A1 RE-DIAGNOSIS (2026-06-23, PROVEN) — supersedes "frequency-cap asymmetry."** The inflated `ghost_frac` (0.13–0.19 vs the true 0.10 hash) and the spurious *negative* lift are **bid-multiplicity / win-history-exit selection, NOT fcap-asymmetry.** Mechanism: a holdout IP never wins → never exits the prospecting pool → re-enters auctions repeatedly → accrues more qualifying bids → concentrates in high-bid-count buckets → the distinct-IP holdout arm over-weights high-frequency (more-visit-prone) IPs → holdout out-visits treatment → negative ITT that is selection, not an ad effect. Proven: single-qualifying-bid IPs split at exactly **0.0988 = the true hash**; `ghost_frac` rises monotonically to **0.47 at 11+ bids**. (fcap is actually **config-OFF** for these prospecting advertisers — `bid_price_log.threshold_failure_reasons` for obj=1 has no fcap tokens, so fcap can't be the mechanism.)
- **De-biasing lever = the `ghost_frac` gate.** Restrict to clean `ghost_frac ∈ [.095,.11]` and the spurious negatives vanish (a pooled High −4.68 z → +0.12 z). Once gated, the per-campaign z-distribution collapses to ~N(0,1) — positive evidence of a *true homogeneous zero*, not failure-to-reject.
- **Measurement gotcha (the single first-bid anchor):** in the cross-day dedup, carry `visited` from the IP's **earliest-dt** row (one 7d window per IP), NOT `MAX(visited)` over per-day rows. `MAX` gives multi-day (holdout) IPs the UNION of their windows → wider observation → ~6.7pp spurious negative. Distinct-IP dedup alone is necessary but not sufficient — *how* `visited` is carried must be dose-invariant.
- `score_band` is **arm ⊥ band** (ghost_frac flat across bands) → band standardization does no bias-correction work; the real confounder (bid-dose multiplicity) is post-treatment, not captured by bands.

**Estimand + validity:**
- **Production primary = bid-grain ITT scaled to ATT by the win rate `w_c` (~0.70).** Arms are exchangeable at t=0 under the random `MD5(advertiser:ip)<100` hash. Win-grain ATT directly is **winner-selected** (A4: treatment=won vs holdout=subsample); ITT = w_c × ATT.
- **DiD is corroboration only (F3)** — a real test launches a fresh holdout at t=0 with no pre-period to difference; the random-hash ITT is the only t=0-valid estimator.
- **The holdout is a >1-YEAR always-on advertiser-level randomized control** (the 10% hash has run for >1yr; only ghost-bid *logging* turned on 2026-05-27). So treatment−holdout = long-run cumulative lift; validity from randomization, not a pre-period.
- **Negative control = visits to OTHER advertisers** (general activity). Clean ~0 certifies exchangeability; the covariate-balance battery confirms arms balanced on everything except focal-advertiser bid dose (the one post-treatment imbalance, scaling with advertiser volume).

**THE PERSUADABLES GRADIENT (2026-06-25, full pop, clean-gf, 100M+ IPs) — directly validates INCR-75.** Pooling clean-gf campaigns surfaces a small but real, monotonic intent gradient (read relative magnitude, not z — z is N-inflated bias-floor everywhere):

| Score band | base visit% | abs ITT (pp) | rel lift |
|---|---:|---:|---:|
| High (top intent) | 1.228 | +0.0025 | **+0.2%** |
| PP | 0.569 | +0.0092 | +1.6% |
| Mid | 0.271 | +0.0090 | **+3.3%** |
| MaxReach (low intent) | 0.194 | +0.0065 | **+3.4%** |
| no_score (reach) | 0.962 | +0.0007 | +0.1% |

Classic persuadables: **top-intent visits anyway (ad adds ~nothing); untargeted reach adds nothing; mid-intent (PP/Mid/MaxReach) is where the ad changes the outcome** (Mid/PP abs ITT ~3.6× High's despite lower baseline). The cross-advertiser **saturation** cut is the same story: the most-cross-targeted IPs (21+ other advertisers bidding, base 1.96%) are incrementally dead (+0.1%) while less-saturated tiers retain +0.6–1.1% — almost certainly the same population as the High band. **Targeting takeaway: incremental opportunity = mid-intent × less-saturated IPs; avoid top-intent / heavily-cross-targeted / untargeted reach.** This is empirical, population-wide backing for INCR-75's core thesis (high IVR/intent = hard to move; lower-intent = more incremental headroom) and reproduces single-advertiser (ElevenLabs: High +0.2%, PP +3.6%). NOTE: still the Beeswax leg (residual bias possible though gf-balanced); signed per-band magnitude needs the MNTN fcap-symmetric leg (INCR-63).

**Reporting rules (rare outcomes / huge N):** (1) report **absolute pp + CI**, not just relative % — at 10⁵–10⁶ IPs a 0.1pp diff is "z=−15" but trivial (E1: significance≠materiality). (2) use **z** as primary for conversions; treat per-campaign `rel%` as display-only behind a min-conversion gate (≥20 holdout conversions) — NaN-drop + small-N inflate a spurious conv positive. (3) always view **median AND mean** (divergence localizes outlier vs broad bias). (4) the clean-gf z-distribution ≈ N(0,1) is the strongest "true-zero" test. (5) don't headline a single advertiser or pool blindly (E3: one advertiser, e.g. WGU/31357, can dominate a pool).

**Publish gate (INCR-69):** until a powered, fully-observed-window run lands on the MNTN leg, the only defensible statement is *"no significant lift detected; effect indistinguishable from zero; underpowered."* Do not publish a non-zero point estimate.
