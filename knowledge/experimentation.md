...
- **FINAL form = lift-adjusted score + score-band tiers (2026-07-05).** To make the value_score read strictly high→low with the tier, **fold the measured lift into value_score** (a `confirmed +` lift adds +30 base + up to +15 by relative magnitude; else +0; keep `apriori_value_score`) and make **tiers SCORE BANDS** (Top ≥90 / Mid 60–90 / Low <60). The 90 cutoff sits just above the max unconfirmed score (89.7, since only confirmed advertisers earn the bonus) → **Top is guaranteed all-confirmed AND the sheet is monotonic by construction.** Final tiers Top 67 / Mid 126 / Low 1,077. **Key lesson: a two-gate tier (power gate + confirmed-lift gate) can never be monotonic with a single score — any tier rule keyed on something other than the score breaks high→low ordering. Once you fold lift into the score, derive the tier FROM the score (bands), and set the top cutoff above the best-possible-unconfirmed score to keep Top all-confirmed.** The future-test MDE gate becomes moot for advertisers whose lift is already measured. Only 17 of the ~1,175 non-confirmed were truly negative; the rest were short-window artifacts. **Verdict buckets:** `confirmed +` (≥20 holdout visits, p<.05, positive) / `flat so far` (≥100 holdout visits, not sig — enough data, ~0) / `too early` (<100 holdout visits — thin) / `no data yet`.
