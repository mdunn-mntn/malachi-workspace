...
- **Pre-visit vs feedback feature leakage**: When testing IP-level features against IVR, guid_log and conversion_log features are outcome-adjacent — they exist BECAUSE the IP visited. Including them in a targeting model produces AUC ~0.999 (leakage), while pre-visit features alone give AUC ~0.896. Always separate features by when they become available (bid time vs post-visit).
- **Matt's composite importance method works**: XGBoost with 3 importance metrics (gain, weight/frequency, cover) averaged into a composite rank produces stable rankings. Feature importance barely changes between 11 and 58 features (AUC stable at 0.896 ± 0.005).
- **Existing Fangorn signals dominate pre-visit features**: `al_avg_segments` (existing MNTN segment count) and `ci_hh_score` (household score) are top pre-visit predictors. New bidstream features (content_genre, device_make) add incremental signal, especially for cold-start IPs with no existing Fangorn scores.
- **Content genre is mid-tier for prediction but high-value for segmentation**: content_genre ranked ~25th for raw visit prediction, but it's the best feature for *vertical classification* (mapping IPs to advertiser categories). Different use case than IVR prediction — both valuable.
- **Scale matters for feature extraction**: augmentor_log = 241 GB/day, bidder_auction_events = ~400 GB/day. Always dry-run first and use 1-hour samples for development. guid_log and win_logs are cheap (~13-75 GB/day).
