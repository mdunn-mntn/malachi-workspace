...
- **The ghost-bid holdout is a FIXED ~10% platform-wide — not a per-test tunable (AUDI-1148, Gruns, 2026-07-22).** There are no plans to raise/lower it, so "use a bigger holdout (25-50%)" is NOT an available lever for powering a single-campaign incrementality read. Power on ghost-bid lift therefore comes only from (a) campaign SIZE (more impressions → more holdout IPs at the fixed 10%) and (b) **POOLING** across many campaigns/advertisers running the same audience — which is exactly how the population-wide persuadables gradient gets its power. **A small, low-VR campaign cannot resolve a few-percent lift on its own, ever.** Canonical: Gruns CGID 126905 ("CTV Prospecting TOFU High DMA", excludes high intent) — 10% holdout of a 0.19% raw-VR prospecting campaign yields only ~19 holdout visits over 3 weeks (Jun 24-Jul 14) → lift +15% rel but 95% CI [-32%, +63%], p=0.53. Holdout accrues ~1 visit/day, so even running to the Aug 1 flight end (~29 holdout visits) stays inconclusive; the binding constraint is holdout visit COUNT, not window length. **Rule: to answer "does audience X improve incrementality?", pool X across advertisers — don't wait on one campaign.** The window ends 7 days before the data edge because `visited` = visit within 7d of first bid, so recent cohorts are right-censored until their 7-day window matures. Deliverable pattern: `enriched.lift__ghost_bid_visits` filters on `campaign_group_id` for a single-campaign read (entry-cohort, exclude the first left-censored day).
