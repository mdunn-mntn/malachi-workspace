...
**augmentor_identity_daily (SQLMesh/BQ version) — disabled due to timeouts**

The BigQuery SQLMesh model `augmentor_identity_daily` was disabled (likely by data-platform) because it was timing out during processing. The Identity team has migrated to a Spark-based GCS pipeline as a replacement. The existing BigQuery table reference for this model may no longer exist. Downstream models that read from it should be audited for breakage.

### [data_catalog] from scotty in #data-platform
**Reason:** Medium confidence — needs verification
**Confidence:** medium

**audience_hll_by_day — data lineage and current status**

`dw-main-silver.aggregates.audience_hll_by_day` was previously generated by a Spark job (owned/run by a member of the data platform team) that landed data into a GCS bucket. CoreDW then read from it via an external readable table. This model was disabled (PR #1024) and the downstream Clickhouse copy DAG (used for UI reach_meter widgets) started failing as a result. A path to revive it is under discussion. The BQ version of this table was running for 6–8 hours and timing out, which is why the Spark approach is preferred.

### [data_knowledge] from Sean Yang in #tgt-infrastructure-squad
**Reason:** Medium confidence — needs verification
**Confidence:** medium

**mntn-first-party Dataproc job — purpose and schedule**

The `mntn-first-party-YYYY-MM-DD-*` Dataproc job is a scheduled daily (or hourly) job that materializes DS2 data from its source. It runs in `mntn-proj-dev` for dev/test and in `mntn-targeting-prj-prod` for production. Note: the DevOps Compass cost attribution tool has been observed to misattribute or hallucinate cost spikes for this job — confirm via BigQuery information schema or job logs before acting on Compass alerts.

### [data_catalog] from Brian McAdams in #tgt-infrastructure-squad
**Reason:** Medium confidence — needs verification
**Confidence:** medium

**GCP artifact registry — targeting team Python registry**

The targeting team's artifact registry is in the `mntn-targeting-prj-prod` GCP project at: `https://console.cloud.google.com/artifacts?project=mntn-targeting-prj-prod`. As of June 2026, only Docker-format repos exist in this registry; no Python-format repo has been created yet. There are also compute images stored at `https://console.cloud.google.com/compute/images?project=mntn-targeting-prj-prod` — this is where Fangorn picks up its Dataproc ephemeral runtime. New repos must be created by DevOps. An alternative for distributing Python wheels is to upload to GCS and install from there as part of the Dataproc setup JSON.

### [mntn_business] from Abbas in #production-ops
**Reason:** Medium confidence — needs verification
**Confidence:** medium
