...
- **"MM" DEFINITION RESOLVED (AUDI-1083, 2026-07-22) + canonical scoring source.** The authoritative scoring doc is **"Prospecting Scores vs Advertiser Scores (Audience Intent)"** https://mntn.atlassian.net/wiki/spaces/TAR/pages/3487891474. It settles the recurring "is it MM" ambiguity: **"MM" / "MM Core" (the product) = the DS19 keyword layer.** The vertical anchors (DS13 legacy / DS46 Fangorn) are "Peak Performance" / "Fangorn" — **scored, but NOT branded "MM."** So **MM ⟺ has DS19 (or DS38/BUK keywords)**. The broad "MM = DS13/19/38/46" reading is a *different lens* = "touched by an MNTN scoring model"; keep the two flags distinct (`is_mm` = keyword layer; `has_mntn_scoring` = any DS13/19/38/46). Do NOT call DS46-only / DS13-only "MM."
- **MM ≠ quality (the counterintuitive kicker, straight from that page).** The **vertical anchor is the scoring engine**; the keyword layer only splits the top tier. Per-campaign prospecting score: **HI 10K = in Vertical ∩ Keywords; PP 8K = in Vertical, not Keywords; MI 3333–6665 = in Vertical bucket (engagement-ranked: page-views/recency → higher, no-engagement → random lower part); "Unscored" (formerly "Max Reach") = in Keywords but NOT in Vertical.** Consequence: a **DS19-only ("MM keywords-only") campaign is Unscored/Max-Reach** (keyword, no vertical to score against — falls back to the advertiser-level vertical score or nothing), while a **DS46-only ("Fangorn vertical-only", NOT "MM") campaign scores to PP (8K)**. So the non-MM vertical config OUT-scores the MM keyword config. The quality axis is the **score ceiling** (HI-capable = vertical+keyword / PP-capable = vertical-only / max-reach = keyword-only), not the "MM" label.
- **Bidder score waterfall (same page):** RTC score → campaign-level prospecting score → advertiser-level intent score → unscored. Advertiser scores are per-(IP, advertiser) vertical-only (HI 10K = in vertical; no PP/keyword split) so an advertiser has intent ready before any campaign runs.
