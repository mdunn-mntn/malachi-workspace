...
- **DS2 architecture (Malachi + Slack 2026-04-22):** Each time an audience is created, MNTN constructs the expression JSON (combining DS13/DS19/CRM/geo/etc. logic) and **materializes the whole expression as a new DS2 `category_id`** — the DS2 cat is the binary identity of that audience for the bidder. Membership (which IPs qualify) is cached in `tmul_daily` (which holds DS2 + DS3 only) so the bidder does fast segment-membership lookups instead of re-evaluating full expressions at bid time. DS2's category_id space is therefore independent — those IDs do NOT appear under DS13/DS19/DS35 (verified in TI-896 by sampling DS2 cats 514936/555664/565599/etc. — exclusively in DS2). The "DS2 disappearance from expressions" observed Oct 27 2025 in TI-896 is most likely an expression-generation pipeline change (e.g., MMv3 stopped emitting DS2 self-references in newly-built expressions, or the materialization step moved) — NOT an advertiser-targeting shift.
