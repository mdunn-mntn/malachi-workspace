...
- **Media Plan algorithm uses 3 signals (priority order)**: (1) Spendability — can the network support the budget? (2) Historical Performance — VVR across similar verticals, (3) Semantic Relevance — industry/vertical content match. **Spendability is prioritized first, performance second.** This explains why the algorithm picks deliverable publishers, not highest-IVR ones (confirmed via Release Brief and Requirements Doc, 2026-03-27).
- **Flex Targeting = 5-15% budget reserve**: Each media plan reserves 5-15% of budget outside specific network allocations. This "Flex" spend goes to opportunistic buying, emerging inventory, or real-time optimization on un-recommended publishers. Explains ±3% deviation between recommended and actual allocations.
