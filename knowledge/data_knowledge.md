...
| 19 | MNTN Matched (keywords for MM 2.0) | MM batch + parallel RTC | — | — | DS19 = keyword half of MM 2.0 state table. **MM and RTC are independent pipelines** producing similar outputs but via separate code paths (Sean Yang, TI team, 2026-05-29 revised): MM batch fills `household_score` via IPDSC scoring against DS19 keyword config; RTC fills `realtime_conquest_score` in real time when an IP matches recent-visit criteria. Both pathways implicitly use the advertiser's account-level DS19 keyword configuration. AUD team to confirm definitively. **Audience-expression `score` block** (Jordan Piepkow + Ryan Kleck, TI team, 2026-05-29 / 2026-06-01): the `select.score.types` array tells the bidder how to evaluate IPs for bidding. Each entry is `{score_type: "rtc" \| ..., id: <int>}`. **Per Ryan Kleck 2026-06-01: the `id` is a `vertical_id`** — the RTC score gives 10K to IPs that match that vertical in real-time. **CRITICAL WRINKLE (Ryan Kleck, 2026-06-01):** "the score doesn't matter if the HHST is not set." So even though `score_type=rtc` appears in 99.9% of prospecting expressions, **RTC is only effective when the campaign has a Household Score Threshold (HHST) set**. For campaigns without HHST configured, RTC is a no-op even though the flag is in the expression JSON. To find truly RTC-active campaigns: lookup bids for the campaign_id in bid_events and check whether the threshold is set. Implication: "RTC-touching = 99.9% of campaigns" overcounts effective RTC. Ryan acknowledges he doesn't fully understand the bidder mechanism; AUD team needed. |
