...
**Canonical MM / 3P / Mixed campaign classifier + the HHST gate is the NORM (AUDI-1141, 2026-07-20).** For "MM vs 3P" questions, classify each prospecting campaign at the segment level (`audience.audience_segments`, type=2/targeted, DS-ids regex-extracted): **MM** = DS13/19/38/46 present & no 3P; **3P** = DS17 ShareThis/DS18 Dstillery/DS35 LiveRamp present & no MM; **Mixed** = both; **Neither** = CRM/1P/IP-list/geo-only. Live-prospecting spend mix (180d): MM 39% / Mixed 36% / 3P 15% / Neither 10% — **pure 3P is only 15% of spend, 446 advertisers** (reinforces Victor's "~72% of 3P campaigns also carry MM"). **The HHST intent gate (`household_score_threshold_archives.threshold>0`) is the default state on MM prospecting: median in-window gated-fraction = 0.99**; un-gated MM is a small minority (74 qualifying advertisers). This matters for perf comparisons: **un-gated MM behaves like unscored 3P** (median VR 1.3 vs 3P 0.9 vs gated-MM 3.9) — the gate is what creates MM's value, so always split MM by gate. **WGU (31357) = ~39% of ALL pure-3P impressions (VR 21)** → any impression-pooled 3P metric is a WGU artifact; use advertiser-weighted (median) for MM-vs-3P. See [[reference_wgu_pixel_case]].
