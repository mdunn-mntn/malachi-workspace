...
## Short flights (<72 hours) AUTO-trigger a 0 HHST gate — the root cause of "gate thrash" (AUDI-1070, confirmed by Tofer/PEX)
CRITICAL bidder/ops rule: any flight run under ~72 hours automatically gets its **HHST gate set to 0 (max-reach / no intent gate) to ensure deliverability** — the system can't guarantee spend against an intent-gated pool in <3 days, so it drops the gate. So an advertiser who runs **short 1–3 day flights (adding spend in bursts)** will repeatedly and automatically have their intent gate forced to 0, serving unscored/low-intent inventory regardless of the audience.
- This is the mechanism behind AUDI-1070 HexClad's "HHST gate changed 51× (0↔10000)": the client runs short flights → each auto-sets the gate to 0. PLUS (a) a MANUAL change to 0 on the main campaign (93373) around Nov 2025 to hit spend, and (b) the gate was then FORGOTTEN / left at 0. All three stack.
- Implication: "gate thrash" / a 0-gate is often NOT an MNTN model or audience problem — it's a CLIENT campaign-management behavior (short flights). **PEX must educate the client to run flights ≥72h** so the intent gate stays engaged.
- Diagnostic: when you see a campaign's HHST at 0/−1, check flight length (campaign active-days per flight) — short flights explain auto-0. Related [[reference_hhst_pacing_lever]].
