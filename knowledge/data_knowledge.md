...
- **DS2 architecture (Malachi + Slack 2026-04-22):** DS2 is a derived/cached segment layer. A unique DS2 `category_id` is created per campaign / audience at creation time, representing the resolved set of IPs matching that campaign's targeting (computed FROM other data sources like DS13/DS19/etc.). Bidder uses DS2 segment-membership lookups (via `tmul_daily`, which holds DS2 + DS3 only) instead of re-evaluating full expressions at bid time. DS2's category_id space is independent — those IDs do NOT appear under DS13/DS19/DS35 (verified in TI-896 by sampling DS2 cats 514936/555664/565599/etc. — exclusively in DS2). The "DS2 disappearance from expressions" observed Oct 27 2025 in TI-896 is therefore most likely a system/pipeline change in how segment expressions reference the DS2 cache (e.g., MMv3 launch may have reshaped expression-generation to skip the DS2 reference) — NOT an advertiser-targeting shift.
