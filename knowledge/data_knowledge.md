...
- **event_log.ip sometimes includes CIDR notation (`/32` suffix).** Older records or certain event types store `216.126.34.185/32` instead of `216.126.34.185`. Exact string matching (`ip = 'x.x.x.x'`) will miss these. Use `ip LIKE 'x.x.x.x%'` or `SPLIT(ip, '/')[OFFSET(0)]` for reliable matching. bid_ip column does NOT have this issue (always bare IP). Discovered TI-650 v16 2026-03-13.
