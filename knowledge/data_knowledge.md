...
This is why aud22 (Geo Includes/Excludes) can flag a false violation. Two confirmed mechanisms: **(1)** the `location_data` metro_id/hierarchy is internally inconsistent for a ZIP, so the impression side (CIL → network_locations) and the audience/TPA side (reads the `location_data` **hierarchy**) resolve the same IP to different DMAs (fix = PR #1147). **(2) Cross-store geo-build skew:** memdb/audience and CIL are on DIFFERENT geo builds, and a network whose DMA was reassigned across builds (e.g. `161.38.248.0/24` flipped 535 Columbus → 547 Toledo at the Jul-13 build) then resolves in-geo to the store on the old build and out-of-geo to the store on the new one. The three geo positions (active build, CIL's served build, memdb's loaded build) can all differ — resolve at the SERVED `geo_version`. See `data_catalog.md` "Geo store versioning" + memory `reference_aud22_geo_reporting_sync`.
