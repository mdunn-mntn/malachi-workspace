...
**S1 pool lookback: 90 days covers 99.99%, 180 days covers 100% (TI-650 v21b, 2026-03-17).** Initial analysis using MIN(impression_time) showed 186-day max gap, but this was biased — selecting the oldest of multiple matches. Using MOST RECENT prior S1 match: max 69 days, median 6 days, P95 29 days, P99 35 days. Verification: 90d lookback = 68,492/68,498 resolved (99.99%, 6 unresolved), 180d = 68,498/68,498 (100%). The 6 edge cases are IPs whose only pre-VV S1 impressions are >90d old BUT also have post-VV S1 events — the lookback analysis masked them by using MAX(all time) instead of MAX(pre-VV only). **For production: use 90d lookback** (0.009% loss is negligible, halves scan cost).
