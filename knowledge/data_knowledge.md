...
- **DS19-only DOES reach HI — verified from the other direction (2026-07-22, 14d RTC-excl):** DS19-only delivers **69% in the HI band (8001–10000), ~1% PP, 7% MI** — it is heavily scored, NOT "Max Reach only." Controls: DS19+DS46 = 47% HI / 8.5% PP; DS19+DS13 = 51% HI(=10000) / 41% PP (v1 categorical). So the rule holds symmetrically: **HI ⟺ has DS19 (keywords); PP ⟺ has vertical anchor (DS13/DS46).** **MECHANISM (resolves "how does DS19-only score HI with no vertical leaf?"):** the vertical membership scoring uses is the **ADVERTISER's vertical** (the Fangorn/scoring model, a property of the IP), NOT a DS13/DS46 leaf in the campaign expression. Expression leaves drive **biddability** (which IPs the campaign bids), not the **score**. A DS19-only campaign bids keyword-matchers; those also in the advertiser's vertical score HI (keyword ∩ vertical). No DS13/DS46 leaf needed for HI — only the keyword layer. Verify query: `tickets/audi_1083_mm_classifying_view/queries/audi_1083_ds13_vs_ds46_score_bands.sql`.
