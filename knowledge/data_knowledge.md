...
5g. **"Why don't my numbers match the UI?" — the 3-knob reconciliation (AUDI-1070, 2026-06-30).** Any gap between a self-computed ROAS/visits/conv number and the client UI/API decomposes into **exactly three knobs** (+ a small pipeline residual). Diagnose in this order:
   1. **SCOPE — the biggest knob.** The client "Performance Report – MoM" chart (pink-visits/blue-spend/green-ROAS) and a naive API pull are often **prospecting-only (S1)**; the UI summary cards are **AID-wide (all stages)**. Mid-funnel S2/S3 (`funnel_level` 2/3) re-serve warm audiences at ROAS ~49–68×, so all-stages blends to ~22–26× vs prospecting's ~9–14×. **Proof of scope = match the spend** (the chart's blue bars sum to the prospecting spend, not AID-wide). Join `campaign_id → bronze.integrationprod.campaigns.funnel_level` (PK=`campaign_id`; there is no `silver.core.campaigns`).
   2. **ATTRIBUTION completeness — the BQ↔UI knob.** `sum_by_*` **default** revenue/conv cols are display last-touch and **omit CTV (`last_tv_touch_*`)**; the UI = last-touch + last-TV-touch (deduped). Avon all-stages: default ROAS 17.3 → +CTV ≈ 22–23 ≈ UI. (Naive lt+tv SUM overshoots ~6% via double-count; CHAPI dedups — see catalog gotcha.)
