...
- **Impression trace paths by type (2026-03-13):**
  - CTV: clickpass_log → event_log (vast_start/vast_impression) → win_logs → impression_log → bid_logs. CTV always has viewable impressions (no non-viewable VVs).
  - Display (viewable VV): clickpass_log → viewability_log → win_logs → impression_log → bid_logs. **viewability_log is the display equivalent of event_log.**
  - Display (non-viewable VV): clickpass_log → impression_log → win_logs → bid_logs. Many display VVs are non-viewable — must check both viewability_log and impression_log.
