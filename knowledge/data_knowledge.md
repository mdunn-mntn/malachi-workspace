...
**Local setup (from `docs/ONBOARDING.md`):** clone → `python3 -m venv .venv && source .venv/bin/activate` → `pip install -r requirements.txt` → `gcloud auth login` AND `gcloud auth application-default login` (ADC). **GOTCHA (AUDI-1172, 2026-07-31): ADC and the `bq` CLI use INDEPENDENT auth.** The Python `bigquery.Client` uses ADC (`application_default_credentials.json`), which expires mid-session and throws `google.auth.exceptions.RefreshError: Reauthentication is needed` — needing an INTERACTIVE `gcloud auth application-default login` (browser); the `bq` CLI (and `bq_run.sh`) use the separate `gcloud auth login` user creds and keep working. So a live-query Python builder can hard-fail on ADC while `bq` is fine. Fix: wrap `client.query(...)` in a try/except that falls back to the last materialized CSV (loud warning, not silent) so a rebuild never dies on auth — pattern in `audi_1172_select_lift.py`. BigQuery + **Cloud SQL state** access is gated — request via a ticket in the Data Platform (DP) Jira backlog. Alt to venv: Dockerized `make build && make plan` (mounts `~/.config/gcloud` for ADC). `sqlmesh` CLI startup is ~45–60s (loads ~1,700 models + connects to Cloud SQL state); `python scripts/sqlmeshd.py` keeps a warm Context (sub-second subsequent commands).
...
2. **`sqlmesh format`** then `sqlmesh lint`. **CI enforces `sqlmesh format --check` (job "Check SQL Formatting" in `sqlmesh-checks`) and FAILS if any model isn't canonically formatted — run `sqlmesh format` and commit the reformatted files BEFORE pushing** (hit 2026-07-24 on AUDI-1083 #1245: format rewrites JS UDFs from `r"""…"""` to single-quote `'…\n…'`, lowercases keywords, relocates comments — cosmetic only, logic identical). Ignore the "Node.js 20 is deprecated" warnings on the checks — repo-wide CI infra, not your PR. **ORDER MATTERS: format BEFORE the `sqlmesh plan` (step 3).** The plan snapshot is keyed to the model FINGERPRINT (exact text), so reformatting AFTER you plan changes the fingerprint and invalidates the snapshot → CI `verify-impact` then fails with "Missing … deployable impact snapshot … A default sqlmesh plan … has not been applied in any environment for this tree." Fix = re-run `sqlmesh plan dev_<user> --auto-apply` on the formatted code (registers the new snapshot in shared state), then `gh run rerun <run-id> --failed` (no new commit needed — verify-impact reads state).
...
**GOTCHA — prod-access guard + Cloud Identity quota project (hit 2026-07-24, AUDI-1083):** the repo monkey-patches a **custom prod-access guard** onto `Context.plan_builder`/`apply`/`run` (`scripts/sqlmesh_prod_access_guard.py` + `prod_operators_allowlist.py`). A DEV plan is *meant* to be allowed for everyone (guard only blocks `target==prod`), BUT it still evaluates `_allowed()`, which does a **Google Cloud Identity group-membership lookup** (`data-platform-squad@mountain.com`). That lookup **throws an uncaught `HttpError`** (guard only catches `PermissionError`/`ValueError`) when the **Cloud Identity API is disabled on your ADC quota project** — default ADC quota project `mntn-coredw-prod` has it DISABLED, so even `sqlmesh plan dev_<user>` crashes with `HttpError 403 … Cloud Identity API has not been used in project mntn-coredw-prod`. Note this is NOT a warehouse/state-DB access problem (both connect fine) and NOT a real dev-access denial. **Fix (CONFIRMED WORKING 2026-07-24):** `gcloud auth application-default set-quota-project dw-main-bronze` — Cloud Identity API IS enabled on `dw-main-bronze` (checked: dw-main-silver and mntn-coredw-prod do NOT have it). After that the guard passes for a dev plan. (Full env setup that worked end-to-end: `python3 -m venv .venv && pip install -r requirements.txt` → sqlmesh 0.0.1.dev4506; `export SSL_CERT_FILE=$(python -m certifi)`; `sqlmesh info` → both connections succeed; then `sqlmesh plan dev_<user> --no-prompts --auto-apply` — note **`--no-prompts` alone still stops at the final "Apply - Backfill Tables [y/n]" and aborts on non-interactive stdin; add `--auto-apply`** to actually apply. Backfill of a small FULL model is cheap.)
...
**The silver LOG tables (event_log / impression_log / viewability_log) have no data before 2025-01-01** (2025-04-08 for viewability). This is NOT a universal floor — other tables reach further back: `cost_impression_log` to 2023-10-01, `conversion_log`/`sum_by_campaign_by_day` to ~2024-01, `ui_visits`/`visits` to 2023-01-01, `all_facts` to ~2020-10 (see data_catalog.md:38 + per-table docs). Check a table's actual `MIN` before assuming 2025-01-01. Pre-2025 *log* data only exists in Greenplum coreDW (deprecated April 30, 2026). When searching for historical IP/impression data, querying the physical tables directly (bypassing the silver VIEW) does NOT extend coverage — it only avoids the UNION overhead.
...
**`raw_visits` is whatever the advertiser's pixel is placed on, including app and API surfaces — sanity-check the daily rate before comparing advertisers.** ElevenLabs (51660) reports **645,567,794 visits over 30 days = 21.5M/day**, against 1.2M/day for WGU (31357) on 350M MNTN impressions and 1.7M/day for Maurices (66784). That is product telemetry, not a marketing-site audience, so any ratio using it as a denominator (share of site visits, visit rate) is not comparable for that advertiser. Rule of thumb: a daily rate far above the advertiser's plausible marketing traffic means the pixel sits on an app or API surface; footnote or exclude rather than act on the ratio.
...
5f-bis. **Third-source confirmation from the public MCP, and the 1.276× factor is advertiser-specific, not universal (verified 2026-08-19, IMP-047).** MNTN's customer-facing Public MCP (`mcp.ex.mountain.com`, the same PAPI/CHAPI surface the client sees) was diffed against `summarydata.sum_by_advertiser_by_day` for **WGU (31357), 2026-08-01 to 2026-08-07**. Impressions, spend, visits and conversions matched **exactly on all seven days** against `views + clicks + competing_views` and `view_conversions + click_conversions + competing_view_conversions`, and a `MarketingObjectiveInfo.Name` x `ChannelInfo.Name` grid matched exactly on all four cells. Two consequences. (1) The §5f mechanism is now confirmed from a **third independent surface** at the **advertiser-day rollup grain**, not just `all_facts` hour grain, so a `sum_by_*` pull plus `competing_*` does reproduce the client level for these metrics and does not require CHAPI or ClickHouse. (2) The **1.276× is Avon's number, not a constant** — WGU runs **1.173×** on visits over this window (186,235 UI vs 158,821 default last-touch). Do not port the 1.276 factor to another advertiser; recompute it. `Graph.UsersReached` from the API sits within **-0.82% to +1.30%** of `HLL_COUNT.EXTRACT(uniques)`, i.e. inside HLL++ error, so the two are the same measure. WGU `Graph.OrderValue` is `$0.00` and the warehouse agrees it is genuinely zero. Detail: [[reference_mntn_public_mcp]].
...
- **`--dry_run` on `INFORMATION_SCHEMA.JOBS_BY_PROJECT` over-estimates the scan by ~27x (AUDI-1316, measured 2026-09-03):** a single-day window returned 4.86 GB estimated bytes but billed 0.178 GB actual (4,863 MB est vs 191 MB actual). The estimate scales with the window (1d 4.86 GB, 7d 16.1 GB, 30d 36.2 GB), so queries against multi-day windows trip the workspace's 5 GB dry-run abort gate while actually costing cents. Validate by running single-day queries and extrapolate; the dry-run cannot be trusted to gate a larger window. Only the single-day measured run is logged in `knowledge/bq_perf_log.jsonl`; multi-day estimates are unverified.
...
**The other direction — a MEASUREMENT cohort must apply the flag, because internal test advertisers pass every statistical quality gate (TI-1313, 2026-09-01).** Advertiser **30506 = "MNTN - No ENG Testing"** (`is_test = TRUE`, `deleted = FALSE`, re-verified 2026-09-02) delivers enough real volume to clear all of the ghost-bid lift gates on `gold.reporting.lift__ghost_bid_results` (`has_valid_holdout`, `meets_min_n`, `meets_min_compliance`, `NOT ghost_frac_inflated`, `NOT arm_imbalance_suspect`) — those gates test sample size, holdout balance and compliance, none of them look at `is_test` — so it enters an incrementality or lift cohort silently and contributes an internal-traffic campaign group to a client-facing read. Filter `is_test`/`deleted` on **`advertisers` AND `campaign_groups`** explicitly for any measurement cohort; the caveat above (keep test-flagged spend) applies to spend/billing rollups, not to lift.

**The LEFT-JOIN filter trap (same ticket, cost a re-run):** a `WHERE is_test = FALSE` placed inside a CTE that is then **LEFT JOINed** to the fact rows **excludes nothing** — the dropped dimension rows simply come back as NULL columns and the fact row survives the join. Either use an INNER JOIN to the filtered dimension, or push the filter down into the base/fact query. Same shape as the `advertiser_id != 90` silent no-op noted under §Additional Gotchas.
...
**"Days live" must be counted from OBSERVED DELIVERY, not from config dates — config can never see a mid-window pause (TI-1313, 2026-09-02).** Three ways to ask "did this campaign group run for most of the window", measured over 877 prospecting campaign groups in a 71-day window at a 75% threshold (≥54 of 71 days): **observed delivery** (distinct days with impressions in `dw-main-silver.summarydata.sum_by_campaign_by_day`) excludes **374**; `bronze.integrationprod.campaign_groups.start_time`/`end_time` excludes **258**; `ui.ui_flights` schedule dates exclude **248**. The three nest exactly — pass-observed & fail-config = **0**, pass-observed & fail-flight = **0**, pass-config & fail-flight = **0** — so the observed-delivery pass set sits inside both config pass sets and **the delivery filter alone subsumes both**; adding the config filters on top changes nothing. The **116 groups caught ONLY by observed delivery are mid-window pauses** (median 35 delivered days of 71): `campaign_groups` is a current-state dimension with no history, so a start/end pair can never show a group that stopped and restarted inside its own window, and a scheduled flight is a schedule, not delivery. **Rule: for any exposure-window, days-live, or "continuously delivering" cohort filter, count delivered days.** (Pairs with the flight-fragmentation note above — the flight view and the delivery view disagree in BOTH directions: many short flights under one continuous delivery run, and one long flight over a paused run.)
...
**Geo is ALWAYS AND-joined to the category block — and for EXCLUDES that AND-of-NOTs is a UNION (Zach Schoenberger, 2026-08-12, Slack; person's word, not a code read).** Two rules behind the `geos.where` structure above:
- **Includes: there is no OR-with-geo, by design.** "We've never supported doing any version of category targeting where the categories are OR'd with geo" — the platform intentionally offers only geo AND categories, because an OR would nearly always be misused. So a buyer ask for "categories OR geo" on the include side is genuinely unsupported, not a config gap.
- **Excludes: the engine applies `NOT IN (RT or CRM) AND NOT IN GEO`, which is identical to `NOT IN (RT or CRM or GEO)`.** An IP in an excluded geo is dropped whether or not it also hits an excluded list, and vice versa (De Morgan: an AND of NOTs across axes = a NOT of the OR). Same direction as item 7b's single-attribute rule (stacked exclude clauses OR, so you inherit every flag) and item 8's AND-NOT exclusion pattern.
- **The audience-builder UI mislabeled this until 2026-08-12.** It joined the exclusion rows with **AND**, which reads as `NOT IN ((RT or CRM) AND GEO)` — exclude only IPs in a list *and* in the geo, a strictly narrower exclusion than what runs. Filed to the PRO team as **PRO-636** (raised by Paulo, confirmed by Zach: "exclude should show as OR to be correct"); the display was fixed the same day, 2026-08-12. Includes still display AND, correctly. **Analytical consequence:** never read exclusion semantics off a UI screenshot or a buyer's UI-derived description (especially pre-2026-08-12) — it understates the exclusion. Parse the stored expression (`audience.audience_segments` targeted segment / TPA expression JSON), and when sizing what an exclusion removes, sum the UNION across axes, not the overlap. Distinct from the aud22 geo *data* mismatch class (see `reference_aud22_geo_reporting_sync`), which is a serve/table issue, not boolean logic.


**Full `geo.location_data.location_type_id` enumeration + zip/local-narrowing detection (AUDI-1141, 2026-07-20).** Completes the geo picture above: **2=country (251), 3=DMA-by-Nielsen-code (210, `location`="500".."881"), 4=DMA-by-name (210), 5=state/region (4,364 incl intl), 6=city (275,686), 7=zip/postal (425,853 incl intl).** Geo targeting is expressed **ONLY** via `location_ids` — there is NO `postal`/`zip`/`region`/`location_type` token in expressions (verified across 41,423 prospecting campaigns). So "is this campaign zip-narrowed?" = does its geos **INCLUDE** block carry a `location_type_id=7` id. **Include vs exclude parse:** geos = `"geos":{"where":{"op":"and","value":[ <INCLUDE or-block>, {"op":"not", <EXCLUDE or-block>} ]}}` — the INCLUDE block is everything before the first `"op":"not"`; grab it with `REGEXP_EXTRACT(expr, r'"geos":\{"where":\{"op":"and","value":\[(.*?)\{"op":"not"')`, pull its `location_ids`, join `geo.location_data` for the type. A **separate** local-targeting form is `geo_radii` (lat/long+radius, `REGEXP_CONTAINS(expr,'geo_radii')`) — 2,284 of 8,224 live prospecting campaigns use it vs 1,665 zip-include and 1,094 city-include; radius is the biggest local signal, don't miss it. Reusable pattern = `tickets/audi_1141_mm_vs_3p_by_vertical/queries/audi_1141_cohort_scorecard.sql`.

**Canonical MM vs 3P campaign classifier: OR-additive vs AND-narrowing, plus the HHST gate is the NORM (AUDI-1141, 2026-07-20).** For "MM vs 3P" questions, classify each prospecting campaign from its latest targeted segment (`audience.audience_segments`, type=2/targeted). MM signal = DS13/19/38/46 positive; 3P = DS17 ShareThis/DS18 Dstillery/DS35 LiveRamp positive. **Do NOT lump all MM+3P campaigns into one "Mixed" group.** Whether 3P narrows MM depends on OR vs AND: ~85% of MM+3P campaigns join the 3P by OR (additive reach, MM still scores) and only ~15% by AND (intersect/narrow). Use the TI-999 Pass 26 JS UDF (LCA tree-walk of `categories.where`) to get semantics OR_include vs AND_include. Groups: **MM** = MM, 3P OR-additive or absent, geo broad; **MM restricted** = MM with AND-include 3P OR narrow geo (zip/city/radius); **3P** = 3P positive no MM; **Neither** = CRM/1P/geo-only. Spend mix (180d): MM 51% / MM restricted 24% / 3P 15% / Neither 10%. **The HHST intent gate (`household_score_threshold_archives.threshold>0`) is the default on MM prospecting (median in-window gated-fraction 0.99)**; split broad MM into gated vs no-gate. Both restrictions hurt: median IVR MM-gated 0.46% vs MM-restricted 0.18% vs MM-no-gate 0.13% vs 3P 0.07% (advertiser-weighted; MM-gated wins IVR and CPV in all 8 sales verticals). **Rates follow TI-999 Pass 26: IVR = visits/impressions, CVR = conversions/impressions, CTR = clicks/impressions (all over impressions).** WGU (31357) is ~39% of pure-3P impressions, so any impression-pooled 3P metric is a WGU artifact; use advertiser-weighted median. See [[reference_wgu_pixel_case]] and TI-999 Pass 26 (`ti_999_pass26_or_vs_and_include.sql`).
...
**A "campaign group" is a FULL FUNNEL, not one campaign — audit at campaign×objective grain (TI-1037 module 00, 2026-07-02).** Each MNTN `campaign_group` bundles one campaign per funnel stage: **stage = `objective_id`** (1=Prospecting, 4=Retargeting, 5=Multi-Touch S2, 6=Multi-Touch S3, 7=Ego). `funnel_level` is NOT a clean stage key — inside the all-retargeting group it is reused as a sub-tier (general/5+PV/cart), so a retargeting campaign can show `funnel_level=1` while `objective_id=4`. **Consequence: grouping delivery by `campaign_group_id` CONFLATES stages** — e.g. Kindred group 69884 "CTV Prospecting High Pop" = Prospecting(F1,CTV) + MT-S2(F2,display) + MT-S3(F3,display) + Ego; its group-level reach/ROAS (as used in TI-1037 modules 12b/12c) blended CTV prospecting with display retargeting. Always filter/split by `objective_id`. Also: **prospecting runs on CTV (`channel_id=8`), the Multi-Touch stages on DISPLAY (`channel_id=1`)** — channel mixes inside one "CTV" group. **Where the money is (Kindred, Jan–May '26): RETARGETING = ~26x ROAS, ~85% of revenue on ~28% of spend (15,758 conv); Prospecting = 62% of spend but 13% of revenue at ~1.9x.** The prospecting YoY decline is a top-funnel-REACH story, not the revenue engine — always map stage economics first (module 00) before deep-diving one stage. Multi-Touch S2 spent ~$18K for ~0 last-touch conv (assist-only / attribution).
  ...
  - **⚠️ CONTRADICTION, appended not overwritten (TI-1313, 2026-09-04).** The line above says the Multi-Touch
    stages run on display (`channel_id=1`). Measured on 433 ghost-bid campaign groups over 22 Jun to 31 Aug
    2026 (`sum_by_campaign_by_day` joined to `public.campaigns`), **stage 2 and stage 3 are MIXED, not
    display-only**: stage 2 = $1,945,567 CTV and $1,601,044 display; stage 3 = $932,652 CTV and $612,723
    display. **Stage 1 is the half that is absolute** — $29,309,489, 100% `channel_id=8`, zero display.
    Evidence for the display-only reading: TI-1037's Kindred group 69884, one advertiser. Evidence for the
    mixed reading: 433 groups across 296 advertisers. Reconciling hypothesis: display-only was true of the
    OLD parent/child group structure and Multi-Touch has since gained a CTV leg, which fits the 48,934
    "Beeswax Television Multi-Touch Plus" campaigns already noted below. **Settles it:** split
    `channel_id` by `funnel_level` AND by group create date. Until then, never assume a stage's channel,
    query it. **The safe direction is stage 1: prospecting is CTV, exactly.**
...
| ✓ | — | — | 57 | 42 | $0.5M | 1.1% | **Peak Performance (v1) only** ("vertical only") — bids all in-vertical IPs → **PP 8000 pin ONLY** (no categorical HI without the keyword layer; verified 2026-07-22 RTC-excl: 83.9% at exactly 8000, 0% at 10000 — see §"CLEAN RULE" below). Earlier "HI 10000 + PP 8000" claim was RTC, corrected; not yet flipped to v2 |
...
- **"MM" DEFINITION — canonical (AUDI-1083, 2026-07-22). MM = any of DS13/19/46 present (BROAD).** Per the canonical taxonomy page §4 (https://mntn.atlassian.net/wiki/spaces/TAR/pages/3691708511): **a "has DS19" test is an UNDERCOUNT — it misses the vertical-only cells ≈7.6% of prospecting spend, which ARE MM.** "MM Core" = DS19 is a **COMPONENT name inside MM** (the keyword component), NOT the definition of MM. So DS46-only / DS13-only ARE MM (vertical-only MM), just without the keyword component. This means the classifier view's original `has_mm = DS13/19/38/46` is already correct; a DS46-only gated/national campaign being `is_unmodified_mm=TRUE` is CORRECT (it is MM, well-configured), and `is_flagship` = DS19+DS46 (the doc's named "flagship") stands. (A user statement 2026-07-22 "it's only MM if DS19" was the MM-Core-vs-MM-umbrella mixup, corrected by this doc.)
- **CLEAN RULE (EMPIRICALLY VERIFIED 2026-07-22, AUDI-1083): HI (10000) requires the keyword layer (DS19); PP (8000) requires the vertical anchor (DS13/DS46).** Per-IP score: HI = in vertical ∩ the campaign's keywords; PP = in vertical, no keyword; MI 3333–6665 = in bucket not vertical; Max Reach 1–3332 = keyword-only outside bucket; non-MM = unscored (−1). Config → reachable tiers: **DS19+anchor → all four** (HI·PP·MI·MaxReach); **DS19-only (mm_keywords_only) → HI·MI·MaxReach, NO PP** (bids keyword-matchers incl. the vertical∩keyword HI slice; PP IPs are vertical-no-keyword, don't match the keyword leaf); **vertical-only with NO DS19 (BOTH fangorn_vertical_only AND vertical_only_legacy) → PP·MI, NO HI**; **non_mm → unscored**. **This CORRECTS a prior documented claim** (this doc's old "PP v1 only → HI 10000 + PP 8000" line + canonical taxonomy 3691708511 §3 "DS13-only → HI+PP"): verified via 30d RTC-excluded delivered `household_score` — **DS13-only = 83.9% at exactly 8000, ZERO at 10000; DS46-only = 0.1% above 8000, ZERO at 10000.** Both vertical-only configs cap at PP; the HI those docs attributed to DS13-only was RTC firing, not categorical scoring. v1 pins at 8000, v2 spreads 6666–8000 — same ceiling.
- **DS19-only DOES reach HI — verified from the other direction (2026-07-22, 14d RTC-excl):** DS19-only delivers **69% in the HI band (8001–10000), ~1% PP, 7% MI** — it is heavily scored, NOT "Max Reach only." Controls: DS19+DS46 = 47% HI / 8.5% PP; DS19+DS13 = 51% HI(=10000) / 41% PP (v1 categorical). **AUTHORITATIVE (Ryan Kleck, `audience_intent` DAG page, the PROSPECTING-scores column = the scores we use): HI (10K) = in Vertical (DS13) AND in Keywords (DS19) — BOTH required; PP (8K) = in vertical, no keyword; MI = in bucket, not vertical; Unscored (prev Max Reach) = outside bucket/vertical but INSIDE keywords.** The rule is NOT symmetric — HI needs both sets. **The DS19-only split maps 1:1 onto Ryan's four rows and CONFIRMS the model** (not contradicts it): 69% HI (keyword∩vertical), ~1% PP, 7% MI (keyword in bucket-not-vertical), ~22% MR/unscored (keyword-matchers OUTSIDE the vertical = exactly Ryan's Unscored definition). **MECHANISM (resolves "how does DS19-only score HI with no vertical leaf?"):** the Vertical (DS13) the score intersects is the **ADVERTISER's vertical**, always fed into the prospecting score; Keywords (DS19) are present only when the campaign carries a DS19 leaf. That asymmetry is why "DS19 unlocks HI" reads as a config rule: the vertical rides along from the advertiser, and the keyword layer completes the vertical∩keyword intersection. Expression leaves drive **biddability** (which IPs the campaign bids); the **advertiser vertical + campaign keywords** drive the **score**. No DS13/DS46 targeting leaf is needed for HI — a vertical-only campaign (no DS19) has no keyword set, so every in-vertical IP is vertical-no-keyword → PP, 0 HI. Verify query: `tickets/audi_1083_mm_classifying_view/queries/audi_1083_ds13_vs_ds46_score_bands.sql`.
- **MM PRODUCT-VERSION NAMES (Alyson, 2026-07-22) — how the team refers to MM configs, mapped to DS-leaf structure:** **mmv1** = vertical categorization era (DS13-only), shipped ~Dec 2023–Jan 2024. **mmv2** = keywords (DS19-only) = "MNTN Matched V2". **mmv3** = keywords + vertical (DS19+DS13), shipped ~**Sept 2024** (AP holds exact release date). Version is NOT purely structural — a **DS13-only** campaign is **mmv1 if created before the mmv3 release, else mmv3** (temporal). Empirically validated: DS13-only campaign creation ramps through 2024, **collapses Sept→Oct 2024 (15→3/mo)**, trickle from late 2025 — a clean ~2024-09-01 boundary (mmv1 cohort's latest create = 2024-08-30). **Fangorn (DS46) = "updated DS13"** (same vertical slot, continuous scoring), so DS19+DS46 = the Fangorn generation of mmv3. AUDI-1083 `mm_class` uses these names in place: `mmv2`/`mmv3`/`mmv1`(-or-mmv3 by `campaign_created`), while `mm_flagship_fangorn` (DS19+DS46) / `fangorn_vertical_only` (DS46-only) / `non_mm` keep structural names.
...
- **SCORING GENERATIONS — v1 = categorical fixed points, v2 = score bands where THE LABEL FOLLOWS THE SCORE (verified 2026-07-08, 7d of delivered CIL, RTC-excluded, all live v1/v2 prospecting).** Per the Fangorn methodology page (https://mntn.atlassian.net/wiki/spaces/TAR/pages/3414917161): Fangorn = continuous 0–1 intent score; raw boundaries 0.6/0.8 divide MaxReach/MI/HI, transformed onto the legacy 3333/6666 pacing points. Empirical delivered-score distribution:
  ...
  - **v2 (DS46): two model passes per IP, each a continuous band with a pin at its top** — PP pass → **6666–8000** (3.8M imps over 1,206 distinct values + 2.1M pinned exactly 8000); HI pass → **8001–10000** (11.0M over 1,868 values + 2.0M pinned exactly 10000). An IP that structurally matches vertical/keywords but scores <0.8 raw lands in MI/MaxReach — **qualifying criteria feed the model; only the score puts an IP in the HI/PP groups** (Malachi's "two scores, only above-bar counts" reading, confirmed).
    - **RECHECK 2026-07-21 (TI-1037, current 7d CIL, same variant-3 query, RTC-excluded): the top pins are GONE.** DS46 now delivers PURELY continuous bands with **zero** impressions at exactly 8000 or exactly 10000: PP band = **6666–7878** (4.2M imps, 1,188 distinct, min 6666 / max 7878), HI band = **8001–9933** (18.0M imps, 1,864 distinct, min 8001 / max 9933), plus an empty gap at 7879–8000. So the two SCORING-SIDE band **floors (6666, 8001) are stable and true**, but the "pinned exactly 8000/10000" behavior in the 2026-07-08 read no longer holds — most likely a model retrain (aug+guid feature store) removed the cap-pinning. Not yet root-caused; treat the exact band TOPS as model-version-dependent (may drift week to week), the floors as fixed. **Dashboard implication (TI-1037 perf_report):** the fixed tier cutoffs (PP 6666–8000, HI 8001–10000) still classify BOTH generations correctly — DS46's continuous PP (≤7878) and HI (8001–9933) sit inside those buckets, and v1's fixed 8000/10000 land in PP/HI — so **no per-campaign DS13-vs-DS46 branching is needed for score-tier bucketing.**
...
- **A ClickHouse reporting export keyed on `cgid` is the PARENT campaign group — expand before joining a BQ group-grain view (verified 2026-08-19, AUDI-1083).** The squad's goal-attainment lists (e.g. the "bad cpa" tab behind the 8/21 Insight Spelunking session) come from ClickHouse, not BQ: `summarydata.all_facts_by_day_ramp_combined` aggregated to `parent_campaign_group_id` via `dictGet('default.parent_campaign_group', ...)`, joined to `info.v_parent_campaign_groups` (name, `goal_type_id`, `goal_value`), `info.goal_types`, `info.v_campaign_group_statuses` (`name = 'Live'`), with attainment from the server-side `campaign_achievement_fn(parent_cg, roas, cpa, cpv, cpcv)` and spend including a `legacy_spend` column BQ does not carry. Its `cgid` output column is therefore the **parent** group. BQ's `bronze.integrationprod.campaign_groups.parent_campaign_group_id` is the bridge: expand each exported id to `{self} ∪ {children}` before rolling Stage-1 flags up, or a parent whose audience lives on a child group silently reads as unlabelled. On the 251-row 8/21 list, 10 ids had a child group (expanding changed no label there, but the join is only correct because of it). Spend also will not tie: that list showed $220,763 for FICO group 81053 where `sum_by_campaign_by_day` shows $369,692 over the same dates — different table, different rollup, `legacy_spend` included. Reconcile deliberately, do not assume.
- **Detecting Peak Performance by a DS13-only regex undercounts it ~4x today (verified 2026-08-19, AUDI-1083).** The ClickHouse-side PP filter in circulation is `match(argMax(expression, _version), '"data_source_id":\\s*13([^0-9]|$)')` against `integrationprod.audiences`, i.e. **DS13 only**. Because the v1→v2 flip swaps 13→46, that misses every PP v2 campaign: on the 8/21 bad-CPA list, 161 of 251 groups are PP and **128 of those are v2 (DS46), only 33 v1 (DS13)**. Any ClickHouse-side PP predicate needs `data_source_id: 13` **or** `46`. Same trap as the BQ-side `tiers_reachable LIKE '%PP%'` above, opposite direction: one over-counts 2.3x, this one under-counts ~4x.
...
- **Select vs PTV spend — never compare on `media_spend` alone (AUDI-1172, 2026-07-29).** For **product_id=2 (MNTN Select)** `platform_spend`=$0 on 100% of rows (verified 59.7M rows, `objective_id=1`, Jun-Jul 2026) because Select bills a single **gross blended CPM** (~$47) with the platform margin bundled INTO `media_spend`. **PTV (product_id=1)** unbundles: net media CPM ~$10 + `platform_spend` ~$16 separate (`integrationprod.channels` id 5 = Platform Fee). So a media-only Select-vs-PTV comparison FLATTERS PTV (drops its biggest cost) and understates Select's true cost. **Use total = `media_spend+data_spend+platform_spend`** (canonical, additive) as the apples-to-apples advertiser cost for both. Residual: whether Select's blended CPM fully captures MNTN's Select margin, or some lands only in `invoice_select_publisher_invoices`/`mntnselect_offering_versions`, is unconfirmed.

- **The same rule fixes the CPM BASIS, and the two bases differ ~2x (AUDI-1213, verified 2026-09-03).** An advertiser-facing CPM is `1000 * (media_spend + data_spend + platform_spend) / impressions`. A CPM computed from `cost_impression_log.media_cost` is a media-cost CPM and is not what the advertiser pays. WGU (AID 31357) trailing-30d reads **CPM 4.607 on the `media_cost` basis vs 9.222 advertiser-facing**: a basis change, not a rate move. Any CPM feeding a spend or budget forecast (for example an MDE power calculator's spend-required conversion) must be the advertiser-facing one.
...
- **Domain ecommerce classification cannot be done from the domain name or the score — MEASURED
  (AUDI-431, 2026-08-11).** Fetching all 2,484 proposed blocklist rows found a **3.06% false-blocklist
  rate: 76 real retailers** that name/knowledge-only judgment and the prod classifier both missed.
  **The systematic blind spot is a content site with a store attached** — recipe/craft/travel blogs
  selling their own goods at `/shop` or on a `shop.<domain>` subdomain (`keviniscooking.com` spice
  rubs, `hearthookhome.com` 224 crochet patterns, `pantrymama.com` cookbooks), plus publishers with
  first-party Shopify stores (`bonappetit.com`, `newyorker.com`, `sfchronicle.com`, `lemonde.fr`).
  Thousands of article URLs bury a handful of product pages, so `med_score` reads "blog":
  `tastesbetterfromscratch.com` scores 0.033 with **0%** of URLs over the 0.4 cutoff and sells a
  $14.99 cookbook. Conversely **`pct_ge_04` near 100% is NOT evidence of a store at any median** —
  of 41 such domains only 2 were real (the rest are template-generated URL sets scoring uniformly
  just above the cutoff). Always check `/shop`, `/store`, `/products`, `/collections` and the nav
  before calling a domain non-ecommerce. **~6-7% of scored domains are dead** (`cootlogix.com`:
  rank 19 at 88M rows/28d, no site connected). Runbook:
  `documentation/runbooks/ecommerce_list_refresh_runbook.md`.
...
**DS47 (CRM Identity Graph Generated) is NOT a per-IP superset of DS4 — the 2026-07-01 DS4→DS47 exclusion-clause migration DROPPED most direct-matched IPs (PS-8572, 2026-08-06).** DS47 membership is 2.2–2.4x LARGER than DS4 in aggregate (graph expansion; Lovepop 58797 upload 28594: 13.87M DS47 IPs vs 5.87M DS4 at dt 7/15), but it covers DIFFERENT IPs: of 1,300 converting IPs that were DS4 exact-matches at ipdsc dt 6/30, only 182 were also DS47 members at 7/02 (281 DS47 members total) — **86% of direct matches absent from DS47**. The bidder exclusion evaluates DS47 ONLY (since the 7/1 release), so the platform-wide DS4→DS47 migration of CRM exclusion clauses silently removed most direct-matched customer IPs from live exclusions. Never reason "DS47 is bigger, so exclusion reach can only grow" — verify per-IP overlap in `ipdsc__v1` at the relevant `dt`. Handed to the Audience squad as a platform issue. [[reference_crm_exclusion_serve_time]]

**CRM exclusion clauses live ONLY on the S1 (prospecting) campaign — S2/S3 stages carry NO CRM/DS21/DS34/DS2 excludes (PS-8572, 2026-08-06).** Verified across every archived segment version (Lovepop segments 738123/738128, 5 versions each, zero CRM hits): the S2/S3 stage campaigns of a campaign_group run only DS16 stage-progression includes + the DS14 gate + holdout. Standalone retargeting groups get no CRM clause by design (Zach rule: CRM unusable in retargeting). Consequences: (a) converter suppression on S2/S3 relies ENTIRELY on S1 gating entry into the DS16 stage segments; (b) an IP already in the S2/S3 pools when its CRM list is attached (or CRM-excluded late) keeps getting served by S2/S3/RT — "ads after converting" complaints are usually sibling-stage serving, not an S1 exclusion failure. Any serve-time leak test must split S1 vs S2/S3/RT before declaring a violation.
...
- **BQ equivalent VALIDATED (AUDI-1173, 2026-07-28)** — use `MOD(ABS(CAST(CONCAT('0x', SUBSTR(TO_HEX(MD5(CONCAT(CAST(advertiser_id AS STRING),':',ip))),1,16)) AS INT64)),1000)`. Takes the first **16** hex chars (NOT 15), buckets 0-99 = platform holdout. This same MD5(AID:ip)-16hex-mod-1000 hash is the RCT bucketer for any new household-randomized experiment; use disjoint bucket ranges (e.g. 100-999) to stay clear of the 0-99 platform holdout. (Full BQ form in the "BQ holdout hash" block below.)
...
2. **Effective window = 8 days OR 30 days — conditional (per-IP epoch decay, not archive overwrite).** membership-db's TTL lookup
   (`server/src/datasource/ttl.rs::get_ttl`) is **audience_type-FIRST**: if DS14 is present in the portal-pushed `data_source_info`
   map it uses `get_audience_type_ttl` (90d for audience_type 3, else `default_ttl=`**30**); only if DS14 is **absent** from that map
...
4. **Scoring is UNGATED by DS14.** `spark/audience_intent/vertical_high.py` / `vertical_mid.py` score every (ip, vertical_id)
   ...
   NO recent pageview/recency. DS14 is a separate DAG; its gate is the DS14/cat1 clause baked into each stored audience-segment
   expression at creation time (evaluated at serving time, NOT injected by membership-db — see Runtime verification below). So the full 31-day universe is scored and
...
**Runtime verification (Compass, 2026-07-30) — code-strong, live-state unconfirmed. Refines three points:**
- **Which builder is live:** code strongly favors `gcp_mntn_global_data` (airflow-ti, Dataproc Serverless/GCS, DAG `dags/mntn_global_data.py`, schedule `2 0 * * *`, `severity=0`). Legacy `run_mntn_global_data` (`SteelHouse/airflow` `dags/tpa/spark_run_mntn_global_data_etl_dag.py`, EMR/S3, `10 0 * * *`) sources its PySpark from `SteelHouse/mntn_global_data`, untouched since 2024-03-05. Multi-source DS14 rewrite landed 2025-12-11 (TI-369). **Pause-state runtime-unconfirmed** (no MWAA/Composer observability path reachable).
- **Category assignment (post-migration, differs from legacy):** augmentor_log → `category_id=1`; `bidder_auction_events` → `category_id=exchange_id` (fan-out; `device_ip`→`ip`); guid_log → `1` PLUS one row per distinct `exchange_id` seen in that day's auction events. `.0` IPs: `0.0.0.0` dropped, any IP ending `.0` has its categories replaced with synthetic `1000`. Legacy S3 job emitted `category_id=1` for every row. **Consequence:** an expression matching `data_source_id:14` with match-any categories now hits a broader exchange-fanned pool than pre-rewrite; a `cats:[1]` gate is unaffected.
- **Fail-soft build = silent-degradation risk:** each of the 3 source reads is wrapped in try/except (warning only). A day where augmentor_log and/or bidder_auction_events fail to land still "succeeds" on guid_log alone; only total absence raises. With `severity=0` and no Grafana/Prometheus job, a multi-day partial degradation shrinks the addressable pool with no alert. → `improvements_backlog.md` IMP-014.
- **No bid-time injection (premise falsified):** there is NO code that ANDs `data_source_id:14, cats:[1]` onto expressions at eval/bid time. DS14 is in every expression because the clause is **written into the STORED audience segment at creation time** (same "always-on, invisible, non-removable" pattern as DS13 = `PEAK_PERFORMANCE_DATA_SOURCE_ID=13` in `mntn-go` `audienceTransformers.ts`). membership-db's only hardcoded DS14/cat1 is a **QA-only `totals_validation_expression`** (reach-sizing RPC fallback, `gateway/config/config.yml`, absent from prod configs, never read by `eval`/`eval_batch`). **Implication for removing DS14 from a test campaign:** edit the stored segment expression / the creation-time builder that writes the clause — NOT a membership-db config change. Caveat: `rtb-rs`/`ad-service`/`ad-builder` not yet grepped, so an upstream injection point is unlikely (fixture evidence) but not formally ruled out.
...
Kleck / Sean Yang / Zach Schoenberger). **$ (order-of-magnitude, Dataproc Serverless):** whole `audience_intent` scoring DAG ≈ $39k/mo; the gate saves ~$1.3k/mo (DS13 verticals) to ~$11k/mo (~$130k/yr) if the DS19 cut is applied to `prospecting_keywords` (34% of DAG cost) where the MM Core volume lands. Tracked in AUDI-1175/1176. **`intent_score_map` itemized 2026-08-07 (was in the unattributed residual): 2,918 DCU-h/run ≈ $260/run list premium + ~$20 shuffle storage ≈ $7.8k/mo at daily cadence** — measured from `batches describe approximateUsage`, this run at 32.5% executor utilization (straggler-pinned fleet; see AUDI-1194 validation run 1). **Consumer audit (2026-07-28):** the input-gate is safe for all serving/bidding paths and Fangorn (training/inference read a separate monthly 1%-sampled feature store, NOT `prospecting_intent`), but NOT globally: the LIVE DDM/Redshift automated HHST threshold recommender (`ETL-DCO-Automated-Threshold-Adjustment.py` → `ddm.hhst_generate_recommendation`, owner **Devon Rogers**) reads the FULL scored `prospecting_intent`/`advertiser_intent` as the `ip_population` denominator that SETS the production HHST gate. **Deeper read + file-verified 2026-07-28: there are TWO HHST systems.** The primary auction-scoped one (airflow-camperbid `intent_score_threshold_v4`, `campaign_bucket_population_runner`) builds its bucket population from bid-side logs (`bid_price_log`/`bidder_bid_events`, ≤24h, filtered to `has_price` OR score-failures), which already EXCLUDE non-biddable IPs, so the scoring gate does NOT starve it (gate-SAFE); it buckets on a bid-time coalesced `hh_score` (conquest→household→advertiser→0), keyword info baked into score bands upstream. Only the DDM/Redshift DCO pilot (`ddm.hhst_bucket_collections`, scoped to `ddm.test_hhst_campaigns`, owner Devon Rogers) reads the full scored `prospecting_intent` — small-scope, adaptable. So the compute prize most likely SURVIVES; de-risk with two shadow queries (bid-log starvation count under a DS14-only population; Max-Reach inflation from unscored auction-visible IPs coalescing to `hh_score=0`). Shadow query 1 (Max-Reach, run 2026-07-28 on `bid_price_log`): Max Reach is already ~99% of the v4 recommender denominator (baseline unscored, gate-neutral — the household-score failures the DDM/Redshift path keys on are ~0). **AUD-5221 resolved** = intent-score-distribution deciles for the shuffling experiment (addressable-safe), NOT the US-population census that `strategic_north_star.md` describes. **Applied HHST lives in `dso.household_score_thresholds`** (CDC `integrationprod.dso_household_score_thresholds`, one row/campaign, live daily writer ~2K camps/day) — NOT `configuration_service_campaign_presets.household_score_threshold_preset`, which is manual ops overrides only (~26 campaigns, human `reason`). Applied-threshold distribution 2026-07-28: **65% of 32,550 campaigns at threshold=0 (Max Reach/ungated), ~69% ≤3332** — Max Reach is the dominant steady state, so a DS14 scoring gate can't worsen the majority and the gated ~31% keep their population (auction-scoped). LiftLab (DS52) confirmed served-only (no scored-universe read). Unknowns to confirm with owners: AUD-5221 deciles (Alex/Zach, no code) and any LiftLab full-scored export (#dev-incremental-lift, none found). **Correction 2026-07-28:** the LIVE writer of `dso_household_score_thresholds` is PTV (`product_id=1`) funnel-1, ~2,078 camps/day — a DIFFERENT system than the Select-scoped camperbid v4 (`product_id=2`). DDM confirmed a test pilot (`inner join ddm.test_hhst_campaigns`; generate step reads wins/spend). Every HHST implementation found is auction-scoped; the only `prospecting_intent` reader is the confined DDM pilot. Gate-safety holds IF the PTV writer is auction-scoped (likely — pattern + 65%-max-reach), but the exact PTV-writer job was NOT pinned — the one residual to confirm (Devon / "BPL v. DSO" Mode notebook) before AUDI-1176 implementation. **RESOLVED 2026-07-28 (code):** camperbid **v3/v4 ARE the PTV writers** — the `product_id=2` clause is a Select-**exclusion** anti-join (`left_anti`), not a filter (v4=Fangorn PTV, v3=non-Fangorn PTV). Population is auction-scoped (`COUNT(DISTINCT ip)` over `bid_price_log`+`bidder_bid_events`, no scored-universe join); the sole physical writer of `dso.household_score_thresholds` org-wide = `SteelHouse/idso` (BOS, `HouseholdScoreThresholdRepository.kt`, `ON CONFLICT (campaign_id) DO UPDATE`, pulling `COALESCE(manual_preset, optimized_intent_thresholds.threshold)`). **Gate-SAFE — the prod HHST recommender does not couple to the scored universe.** 3-hop chain: v3/v4 compute → `performance.optimized_intent_thresholds` sync → `SteelHouse/idso` BOS hourly-cron upsert (the sole applied-table writer). EMR `dags/camperbid/intent_score_threshold/main` + legacy Redshift `dags/performance/intent_score_threshold` are superseded predecessors (Redshift→EMR→BQ; even the EMR's scored read is commented out → auction-only there). **Several** DDM pilot routines read the full scored `prospecting_intent`/`advertiser_intent` (`hhst_bucket_collections`, `cache_hhst_population_filters`, `cache_hhst_win_conditions`, `cache_hhst_augmentor_volume`, `cache_hhst_augmentor_test_volume`), but each reads `dso.household_score_thresholds` only as a join INPUT and writes only `ddm.*` cache/bucket tables; the threshold-recommendation step (`hhst_generate_recommendation`) is fenced to `ddm.test_hhst_campaigns`. **None writes the applied `dso.household_score_thresholds`** (idso is the sole writer) — so a DS14 gate shifts only these DDM pilot/monitoring inputs, never any production threshold. Independently re-confirmed 2026-07-28 (corrected: it is 5 DDM routines, not only `cache_hhst_population_filters`). **Implementation nuances (adversarial RFD review, 2026-07-29):** (a) **Gate predicate ≠ sizing predicate** — the sizing counted DS14 under ALL categories, but the bidder gate is `category_id=1`; auction-only IPs get an exchange category, not 1 (`create_mntn_global_data_pyspark.py`), so gate scoring on the SAME predicate the bidder enforces (and re-size on it) or it is real coverage loss. (b) **RTC ≠ the MM/vertical household score** — RTC (`conquest_score`, 1-day TTL) fills only conquest matches; a re-entering non-conquest IP has ≤24h household-score staleness until the next daily batch, so "zero coverage loss" is really **≤24h staleness on that cohort** (check score QUALITY on the holdout, not just impression counts). (c) **Savings are sub-linear, not the raw 69%/39%** — `prospecting_keywords` cost = the ~33.8B-row parquet WRITE (RDD `combineByKey` + broadcast campaign dict, engineered to AVOID a shuffle); gating a ~259M-IP DS14 set adds a large shuffle. `vertical_high/mid` read all 31 DS13 partitions regardless (DS14 is not a partition key of DS13 IPDSC) → read-I/O unchanged, only post-read stages shrink. Treat 69%/39% as an UPPER BOUND on compute; the real $ is IAM-walled from the analyst (Gate B → data-platform).

### Ghost-bid intent bands do NOT match `cost_impression_log.household_score` (TI-1313, 2026-09-02)

**The ghost-bid pipeline's `score_band` strata and the impression log's stored `household_score` are two
different values. Never use one to weight, allocate, or validate the other.** Measured across the 381
campaign groups present in both: mean total variation distance between the two band mixes **0.284**, and
**55.9% of campaigns disagree by more than 10 percentage points**. Mean mix, platform strata vs impression
log: High **62.8% / 36.7%**, Unscored **29.8% / 48.3%** (PP/Mid/MaxReach all a few points higher in the log).

The clearest tell: campaign groups **96237, 81053, 106777, 117662** carry High-band strata in
`gold.reporting.lift__ghost_bid_results` while every one of their `cost_impression_log` rows tops out at
`household_score = 8000` **exactly** — zero impressions at the High cut (>= 8001), across 4.5M to 23.5M
impressions each. The platform bands at **bid time**; the log stores an impression-time value that is
clamped or sourced differently.

**Consequence.** A per-band cost figure built by joining log spend onto the gold strata silently loses
spend (15% on the TI-1313 cohort) and biases the High band low. Allocate by `bid_count_treatment` from the
gold table instead (see `data_catalog.md` ghost-bid gotcha 12). This does NOT refute the log's own band
economics, which are real and separately useful: unscored prospecting delivery runs **$13.01 CPM against
...
- **`dw-main-silver.perml.flight_cid_day_audience_sizes`** (ACCESSIBLE; cols: `campaign_id, campaign_group_id,
  ...
  **Date floor = 2025-02-01** (the sibling `perml.stg_mmmp_audience_sizes_historical` TABLE also floors at 2025-02-01;
  the `flight_cid_day_audience_sizes` VIEW runs to current). **Confirmed usable at campaign-group grain (TI-1313, 2026-09-02): joining on `campaign_id` to funnel-1/objective-1 campaigns covered 890 of 890 powered ghost-bid campaign groups, no gaps.** Take the **median across delivered days**, not the last day, so a mid-window audience edit does not decide the value; on that cohort median 4.1M, range 35,904 to 91.4M. Audience size correlates strongly with delivery (Spearman vs average frequency **-0.487**, vs impressions per audience member -0.701) and **not at all with ghost-bid visit lift (+0.045, p=0.54)**. **No 2024 addressable-pool history exists in either** — a
...
3. **S4 has zero scores** for both cohorts — S4 doesn't use `household_score` at all (likely pixel-exclusion-driven retargeting only).
   - **⚠ THE FOOTGUN THIS CREATES (AUDI-1208, 2026-08-18): the S3 flat 10,000 lands inside the HI band (8001–10000), so ANY query that bands `household_score` off `prospecting_intent` without a funnel filter counts every S3 campaign's ENTIRE scored IP set as High Intent.** `models/audience_intent/prospecting_join.py` (airflow-ti) keeps the pipeline score only when `campaign_template_id = 10 OR funnel_level IN (1,2)`; everything else is overwritten with `REGULAR_SCORE_WITH_KEYWORD = 10000`. Measured on 2026-08-17: of 4,907 active scored campaigns, **1,426 were `funnel_level=3` and ALL 1,426 sat at `hi_ips = all_ips` exactly** — 100% "HI" — while **zero** funnel-1/2 campaigns did. Leaving them in inflated the mean HI pool per audience **3.8x (18.3M, vs 4.8M once removed)**.
     - **The rule: always scope to `funnel_level = 1` (prospecting) or `IN (1,2)`, joining `integrationprod.campaigns` on `campaign_id`.** `objective_id` cannot catch this — all 4,907 carried `objective_id = 1` (consistent with the standing "objective_id UNRELIABLE → use funnel_level" rule).
     - **The tell, if you inherit a suspect number:** compute HI ÷ all-scored per campaign. A clean prospecting population sits ≤50%; contamination shows as an **empty 50–99% band with a spike at exactly 100%** (2026-08-17: 296 campaigns <1%, 3,185 at 1–50%, **0 at 50–99%**, 1,426 at 100%).
     - S1-vs-S1+S2 barely matters (funnel 1: mean 4,772,375 / median 3,553,726; funnel 1+2: mean 4,757,882 / median 3,564,058). Only funnel 3 breaks it. Queries: `tickets/audi_1208_vertical_hi_audience_sizing/queries/audi_1208_hi_subset_by_audience.sql`.

**Audience EXCLUSIONS are invisible to the scoring pipeline — a "HI pool" from `prospecting_intent` is ALWAYS pre-exclusion (AUDI-1208, 2026-08-18, read from source).** `models/audience_intent/prospecting_join.py` reads `gs://household-scoring-prod/output/data_aggregation/prospecting_active_campaign_categories/` (PACC — one row per campaign × `data_source_id` × `data_source_category_id`, carrying an `include` BOOL where **`include = false` IS the exclusion clause**) and immediately does `.groupBy("advertiser_id","campaign_group_id","campaign_id","campaign_template_id").agg(F.count("*").alias("_c")).drop("_c")`. That **discards `include`, `data_source_id`, and `data_source_category_id`.** PACC is used purely as a dimension to fetch `campaign_template_id` / `funnel_level`, then LEFT-joined to the scores. `include = false` **never filters anything in the scoring path.**
- **Consequence for any sizing question:** scoring sizes the *addressable* universe; the bidder removes the excluded slice later, at serve time, against the DS the bidder actually evaluates (DS47 since the 2026-07-01 release — memory `reference_crm_exclusion_serve_time`). You cannot answer "how much does excluding cost this audience?" from `prospecting_intent` at all.
- **The trap this sets.** Splitting audiences into with- vs without-exclusion cohorts and comparing HI pool sizes looks like it measures the exclusion's effect. It does not — both sides are pre-exclusion. Measured 2026-08-17 (funnel 1, n=2,063): **with-exclusion audiences report a HIGHER median HI pool (3,725,338 vs 3,486,590)**, because carrying an exclusion correlates with being a larger, more mature account. Read as an exclusion effect, that inverts the truth. 721 of 2,063 prospecting audiences (35%) carry at least one `include = false` clause.
- A true post-exclusion HI size needs each campaign's exclusion sets resolved against `ipdsc__v1` at the same `dt` and subtracted from its HI pool. Not done as of 2026-08-18.

**7.6% of live prospecting audiences are structurally incapable of reaching HI (AUDI-1208, 2026-08-18).** On 2026-08-17, **156 of 2,063** MM prospecting audiences scored **exactly zero** HI IPs, and that set is **exactly** the set with no DS19 leaf in its include list — 156 of 156, zero exceptions either way (all 1,907 DS19-carrying audiences had HI > 0). This is the CLEAN RULE observed at portfolio scale, and it is a clean natural experiment confirming it. They are not small audiences: median 18.7M total scored IPs (max 140.6M) and a median 8.4M PP band — they are capped one band below HI by configuration. **Sizing consequence: averaging them in drags the mean HI pool down 8.2%** (4,772,375 with them, 5,162,773 without). State which convention you used.

**AUDI-1208 sizing baseline (2026-08-17) — the numbers to start from when someone asks "how big is X".** Verticals (148): mean 9,479,187 distinct IPs, median 6,557,786, Q1 3,960,892, Q3 11,962,638, min 919,345 (`101005`), max 76,274,119 (`124000 Current Affairs`). Buckets (37): mean 25,952,755, median 20,852,312, Q1 12,756,804, Q3 33,091,717. HI pool per MM prospecting audience (funnel 1, n=2,063): mean 4,772,375, median 3,553,726, Q1 1,649,295, Q3 5,956,302, max 41,760,550; same audiences at ANY score: mean 51,321,823 / median 43,120,471. **Both distributions are strongly right-skewed (mean ≈ 1.4x median for verticals, 1.34x for HI pools) — quote quartiles, not the mean alone. Quartiles use LINEAR interpolation (Spark `percentile()` / numpy); Python's `statistics.quantiles` default is `exclusive` and differs 12% at Q1 on the 37 buckets.** Verticals overlap by design: an IP averages ~6.6 verticals, so the 148 sizes sum to ~1.40B against a 214,079,274-IP base — **never add category sizes.** Full record: `tickets/audi_1208_vertical_hi_audience_sizing/summary.md`.
...
- **Migration:** Aerospike → **ScyllaDB** ("our future is Scylla") + Redis. Driver: Aerospike is expensive / poor support; Scylla cheaper. Decision above Abbas. **DONE for the household-profile/membership-consumer path (Eric Salinger 2026-08-25: "already ScyllaDB, has been for some time")** — Aerospike references in this section describe the June 2026 state; spend/recency/frequency-store status unverified since.
...
2. **Membership consumer (the path TI scores travel):** scoring team writes scores to a **GCS bucket** → GCS event trigger → **PubSub** → **RabbitMQ** (messages carry GCS file URLs). Consumer downloads + processes the (large) file → writes ~~Aerospike~~ **ScyllaDB** (store confirmed migrated, Eric Salinger 2026-08-25). Logic: intent scores **batched** (not line-by-line); ~~IP with empty segment list → deleted~~ **(CORRECTED 2026-08-25: nothing is ever deleted — an empty segment list executes a full `REPLACE` that clears segment columns, keeps the row, and RESETS its TTL; the June "deleted" claim was Abbas describing either the old Kotlin consumer or unshipped intent)**; special handling for holdout IPs. Runs in **Kubernetes**. Handles everything household-profile-related. The current Rust service is `rtb-membership-consumer-service` (predecessor "membership consumer oracle", Kotlin, decommissioned).
   - **Segment path detail (Eric Salinger meeting + doc, AUDI-1016 2026-08-25):** segment scores arrive over TWO paths from the same producer (`membership-db/etl`): the **4-hourly GCS full-snapshot dump** (bucket → PubSub notification → consumer reads file-by-file; the RabbitMQ hop above is the intent-score path description from June — Eric describes the segment path as PubSub-direct) and the **Kafka topic `segment-updates-burnin-proto`** (activity-driven; per Zach a SUBSET of the full dump, per Eric mostly retargeting notifications — unreconciled with the doc's 1.03B msgs/day). **~92% of incoming segment messages are empty duplicates** (dump re-sends every empty IP every 4h; peak ~402k records/s). The dump is a **shared data product (~8 consumers: attribution, incrementality, reporting)** — it cannot be deleted, only the bidder's consumption of it. Scylla row TTL: Eric says ~30 days, his code-derived doc says 7 (`DEFAULT_TTL_SECS`, `rtb-scylla-models`) — unresolved; either way every 4-hourly write resets it, so empty rows never expire. Fix direction + full conflict table: `tickets/ti_1016_memdb_bidder_cache_optimization/summary.md` §4.4-§5b. **Outcome (2026-08-25, AUDI-1016 closed): no new build — ongoing MemDB dedup work plus the MNTN-ID migration (household keying moves off bare IP) are expected to vastly reduce the empty-write volume (Eric via Zach).**
   - **Terminology collision (cross-team):** the bidder team's "segment scores" ≈ what TI/scoring calls the household score / campaign intent score; the bidder's "intent scores" ≈ the advertiser score. Eric's rule of thumb: prefix any score with the owning team's name.
   - **Producer-side ownership (settled in-thread by Zach, 2026-08-25 PM):** segment scores = **TMUL, owned/created by Zach Schoenberger** (the membership-db sweep producing the GCS dump); **RTC = Ryan Kleck + Weiang Li** (RTC's only real-time publish is DS13+rtc-score into MemDB via topic `vertical-classifications` — NOT the empties source, and staying). **The empty-segment fix is Eric ↔ AP (Mike Dolt, Jaime Mutale, Daniel Hartnett) + Zach; stakeholder DPlat (Dustin Niehoff). Not an AUDI problem** ("we do scores, but not the scores that affect this"). Zach 2026-08-25: ongoing MemDB work already reduces downstream duplicates. Read-before-write dedup on the consumer is a dead end by Eric's metric (goal = net Scylla REQUESTS; a Redis front-cache worked but cost ~$20k/mo — cpu+memory+throughput, >100GB, replicas — vs cheaper Scylla scale-up). DPlat wants the bidder to stream intent scores; Eric's precondition is the segments fix.
...
- **Beeswax era (being retired):**
  ...
  - **rtb-bidder-service (JVM Bidder)** — Beeswax sends it the eligible campaigns; it produces bids/campaign, handling **pacing (IHP = In-House Pacing), fcap, pricing**. **Runs as TWO live deployments on the prod clusters (AUDI-1223, 2026-08-25): namespace `bidder` (`env='prod'`) and `bidder-burnin` (`env='burnin'`, the release-soak stage; per Abbas via Matt Brorby the only functional difference is IHP). Campaigns opt into burnin via `burnin_bidding_enabled` in `integrationprod.beeswax_*_sync_config` (coalesced campaign → group → advertiser; ThirdLove CG 115424 = true, PER-6332, May 2026). Both log to `bid_price_log` with their `env` stamp — burnin is the larger deployment by row volume.** **Beeswax quirk: we don't learn win/loss until much later (minutes) via the spend pipeline** — significant downstream implication. Writes **bid logs**. Its `household-profiles` cache = "everything but segments": `geo_version`, `timestamp`, `hhs:timestamp`, `hhs:segment:ttl`, `hhs:campaign` (per-campaign household scores, present on ~63% of records), `hhs:advertiser` (per-advertiser scores, ~35%), `hhs:segment` (usually empty), `segments` (~4%). *(The Confluence "Bidder" PDF rendered these bins as `is:*`; the authoritative Aerospike datastore dump shows they are `hhs:*` = household score.)* Also caches `recency`, `settings`, `spend`, `price`, `bid volume`/`inflight bids` (IHP).
...
-- RE-VALIDATED AUDI-1173 (2026-07-28): 16 hex chars is correct; this is the canonical
--   household-randomized RCT bucketer. For a NEW experiment use disjoint bucket ranges
--   (e.g. 100-999) so treatment arms never collide with the 0-99 platform holdout.
```
...
**Both `expression` columns are LIVE CONFIG, not point-in-time — check `update_time` before attributing them to a past window (TI-1313, 2026-09-02).** `audience.audiences.expression` and `audience.audience_segments.expression` hold current state only, with no history. Reading either to characterize a **trailing measurement window** stamps today's targeting onto delivery that already happened. Measured on the TI-1313 cohort: **397 of 877 campaign groups (45%) carried an `update_time` inside the 71-day window** — so for nearly half the cohort the stored expression is provably not the one that ran for the whole window. This bites any config-derived campaign attribute (the TI-1313 case was a CRM-exclusion flag read off `audiences.expression`, `"data_source_id":4` inside the `$.interest.exclude` block, 276 of 877 groups = 31.5%). **Mitigations:** report the mid-window-edit share alongside the attribute, restrict to groups with `update_time` before the window, or resolve as-of-date from `bronze.integrationprod.archives_audience_segment_archives` ordered by `create_time` (see §archives_audience_segment_archives.version is NON-MONOTONIC). Same failure class as `audience.mm_campaign_classifier` being a current-state snapshot.
...
**Exclusion-side corollary to rule 1 (PS-8572, 2026-08-06):** the rule cuts both ways — S2/S3 stage campaigns and standalone retargeting groups carry NO CRM *exclusion* clauses either (verified across all archived Lovepop segment versions); converter suppression outside S1 rests entirely on S1 gating DS16 stage entry. Full serve-time picture: "CRM Upload Flow (DS 4)" section.
...
- **VV (Verified-Visit) lookback WINDOW — source of truth + change history (TI-1037 workflow, 2026-07-02):** the VV
  ...
  (NOT `advertiser_configurations` — its `conversion_lookback_window`/`page_view_lookback_window` are the **BLOCK
  lookbacks** (the block_conversion / block_first_party suppression horizon; corrected PS-8572 2026-08-06 from the
  earlier "taxonomy/RT-membership lookback" read), a different knob, and `conversion_lookback_window` is NULL for
  most advertisers; NOT `r2_advertiser_settings` — no window cols).
...
### Audience Exclusions is an OPT-IN, prospecting-only UI surface with THREE toggles — and it is usually OFF (2026-08-27)
Advertiser UI path **Audience > Exclusions > Audience Exclusions**. Screen copy: *"Block recent converters and/or site visitors from seeing your ads to focus on gaining new ones. These exclusions will only apply to your prospecting campaigns."* Three independent toggles, each defaulting to **OFF** with a 90-day field:
- **Converter Exclusions** — "Exclude users that converted within the last N days" → `audience_advertiser_configurations.block_conversion` + `conversion_lookback_window`.
- **Site Visitor Exclusions** — same for visitors → `block_first_party` + `page_view_lookback_window`.
- **Audience Isolation** — "ensures your campaigns split audiences evenly without overlap, enabling clean experimentation and accurate performance comparison." **This is the OLD contaminated A/B split method** (impression from one campaign adds the IP to the other's block list; processing delay lets IPs land in both arms). It is still exposed in the product. **Do not use it to split an experiment — use the MD5(AID:IP) hash-bucket split** (`experimentation.md` § BUK Exp 1 lesson).

**Load-bearing: this is opt-in and rarely touched, so never assume converter suppression is on.** `summarydata.advertiser_audience_tooling_change_log` holds only **245 change rows across 157 advertisers** lifetime (as of 2026-07-29) out of ~1,859 active advertisers — most accounts have never touched these settings. Verify per advertiser before building any argument that depends on converters leaving the pool.

**Corollary — what the exclusions do and do NOT drain.** DS21/DS34 in the shared prospecting `NOT(...)` block remove **converters and site visitors, not previously-served IPs**. Prospecting keeps re-serving the same households unless a **DS16 net-new gate** is on (§"DS16 = the advertiser's OWN funnel tags"). So a prospecting pool does **not** drain merely from being hit — any "we burn through the audience" argument needs the exclusion toggle ON, and even then the drain runs on visits, not impressions.

**Removal rate per exposure — run a depletion argument on VISITS, never conversions (2026-08-27).** Per-exposure conversion hazard is **0.038% pooled / 0.021% for WGU** (75,690 converting IPs ÷ 14,910,370 distinct IPs over 24.66 imps/IP, `incr_75_advertiser_metrics.csv`). Twenty exposures therefore strip only **0.41% (WGU) to 0.77% (pooled)** of the pool — far too little mass for adverse selection to bite. Site-visitor removal runs **~0.5–0.65% per exposure, ~9–12% over twenty**, which is a real drain. Sanity check on any illustrative rate: a 10%-per-pass figure at WGU's 24.66 exposures would convert 13.8M of 14.9M IPs in 30 days (92.6%) at a $0.24 CPA against its actual $44.29.

- **THREE independent lookback knobs — never conflate (PS-8572, 2026-08-06):** (1) **VV windows** =
  `advertisers.clickpass_acquisition_ttl` (PRO) / `clickpass_click_ttl` (RT) + `conversion_window` (attribution);
  (2) **BLOCK lookbacks** = `audience_advertiser_configurations.conversion_lookback_window` /
  `page_view_lookback_window` (the horizon block_conversion / block_first_party suppress over);
  (3) **per-clause lookbacks** = the `lookback_window` INSIDE a DS21/DS34 clause of the `audience_segments`
...
- **GCP data floor: January 1, 2025 (Zach, 2026-03-24).** No BQ table (silver or bronze) has data before 2025-01-01. Pre-2025 data only exists in Greenplum coreDW (deprecated April 30, 2026). All-time lookback queries are bounded by this floor. *(Superseded 2026-07-08/2026-07-27: the 2025-01-01 floor holds only for the silver LOG tables; `cost_impression_log` reaches 2023-10-01, `ui_visits`/`visits` 2023-01-01, `all_facts` ~2020-10, `conversion_log`/`sum_by_campaign_by_day` ~2024-01. Verify per table.)*
...
- **31357 = WGU (Western Governors University).** ~30% of MNTN monthly spend (entire business). Largest single advertiser. Has abnormally long S3 lookback window per Zach. Online degree program. Any analysis using this advertiser as test case should note it's an extreme outlier in spend and funnel depth. (Zach confirmed 2026-03-17)
  - **Audience Exclusions all OFF (2026-08-27).** Converter Exclusions, Site Visitor Exclusions and Audience Isolation are all off in the UI (screenshot, Matt Brorby). Corroborated from source: `audience_advertiser_configurations` reads `block_conversion=FALSE`, `block_first_party=FALSE`, both lookback windows NULL, and all 74 live prospecting segments carry zero DS21 and zero DS34 clauses; the only exclusion is a static uploaded IP list (DS8 categories 1598–1602). **Caveat:** TI-1061 records that this table has unreliable presence/absence semantics, so treat the source half as strong-but-unverified against the bidder's effective config; the UI screenshot is the firmer evidence. **Consequence:** converters are never removed from WGU's prospecting pool, so any converter-depletion or adverse-selection argument does not apply to WGU as configured.
  - **CONTRADICTION on the ~30% share — measured 8.75%, unresolved 2026-08-27. Rank #1 survives; the share does not.** The 30% figure is Zach's word (2026-03-17) and is repeated in `experimentation.md`. A direct sum of the full-universe 30d pull (`tickets/incr_75_eligible_advertisers/outputs/incr_75_advertiser_metrics.csv`, 1,859 advertisers, pulled 2026-08-19) gives WGU **$3,352,209** of **$38,293,945** = **8.75%**; an independent 30d total of $42.3M puts it at 7.9%. Both land 8–9%, ~3.4x below the recorded claim, and the CSV is an eligible-advertiser cohort so its denominator is a floor — a fuller denominator moves the share DOWN, not up. What does survive: WGU is unambiguously **#1 by spend at 3.58x #2** (ElevenLabs 51660, $937,510), and is an extreme outlier on frequency (24.66 imps/IP over 14,910,370 distinct IPs, 98.4th percentile of 1,859; median 3.36). Reconciling hypotheses: Zach may have meant gross billings or a single business line rather than 30d media spend, or the share was accurate in Mar 2026 and has since fallen as the platform grew. **Settling check:** monthly WGU spend ÷ platform spend across 2026 from `agg__daily_sum_by_campaign` (or `sum_by_campaign_by_day` for the long pre-period) to see whether it was ever ~30%. **Until settled, do not quote "~30% of MNTN spend" in any deliverable — quote rank #1 and the 3.58x gap instead.**
...
- **`agg__daily_sum_by_campaign` is FROZEN, not merely lagging** (escalated 2026-08-11; was "stale, 35 days behind" on 2026-06-04): `INFORMATION_SCHEMA.PARTITIONS` = **242 partitions, 2025-09-01 → 2026-04-30**, unchanged for 3+ months. **Any trailing or historical window outside that range returns ZERO rows, silently.** The old advice that it "is still acceptable for trailing-12mo *patterns*" is now WRONG — a 12-month pattern computed today lands entirely outside its range. **Replacement for advertiser × day spend: `summarydata.sum_by_advertiser_by_day`** (one row per advertiser × day, 2024-01-01 → current, fresh, `require_partition_filter=TRUE`; total spend = `media_spend + data_spend + platform_spend`; project narrow columns, `SELECT *` is ~613x more expensive). For campaign grain use `sum_by_campaign_by_day` (2024-01-01+). For trailing-30d "current state" source `cost_impression_log` + `clickpass_log` + `ui_conversions` directly. Pipeline owner still unknown — flag if blocking work.
...
- **`advertiser_configurations.conversion_lookback_window`**: NULL for most advertisers (use system default of 30 days). Only ~620 advertisers have non-null values (mostly 30, some 33/35/45/60 days). It is the BLOCK lookback (block_conversion horizon), NOT the attribution conversion window — see "THREE independent lookback knobs" in the VV section (PS-8572, 2026-08-06).
...
- **"Shopper Graph" = the original name for MNTN Matched** (confirmed Alyson Lefkowitz + Brian McAdams, INC-006 deploy 2026-07-30). The `SteelHouse/shopper_graph` service backs the **ENTIRE MNTN Match backend** — building a MNTN Match campaign hits it directly. **Owned by the DS team (Alyson's org)**, not another team. **Deliberately deployed MANUALLY (`workflow_dispatch`)** because it's a critical, overloaded/finicky API they don't want accidental deploys to: **merging to main does nothing until a deploy workflow is run.** The repo ships **three separate images via three deploy workflows** — `steelhousedev/shopper-graph` (middleware API, argocd), `openai_batch_runner` (the batch pipeline `batch_*` tasks, `DbtImageName.OPEN_AI_BATCH`), and `mntn_matched_data_pipeline` (dbt, `DbtImageName.SHOPPER_GRAPH`). **A change to `openai/…/batch_fetcher.py` (INC-006 fix #296) ships ONLY via `deploy_openai_dockerhub_gcp.yml`, NOT the middleware deploy.** Full deploy-workflow decision rule + access/QA-env notes → MEMORY `reference_shopper_graph_deploy`.
- **/vertical endpoint semantics (verified from code, SteelHouse/shopper_graph@6626756, adversarially re-verified, AUDI-1142 2026-08-24):** the "/vertical cache" is Postgres `fpa.advertiser_verticals` keyed on `advertiser_id` — a table, not a cache object. **POST /vertical without a `vertical_id` ALWAYS runs the scrape+GPT path** (fine-tuned model `ft:gpt-4.1-mini-2025-04-14:mntn::BMhxthUA`); an existing row only flips the INSERT to an UPDATE — there is no caching on the POST path at all. **`/vertical` never consults `fpa.mm_domain_map`** (the hoteling map); `/autopilot` DOES (`autopilot_wrapper.py:312-327`, `get_mapping` fallback to the root advertiser's profile) — so a hoteled AID sharing another AID's domain re-scrapes or 400s on /vertical. Spec-vs-reality: `api_spec.yaml` marks `companyUrl` `required: false` on POST (in practice the expensive path 400s without a resolvable URL) and GET /vertical has NO company_url param. `/autopilot_from_url` throttle = `Semaphore(1)` per pod returning 429 + Retry-After: 30.
- **Who POSTs /vertical (AUDI-1142, 2026-08-24):** dominant caller is the airflow-ti precache DAG — `models/vertical_categorization/verticals_auto_assignment.py`, up to 200 AIDs/run, cron every 30 min via `dags/machine_learning/mntn_match_verticals_precache_v1_1.py`. gary-ql `storeCompanyVertical()` (`src/utils/services/MntnMatched.ts`) fires per registration / first company_url set, fire-and-forget, errors swallowed. select-app's client is GET-only. gary-ql vertical READS are DB-direct (sequelize over `fpa.advertiser_verticals` + Redis 1h TTL), never shopper-graph HTTP.
- **Gotcha — 97% of prod POST /vertical requests fail with 400** (pod logs 2026-08-22: 622 of 636; 2026-08-23: 626 of 643). Failure mode is scrape-returns-invalid-text (bot challenge / too-short) → self-selection fallback fails, NOT the `SCRAPING FAILED` exception (zero hits). **563 identical AIDs re-fail on both days** — a recurring population the precache DAG re-hammers daily, wasted LLM spend (IMP-067).
- **Select DLQ structural bypass for shopper-graph errors (select-app@eaf611f, AUDI-1142 2026-08-24):** `SHOPPER_GRAPH_CLIENT_ERROR` with `SHOPPER_GRAPH_CLIENT_ERROR_RETRY_DISABLED` (code default TRUE — a QA workaround for coredbdev being unsynced) causes the embedding-path message to be ACKED: no retry, no DLQ, so /vertical outages are invisible to the DLQ alert on that path. Recommendations moved to Temporal (`apps/select-workflow`), where shopper-graph failures surface as workflow retries / OTel spans, not RMQ DLQ messages. The one recorded DLQ incident (2026-06-01, 486 msgs in select-dlq.v3) was Gary → audience-service, not shopper graph.
...
- **`site_network_hourly` SUCCEEDS with a missing or partial hour; `_SUCCESS` per `hh=` dir is the completeness marker, the batch state is not (AUDI-1275, verified 2026-09-02 from four event logs + the GCS listing).** The model wraps each hour's write in `except Exception` (`models/bidstream_hourly/site_network_hourly.py` L245-250), so when a Spark job dies the batch still reports SUCCEEDED: `app-20260901205120829-0863` (batch `sit-net-hou-y3a-20260901-195000-1`, 2 h 25 min) lost both hour jobs to `FetchFailed` storms from dynamic-allocation scale-down shuffle loss (8,244 FetchFailed task ends, stage 8 re-submitted 116 times, "ShuffleMapStage 9 / 25 has failed the maximum allowable number of times: 4") and wrote 0 bytes, batch SUCCEEDED. A stage re-attempt under FileOutputCommitter v2 can also leave a PARTIAL partition with task-committed files and no `_SUCCESS`: `dt=2026-09-02/hh=05` has 29 part files (0.482 GB) carrying two timestamps (09:45 and 10:02 UTC) from `app-20260902085134727-0157`, whose hour-05 write job failed at `ResultStage 19`. Baseline 2026-08-27 to 2026-09-02: 167 hour partitions, 166 with `_SUCCESS`, `dt=2026-08-31/hh=18` absent. Each hour is written twice by consecutive runs, so the next run normally repairs it. Neither behaviour is caused by speculation (airflow-ti #1271 turns `spark.speculation=true` on here as the AUDI-1275 canary from 2026-09-03; the post-merge watch must not blame these on it). The storms are a separate defect, IMP-104. Detail: memory `reference_dataproc_eventlog_profiling` § FetchFailed storms; ticket `tickets/audi_1290_pipeline_optimization_hackathon/audi_1275_straggler_gcs_writers/summary.md` §4.6.
...
**Also: the MNTN PSA advertiser is `advertiser_id = 9090`, NOT 90** (corrected 2026-08-11 — the earlier "AID 90" here was wrong and propagated into query filters). PSA impressions are served to holdout IPs intentionally (they show up in `cost_impression_log` for buckets 0-99). Exclude **9090** from any holdout-based lift analysis — Alex Knorr hit this as a confusing anomaly before the PSA fact was surfaced.

**Verification (2026-08-11):** `advertiser_id = 90` has **no row** in `bronze.integrationprod.advertisers` and **zero rows** in CIL; `9090` = "Public Service Announcement" (`active=TRUE, deleted=FALSE, is_test=FALSE`), 2,275 impressions / $197 on 2026-08-05 alone. **So a filter written `advertiser_id != 90` is a silent NO-OP** — it excludes nothing and lets PSA into the universe. This bit INCR-75's `incr_75_advertiser_metrics.sql` (both the 30d and 56d CTEs); PSA entered the 2,009-advertiser universe and passed the clean/active and non-B2B filters, only falling out at measurable-IVR because its `p_visit = 0.0`. The eligible set was unaffected, but grep any analysis inheriting that filter.
...
This is why aud22 (Geo Includes/Excludes) can flag a false violation. Two confirmed mechanisms: **(1)** the `location_data` metro_id/hierarchy is internally inconsistent for a ZIP, so the impression side (CIL → network_locations) and the audience/TPA side (reads the `location_data` **hierarchy**) resolve the same IP to different DMAs (fix = PR #1147). **(2) memdb geo-ingest mapping gap:** a network whose DMA was reassigned across builds (e.g. `161.38.248.0/24` flipped 535 Columbus → 547 Toledo at the Jul-13 build) can resolve in-geo to memdb and out-of-geo to CIL. Confirmed 2026-07-31: memdb's `geo_version` field read Jul-13 but its mapping for that `/24` still returned the Jun-29 value 535, while the table holds 547 at Jul-13 — memdb's per-network mapping did NOT refresh when its version pointer advanced (a single network block covers the IP, so not ambiguous records, and not a version-pointer lag). Resolve at the SERVED `geo_version`; memdb's past state isn't preserved, so catch the next live example to diff table-vs-memdb. See `data_catalog.md` "Geo store versioning" + memory `reference_aud22_geo_reporting_sync`.
...
**Label correction (2026-09-02, TI-1313):** advertiser **30506** is `MNTN - No ENG Testing`
(`is_test = TRUE`, `deleted = FALSE`) in `bronze.integrationprod.advertisers`, not "Vivint" — the
table above carries a TI-884 working label, and the same id appears correctly named in the CIL
`campaign_id = -3` note. The measured ρ is unaffected; the advertiser identity is (it is an internal
test account, see §is_test / deleted Filters).
...
- **DS14:** MNTN global data — automatically added to all audience expressions to filter down to only IPs seen in `guid_log` (4-day window) and `augmentor_log` (1-day window). Functions as an activity recency filter. Implemented as a **global filter at MembershipDB / audience-service level** (Sean Yang, 2026-07-16 readout); expanding the pool by admitting other site_visit_signal IPs into DS14 was floated at the readout — overlap + option sizing in AUDI-1117. (NOTE: the "~7-day augmentor-log" reading at §DS14 cat [1] was a decode error, corrected 2026-07-30 — effective window = 1-4d build lookback + 8-day serving TTL ≈ 9-12d from last sighting; AUDI-1117 = the empirical no-cliff detail.) **AUDI-1117 empirical (all imps 2026-07-01): the documented windows are NOT a hard universal cliff — lag distributions decay smoothly with no cliff at 1d/4d/7d.** Display delivery is a **100.00% same-day augmentor echo by construction** (aug_log mirrors the display bid stream), so DS14-gate evidence must come from CTV; CTV-prospecting has only a **soft edge** (12.2% of imps outside aug(1d)|guid(4d), 4.3% outside both free logs within 11d). **(Refined 2026-07-30, Compass: "global filter at MembershipDB" = the DS14/cat1 clause is written into the STORED segment expression at creation time, not injected at eval/bid time by membership-db; see DS14-opt block Runtime verification.)**
...
**⚠️ CONTRADICTION, unresolved 2026-09-03:** the trailing-30d pull ending 2026-09-03 reads AID 51660 at IVR **0.58%** / CPM **$31.80** against **3.07%** / **$8.58** in the June run. AID 51660 paused a $770K campaign group 2026-08-20 (AUDI-1215), inside that window, and the same 2026-09-03 run moved CPM to the advertiser-facing basis (media + data + platform), so the CPM half is not like-for-like. Until reconciled, treat the ~$36K visit-lift figure as June-conditioned. Settling check: per-day IVR for 51660 across 2026-06-01 to 2026-09-03 to see whether the drop tracks the pause date.

**Ghost-bid / holdout negative-lift bias (Matt Brorby, 2026-06-23; CORRECTED per leg AUDI-1173, 2026-07-28):** the 10% hash holdout is clean at assignment, but the frequency-cap asymmetry (bias A1) is **leg-dependent, not universal**:
...
**UPDATE 2026-09-03: the in-product path moved off `usersreached` on 2026-08-25.** The premier incrementality Testing tab and the `gary-ql` `forecasted_mde_percent` resolver no longer read `graph.usersreached` or the FPA `totalConversionRate`. Both now take the MDE baseline from the ChAPI per-IP-user rate columns `Graph.IPUserSiteVisitorRate` (visits goal) and `Graph.IPUserConversionRate` (conversions goal), selected by the experiment's `goal_metric` (RX-7420 series: `gary-ql` #4664 `27ffe77c`, plus #4662/#4665/#4666; code read at `SteelHouse/gary-ql@cbae0e94` and `SteelHouse/premier-ui@aaf65d59`). `getAudienceFpaReportTotalsByAdvertiserId` is now dead code. The mixed-key gotcha above is unchanged and still applies to anyone dividing `sitevisitors / usersreached` by hand.
...
- **Old and new group structures COEXIST per advertiser (verified Bouqs 2026-07-16, AUDI-1037):** the old parent/child structure minted a **separate sibling "- Multi-Touch" GROUP** (e.g. 25430 "August CTV Prospecting" = 1 prospecting campaign; 25431 "… - Multi-Touch" = the obj=5/6 MT campaigns), while newer groups hold all stages inside one group (119362 = 6 campaigns incl. obj=1 fl=2/3 MT). Consequence: a campaign_group-grain list **reads like a campaign-grain list** ("X" / "X - Multi-Touch" pairs with adjacent IDs) — don't "fix" it to groups, it already is groups. Label such lists "Campaign groups" to preempt the confusion.
...
## Display multi-touch is an ADVERTISER-level feature flag — there is NO campaign-level toggle (TI-1313, 2026-09-02)
The switch is `dw-main-bronze.integrationprod.core_advertisers_x_features` with **`feature_id = 1 AND active`**; `feature_id 1 = MULTI_TOUCH_ENABLED` per `core_features` (verified 2026-09-02: 3,481 rows / 3,481 distinct advertisers, so one row per advertiser). A second, independent advertiser-level setting is `ui_advertiser_premier_attributes.multi_touch_disabled_by_default`, and `core_features` also carries `feature_id 20 = TV_RETARGETING_MULTI_TOUCH_DISABLED`.

**There is no campaign-group or campaign display-multi-touch toggle anywhere in BigQuery.** Checked `core_campaign_group_channel_margins`, `public.campaigns` / `campaign_groups` / `campaign_groups_raw`, the `ui_*` and `dso_*` config-override tables, and every `integrationprod` column matching enabled/disabled/toggle/display/channel/stage.

**Consequence for any per-campaign attribute table:** "runs display multi-touch" is an **advertiser** attribute. Joining it onto campaign groups gives every group of the same advertiser the identical value, so it cannot explain between-group variation inside an advertiser and will read as a campaign-level effect in a per-campaign model when it is really an advertiser fixed effect. Label it as advertiser-grain, or drop it from campaign-grain work. What a group actually DELIVERED on display is observable instead: `campaigns.channel_id = 1` (multi-touch/display) vs `8` (television/CTV).
...
- **Measured replacement, the number to quote (AUDI-1070, HexClad): 68–70% of each month's distinct HI reach had NOT been served in the prior 30 days** — every month, including the peak over-spend month (`outputs/hexclad_hi_replacement_rate.csv`). Two thirds of the pool turns over monthly, so there is no monotonic 10k→9k→8k countdown to run against it.
- **Frequency does not run away either: Caraway's HI frequency held FLAT at 1.45–2.07 imps per household per month for twelve months while spend rose +191%** (`audi_1070 outputs/diag_caraway/hi_reach_frequency.csv`). Headline imps/IP figures (e.g. WGU's 24.66) are shared-IP plus cap-counter-grain artifacts, not twenty hits on one prospecting household.
...
- **Scope = campaign AND campaign_group; the counter key is ALWAYS the IP.** **CORRECTION (AUDI-1173, codebase 2026-07-28):** `household_id_for_fcap()` always returns the IP — enum `HouseholdIdentifier` has only `Ipv4`/`Ipv6`/`Both`, there is **no MNTN-id branch**. The `uses_mntn_id` flag drives **candidate fetch + holdout routing**, NOT the fcap counter key. (Supersedes the prior "keyed on IPv4, MNTN-ID only when `uses_mntn_id`" line — MNTN-id never keys the fcap counter.) **No advertiser dimension in the counter and no rollup key** → no cross-advertiser cap; frequency counts each campaign_group's IP independently.
- **Cap ARMS need new bidder code, not config (AUDI-1173).** `CampaignModel` has **no per-household cap field** → assigning experimental per-household caps (e.g. cap-8 / cap-3 arms) requires a **new @SteelHouse/rtb bidder feature**; it cannot be done via config or a `bidder.frequency_caps` sync alone.
- **Config tables (AUDI-1173):** `advertiser_frequency_caps` is **EMPTY (0 rows)** — no advertiser-scoped cap exists in prod (the advertiser-rollup "leakage" is a genuine capability GAP, not a mis-set knob); `integrationprod.campaign_group_frequency_caps` + `dso_frequency_caps` are the populated config. See data_catalog "frequency_caps config tables."
- **Frequency LEAKS across an advertiser's campaign_groups, but the leakage-$ magnitude over `(ip, advertiser)` is SHARED-IP CONFOUNDED (AUDI-1173 — RETRACTS the old 4.8%/13% figures).** Apparent cross-group/stage leakage rises **monotonically with the distinct-advertiser count per IP**: ~76% of "leaked" impressions sit on IPs shared by **5+ advertisers** (i.e. NAT/office/shared IPs, not one household seeing extra ads). A naive high-threshold purge trips for only **~0.34% of households**. So any per-`(ip, advertiser)` "household" frequency-leakage dollar estimate is **unreliable without a real shared-IP (identity/household) purge** — treat the qualitative "no advertiser rollup → leakage exists" as true, the sized $ as retracted.
...
- **CONTRADICTION on the universal default window — 30min (code) vs 15min (config), unresolved 2026-08-27.** The codebase read (2026-07, above and in [[reference_frequency_capping]]) says the appended universal policy is **1 imp / 30 min**. A direct read of `dw-main-bronze.integrationprod.dso_frequency_caps` (2026-08-27) says `universal_cap=1, universal_duration=900` (**15 min**) on **10,324 of 10,428** campaigns that delivered in the 7 days ending 2026-08-26. Both stand until settled. Reconciling hypothesis: the config table carries the synced 900s row while the crate appends its own 1800s policy at request time, so the two are different layers rather than a changed value; alternative is that the default was retuned 30min→15min after the July code read. **Settling check:** read the universal-policy constant in the `fcap` crate (`SteelHouse/rtb-campaign-service`) and compare to `universal_duration` for the same campaign_id, or measure delivered same-IP inter-impression gaps against both 900s and 1800s.
- **Caps ARE set in practice, on essentially every live campaign (BQ census, 7 days ending 2026-08-26).** Of **10,430** campaigns that delivered impressions, **10,428** carry a `dso_frequency_caps` row (10,422 universal, 9,129 secondary, 3,262 primary); only **4** are uncapped at any tier. Scope of that window: 1,627 live advertisers / 3,539 campaign groups / 10,428 campaigns. Most common live configurations: **1 imp/day + 6 imps/30d (16.0%)**, 1 imp/3h + 24/3d (11.1%), 1 imp/14d (10.0%), 6 imps/15d (7.0%), 24 imps/day (6.6%). Taking `LEAST()` across the four tiers normalized to a 30-day window, ~27% of live campaigns sit at ≤8 imps/30d. **So "are caps configured?" is answered yes** — the open question was never tightness, it is grain (next line). Advertiser-set custom caps remain rare: `campaign_group_frequency_caps` = 1,255 rows across 462 campaign groups (identical count in the `ui_` surface), most common custom value 1 imp/30d (538 rows, 188 groups).
- **A tight per-campaign cap does not bound per-household frequency, and does not protect a gated pool from saturation — it converts the failure.** Because the counter has no advertiser or cross-group rollup, an advertiser running N campaign_groups × M funnel stages can serve one household N×M times its configured cap by construction. WGU (31357) is the worked example: every campaign carries a cap, yet 30d delivery is 367,710,012 imps over 14,910,370 distinct IPs = **24.66 imps/IP**, the 98.4th percentile of 1,859 advertisers (median 3.36). And where the cap *does* bind against a narrowed high-intent pool at fixed budget, the documented system response is ordered — the bidder first raises frequency on existing HI up to the cap (§"HI pool is a FLOW"), then the pacing controller lowers the HHST gate to find fill ([[reference_hhst_pacing_lever]]), then under-delivers. The cost moves from frequency to gate relaxation to under-delivery; it does not disappear. Bid inflation on a thinner pool is **not** a documented fourth path — the only recorded pricing rule is a rolling ~7-day publisher CPM with no score term. Do not assert it.
...
- **Ownership (AUDI-1173):** the `rtb-campaign-service` fcap crate is owned by **@SteelHouse/rtb** — authors/maintainers **snowsignal (Jane Lewis, crate author), rogusdev (Chris Rogus), RockyGitHub (Chris Davidoff)**. NOT jtang, NOT "Zach/Jordan" (those are audience-tools / `campaign.rs` routing). See mntn_business.
...
## Total-site-visit unit — guid_log = page-views, NOT visits (AUDI-1173, 2026-07-28)
The attribution-independent "did this household visit the advertiser site" signal comes from **`logdata.guid_log`** — but guid_log rows are **page-views**, not visits. `clickpass_log` is the (MNTN-attributed) **visit** log. For a total-site-visit metric (e.g. a frequency RCT's primary outcome):
