...
| 46 | ML Audience Intent Scoring Model | ✗ | 241 | $1.70M | 0 | Fangorn ML-driven intent scoring. **Audience overlay mechanic** (Ryan Kleck, 2026-06-01): when MNTN switches an advertiser to Fangorn, the change is applied via the "audience overlay" feature that updates `audience.audience_segments` to reference DS46, but does NOT change the base `audience.audiences` row — which still references DS13 (Vertical) and DS19 (Keywords). So a campaign showing DS46 in `audience_segments` is **still effectively MM** — Fangorn replaces the scoring algorithm, not the underlying audience config. **TI-999 implication:** DS46 in `audience_segments` represents a Fangorn-overlaid MM campaign, NOT a new audience type — counts as MM-touching. DS13→DS46 swap rolled out to first three advertisers week of 2026-04-30 (`mntn_business.md`). |
...
| 11 | LiveRamp (legacy) | ✓ | 0 | 0 | 0 | **Deprecated old LiveRamp** (Sean Yang, TI team, 2026-05-29). Used device_id→IP mapping for targeting; DS35 replaced it by having LiveRamp send IPs directly. Retained in `tpa.categories` because reporting still needs the historical category references. 5 inactive prospecting campaigns still reference it positively (zero impressions in 30d). **CUTOVER DATE (AUDI-1070, 2026-06-30, from `archives_audience_segment_archives` expression DS-id counts by month): the DS11→DS35 flip is platform-wide and sharp — through 2025-04 DS35=0 campaigns / DS11~1,300; 2025-05 = overlap month (DS11 6,805, DS35 699 first appearance); 2025-06 DS11 collapses to 4, DS35 jumps to 7,037; 2025-07+ DS11 effectively dead. So the 3P-interest layer switched LiveRamp source in **May–June 2025** — coincident with the mid-2025 perf-inflection window, but it's a 3P-layer *upgrade* (direct-IP, fresher), not an MM-core scoring change.** |
...
| 33 | Sovrn | ✗ | 0 | 0 | 0 | **ACTIVE MM site-visit DDP** ($0.50 CPM, live 2025-05-27). **77% of URLs malformed** (doubled protocol `https://Xhttps://X`) → fail `NET.REG_DOMAIN` — only ~23% classifier-usable (AUDI-1089 q1c). **Junk IS billed:** June billed domains include the malformed hosts themselves (msn.comhttps 3.3%, yahoo.comhttps 2.2% of attributed imps — AUDI-1089 q1d). AUDI-1089 verdict: DROP (~50-200× over band). |
...
| 40 | 33Across API | ✗ | 0 | 0 | 0 | **ACTIVE MM site-visit DDP** ($0.50 CPM, live 2025-07-01). Modal URL = openwebmp.com RTB endpoint (17.8%; top-5 domains 58%) — ad-infra heavy, 26% path share (AUDI-1089 q1b/q1c). **Top billed domains are cookie-sync endpoints** (cookies.nextmillmedia.com 9.2%, sync.programmaticx.ai 8.2% — q1d): we pay for ad-infra hits. AUDI-1089 verdict: DROP/renegotiate (~4-18× over band). |
...
| 51 | Bombora | ✗ | 0 | 0 | 0 | B2B intent/ABM. **DS51 now delivers IPDSC volume directly as of ~2026-07-06** (superseded the earlier "0 IPDSC volume" state — the "~July 2026" milestone landed). **Intermittent:** the Bombora feed drops ~every other day → `gs://mntn-data-archive-prod/ipdsc/dt=D/data_source_id=51/` (~49 files/present-day), source `partners/bombora/segments/{D-1}`; skip days = no ipdsc partition **that day** (self-heals next delivery; on-call INC-001) but membership-db persists (~90d TTL per Jordan), so a skip does **NOT** imply 0 downstream serving (07-25 was a skip and the Bombora campaigns still served 104K). Present days ≈ 108K-141K DS51 impressions in `analytics_curated.enriched_impressions`, but **all from ONE internal test advertiser** (30506 "MNTN - No ENG Testing", CG 131563, live since 07-24) — pipe-validation traffic, NOT production client delivery yet. LiveRamp custom-segment pipe (DS35, join key `SegmentID`, ~3-day lead) also still live. See `mntn_business.md` § Bombora + on-call INC-001. (Left flag/count cols reflect an earlier snapshot, not re-verified.) |
...
- **DS14 — Zach confirms freshness filter framing:** ✅ Zach Schoenberger (TI team, 2026-06-01): "ds14 is if we have seen the ip in our logs. it limits our segments to recently seen ip's." Matches Sean's earlier framing (guid_log 4d + augmentor_log 1d). **Update 2026-07-28 (DS14-opt):** effective serving window = **8-day TTL** (membership-db `data_source_ttls['14']=8`, per-IP epoch decay), not the 1-4d build; DS14 IS materialized in `ipdsc__v1` (`data_source_id=14`, ~149M IPs/day). Scoring is ungated: full 31d DS13 universe scored, only ~8d addressable → **39% (DS13) / 69% (DS19 MM Core) of scored IPs are non-biddable**. Origin: born 2025-08-13, prod 2025-12-11 (AUDI-369), post-MM. **Source code (GitHub SteelHouse org, verified 2026-07-30):** build + lookback = `mntn_global_data`/`airflow-ti` `spark/create_mntn_global_data_pyspark.py` (`augmentor_log_lookback_days = 1`, `guid_log_lookback_days = 4`; airflow-ti twin adds `bidder_auction_events` 1d); materialization = `airflow-ti` `spark/data_source/populate_data_source.py::_get_data_source_14_df` (L933-953) → `ipdsc__v1`; TTL = `membership-db` `server/config/config.yml` `data_source_ttls['14']=8`. See `data_knowledge.md` DS14-opt block for the full citation. **Runtime verification (Compass, 2026-07-30):** live builder code-favored `gcp_mntn_global_data` (airflow-ti/Dataproc); the legacy EMR/S3 job's PySpark is untouched since 2024. TTL lookup is **audience_type-FIRST**, so `'14':8` is a **fallback** — effective 8d only if DS14 is absent from the portal `data_source_info` map, else 30d (`default_ttl`); which is live is unresolved. DS14 is **NOT injected at bid time** — the DS14/cat1 clause is baked into the stored segment at creation (like DS13's `PEAK_PERFORMANCE_DATA_SOURCE_ID=13` in `mntn-go`), so removing it for a campaign = edit the segment/builder, not membership-db.
- **membership-db serving TTLs (verified from live config, `SteelHouse/membership-db` `server/config/config.yml` @ default-branch commit 3e9ce5c, 2026-07-24):** per-`data_source_id` retention in the serving store (per-IP epoch decay), separate from each source's build/refresh window. `data_source_ttls` in days: DS1=3650, DS9=180, **DS14=8**, DS16=30, DS18=3650, DS21=365, DS34=365, DS42=3, DS43=3, DS51=90. Data sources NOT listed (e.g. DS47 CRM) fall back to `audience_type_ttls.default_ttl=30`. This confirms the DS51 "~90d TTL per Jordan" note (DS51 row above) from source. Also `score_type_ttls_in_secs: rtc=86400` (= 1 day; the inline "30 days in seconds" comment in the config is wrong, the value is 1 day, matching the known RTC 1-day TTL).
...
- **MM adoption by advertiser headcount = ~83%** (distinct from spend share). Of active advertisers, **~83% run ≥1 MM campaign** (1,756 / 2,119; ~87% / 1,755 / 2,015 among prospecting advertisers), de-duped from live `audience_segments` with MM = DS13/19/38/46. **DS19 is always part of MM** (keywords) — excluding it wrongly halves this to ~47%, the source of legacy "~half of advertisers" figures. Window 2026-05-01→06-15; canonical query `tickets/ti_999_interest_segment_sizing/queries/ti_999_mm_adoption_current.sql`; full methodology + the DS19 swing in `data_knowledge.md` § "MM advertiser adoption — canonical answer + the DS19 swing". Count from live `audience_segments`, not `audience_audiences` templates (templates over-count ~5 pts at the campaign-group level).
