...
1. **RTC vs MM pipeline relationship (AUD squad):** Sean's two statements differ — first read "RTC = MM real-time variant"; revised read "RTC is independent of MM, MM is batch via IPDSC, RTC is real-time match-and-tag." AUD team (Mike Dolt or Jordan Piepkow) to confirm whether RTC and MM share the same scoring engine or are genuinely separate pipelines.
2. **HHST gates effective RTC (Ryan Kleck flagged 2026-06-01):** RTC only affects bidding when HHST is set. **Where to find HHST settings — campaigns table? campaign_configs? bidder runtime?** And critically: query `bid_events` for the 3P-only baseline campaigns (404 camps) to see which have HHST set. Cleanest 3P baseline = 3P-only WITHOUT HHST set.
3. **Does Fangorn (DS46) bake in RTC?** User question 2026-06-01. If DS46 campaigns also include RTC scoring, that's another dimension of overlap between MM-batch and RTC that needs unpacking.
