...
4. **self_improvement kernels** (pri 4) — ✅ **SHIPPED 2026-07-20.** (a) `health_scorecard.py` (read-only: days-since-`/capture` via the `: capture` ritual-commit grep, orphan docs >120d, dup-H1-titles) wired into the SessionStart print. (b) `log_request.py` `UserPromptSubmit` hook → gitignored keyword-only `knowledge/.request_log.jsonl` (verb + ≤10 nouns + one-way hash, never the raw prompt) + `request_digest.py` on-demand digest that PROPOSES a `/skill` for recurring shapes. Read/append-only, no delete authority; nothing auto-created.
5. **deck_structure kernel** (pri 5) — ✅ **SHIPPED 2026-07-20.** Added "Part 10: The De-Slop Pass — Specificity Is Proof of Work" to `presentation_playbook.md` (slop list + one-line advisory grep + explicit scope note: removes filler only, does NOT mandate a style or override facts-not-presentation for technical/internal decks); wired the same grep as warn-only step 11 in `presentation_critique.md`. No `new_deck.sh`/`lint_deck.py`/per-slide-YAML/mandated-style (all anti-goals).
6. **execution_engine** (pri 6, already-built) — ✅ **BLESSED 2026-07-20.** Appended the 6 live-crawl lessons (session-resume, `sql_tables` `__`-truncation fix, `coverage_state`/`lint_tickets` gate, READ-ONLY prose-oracle, false-stale `schema_synced` fallback, cross-project seeding) to `agent_pass_runbook.md` as durable doctrine; blessed the 7-agent roster + linters + manifest-as-queue as THE factory. No parallel `RESTRUCTURE_GUIDE.md` / whole-workspace bulk port.

---

**All six verdicts resolved (2026-07-20).** Phase-0 provenance + all five kernels shipped; the engine is blessed. The thin extension the thesis demanded — one toolchain, one provenance store, one front-matter model, two prose sections, a read-only scorecard + keyword-only request log — is live on `main`. None of the named anti-goals were built.
