...
- `lib/mntn_xlsx.py` — the shared branded `.xlsx` builder (`MntnWorkbook`). The default shareable deliverable. **It refuses to write the file on a rule break**, so read the build errors rather than working around them: char caps, jargon/abbreviated/negated column headers, `By `-prefixed or Title Case tab names, placeholder headers, ordinal group labels, mid-word column widths. Rules and the why: `documentation/docs/xlsx_deliverable_standard.md`, memory `reference_xlsx_master_format`.
