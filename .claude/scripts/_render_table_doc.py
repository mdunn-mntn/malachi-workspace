...
def schema_block(tcols):
    rows = ["| column | type | nullable | partition | cluster# |",
            "|--------|------|----------|-----------|----------|"]
    for c in tcols:
        ...
        rows.append(f"| {c['column_name']} | {c['data_type']} | "
                    f"{c.get('is_nullable', '')} | {part} | {clus} |")
    return "\n".join(rows)


def derive(tcols):
    part = next((c["column_name"] for c in tcols
                 if str(c.get("is_partitioning_column", "")).upper() == "YES"), "none")
    clus = [c["column_name"] for c in sorted(
...
def write_doc(dataset, out_dir, name, tcols, storage, today, ttype="BASE TABLE", definition=None):
    ...
    if os.path.exists(path):
        ...
        schema_changed = True
        if m:
            schema_changed = (content[m.end(1):m.start(2)] != schema)   # compare the OLD schema block to the new
        ...
        if not schema_changed:
            upd.pop("schema_synced", None)   # no-op reintrospect: don't re-stamp the machine date (would falsely flag verified docs stale)
        content = set_fm(content, upd)
        action = "updated"
    else:
        rows, lbytes = _int(storage.get("total_rows")), _int(storage.get("total_logical_bytes"))
        grain_default = "N/A — derived view" if is_view else "one row per <fill me>"
        summary_default = (f"<what this {ttype.lower()} provides>" if is_view
        ...
        content = NEW_DOC.format(
            ds=dataset, tbl=name, ttype=ttype, part_display=part_display, physical=physical,
            clus=", ".join(clus),
            rows=rows if rows is not None else "null",
            lbytes=lbytes if lbytes is not None else "null",
            today=today, schema=schema,
            grain_default=grain_default, summary_default=summary_default)
...
def main():
    if len(sys.argv) not in (6, 7):
        sys.exit("usage: _render_table_doc.py <dataset> <out_dir> <columns.json> "
    ...
    for t in tables:
        ...
        write_doc(dataset, out_dir, name, tcols, storage.get(name, {}), today, ttype, defs.get(name))
