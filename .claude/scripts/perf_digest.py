...
def by_table(rows, top):
    ...
    out = ["## Cost by table", "",
           ...
           "|---|--:|--:|--:|--:|--:|--:|---|"]
    ranked = sorted(agg.items(), key=lambda kv: -(pctl([x.get("gb_billed") for x in kv[1]], 90) or 0))
...
def offenders(rows, top):
    ...
    out = ["## Top offenders (single most expensive runs)", "",
           "| gb_billed | slot_s | tables | ticket | label | ts |", "|--:|--:|---|---|---|---|"]
    for r in ok[:top]:
        out.append(f"| {r['gb_billed']} | {round((r.get('slot_ms') or 0)/1000,1)} | "
...
def main():
    ...
    ap.add_argument("--log", default=os.path.join(here, "..", "..", "knowledge", "bq_perf_log.jsonl"))
    ...
    blocks = {"by-table": by_table, "offenders": offenders,
              "repeats": repeats, "phase-accuracy": phase_accuracy}
    print(f"<!-- perf_digest {a.mode} · {len(rows)} records"
