...
"${RS[@]}" "$SRC/workflows/" "$B/workflows/"
"${RS[@]}" "$SRC/tickets/_template/" "$B/tickets/_template/"
mkdir -p "$B/lib"; cp "$SRC/lib/mntn_xlsx.py" "$B/lib/mntn_xlsx.py"; cp "$SRC/lib/mntn_xlsx_demo.py" "$B/lib/mntn_xlsx_demo.py"
...
mv "$B/lib/mntn_xlsx_demo.py" "$B/lib/xlsx_demo.py"

# 5. All text transforms in one deterministic python pass ----------------------
python3 - "$B" "$MAP" <<'PY'
import os, sys
B, MAP = sys.argv[1], sys.argv[2]

# ordered literal find->replace from the map file
pairs = []
...
XLSX = {os.path.join(B, "lib", "xlsx_builder.py"), os.path.join(B, "lib", "xlsx_demo.py")}
...
for dp, _, fns in os.walk(B):
    for fn in fns:
        ...
        for f, r in pairs:                 # sanitize map (ordered)
            s = s.replace(f, r)
