...
| §11 Kit compliance | any `verify.sh` ✗ — a lint violation, a stale index, or a failed hook self-test | **Tier 1 Safe:** stale index → `.claude/scripts/verify.sh --fix` (rebuilds + stages); a broken hook → name the hook + its self-test assertion. **Tier 2 Judgment:** a real front-matter violation → the exact file + field to fix. These are the SAME checks the commit gate enforces per-commit; a §11 failure means drift landed via `--no-verify` or predates the gate. Fixing gets the repo back to gate-clean |
