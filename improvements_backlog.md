...
| IMP-003 | 2026-07-28 | airflow-ti / targeting-infra — Fangorn Vertex | `TiVertexPipelineOperator` always injects `reference_date` into every submitted pipeline's `parameter_values`; a template whose date-param name drifts (drift pipeline used `run_date`) fails only at task-exec with a ValueError, not at DAG-parse. Standardize Fangorn template param names on `reference_date`, or validate the operator↔template param contract at parse time (fail fast / CI check). | INC-003: drift template declared `run_date` not `reference_date` → hard ValueError, PagerDuty page, retries exhausted. One-off fixed by PR #1158, but the silent-inject pattern will re-bite any new mis-named template. | S | Fangorn/ML + infra (Brian) | idea | on-call INC-003 |
