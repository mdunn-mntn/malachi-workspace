...
| IMP-005 | 2026-07-29 | airflow-ti / audience_intent — Dataproc | Investigate the `fangorn_score_monitor` batch runtime (~66 min on 2026-07-28) and either raise its `ttl` (5400s) or reduce work (the 20K-file `prospecting_intent` read + ~5.2GB `ipdsc_geo` join dominate). It runs at ~73% of the 90-min TTL, so a heavier-data day risks a hard TTL failure. | INC-004 re-run: batch SUCCEEDED in ~66 min vs 90-min TTL, ~24 min headroom. No historical durations (serverless batches auto-deleted) so can't tell if 66 min is typical or an outlier — worth one look. | S | targeting (Ryan) | idea | on-call INC-004 |
| IMP-004 | 2026-07-29 | airflow-ti / audience_intent | Add a `GCSObjectExistenceSensor` on `gs://mntn-data-archive-prod/ipdsc_geo/dt={{ds}}/_SUCCESS` before `fangorn_score_monitor` (mirror INC-001's `precondition_*` pattern), or widen its retry window, so it stops racing the `ipdsc_geo` producer. | INC-004: monitor reads `ipdsc_geo/dt=<run_date>`; producer lands on D+1 with ~3.5h-variable timing (04:56–08:17Z over 4 days); monitor's `retries=2×10min` gives only ~30-40min slack → pages on late-producer days with a `PATH_NOT_FOUND`. | S | targeting (Ryan) | proposed | on-call INC-004; PR SteelHouse/airflow-ti#1160 |
