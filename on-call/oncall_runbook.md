...
| `keyword_ddp_reporting / wait_for_product_categorization` | Two variants of the SAME sensor: **(a)** 6h `AirflowSensorTimeout` (307 pokes) = upstream still running/not-ready (INC-006); **(b)** fast-fail `ExternalTaskFailedError` in ~9s (single poke) = upstream `product_categorization` in `failed`/`skipped`/**`upstream_failed`** (post-PR #1162, INC-007). Both waiting for `mntn_match_incrementals_fetch.batch_post.product_categorization` = success at logical `−6h`. | Downstream **symptom** — real cause is upstream in the DS13/DS19 MNTN Matched OpenAI pipeline. GCS `dt=` partition **absent**. INC-006 = `batch_fetch` loop-abort (fix shopper_graph#296). INC-007 = `batch_submit` failed 3 levels up on the OpenAI file-quota (see next row). | **real_upstream_failure** — audit the GCS chain `openai_batch_submissions→results→results_joined→product_categorization` for the missing `dt` to find WHICH stage broke (a missing `submissions/dt` means it failed at *submit*, not fetch). Do NOT clear the sensor until `product_categorization/dt` lands (clear just re-fails fast / re-waits). PR #1162 makes it fail fast; #296 fixes the fetch bug. **#296 ships ONLY via `deploy_openai_dockerhub_gcp.yml` (openai_batch_runner image, context `openai/`) — NOT the middleware deploy; deployed 2026-07-30, first exercised 07-31 09:00Z. Deploy-workflow map → memory `reference_shopper_graph_deploy`.** **New upstream signature after shopper_graph#305 deploys (AUDI-1279, PR open 2026-09-03, NOT deployed yet):** a dead cohort fails `mntn_match_incrementals_fetch / batch_fetch` itself with `DeadCohortError: dead cohort dt=<D>: 0 of <N> batches progressed <age>h after submit (threshold 12.0h); first error: <code>: <message>`, after one `batch=<id> ... status=<status> ... error=...` line per receipt and a `cohort dt=<D>: n=..` summary in the `batch_transition`/`batch_fetch` task logs (`[base]` lines; the Slack text stays the generic `Pod ... returned a failure`). Routing is the existing ML path (`#monitor-emr` + ML squad email, no PagerDuty at severity 5). Verdict = real_upstream_failure on the OpenAI side (org-side file access until AUDI-1301); read the first error from the log, do NOT clear `batch_fetch` (each retry re-queries OpenAI and self-clears if the cohort wakes). A missing receipts partition returns instead of raising there; `get_batch_ids` still raises `FileNotFoundError` on it. Alarm semantics: memory `reference_mntn_matched_batch_pipeline` § 2026-09-03. | INC-006, INC-007 |
...
- **Access:** Malachi had fewer Databricks perms than Ryan/Brian ("producers/dev/users" vs Brian "admins/users"); Ryan granted **temp admin** so the Spark UI was reachable. **That grant persisted — `malachi@mountain.com` is in `admins` as of 2026-08-20.** Workspace admin still does NOT grant Unity Catalog `system` schema access (`system.lakeflow` denies `USE SCHEMA`). Permanent programmatic access (API token/MCP) is gone (vault/security policy — same class as the decommissioned Slack bot). **CORRECTION (2026-08-27): programmatic access exists and is key-free.** The 2026-07-31 line above describes API tokens, which stay banned; the U2M OAuth CLI profile `malachi@mountain.com` has read Databricks programmatically since 2026-08-03 (verified by AUDI-1191's `databricks_rca.py` live-fires), and as of 2026-08-26 every `system.*` schema is granted to both `malachi@mountain.com` and the `spark_optimizer` service principal, verified by reading rows. Detail: memory `reference_databricks`, `reference_databricks_system_schema_grants`.
...
Durable fix logged as IMP-060 (a cross-DAG guard, or a non-destructive materialization).
