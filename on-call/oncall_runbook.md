...
| `fangorn_inference_pipeline_run / inference_pipeline` | `RuntimeError: Job failed with: code: 9 … failed tasks are: [create-dataproc-cluster]` (PagerDuty, retries exhausted). The 290-worker cluster needs **~4,672 N2_CPUS (~93% of the 5,000 us-central1 quota)** + ~145,500 GB disk, so it fails on the CAPACITY CEILING via one of 3 surfaces: a Dataproc-op **zonal `code 14 UNAVAILABLE`** stockout, a Vertex/worker-pool **`Insufficient N2_CPUS/DISKS_TOTAL_GB quota`** error, or (INC-002) starvation by a truly-concurrent job. | The cluster at ~93% of N2 quota has no headroom → fails on: (a) **quota exhaustion** (INC-008) — incl. its OWN prior failed cluster's VMs lingering (self-block) or another N2 consumer; (b) **zonal stockout** (INC-008) — a single zone short on large instances; (c) **real concurrent-job contention** (INC-002). ⚠ `inference_pipeline` is UPSTREAM of `challenger_inference_pipeline` (sequential) — the challenger is NEVER the contender. | **Pull ALL 3 surfaces + reconcile** (don't name a cause from one): `dataproc operations describe <failed-create>` (zonal), the Vertex `service`/worker-pool log (quota), and `gcloud compute regions describe us-central1` quota-vs-usage. Check no prior ERROR cluster still holds VMs (delete it). Quota free + no zonal stockout → re-run provisions. Persistent → owner+infra raise quota / auto-delete failed clusters / multi-zone / smaller cluster (IMP-014). | INC-002 (concurrent-job contention), INC-008 (quota ceiling + zonal, 07-30) |
...
### INC-008 — `fangorn_inference_pipeline_run` `inference_pipeline` — 290-worker Dataproc create failed against the us-central1 COMPUTE QUOTA CEILING (~93% of N2_CPUS) — quota exhaustion + zonal stockout, NOT champion/challenger contention
...
**STATUS: OBSERVED — root cause RECONCILED (compute-quota ceiling); as of 21:2xZ quota is free again (N2 usage 80 / 4920 free) so a re-run should now provision. Flip to RESOLVED when the champion create succeeds.**
...
**Verdict: RESOURCE/QUOTA CEILING (`transient_infra` + capacity-limited).** The cluster at ~93% of N2 quota is fragile to BOTH zonal physical capacity AND quota headroom — including headroom consumed by its own uncleaned failed attempt. This IS the "~94% cap" of INC-002 (4,672/5,000 = 93.4%), but the contender here was the champion's own dead cluster, NOT the challenger. **Both my earlier calls were wrong:** (1) champion-vs-challenger contention — refuted (challenger deleted 18:53Z, 62 min before the champion's 19:55Z create; and `inference_pipeline` is UPSTREAM of `challenger_inference_pipeline`, so they're sequential and never concurrent); (2) "pure external zonal stockout" — incomplete (the STOCKOUT wording on attempts 1/3 masked that the binding ceiling is the project's own N2_CPUS/DISKS quota, per Brian's quota error + the quota API).
