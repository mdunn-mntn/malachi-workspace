...
**Durable fix (route to the Targeting/TI owner; never hot-patch a prod DAG):** align the cross-DAG contract with the producer's intentional skip — set `skipped_states=[State.SKIPPED]` on `wait_fpa` so a legitimate no-data upstream skip makes the consumer **SKIP** (not FAIL + page), OR gate `hashed_email_ds_26_signals` with the same `source_available_dsid26` short-circuit the producer uses. Logged IMP-026. **Fix SHIPPED as [airflow-ti#1175](https://github.com/SteelHouse/airflow-ti/pull/1175) (2 DAGs: this one + sibling `hashed_email_guid_log_signals`/DS23, same latent bug; `keyword_ddp_reporting` NOT changed, its skip is a real break). Tracked AUDI-1195 (Spike). Moves `skipped` from `failed_states` to `skipped_states`.**
