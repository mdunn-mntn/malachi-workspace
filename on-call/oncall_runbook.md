...
**Durable fix (route to the producing/TI owner; never hot-patch):** IMP-043 — raise `wait_fpa`'s `timeout` on both DAGs to cover the real producer distribution (a 50-min tail exists; 900s covers ~94% of runs), and/or move the two consumers to the producer's `all_batches_done` boundary. Raising `retries` alone does NOT help: `AirflowSensorTimeout` skips retries entirely. Worth a separate look: the recurring ~10-min gap between `get_dt_hh_by_dsid` and `source_available_*` is scheduling latency inside the producer, not task work.
