...
| `fangorn_inference_pipeline_run / inference_pipeline` | `RuntimeError: Job failed with: code: 9 … failed tasks are: [create-dataproc-cluster]` (PagerDuty, retries exhausted). **TWO distinct root causes** behind the same wrapper — the inner Dataproc error (`operations describe <failed-create>`) is the tell: a Dataproc **quota** error vs a GCE **zonal `code 14 UNAVAILABLE`** stockout. | (a) **resource_contention** (INC-002): another Dataproc job's cluster lifetime OVERLAPS the failed create → starves it. (b) **transient_infra** (INC-008): external `us-central1-a` zonal stockout for the 290-worker cluster, NO concurrent MNTN job. ⚠ `inference_pipeline` is UPSTREAM of `challenger_inference_pipeline` (sequential) — the challenger is NEVER the champion's contender. | **Run the discriminating test first:** `operations describe` the failed create for the real error, AND check whether ANY other cluster's [create→delete] window overlaps the failed-create time. **Overlap → contention:** let the other job finish, re-trigger champion (INC-002). **No overlap → zonal stockout:** re-run once (autozone re-picks a zone); persists → owner+infra widen zones/capacity (INC-008). | INC-002 (contention), INC-008 (zonal stockout, 07-30) |
...
### INC-008 — `fangorn_inference_pipeline_run` `inference_pipeline` — Dataproc create failed on a `us-central1-a` ZONAL STOCKOUT (external GCE capacity — NOT resource contention)
...
**Error (Airflow log tail):** `RuntimeError: Job failed with: code: 9 … failed tasks are: [create-dataproc-cluster]` (same wrapper as INC-002 — but a different root cause; see below).
...
**STATUS: OBSERVED — verdict CORRECTED to `transient_infra`; us-central1 is STOCKED OUT for the 290-worker cluster across ≥2 zones (re-run has NOT recovered yet).** Champion 290-worker create has failed in **us-central1-a** (20:02Z) and **us-central1-c** (21:15Z, explicit `state:STOCKOUT`) while small clusters (4-worker, daily-drift) provision fine → a regional large-instance capacity crunch, not a one-zone blip. A single re-run does NOT reliably clear it. Flip to RESOLVED only when the champion create actually succeeds.

**⚠ VERDICT CORRECTION — my first-pass call (resource contention, champion vs challenger) was WRONG.** Owner Brian McAdams pushed back ("they don't run at the same time"); the empirical test proved him right on two independent lines:
1. **Cluster lifetimes do NOT overlap.** Challenger `fangorn-challenger-62e32a7e` (290 workers): created 18:36Z, up 18:40Z, **DELETED 18:52–18:53Z**. Champion `fangorn-inference-445fba60` (290 workers) create: **PENDING 19:55:44Z → FAILED 20:02:39Z** — the champion tried **62 min AFTER the challenger was already torn down.** At 19:55Z there was **no** concurrent MNTN Dataproc cluster in `mntn-targeting-prj-prod`/`us-central1`. Two further 290-worker creates also failed on the same stockout (20:02Z, 21:07Z) while a smaller `fangorn-daily-drift` cluster **created fine at 20:48Z** → `us-central1-a` lacked capacity for **large (290-worker)** clusters specifically, not for us in aggregate.
2. **`inference_pipeline` is UPSTREAM of `challenger_inference_pipeline`** in the DAG (`wait_for_features → inference_pipeline → challenger_inference_pipeline → daily_drift_pipeline`; graph view + owner confirm). They run **sequentially by design and never hold Dataproc clusters concurrently.** The 18:40Z challenger cluster belonged to an earlier cycle and was gone before the champion's create. The challenger can NEVER be the champion's contender.

**Verdict: TRANSIENT INFRA — a genuine external `us-central1-a` zonal compute stockout for the 290-worker cluster. NOT resource contention (no concurrent MNTN job), NOT config.** A GCE zone is a global, multi-tenant resource; external demand can drain the machine type. Autozone had placed the cluster in `us-central1-a`; capacity vanished during provisioning, and repeatedly so for large clusters that afternoon.

**Action (per `transient_infra`, escalated — stockout is RECURRING across zones):** a first re-run re-picks a zone via autozone, but here creates have **already stocked out in 2 zones (a, c)**, so blind re-running is not reliably clearing it. Escalation ladder: (1) one more re-run may land in `us-central1-b`/`-f` if those have capacity; (2) else **wait** — GCE stockouts are transient (minutes–hours) and clear on their own; (3) if it stays blocked, route to **owner + infra**: this is external GCP large-instance capacity in us-central1 — options are a capacity reservation, a secondary region, or a smaller cluster (IMP-014). Not a hot-patch. Keep owner (Brian/ML) informed since the daily inference output is delayed while it's stocked out.

**Method lesson (mine — the reasoning trap, logged):** I inferred contention from a grid screenshot ("challenger running") + two 290-worker creates existing on the same day, WITHOUT running the discriminating test — **the challenger cluster's DELETE time vs the champion's CREATE time.** Delete 18:53Z vs create 19:55Z refutes overlap outright; and the DAG dependency (inference upstream of challenger) makes concurrency impossible by design. This is the mirror of INC-001's lesson: there I wrongly folded, here I wrongly held — both because I skipped the discriminating test. **For "create-dataproc-cluster code 9": (a) `operations describe` the failed create for the REAL error, and (b) prove overlap by comparing another cluster's [create→delete] window to the failed-create window — never infer overlap from a task-state screenshot or from two creates sharing a day.**

**Diagnosis run (copy-paste — the delete-vs-create overlap test is the crux):**
```bash
# 1. Real error (Airflow log only says "create-dataproc-cluster failed"): describe the FAILED create op
gcloud dataproc operations describe <FAILED_CREATE_OP> --project=mntn-targeting-prj-prod --region=us-central1 \
  --format="value(error)"          # -> code 14 zone UNAVAILABLE = external stockout (transient_infra)
# 2. OVERLAP TEST (contention vs stockout): full op chronology with real timestamps — does ANY other
#    cluster's [create -> delete] window contain the failed champion CREATE time?
gcloud dataproc operations list --project=mntn-targeting-prj-prod --region=us-central1 --format=json \
  | python3 -c "import sys,json; [print(o['metadata']['statusHistory'][0]['stateStartTime'], o['metadata']['status']['stateStartTime'], o['metadata'].get('operationType'), o['metadata'].get('clusterName')) for o in json.load(sys.stdin)]" | sort
#    No overlapping cluster around the failed create -> transient_infra (INC-008). Overlap -> contention (INC-002).
```

**Durable fix:** single-zone (`us-central1-a`) placement makes the 290-worker Fangorn inference cluster fragile to zonal stockouts. Fix = give the cluster fallback zones / multi-zone selection (or a secondary region) so one zone's stockout doesn't page — owner = Fangorn/ML + infra (template in `targeting-infra`, not `airflow-ti`). Tracked as **IMP-014** in `improvements_backlog.md`. This is distinct from INC-002/IMP-002 (real champion-vs-concurrent-job contention). Do NOT hot-patch.
